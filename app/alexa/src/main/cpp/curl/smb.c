...
static void conn_state(struct connectdata *conn, enum smb_conn_state newstate) {
    ...
    if(smbc->state != newstate) infof(conn->data, "SMB conn %p state change from %s to %s\n", (void*)smbc, names[smbc->state], names[newstate]);
#endif
    smbc->state = newstate;
}
static void request_state(struct connectdata *conn, enum smb_req_state newstate) {
    ...
    if(req->state != newstate) infof(conn->data, "SMB request %p state change from %s to %s\n", (void*)req, names[req->state], names[newstate]);
#endif
    req->state = newstate;
}
static CURLcode smb_setup_connection(struct connectdata *conn){
...
static CURLcode smb_connect(struct connectdata *conn, bool *done) {
    ...
    char *slash;
    (void)done;
    if(!conn->bits.user_passwd) return CURLE_LOGIN_DENIED;
    smbc->state = SMB_CONNECTING;
    smbc->recv_buf = malloc(MAX_MESSAGE_SIZE);
    if(!smbc->recv_buf) return CURLE_OUT_OF_MEMORY;
    connkeep(conn, "SMB default");
    slash = strchr(conn->user, '/');
    if(!slash) slash = strchr(conn->user, '\\');
    if(slash) {
        smbc->user = slash + 1;
        smbc->domain = strdup(conn->user);
        if(!smbc->domain) return CURLE_OUT_OF_MEMORY;
...
static CURLcode smb_recv_message(struct connectdata *conn, void **msg) {
    ...
    if(!bytes_read) return CURLE_OK;
    smbc->got += bytes_read;
    if(smbc->got < sizeof(unsigned int)) return CURLE_OK;
...
static CURLcode smb_flush(struct connectdata *conn) {
    ...
    if(result) return result;
    if(bytes_written != len) smbc->sent += bytes_written;
    else smbc->send_size = 0;
    return CURLE_OK;
}
...
static CURLcode smb_send_setup(struct connectdata *conn) {
    ...
    if(byte_count > sizeof(msg.bytes)) return CURLE_FILESIZE_EXCEEDED;
...
static CURLcode smb_send_open(struct connectdata *conn) {
    ...
    size_t byte_count;
    if((strlen(req->path) + 1) > sizeof(msg.bytes)) return CURLE_FILESIZE_EXCEEDED;
    memset(&msg, 0, sizeof(msg));
...
static CURLcode smb_send_write(struct connectdata *conn) {
    ...
    if(upload_size >= MAX_PAYLOAD_SIZE - 1) upload_size = MAX_PAYLOAD_SIZE - 1;
    memset(msg, 0, sizeof(*msg));
...
static CURLcode smb_send_and_recv(struct connectdata *conn, void **msg) {
    ...
    *msg = NULL;
    if(!smbc->send_size && smbc->upload_size) {
        size_t nread = smbc->upload_size > conn->data->set.upload_buffer_size ? conn->data->set.upload_buffer_size : smbc->upload_size;
        conn->data->req.upload_fromhere = conn->data->state.ulbuf;
        result = Curl_fillreadbuffer(conn, nread, &nread);
        if(result && result != CURLE_AGAIN) return result;
        if(!nread) return CURLE_OK;
    ...
    if(smbc->send_size || smbc->upload_size) return CURLE_AGAIN;
...
static void get_posix_time(time_t *out, curl_off_t timestamp) {
    ...
    if(timestamp > TIME_T_MAX) *out = TIME_T_MAX;
    else if(timestamp < TIME_T_MIN) *out = TIME_T_MIN;
    else
#endif
    ...
    switch(req->state) {
        case SMB_TREE_CONNECT:
            if(h->status) {
                ...
                if(h->status == smb_swap32(SMB_ERR_NOACCESS))req->result = CURLE_REMOTE_ACCESS_DENIED;
                break;
            }
        ...
        case SMB_OPEN:
            ...
            if(conn->data->set.upload) {
                ...
                if(conn->data->req.size < 0) {
                    ...
                    if(conn->data->set.get_filetime) get_posix_time(&conn->data->info.filetime, smb_m->last_change_time);
        ...
        case SMB_DOWNLOAD:
            ...
            if(len > 0) {
              ...
              } else result = Curl_client_write(conn, CLIENTWRITE_BODY, (char *)msg + off + sizeof(unsigned int), len);
              if(result) {
                  req->result = result;
                  next_state = SMB_CLOSE;
                  break;
        ...
        case SMB_UPLOAD:
            ...
            if(conn->data->req.bytecount >= conn->data->req.size) next_state = SMB_CLOSE;
            else next_state = SMB_UPLOAD;
            break;
        case SMB_CLOSE: next_state = SMB_TREE_DISCONNECT; break;
        case SMB_TREE_DISCONNECT: next_state = SMB_DONE; break;
        default:
            smb_pop_message(conn);
            return CURLE_OK;
    }
...
static CURLcode smb_parse_url_path(struct connectdata *conn) {
    ...
    char *path;
    char *slash;
    CURLcode result = Curl_urldecode(data, data->state.up.path, 0, &path, NULL, REJECT_CTRL);
    if(result) return result;
    smbc->share = strdup((*path == '/' || *path == '\\') ? path + 1 : path);
    free(path);
    if(!smbc->share) return CURLE_OUT_OF_MEMORY;
    slash = strchr(smbc->share, '/');
    if(!slash) slash = strchr(smbc->share, '\\');
    if(!slash) {
        Curl_safefree(smbc->share);
        return CURLE_URL_MALFORMAT;
    }
