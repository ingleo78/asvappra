...
static void SHA256_Final(unsigned char *digest, SHA256_CTX *ctx) {
    ...
    if(length == SHA256_DIGEST_LENGTH) CryptGetHashParam(ctx->hHash, HP_HASHVAL, digest, &length, 0);
...
static int sha256_compress(struct sha256_state *md, unsigned char *buf) {
    ...
    for(i = 0; i < 16; i++) W[i] = WPA_GET_BE32(buf + (4 * i));
...
#define block_size 64
    ...
    while(inlen > 0) {
        if(md->curlen == 0 && inlen >= block_size) {
            if(sha256_compress(md, (unsigned char *)in) < 0) return -1;
            ...
            if(md->curlen == block_size) {
                if(sha256_compress(md, md->buf) < 0) return -1;
...
static int SHA256_Final(unsigned char *out, struct sha256_state *md) {
    ...
    if(md->curlen > 56) {
        while(md->curlen < 64) md->buf[md->curlen++] = (unsigned char)0;
        sha256_compress(md, md->buf);
        md->curlen = 0;
    }
    while(md->curlen < 56) md->buf[md->curlen++] = (unsigned char)0;
    WPA_PUT_BE64(md->buf + 56, md->length);
    sha256_compress(md, md->buf);
    for(i = 0; i < 8; i++) WPA_PUT_BE32(out + (4 * i), md->state[i]);
    return 0;
}
#endif
