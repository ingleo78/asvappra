...
static void state(struct connectdata *conn, pop3state newstate) {
    ...
    if(pop3c->state != newstate) infof(conn->data, "POP3 %p state change from %s to %s\n", (void *)pop3c, names[pop3c->state], names[newstate]);
#endif
...
static CURLcode pop3_perform_upgrade_tls(struct connectdata *conn) {
    ...
    if(!result) {
        if(pop3c->state != POP3_UPGRADETLS) state(conn, POP3_UPGRADETLS);
        if(pop3c->ssldone) {
            pop3_to_pop3s(conn);
...
static CURLcode pop3_perform_user(struct connectdata *conn) {
    ...
    if(!result) state(conn, POP3_USER);
    return result;
}
...
static CURLcode pop3_perform_apop(struct connectdata *conn) {
    ...
    for(i = 0; i < MD5_DIGEST_LEN; i++) msnprintf(&secret[2 * i], 3, "%02x", digest[i]);
    result = Curl_pp_sendf(&pop3c->pp, "APOP %s %s", conn->user, secret);
    if(!result) state(conn, POP3_APOP);
    return result;
}
#endif
static CURLcode pop3_perform_auth(struct connectdata *conn, const char *mech, const char *initresp) {
    CURLcode result = CURLE_OK;
    struct pop3_conn *pop3c = &conn->proto.pop3c;
    if(initresp) result = Curl_pp_sendf(&pop3c->pp, "AUTH %s %s", mech, initresp);
    else result = Curl_pp_sendf(&pop3c->pp, "AUTH %s", mech);
    return result;
}
static CURLcode pop3_continue_auth(struct connectdata *conn, const char *resp) {
    struct pop3_conn *pop3c = &conn->proto.pop3c;
...
static CURLcode pop3_perform_authentication(struct connectdata *conn) {
    ...
    if(pop3c->authtypes & pop3c->preftype & POP3_TYPE_SASL) {
        ...
        if(!result)
          if(progress == SASL_INPROGRESS) state(conn, POP3_AUTH);
    }
    if(!result && progress == SASL_IDLE) {
    #ifndef CURL_DISABLE_CRYPTO_AUTH
        if(pop3c->authtypes & pop3c->preftype & POP3_TYPE_APOP) result = pop3_perform_apop(conn);
        else
    #endif
        if(pop3c->authtypes & pop3c->preftype & POP3_TYPE_CLEARTEXT) result = pop3_perform_user(conn);
        else {
...
static CURLcode pop3_perform_command(struct connectdata *conn) {
    ...
    if(pop3->id[0] == '\0' || conn->data->set.ftp_list_only) {
        command = "LIST";
        if(pop3->id[0] != '\0')  pop3->transfer = FTPTRANSFER_INFO;
    } else command = "RETR";
    if(pop3->id[0] != '\0') result = Curl_pp_sendf(&conn->proto.pop3c.pp, "%s %s", (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command), pop3->id);
    else result = Curl_pp_sendf(&conn->proto.pop3c.pp, "%s", (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command));
    if(!result) state(conn, POP3_COMMAND);
    return result;
}
static CURLcode pop3_perform_quit(struct connectdata *conn) {
    CURLcode result = Curl_pp_sendf(&conn->proto.pop3c.pp, "%s", "QUIT");
    if(!result) state(conn, POP3_QUIT);
    return result;
}
        ...
        if(len >= 4 && line[len - 2] == '>') {
            ...
            for(i = 3; i < len - 2; ++i) {
                if(line[i] == '<') {
                    ...
                    char *at;
                    if(!timestamplen) break;
                    pop3c->apoptimestamp = (char *)calloc(1, timestamplen + 1);
                    if(!pop3c->apoptimestamp) break;
                    memcpy(pop3c->apoptimestamp, line + i, timestamplen);
                    pop3c->apoptimestamp[timestamplen] = '\0';
                    at = strchr(pop3c->apoptimestamp, '@');
                    if(!at) Curl_safefree(pop3c->apoptimestamp);
...
static CURLcode pop3_state_capa_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    ...
    (void)instate;
    if(pop3code == '*') {
        if(len >= 4 && !memcmp(line, "STLS", 4)) pop3c->tls_supported = TRUE;
        else if(len >= 4 && !memcmp(line, "USER", 4)) pop3c->authtypes |= POP3_TYPE_CLEARTEXT;
            ...
            for(;;) {
                ...
                if(mechbit && llen == wordlen) pop3c->sasl.authmechs |= mechbit;
                line += wordlen;
                len -= wordlen;
            }
        }
        ...
        if(data->set.use_ssl && !conn->ssl[FIRSTSOCKET].use) {
              if(pop3c->tls_supported) result = pop3_perform_starttls(conn);
              else if(data->set.use_ssl == CURLUSESSL_TRY) result = pop3_perform_authentication(conn);
              else {
                failf(data, "STLS not supported.");
                result = CURLE_USE_SSL_FAILED;
              }
        } else result = pop3_perform_authentication(conn);
    } else {
...
static CURLcode pop3_state_starttls_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    ...
    } else result = pop3_perform_upgrade_tls(conn);
    return result;
}
...
static CURLcode pop3_state_user_resp(struct connectdata *conn, int pop3code, pop3state instate) {
    ...
    if(pop3code != '+') {
        ...
        result = CURLE_LOGIN_DENIED;
    } else  result = Curl_pp_sendf(&conn->proto.pop3c.pp, "PASS %s", conn->passwd ? conn->passwd : "");
    if(!result) state(conn, POP3_PASS);
    return result;
}
...
static CURLcode pop3_statemach_act(struct connectdata *conn) {
    ...
    struct pingpong *pp = &pop3c->pp;
    size_t nread = 0;
    if(pop3c->state == POP3_UPGRADETLS) return pop3_perform_upgrade_tls(conn);
    ...
    do {
        ...
        if(!pop3code) break;
        switch(pop3c->state) {
            case POP3_SERVERGREET: result = pop3_state_servergreet_resp(conn, pop3code, pop3c->state); break;
            case POP3_CAPA: result = pop3_state_capa_resp(conn, pop3code, pop3c->state); break;
            case POP3_STARTTLS: result = pop3_state_starttls_resp(conn, pop3code, pop3c->state); break;
            case POP3_AUTH: result = pop3_state_auth_resp(conn, pop3code, pop3c->state); break;
        #ifndef CURL_DISABLE_CRYPTO_AUTH
            case POP3_APOP: result = pop3_state_apop_resp(conn, pop3code, pop3c->state); break;
        #endif
            case POP3_USER: result = pop3_state_user_resp(conn, pop3code, pop3c->state); break;
            case POP3_PASS: result = pop3_state_pass_resp(conn, pop3code, pop3c->state); break;
            case POP3_COMMAND: result = pop3_state_command_resp(conn, pop3code, pop3c->state); break;
            case POP3_QUIT: default: state(conn, POP3_STOP); break;
        }
...
static CURLcode pop3_multi_statemach(struct connectdata *conn, bool *done) {
    ...
    if((conn->handler->flags & PROTOPT_SSL) && !pop3c->ssldone) {
        ...
        if(result || !pop3c->ssldone) return result;
    }
...
static CURLcode pop3_block_statemach(struct connectdata *conn, bool disconnecting) {
    CURLcode result = CURLE_OK;
    struct pop3_conn *pop3c = &conn->proto.pop3c;
    while(pop3c->state != POP3_STOP && !result) result = Curl_pp_statemach(&pop3c->pp, TRUE, disconnecting);
    return result;
}
static CURLcode pop3_init(struct connectdata *conn) {
    ...
    if(!pop3) result = CURLE_OUT_OF_MEMORY;
    return result;
}
static int pop3_getsock(struct connectdata *conn, curl_socket_t *socks) {
...
static CURLcode pop3_perform(struct connectdata *conn, bool *connected, bool *dophase_done) {
    ...
    if(conn->data->set.opt_no_body)  pop3->transfer = FTPTRANSFER_INFO;
    *dophase_done = FALSE;
...
static CURLcode pop3_disconnect(struct connectdata *conn, bool dead_connection) {
    ...
    if(!dead_connection && pop3c->pp.conn && pop3c->pp.conn->bits.protoconnstart)
        if(!pop3_perform_quit(conn)) (void)pop3_block_statemach(conn, TRUE);
...
static CURLcode pop3_regular_transfer(struct connectdata *conn, bool *dophase_done) {
    ...
    if(!result && *dophase_done) result = pop3_dophase_done(conn, connected);
    return result;
}
static CURLcode pop3_setup_connection(struct connectdata *conn) {
...
static CURLcode pop3_parse_url_options(struct connectdata *conn) {
    ...
    if(pop3c->preftype != POP3_TYPE_APOP)
        switch(pop3c->sasl.prefmech) {
            case SASL_AUTH_NONE: pop3c->preftype = POP3_TYPE_NONE; break;
            case SASL_AUTH_DEFAULT: pop3c->preftype = POP3_TYPE_ANY; break;
            default: pop3c->preftype = POP3_TYPE_SASL; break;
        }
    return result;
}
static CURLcode pop3_parse_url_path(struct connectdata *conn) {
...
static CURLcode pop3_parse_custom_request(struct connectdata *conn) {
    ...
    if(custom) result = Curl_urldecode(data, custom, 0, &pop3->custom, NULL, REJECT_CTRL);
    return result;
}
CURLcode Curl_pop3_write(struct connectdata *conn, char *str, size_t nread) {
    ...
    for(i = 0; i < nread; i++) {
        ...
        switch(str[i]) {
            case 0x0d:
                ...
                } else if(pop3c->eob == 3) pop3c->eob++;
                else pop3c->eob = 1;
                break;
            case 0x0a:
                if(pop3c->eob == 1 || pop3c->eob == 4) pop3c->eob++;
                else pop3c->eob = 0;
                break;
            case 0x2e:
                if(pop3c->eob == 2) pop3c->eob++;
                else if(pop3c->eob == 3) {
                    strip_dot = TRUE;
                    pop3c->eob = 0;
                } else pop3c->eob = 0;
                break;
            default: pop3c->eob = 0; break;
        }
    ...
    if(pop3c->eob) return CURLE_OK;
    if(nread - last) result = Curl_client_write(conn, CLIENTWRITE_BODY, &str[last],nread - last);
    return result;
}
#endif