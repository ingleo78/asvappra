...
timediff_t Curl_timeleft(struct Curl_easy *data, struct curltime *nowp, bool duringconnect) {
    ...
    struct curltime now;
    if (data->set.timeout > 0) timeout_set |= 1;
    if (duringconnect && (data->set.connecttimeout > 0)) timeout_set |= 2;
    switch(timeout_set) {
        case 1: timeout_ms = data->set.timeout; break;
        case 2: timeout_ms = data->set.connecttimeout; break;
        case 3:
            if(data->set.timeout < data->set.connecttimeout) timeout_ms = data->set.timeout;
            else timeout_ms = data->set.connecttimeout;
            break;
        default: if(!duringconnect) return 0;
    }
    if (!nowp) {
        now = Curl_now();
        nowp = &now;
    }
    if (duringconnect) timeout_ms -= Curl_timediff(*nowp, data->progress.t_startsingle);
...
static CURLcode bindlocal(struct connectdata *conn, curl_socket_t sockfd, int af, unsigned int scope) {
    ...
    if (dev && (strlen(dev) < 255) ) {
        ...
        if  (!is_host) {
                ...
                if(setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, dev, (curl_socklen_t)strlen(dev) + 1) == 0) return CURLE_OK;
            #endif
        ...
        if (!is_interface) {
            ...
            int rc;
            if (af == AF_INET) conn->ip_version = CURL_IPRESOLVE_V4;
            #ifdef ENABLE_IPV6
                else if(af == AF_INET6)
                conn->ip_version = CURL_IPRESOLVE_V6;
            #endif
            rc = Curl_resolv(conn, dev, 0, 0, &h);
            ...
            if (h) {
                ...
                if (af != h->addr->ai_family) return CURLE_UNSUPPORTED_PROTOCOL;
                done = 1;
            } else done = -1;
        }
        if(done > 0) {
            ...
            if (af == AF_INET6) {
                ...
                if(Curl_inet_pton(AF_INET6, myhost, &si6->sin6_addr) > 0) {
                    ...
                    #ifdef HAVE_SOCKADDR_IN6_SIN6_SCOPE_ID
                        if(scope_ptr) si6->sin6_scope_id = atoi(scope_ptr);
                    #endif
                }
    ...
    for( ; ; ) {
        ...
        if(--portnum > 0) {
            ...
            port++;
            if(sock->sa_family == AF_INET) si4->sin_port = ntohs(port);
        #ifdef ENABLE_IPV6
...
#endif
#endif
    if (0 != getsockopt(sockfd, SOL_SOCKET, SO_ERROR, (void *)&err, &errSize)) err = SOCKERRNO;
#ifdef _WIN32_WCE
...
#endif
    if ((0 == err) || (EISCONN == err)) rc = TRUE;
    else rc = FALSE;
    if(error) *error = err;
#else
    (void)sockfd;
    if(error) *error = SOCKERRNO;
#endif
    return rc;
}
static struct Curl_addrinfo *ainext(struct connectdata *conn, int tempindex, bool next) {
    ...
    if (ai && next) ai = ai->ai_next;
    while(ai && (ai->ai_family != conn->tempfamily[tempindex])) ai = ai->ai_next;
...
static CURLcode trynextip(struct connectdata *conn, int sockindex, int tempindex) {
    ...
    if(fd_to_close != CURL_SOCKET_BAD) Curl_closesocket(conn, fd_to_close);
    return result;
}
...
static CURLcode connect_SOCKS(struct connectdata *conn, int sockindex, bool *done) {
    ...
    if(conn->bits.socksproxy) {
        const char * const host = conn->bits.httpproxy ? conn->http_proxy.host.name : conn->bits.conn_to_host ? conn->conn_to_host.name :
                sockindex == SECONDARYSOCKET ? conn->secondaryhostname : conn->host.name;
        const int port = conn->bits.httpproxy ? (int)conn->http_proxy.port : sockindex == SECONDARYSOCKET ? conn->secondary_port :
                conn->bits.conn_to_port ? conn->conn_to_port : conn->remote_port;
        switch(conn->socks_proxy.proxytype) {
            case CURLPROXY_SOCKS5: case CURLPROXY_SOCKS5_HOSTNAME:
                result = Curl_SOCKS5(conn->socks_proxy.user, conn->socks_proxy.passwd, host, port, sockindex, conn, done);
                break;
            case CURLPROXY_SOCKS4: case CURLPROXY_SOCKS4A: result = Curl_SOCKS4(conn->socks_proxy.user, host, port, sockindex,conn, done); break;
            default:
...
static void post_SOCKS(struct connectdata *conn, int sockindex, bool *connected) {
    ...
    *connected = 1;
    if(sockindex == FIRSTSOCKET) Curl_pgrsTime(conn->data, TIMER_CONNECT);
...
CURLcode Curl_is_connected(struct connectdata *conn, int sockindex, bool *connected) {
    ...
    *connected = 0;
    ...
    if (SOCKS_STATE(conn->cnnct.state)) {
        ...
        if (!result && *connected) post_SOCKS(conn, sockindex, connected);
        return result;
    }
    for(i = 0; i<2; i++) {
    const int other = i ^ 1;
    if(conn->tempsock[i] == CURL_SOCKET_BAD) continue;
    error = 0;
#ifdef ENABLE_QUIC
      ...
      infof(data, "Connection failed\n");
        ...
        if(error) {
            ...
            if(conn->tempaddr[i]) {
                ...
                if ((status != CURLE_COULDNT_CONNECT) || conn->tempsock[other] == CURL_SOCKET_BAD) result = status;
            }
        }
    }
    ...
    #ifndef CURL_DISABLE_PROXY
        if (conn->bits.socksproxy) hostname = conn->socks_proxy.host.name;
        else if (conn->bits.httpproxy) hostname = conn->http_proxy.host.name;
        else
    #endif
    ...
    #ifdef WSAETIMEDOUT
        if (WSAETIMEDOUT == data->state.os_errno) result = CURLE_OPERATION_TIMEDOUT;
    #elif defined(ETIMEDOUT)
        if (ETIMEDOUT == data->state.os_errno) result = CURLE_OPERATION_TIMEDOUT;
    #endif
...
void Curl_sndbufset(curl_socket_t sockfd) {
    ...
    if(detectOsState == DETECT_OS_VISTA_OR_LATER) return;
    ...
    *sockp = CURL_SOCKET_BAD;
...
#endif
    if (is_tcp && data->set.tcp_nodelay) tcpnodelay(conn, sockfd);
    nosigpipe(conn, sockfd);
    Curl_sndbufset(sockfd);
    if (is_tcp && data->set.tcp_keepalive) tcpkeepalive(data, sockfd);
    if (data->set.fsockopt) {
        Curl_set_in_callback(data, true);
        error = data->set.fsockopt(data->set.sockopt_client, sockfd, CURLSOCKTYPE_IPCXN);
        Curl_set_in_callback(data, false);
        if (error == CURL_SOCKOPT_ALREADY_CONNECTED) isconnected = 1;
        else if(error) {
            Curl_closesocket(conn, sockfd);
    ...
    if(addr.family == AF_INET
       ...
       || addr.family == AF_INET6
        ...
        if(result) {
            Curl_closesocket(conn, sockfd);
            if(result == CURLE_UNSUPPORTED_PROTOCOL) return CURLE_COULDNT_CONNECT;
            return result;
        }
    }
    ...
    if (!isconnected && (conn->transport != TRNSPRT_UDP)) {
        if (conn->bits.tcp_fastopen) {
            ...
            } else rc = connect(sockfd, &addr.sa_addr, addr.addrlen);
        #else
            ...
            if(conn->given->flags & PROTOPT_SSL) rc = connect(sockfd, &addr.sa_addr, addr.addrlen);
            else rc = 0;
        #endif
        } else rc = connect(sockfd, &addr.sa_addr, addr.addrlen);
        if (-1 == rc) error = SOCKERRNO;
    #ifdef ENABLE_QUIC
...
CURLcode Curl_connecthost(struct connectdata *conn, const struct Curl_dns_entry *remotehost) {
    ...
    CURLcode result = CURLE_COULDNT_CONNECT;
    int i;
    timediff_t timeout_ms = Curl_timeleft(data, &before, 1);
    if (timeout_ms < 0) {
        failf(data, "Connection time-out");
        return CURLE_OPERATION_TIMEDOUT;
    }
...
bool Curl_connalive(struct connectdata *conn) {
    if(conn->ssl[FIRSTSOCKET].use) {
        if(!Curl_ssl_check_cxn(conn)) return false;
    }
#ifdef MSG_PEEK
    else if(conn->sock[FIRSTSOCKET] == CURL_SOCKET_BAD) return false;
    else {
        char buf;
        if(recv((RECV_TYPE_ARG1)conn->sock[FIRSTSOCKET], (RECV_TYPE_ARG2)&buf, (RECV_TYPE_ARG3)1, (RECV_TYPE_ARG4)MSG_PEEK) == 0) return false;
    }
#endif
    return true;
}
int Curl_closesocket(struct connectdata *conn, curl_socket_t sock) {
    if(conn && conn->fclosesocket) {
        if((sock == conn->sock[SECONDARYSOCKET]) && conn->bits.sock_accepted) conn->bits.sock_accepted = 0;
        else {
            int rc;
    ...
    if(conn) Curl_multi_closed(conn->data, sock);
    close(sock);
    return 0;
}
CURLcode Curl_socket(struct connectdata *conn, const struct Curl_addrinfo *ai, struct Curl_sockaddr_ex *addr, curl_socket_t *sockfd) {
    ...
    if (addr->addrlen > sizeof(struct Curl_sockaddr_storage)) addr->addrlen = sizeof(struct Curl_sockaddr_storage);
    ...
    if(*sockfd == CURL_SOCKET_BAD) return CURLE_COULDNT_CONNECT;
    if(conn->transport == TRNSPRT_QUIC) (void)curlx_nonblock(*sockfd, 1);
...
void Curl_conncontrol(struct connectdata *conn, int ctrl
    ...
    DEBUGASSERT(conn);
    if((ctrl == CONNCTRL_STREAM) && (conn->handler->flags & PROTOPT_STREAM)) DEBUGF(infof(conn->data, "Kill stream: %s\n", reason));
...
bool Curl_conn_data_pending(struct connectdata *conn, int sockindex) {
    int readable;
    DEBUGASSERT(conn);
    if (Curl_ssl_data_pending(conn, sockindex) || Curl_recv_has_postponed_data(conn, sockindex)) return true;
