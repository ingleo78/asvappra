...
static void showtime(struct Curl_easy *data, const char *text, time_t stamp) {
    ...
    if(result) return;
    msnprintf(str, sizeof(str), "\t %s: %s, %02d %s %4d %02d:%02d:%02d GMT", text, Curl_wkday[tm->tm_wday?tm->tm_wday-1:6], tm->tm_mday, Curl_month[tm->tm_mon],
...
static gnutls_datum_t load_file(const char *file) {
    ...
    void *ptr;
    f = fopen(file, "rb");
    if(!f) return loaded_file;
    if(fseek(f, 0, SEEK_END) != 0 || (filelen = ftell(f)) < 0 || fseek(f, 0, SEEK_SET) != 0 || !(ptr = malloc((size_t)filelen))) goto out;
...
static CURLcode handshake(struct connectdata *conn, int sockindex, bool duringconnect, bool nonblocking) {
    ...
    for( ; ; ) {
        ...
        if((rc == GNUTLS_E_AGAIN) || (rc == GNUTLS_E_INTERRUPTED)) {
            ...
            if(strerr == NULL) strerr = gnutls_strerror(rc);
...
static gnutls_x509_crt_fmt_t do_file_type(const char *type) {
    if(!type || !type[0]) return GNUTLS_X509_FMT_PEM;
    if(strcasecompare(type, "PEM")) return GNUTLS_X509_FMT_PEM;
    if(strcasecompare(type, "DER")) return GNUTLS_X509_FMT_DER;
    return -1;
}
...
static CURLcode gtls_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    if(connssl->state == ssl_connection_complete) return CURLE_OK;
    if(!gtls_inited) Curl_gtls_init();
    *certverifyresult = 0;
    if(SSL_CONN_CONFIG(version) == CURL_SSLVERSION_SSLv2) {
    failf(data, "GnuTLS does not support SSLv2");
    return CURLE_SSL_CONNECT_ERROR;
    } else if(SSL_CONN_CONFIG(version) == CURL_SSLVERSION_SSLv3) sni = FALSE;
    ...
    if(SSL_CONN_CONFIG(verifypeer) && !(SSL_CONN_CONFIG(CAfile) || SSL_CONN_CONFIG(CApath))) gnutls_certificate_set_x509_system_trust(backend->cred);
#endif
    ...
    if(rc != GNUTLS_E_SUCCESS) return CURLE_SSL_CONNECT_ERROR;
    switch(SSL_CONN_CONFIG(version)) {
    case CURL_SSLVERSION_SSLv3: prioritylist = GNUTLS_CIPHERS ":-VERS-TLS-ALL:+VERS-SSL3.0"; break;
    case CURL_SSLVERSION_DEFAULT:
    case CURL_SSLVERSION_TLSv1: prioritylist = GNUTLS_CIPHERS ":-VERS-SSL3.0"
    #ifdef HAS_TLS13
        ":+VERS-TLS1.3"
    #endif
        ;
    ...
    if(SSL_SET_OPTION(authtype) == CURL_TLSAUTH_SRP) {
        ...
        if(!prioritysrp) return CURLE_OUT_OF_MEMORY;
        strcpy(prioritysrp, prioritylist);
        strcpy(prioritysrp + len, ":" GNUTLS_SRP);
        rc = gnutls_priority_set_direct(session, prioritysrp, &err);
        free(prioritysrp);
        if((rc == GNUTLS_E_INVALID_REQUEST) && err) infof(data, "This GnuTLS does not support SRP\n");
    } else {
#endif
...
static CURLcode pkp_pin_peer_pubkey(struct Curl_easy *data, gnutls_x509_crt_t cert, const char *pinnedpubkey) {
    ...
    do {
        ...
        if(ret != GNUTLS_E_SHORT_MEMORY_BUFFER || len1 == 0) break;
        buff1 = malloc(len1);
        if(NULL == buff1) break;
        len2 = len1;
        ret = gnutls_pubkey_export(key, GNUTLS_X509_FMT_DER, buff1, &len2);
        if(ret < 0 || len1 != len2) break;
        result = Curl_pin_peer_pubkey(data, pinnedpubkey, buff1, len1);
    } while(0);
    if(NULL != key) gnutls_pubkey_deinit(key);
    Curl_safefree(buff1);
    return result;
}
...
static CURLcode gtls_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    if(SSL_CONN_CONFIG(verifystatus)) {
        if(gnutls_ocsp_status_request_is_checked(session, 0) == 0) {
            ...
            switch(status) {
                case GNUTLS_OCSP_CERT_GOOD: break;
                case GNUTLS_OCSP_CERT_REVOKED: {
                        const char *crl_reason;
                        switch(reason) {
                            case GNUTLS_X509_CRLREASON_UNSPECIFIED: crl_reason = "unspecified reason"; break;
                            case GNUTLS_X509_CRLREASON_KEYCOMPROMISE: crl_reason = "private key compromised"; break;
                            case GNUTLS_X509_CRLREASON_CACOMPROMISE: crl_reason = "CA compromised"; break;
                            case GNUTLS_X509_CRLREASON_AFFILIATIONCHANGED: crl_reason = "affiliation has changed"; break;
                            case GNUTLS_X509_CRLREASON_SUPERSEDED: crl_reason = "certificate superseded"; break;
                            case GNUTLS_X509_CRLREASON_CESSATIONOFOPERATION: crl_reason = "operation has ceased"; break;
                            case GNUTLS_X509_CRLREASON_CERTIFICATEHOLD: crl_reason = "certificate is on hold"; break;
                            case GNUTLS_X509_CRLREASON_REMOVEFROMCRL: crl_reason = "will be removed from delta CRL"; break;
                            case GNUTLS_X509_CRLREASON_PRIVILEGEWITHDRAWN: crl_reason = "privilege withdrawn"; break;
                            case GNUTLS_X509_CRLREASON_AACOMPROMISE: crl_reason = "AA compromised"; break;
                            default: crl_reason = "unspecified reason";
                        }
                        failf(data, "Server certificate was revoked: %s", crl_reason);
                        break;
                    }
                default: failf(data, "Server certificate status is unknown");
            }
    ...
    if(chainp) gnutls_x509_crt_import(x509_cert, chainp, GNUTLS_X509_FMT_DER);
    ...
    if(rc) infof(data, "error fetching CN from cert:%s\n", gnutls_strerror(rc));
    ...
    if(!rc) {
        ...
        size_t addrlen = 0;
        if(Curl_inet_pton(AF_INET, hostname, addrbuf) > 0) addrlen = 4;
    #ifdef ENABLE_IPV6
        else if(Curl_inet_pton(AF_INET6, hostname, addrbuf) > 0) addrlen = 16;
    #endif
        if(addrlen) {
            ...
            for(i = 0; ; i++) {
                ...
                if(ret == GNUTLS_E_SHORT_MEMORY_BUFFER) continue;
                if(ret < 0) break;
                if(ret != GNUTLS_SAN_IPADDRESS) continue;
    ...
    if(!rc) {
        ...
        if(SSL_CONN_CONFIG(verifyhost)) {
            failf(data, "SSL: certificate subject name (%s) does not match target host name '%s'", certname, dispname);
            gnutls_x509_crt_deinit(x509_cert);
            return CURLE_PEER_FAILED_VERIFICATION;
    ...
    if(rc != 0) return CURLE_OUT_OF_MEMORY;
    ...
    if(conn->bits.tls_enable_alpn) {
        ...
        #endif
            if(proto.size == ALPN_HTTP_1_1_LENGTH && !memcmp(ALPN_HTTP_1_1, proto.data, ALPN_HTTP_1_1_LENGTH)) conn->negnpn = CURL_HTTP_VERSION_1_1;
    ...
    if(SSL_SET_OPTION(primary.sessionid)) {
        ...
        if(connect_sessionid) {
            ...
            if(incache) Curl_ssl_delsessionid(conn, ssl_sessionid);
...
static bool Curl_gtls_data_pending(const struct connectdata *conn, int connindex) {
    ...
    bool res = FALSE;
    struct ssl_backend_data *backend = connssl->backend;
    if(backend->session && 0 != gnutls_record_check_pending(backend->session)) res = TRUE;
...
static int Curl_gtls_shutdown(struct connectdata *conn, int sockindex) {
    ...
    if(data->set.ftp_ccc == CURLFTPSSL_CCC_ACTIVE) gnutls_bye(backend->session, GNUTLS_SHUT_WR);
#endif
    if(backend->session) {
        ssize_t result;
        bool done = FALSE;
        ...
        while(!done) {
            ...
            if(what > 0) {
                ...
                switch(result) {
                    ...
                    case GNUTLS_E_AGAIN:
                    case GNUTLS_E_INTERRUPTED: infof(data, "GNUTLS_E_AGAIN || GNUTLS_E_INTERRUPTED\n"); break;
                    default:
                        retval = -1;
                        done = TRUE;
                        break;
...
static ssize_t gtls_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
    ...
    if(ret == GNUTLS_E_REHANDSHAKE) {
        ...
        if(result) *curlcode = result;
        else *curlcode = CURLE_AGAIN; /* then return as if this was a wouldblock */
        return -1;
    }
    if(ret < 0) {
...
static int Curl_gtls_seed(struct Curl_easy *data) {
  ...
  if(!ssl_seeded || data->set.str[STRING_SSL_RANDOM_FILE] || data->set.str[STRING_SSL_EGDSOCKET]) ssl_seeded = TRUE;
  return 0;
}
#endif
static CURLcode Curl_gtls_random(struct Curl_easy *data, unsigned char *entropy, size_t length) {
    ...
    if(data) Curl_gtls_seed(data); /* Initiate the seed if not already done */
    gcry_randomize(entropy, length, GCRY_STRONG_RANDOM);
#endif
    return CURLE_OK;
}
static CURLcode Curl_gtls_md5sum(unsigned char *tmp, size_t tmplen, unsigned char *md5sum, size_t md5len) {
#if defined(USE_GNUTLS_NETTLE)
...
static CURLcode Curl_gtls_sha256sum(const unsigned char *tmp, size_t tmplen, unsigned char *sha256sum, size_t sha256len) {
#if defined(USE_GNUTLS_NETTLE)
