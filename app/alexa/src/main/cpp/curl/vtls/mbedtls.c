...
static CURLcode
mbed_connect_step1(struct connectdata *conn, int sockindex) {
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
    ...
    if(ssl_cafile) {
        ...
        if(ret<0) {
            ...
            if(verifypeer) return CURLE_SSL_CACERT_BADFILE;
        }
    }
    if(ssl_capath) {
    ...
    if(SSL_SET_OPTION(key)) {
        ...
        if(ret == 0 && !(mbedtls_pk_can_do(&backend->pk, MBEDTLS_PK_RSA) || mbedtls_pk_can_do(&backend->pk, MBEDTLS_PK_ECKEY))) ret = MBEDTLS_ERR_PK_TYPE_MISMATCH;
        if(ret) {
        #ifdef MBEDTLS_ERROR_C
            mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
        #endif
    ...
    switch(SSL_CONN_CONFIG(version)) {
        ...
        break;
        case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1: CURL_SSLVERSION_TLSv1_2: CURL_SSLVERSION_TLSv1_3:
    ...
    if(SSL_SET_OPTION(key)) mbedtls_ssl_conf_own_cert(&backend->config, &backend->clicert, &backend->pk);
    ...
    #ifdef USE_NGHTTP2
        if(data->set.httpversion >= CURL_HTTP_VERSION_2) *p++ = NGHTTP2_PROTO_VERSION_ID;
    #endif
        *p++ = ALPN_HTTP_1_1;
        *p = NULL;
        ...
        for(p = &backend->protocols[0]; *p; ++p) infof(data, "ALPN, offering %s\n", *p);
    }
#endif
#ifdef MBEDTLS_DEBUG
...
static CURLcode mbed_connect_step2(struct connectdata *conn, int sockindex) {
    ...
    if(!SSL_CONN_CONFIG(verifyhost)) ret &= ~MBEDTLS_X509_BADCERT_CN_MISMATCH;
    if(ret && SSL_CONN_CONFIG(verifypeer)) {
        if(ret & MBEDTLS_X509_BADCERT_EXPIRED) failf(data, "Cert verify failed: BADCERT_EXPIRED");
        else if(ret & MBEDTLS_X509_BADCERT_REVOKED) failf(data, "Cert verify failed: BADCERT_REVOKED");
        else if(ret & MBEDTLS_X509_BADCERT_CN_MISMATCH) failf(data, "Cert verify failed: BADCERT_CN_MISMATCH");
        else if(ret & MBEDTLS_X509_BADCERT_NOT_TRUSTED) failf(data, "Cert verify failed: BADCERT_NOT_TRUSTED");
        else if(ret & MBEDTLS_X509_BADCERT_FUTURE) failf(data, "Cert verify failed: BADCERT_FUTURE");
    ...
    if(conn->bits.tls_enable_alpn) {
        ...
        #endif
            if(!strncmp(next_protocol, ALPN_HTTP_1_1, ALPN_HTTP_1_1_LENGTH) && !next_protocol[ALPN_HTTP_1_1_LENGTH]) conn->negnpn = CURL_HTTP_VERSION_1_1;
...
static CURLcode mbed_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    if(SSL_SET_OPTION(primary.sessionid)) {
        ...
        if(ret) {
            if(ret != MBEDTLS_ERR_SSL_ALLOC_FAILED) mbedtls_ssl_session_free(our_ssl_sessionid);
            free(our_ssl_sessionid);
            failf(data, "mbedtls_ssl_get_session returned -0x%x", -ret);
            return CURLE_SSL_CONNECT_ERROR;
        }
        Curl_ssl_sessionid_lock(conn);
        if(!Curl_ssl_getsessionid(conn, &old_ssl_sessionid, NULL, sockindex)) Curl_ssl_delsessionid(conn, old_ssl_sessionid);
...
static CURLcode mbed_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    ...
    while(ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state || ssl_connect_2_writing == connssl->connecting_state) {
        ...
        if(retcode || (nonblocking && (ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state ||
...
#else
    if(mbedtls_sha256_ret(input, inputlen, sha256sum, 0) != 0) return CURLE_BAD_FUNCTION_ARGUMENT;
#endif
    return CURLE_OK;
}
