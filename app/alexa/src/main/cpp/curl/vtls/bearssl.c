...
static void append_dn(void *ctx, const void *buf, size_t len) {
    ...
    if(ca->err != CURLE_OK || !ca->in_cert) return;
...
static void x509_push(void *ctx, const void *buf, size_t len) {
    struct cafile_parser *ca = ctx;
    if(ca->in_cert) br_x509_decoder_push(&ca->xc, buf, len);
}
static CURLcode load_cafile(const char *path, br_x509_trust_anchor **anchors, size_t *anchors_len) {
    ...
    size_t n, i, pushed;
    fp = fopen(path, "rb");
    if(!fp) return CURLE_SSL_CACERT_BADFILE;
    ca.err = CURLE_OK;
    ca.in_cert = FALSE;
    ca.anchors = NULL;
    ca.anchors_len = 0;
        ...
        while(n) {
            ...
            switch(br_pem_decoder_event(&pc)) {
                ...
                case BR_PEM_BEGIN_OBJ: {
                        ...
                        if(strcmp(name, "CERTIFICATE") && strcmp(name, "X509 CERTIFICATE")) break;
                ...
                case BR_PEM_END_OBJ: {
                        ...
                        ta->flags = 0;
                        if(br_x509_decoder_isCA(&ca.xc)) ta->flags |= BR_X509_TA_CA;
                        ...
                        case BR_KEYTYPE_RSA: ta_size += pkey->key.rsa.nlen + pkey->key.rsa.elen; break;
                        case BR_KEYTYPE_EC: ta_size += pkey->key.ec.qlen; break;
                        default: ca.err = CURLE_FAILED_INIT; goto fail;
                        }
                    ...
                    break;
                default: ca.err = CURLE_SSL_CACERT_BADFILE; goto fail;
            }
        }
    }
    if(ferror(fp)) ca.err = CURLE_READ_ERROR;
fail:
    fclose(fp);
    if(ca.err == CURLE_OK) {
        *anchors = ca.anchors;
        *anchors_len = ca.anchors_len;
        ...
        for(i = 0; i < ca.anchors_len; ++i) free(ca.anchors[i].dn.data);
        free(ca.anchors);
    }
    return ca.err;
}
static void x509_start_chain(const br_x509_class **ctx, const char *server_name) {
    ...
    if(!x509->verifyhost) server_name = NULL;
...
static unsigned x509_end_chain(const br_x509_class **ctx) {
    ...
    if(err && !x509->verifypeer) err = BR_ERR_OK;
    return err;
}
...
static CURLcode bearssl_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    if(!br_ssl_client_reset(&backend->ctx, hostname, 0)) return CURLE_FAILED_INIT;
    backend->active = TRUE;
...
static CURLcode bearssl_run_until(struct connectdata *conn, int sockindex, unsigned target) {
    ...
    for( ; ; ) {
        ...
        if(state & target) return CURLE_OK;
        if(state & BR_SSL_SENDREC) {
            buf = br_ssl_engine_sendrec_buf(&backend->ctx.eng, &len);
            ret = swrite(sockfd, buf, len);
            if(ret == -1) {
                if(SOCKERRNO == EAGAIN || SOCKERRNO == EWOULDBLOCK) {
                    if(connssl->state != ssl_connection_complete) connssl->connecting_state = ssl_connect_2_writing;
                    return CURLE_AGAIN;
                }
            ...
            if(ret == -1) {
                if(SOCKERRNO == EAGAIN || SOCKERRNO == EWOULDBLOCK) {
                    if(connssl->state != ssl_connection_complete) connssl->connecting_state = ssl_connect_2_reading;
                    return CURLE_AGAIN;
                }
                return CURLE_READ_ERROR;
            }
...
static CURLcode bearssl_connect_step2(struct connectdata *conn, int sockindex) {
    ...
    if(ret == CURLE_AGAIN) return CURLE_OK;
    if(ret == CURLE_OK) {
...
static CURLcode bearssl_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    if(conn->bits.tls_enable_alpn) {
        ...
        #ifdef USE_NGHTTP2
            if(!strcmp(protocol, NGHTTP2_PROTO_VERSION_ID)) conn->negnpn = CURL_HTTP_VERSION_2;
            else
        #endif
            if(!strcmp(protocol, ALPN_HTTP_1_1)) conn->negnpn = CURL_HTTP_VERSION_1_1;
    ...
    if(SSL_SET_OPTION(primary.sessionid)) {
        ...
        if(incache) Curl_ssl_delsessionid(conn, oldsession);
...
static ssize_t bearssl_send(struct connectdata *conn, int sockindex, const void *buf, size_t len, CURLcode *err) {
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
    unsigned char *app;
...
static ssize_t bearssl_recv(struct connectdata *conn, int sockindex, char *buf, size_t len, CURLcode *err) {
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
    unsigned char *app;
    size_t applen;
...
static CURLcode bearssl_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    CURLcode ret;
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    curl_socket_t sockfd = conn->sock[sockindex];
    ...
    if(ret || (nonblocking && (ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state ||
...
static CURLcode Curl_bearssl_random(struct Curl_easy *data UNUSED_PARAM, unsigned char *entropy, size_t length) {
    ...
    if(!seeded) {
        ...
        if(!seeder || !seeder(&ctx.vtable)) return CURLE_FAILED_INIT;
        seeded = TRUE;
    }
...
static CURLcode Curl_bearssl_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
    return bearssl_connect_common(conn, sockindex, TRUE, done);
}
static void *Curl_bearssl_get_internals(struct ssl_connect_data *connssl, CURLINFO info UNUSED_PARAM) {
    struct ssl_backend_data *backend = connssl->backend;
    return &backend->ctx;
}
static void Curl_bearssl_close(struct connectdata *conn, int sockindex) {
    ...
    for(i = 0; i < backend->anchors_len; ++i) free(backend->anchors[i].dn.data);
    free(backend->anchors);
}
static void Curl_bearssl_session_free(void *ptr) {
    free(ptr);
}
static CURLcode Curl_bearssl_md5sum(unsigned char *input, size_t inputlen, unsigned char *md5sum, size_t md5len UNUSED_PARAM) {
    br_md5_context ctx;
    br_md5_init(&ctx);
...
static CURLcode Curl_bearssl_sha256sum(const unsigned char *input, size_t inputlen, unsigned char *sha256sum, size_t sha256len UNUSED_PARAM) {
