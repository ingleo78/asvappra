...
static OSStatus SocketRead(SSLConnectionRef connection, void *data, size_t *dataLength) {
    ...
    *dataLength = 0;
    for( ; ; ) {
        ...
        if(rrtn <= 0) {
            theErr = errno;
            if(rrtn == 0) rtn = errSSLClosedGraceful;
            else
            switch(theErr) {
                case ENOENT: rtn = errSSLClosedGraceful; break;
                case ECONNRESET: rtn = errSSLClosedAbort; break;
                case EAGAIN:
...
CF_INLINE const char *TLSCipherNameForNumber(SSLCipherSuite cipher) {
    switch(cipher) {
        ...
        case TLS_RSA_PSK_WITH_AES_256_GCM_SHA384: return "TLS_PSK_WITH_AES_256_GCM_SHA384";
        ...
        case TLS_DHE_PSK_WITH_NULL_SHA384: return "TLS_RSA_PSK_WITH_NULL_SHA384";
...
CF_INLINE void GetDarwinVersionNumber(int *major, int *minor) {
    ...
    if(sysctl(mib, 2, NULL, &os_version_len, NULL, 0) == -1) return;
    os_version = malloc(os_version_len*sizeof(char));
    if(!os_version) return;
...
CF_INLINE CFStringRef getsubject(SecCertificateRef cert) {
    ...
    if(SecCertificateCopyLongDescription != NULL) server_cert_summary = SecCertificateCopyLongDescription(NULL, cert, NULL);
    else
#endif
#if CURL_BUILD_MAC_10_6
    if(SecCertificateCopySubjectSummary != NULL) server_cert_summary = SecCertificateCopySubjectSummary(cert);
    else
#endif
...
static OSStatus CopyIdentityWithLabel(char *label, SecIdentityRef *out_cert_and_key) {
    ...
    if(SecItemCopyMatching != NULL && kSecClassIdentity != NULL) {
        ...
        query_dict = CFDictionaryCreate(NULL, (const void **)keys, (const void **)values, 5L, &kCFCopyStringDictionaryKeyCallBacks,
        ...
        if(keys_list) CFRelease(keys_list);
...
CF_INLINE bool is_file(const char *filename) {
    ...
    if(stat(filename, &st) == 0) return S_ISREG(st.st_mode);
    return false;
}
#if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
...
static CURLcode set_ssl_version_min_max(struct connectdata *conn, int sockindex) {
...
#if (CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11) && HAVE_BUILTIN_AVAILABLE == 1
    if(__builtin_available(macOS 10.13, iOS 11.0, *)) max_supported_version_by_os = CURL_SSLVERSION_MAX_TLSv1_3;
    else max_supported_version_by_os = CURL_SSLVERSION_MAX_TLSv1_2;
#else
    max_supported_version_by_os = CURL_SSLVERSION_MAX_TLSv1_2;
#endif
    switch(ssl_version) {
        case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1: ssl_version = CURL_SSLVERSION_TLSv1_0;
    }
    switch(ssl_version_max) {
        case CURL_SSLVERSION_MAX_NONE: case CURL_SSLVERSION_MAX_DEFAULT: ssl_version_max = max_supported_version_by_os;
    }
#if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
    if(SSLSetProtocolVersionMax != NULL) {
        ...
        for(; i <= (ssl_version_max >> 16); i++) {
            switch(i) {
                case CURL_SSLVERSION_TLSv1_0: (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol1, true); break;
                case CURL_SSLVERSION_TLSv1_1: (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol11, true); break;
                case CURL_SSLVERSION_TLSv1_2: (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol12, true); break;
                case CURL_SSLVERSION_TLSv1_3:
...
static CURLcode sectransp_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    if(SSLCreateContext != NULL) {
        if(backend->ssl_ctx) CFRelease(backend->ssl_ctx);
    ...
    #if CURL_SUPPORT_MAC_10_8
        if(backend->ssl_ctx) (void)SSLDisposeContext(backend->ssl_ctx);
    ...
    if(SSLSetProtocolVersionMax != NULL) {
        switch(conn->ssl_config.version) {
            ...
            #if (CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11) && HAVE_BUILTIN_AVAILABLE == 1
                if(__builtin_available(macOS 10.13, iOS 11.0, *)) (void)SSLSetProtocolVersionMax(backend->ssl_ctx, kTLSProtocol13);
                else (void)SSLSetProtocolVersionMax(backend->ssl_ctx, kTLSProtocol12);
            #else
                (void)SSLSetProtocolVersionMax(backend->ssl_ctx, kTLSProtocol12);
            #endif
                break;
            case CURL_SSLVERSION_DEFALUT: case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1: case CURL_SSLVERSION_TLSv1_2: case CURL_SSLVERSION_TLSv1_3:
        ...
        switch(conn->ssl_config.version) {
            case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1:
                ...
                break;
            case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1: case CURL_SSLVERSION_TLSv1_2: case CURL_SSLVERSION_TLSv1_3:
                CURLcode result = set_ssl_version_min_max(conn, sockindex);
                if(result != CURLE_OK) return result;
                break;
            case CURL_SSLVERSION_SSLv3:
    ...
    (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kSSLProtocolAll, false);
    switch(conn->ssl_config.version) {
        case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_TLSv1_0:
            (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol1, true);
            break;
        case CURL_SSLVERSION_TLSv1_1:
...
#endif
    if(SSL_SET_OPTION(key)) infof(data, "WARNING: SSL: CURLOPT_SSLKEY is ignored by Secure Transport. The private key must be in the Keychain.\n");
    if(ssl_cert || ssl_cert_blob) {
        ...
        if(err == noErr && cert_and_key) {
            ...
            if(err == noErr) {
                ...
                CFRelease(cert);
                if(result == CURLE_PEER_FAILED_VERIFICATION) return CURLE_SSL_CERTPROBLEM;
    ...
    if(conn->ssl_config.verifyhost) {
        ...
        strlen(hostname));
        if(err != noErr) infof(data, "WARNING: SSL: SSLSetPeerDomainName() failed: OSStatus %d\n", err);
    ...
    #if CURL_BUILD_MAC
        if(darwinver_maj == 12 && darwinver_min <= 3 && all_ciphers[i] >= 0xC001 && all_ciphers[i] <= 0xC032) continue;
    #endif
...
static long pem_to_der(const char *in, unsigned char **out, size_t *outlen) {
    ...
    unsigned char *b64;
    sep_start = strstr(in, "-----");
    if(sep_start == NULL) return 0;
    cert_start = strstr(sep_start + 1, "-----");
    if(cert_start == NULL) return -1;
    cert_start += 5;
    cert_end = strstr(cert_start, "-----");
    if(cert_end == NULL) return -1;
    sep_end = strstr(cert_end + 1, "-----");
    if(sep_end == NULL) return -1;
    sep_end += 5;
...
static int append_cert_to_array(struct Curl_easy *data, unsigned char *buf, size_t buflen, CFMutableArrayRef array) {
    ...
    switch(result) {
        case CURLE_OK: break;
        case CURLE_PEER_FAILED_VERIFICATION: return CURLE_SSL_CACERT_BADFILE;
        case CURLE_OUT_OF_MEMORY: default: return result;
    }
    free(certp);
...
static CURLcode verify_cert(const char *cafile, struct Curl_easy *data, SSLContextRef ctx) {
    ...
    switch(trust_eval) {
        case kSecTrustResultUnspecified: case kSecTrustResultProceed: return CURLE_OK;
        case kSecTrustResultRecoverableTrustFailure: case kSecTrustResultDeny: default:
...
static CURLcode pkp_pin_peer_pubkey(struct Curl_easy *data, SSLContextRef ctx, const char *pinnedpubkey) {
    ...
    do {
        SecTrustRef trust;
        OSStatus ret = SSLCopyPeerTrust(ctx, &trust);
        if(ret != noErr || trust == NULL) break;
        SecKeyRef keyRef = SecTrustCopyPublicKey(trust);
        CFRelease(trust);
        if(keyRef == NULL) break;
    #ifdef SECTRANSP_PINNEDPUBKEY_V1
        publicKeyBits = SecKeyCopyExternalRepresentation(keyRef, NULL);
        CFRelease(keyRef);
        if(publicKeyBits == NULL) break;
    #elif SECTRANSP_PINNEDPUBKEY_V2
        OSStatus success = SecItemExport(keyRef, kSecFormatOpenSSL, 0, NULL, &publicKeyBits);
        CFRelease(keyRef);
        if(success != errSecSuccess || publicKeyBits == NULL) break;
    #endif
        pubkeylen = CFDataGetLength(publicKeyBits);
        pubkey = (unsigned char *)CFDataGetBytePtr(publicKeyBits);
        switch(pubkeylen) {
            case 526: spkiHeader = rsa4096SpkiHeader; break;
            case 270: spkiHeader = rsa2048SpkiHeader; break;
        #ifdef SECTRANSP_PINNEDPUBKEY_V1
            case 65:
            ...
            case 97:
                ...
                spkiHeaderLength = 23;
                break;
            default: infof(data, "SSL: unhandled public key length: %d\n", pubkeylen);
        #elif SECTRANSP_PINNEDPUBKEY_V2
            default: result = Curl_pin_peer_pubkey(data, pinnedpubkey, pubkey, pubkeylen);
        #endif
            continue;
        }
    ...
    if(publicKeyBits != NULL) CFRelease(publicKeyBits);
    return result;
}
#endif
static CURLcode sectransp_connect_step2(struct connectdata *conn, int sockindex) {
    ...
    if(err != noErr) {
        switch(err) {
            ...
            case errSSLPeerDecodeError: failf(data, "Decode failed"); break;
            case errSSLDecryptionFail: case errSSLPeerDecryptionFail: failf(data, "Decryption failed"); break;
            case errSSLPeerDecryptError: failf(data, "A decryption error occurred"); break;
            case errSSLBadCipherSuite: failf(data, "A bad SSL cipher suite was encountered"); break;
            case errSSLCrypto: failf(data, "An underlying cryptographic error was encountered"); break;
        #if CURL_BUILD_MAC_10_11 || CURL_BUILD_IOS_9
            case errSSLWeakPeerEphemeralDHKey: failf(data, "Indicates a weak ephemeral Diffie-Hellman key"); break;
        #endif
            case errSSLBadRecordMac: case errSSLPeerBadRecordMac:
            failf(data, "A record with a bad message authentication code (MAC) was encountered");
            break;
            case errSSLRecordOverflow: case errSSLPeerRecordOverflow: failf(data, "A record overflow occurred"); break;
            case errSSLPeerDecompressFail: failf(data, "Decompression failed"); break;
            case errSSLPeerAccessDenied: failf(data, "Access was denied"); break;
            case errSSLPeerInsufficientSecurity: failf(data, "There is insufficient security for this operation"); break;
            ...
            case errSSLCertNotYetValid:
                ...
                return CURLE_PEER_FAILED_VERIFICATION;
            case errSSLCertExpired: case errSSLPeerCertExpired:
            ...
            case errSSLPeerCertUnknown:
                ...
                return CURLE_PEER_FAILED_VERIFICATION;
            case errSecAuthFailed: failf(data, "SSL authentication failed"); break;
            ...
            case errSSLHostNameMismatch:
                ...
                return CURLE_PEER_FAILED_VERIFICATION;
            case errSSLNegotiation: failf(data, "Could not negotiate an SSL cipher suite with the server"); break;
            case errSSLBadConfiguration: failf(data, "A configuration error occurred"); break;
            case errSSLProtocol: failf(data, "SSL protocol error"); break;
            case errSSLPeerProtocolVersion: failf(data, "A bad protocol version was encountered"); break;
            case errSSLPeerNoRenegotiation: failf(data, "No renegotiation is allowed"); break;
            case errSSLConnectionRefused: failf(data, "Server dropped the connection during the SSL handshake"); break;
            case errSSLClosedAbort: failf(data, "Server aborted the SSL handshake"); break;
            case errSSLClosedGraceful: failf(data, "The connection closed gracefully"); break;
            case errSSLClosedNoNotify: failf(data, "The server closed the session with no notification"); break;
            case paramErr: case errSSLInternal: case errSSLPeerInternalError:
            failf(data, "Internal SSL engine error encountered during the SSL handshake");
            break;
            case errSSLFatalAlert: failf(data, "Fatal SSL engine error encountered during the SSL handshake"); break;
            case errSSLBufferOverflow: failf(data, "An insufficient buffer was provided"); break;
            case errSSLIllegalParam: failf(data, "An illegal parameter was encountered"); break;
            case errSSLModuleAttach: failf(data, "Module attach failure"); break;
            case errSSLSessionNotFound: failf(data, "An attempt to restore an unknown session failed"); break;
            case errSSLPeerExportRestriction: failf(data, "An export restriction occurred"); break;
            case errSSLPeerUserCancelled: failf(data, "The user canceled the operation"); break;
            case errSSLPeerUnexpectedMsg: failf(data, "Peer rejected unexpected message"); break;
        #if CURL_BUILD_MAC_10_11 || CURL_BUILD_IOS_9
            case errSSLClientHelloReceived: failf(data, "A non-fatal result for providing a server name indication"); break;
        #endif
        #if CURL_BUILD_MAC_10_6
            case errSSLClientCertRequested: failf(data, "The server has requested a client certificate"); break;
        #endif
        #if CURL_BUILD_MAC_10_9
            case errSSLUnexpectedRecord: failf(data, "Unexpected (skipped) record in DTLS"); break;
        #endif
            default: failf(data, "Unknown SSL protocol error in connection to %s:%d",hostname, err);
        }
        return CURLE_SSL_CONNECT_ERROR;
    } else {
        ...
        switch(protocol) {
            case kSSLProtocol2: infof(data, "SSL 2.0 connection using %s\n", SSLCipherNameForNumber(cipher)); break;
            case kSSLProtocol3: infof(data, "SSL 3.0 connection using %s\n", SSLCipherNameForNumber(cipher)); break;
            case kTLSProtocol1: infof(data, "TLS 1.0 connection using %s\n", TLSCipherNameForNumber(cipher)); break;
        #if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
            case kTLSProtocol11: infof(data, "TLS 1.1 connection using %s\n", TLSCipherNameForNumber(cipher)); break;
            case kTLSProtocol12: infof(data, "TLS 1.2 connection using %s\n", TLSCipherNameForNumber(cipher)); break;
        #endif
        #if CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11
            case kTLSProtocol13: infof(data, "TLS 1.3 connection using %s\n", TLSCipherNameForNumber(cipher)); break;
        #endif
            default: infof(data, "Unknown protocol connection\n");
        }
    #if(CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11) && HAVE_BUILTIN_AVAILABLE == 1
        if(conn->bits.tls_enable_alpn) {
            if(__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
                ...
                if(err == noErr && alpnArr && CFArrayGetCount(alpnArr) >= 1) chosenProtocol = CFArrayGetValueAtIndex(alpnArr, 0);
            #ifdef USE_NGHTTP2
                if(chosenProtocol && !CFStringCompare(chosenProtocol, CFSTR(NGHTTP2_PROTO_VERSION_ID), 0)) conn->negnpn = CURL_HTTP_VERSION_2;
                else
            #endif
                if(chosenProtocol && !CFStringCompare(chosenProtocol, CFSTR(ALPN_HTTP_1_1), 0)) conn->negnpn = CURL_HTTP_VERSION_1_1;
...
static CURLcode sectransp_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
#ifndef CURL_DISABLE_VERBOSE_STRINGS
    if(data->set.verbose) show_verbose_server_cert(conn, sockindex);
#endif
...
static CURLcode sectransp_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    CURLcode result;
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    curl_socket_t sockfd = conn->sock[sockindex];
...
static CURLcode Curl_sectransp_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
...
static void Curl_sectransp_close(struct connectdata *conn, int sockindex) {
    ...
    #if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
        if(SSLCreateContext != NULL) CFRelease(backend->ssl_ctx);
    #if CURL_SUPPORT_MAC_10_8
...
static int Curl_sectransp_check_cxn(struct connectdata *conn) {
    ...
    if(backend->ssl_ctx) {
        ...
        if(err == noErr) return state == kSSLConnected || state == kSSLHandshake;
        return -1;
    }
    return 0;
}
static bool Curl_sectransp_data_pending(const struct connectdata *conn, int connindex) {
    ...
    if(backend->ssl_ctx) {
        ...
        if(err == noErr) return buffer > 0UL;
        return false;
    } else return false;
}
static CURLcode Curl_sectransp_random(struct Curl_easy *data UNUSED_PARAM, unsigned char *entropy, size_t length) {
    size_t i;
    u_int32_t random_number = 0;
    (void)data;
    for(i = 0 ; i < length ; i++) {
        if(i % sizeof(u_int32_t) == 0) random_number = arc4random();
...
static CURLcode Curl_sectransp_md5sum(unsigned char *tmp, size_t tmplen, unsigned char *md5sum, size_t md5len) {
    (void)md5len;
    (void)CC_MD5(tmp, (CC_LONG)tmplen, md5sum);
    return CURLE_OK;
}
static CURLcode Curl_sectransp_sha256sum(const unsigned char *tmp, size_t tmplen, unsigned char *sha256sum, size_t sha256len) {
...
#if CURL_BUILD_MAC_10_9 || CURL_BUILD_IOS_7
    if(SSLSetSessionOption != NULL) return TRUE;
#endif
    return FALSE;
}
static ssize_t sectransp_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
...
static ssize_t sectransp_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
    ...
    if(err != noErr) {
        switch(err) {
            case errSSLWouldBlock:
                if(processed) return (ssize_t)processed;
                *curlcode = CURLE_AGAIN;
                return -1L;+
            case errSSLClosedGraceful: case errSSLClosedNoNotify:
                *curlcode = CURLE_OK;
                return -1L;
            case -9841:
                if(SSL_CONN_CONFIG(CAfile) && SSL_CONN_CONFIG(verifypeer)) {
                    CURLcode result = verify_cert(SSL_CONN_CONFIG(CAfile), conn->data, backend->ssl_ctx);
                    if(result) return result;
                }
                goto again;
            default:
...
static void *Curl_sectransp_get_internals(struct ssl_connect_data *connssl, CURLINFO info UNUSED_PARAM) {
    struct ssl_backend_data *backend = connssl->backend;
    (void)info;
