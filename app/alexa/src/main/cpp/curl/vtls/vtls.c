...
#define CLONE_STRING(var)                 \
    if(source->var) {                     \
        ...
        if(!dest->var) return FALSE;      \
    } else dest->var = NULL;
#define CLONE_BLOB(var) if(blobdup(&dest->var, source->var)) return FALSE;
static CURLcode blobdup(struct curl_blob **dest, struct curl_blob *src) {
    ...
    if(src) {
        ...
        if(!d) return CURLE_OUT_OF_MEMORY;
        d->len = src->len;
...
bool Curl_ssl_config_matches(struct ssl_primary_config *data,struct ssl_primary_config *needle) {
    if((data->version == needle->version) && (data->version_max == needle->version_max) && (data->verifypeer == needle->verifypeer) &&
       (data->verifyhost == needle->verifyhost) && (data->verifystatus == needle->verifystatus) && blobcmp(data->cert_blob, needle->cert_blob) &&
       Curl_safe_strcasecompare(data->CApath, needle->CApath) && Curl_safe_strcasecompare(data->CAfile, needle->CAfile) &&
       Curl_safe_strcasecompare(data->clientcert, needle->clientcert) && Curl_safe_strcasecompare(data->random_file, needle->random_file) &&
       Curl_safe_strcasecompare(data->egdsocket, needle->egdsocket) && Curl_safe_strcasecompare(data->cipher_list, needle->cipher_list) &&
       Curl_safe_strcasecompare(data->cipher_list13, needle->cipher_list13) && Curl_safe_strcasecompare(data->pinned_key, needle->pinned_key)) {
        return TRUE;
    }
    return FALSE;
}
...
static CURLcode ssl_connect_init_proxy(struct connectdata *conn, int sockindex) {
    ...
    if(ssl_connection_complete == conn->ssl[sockindex].state && !conn->proxy_ssl[sockindex].use) {
        ...
        if(!(Curl_ssl->supports & SSLSUPP_HTTPS_PROXY)) return CURLE_NOT_BUILT_IN;
...
#endif
    if(!ssl_prefs_check(conn->data)) return CURLE_SSL_CONNECT_ERROR;
    conn->ssl[sockindex].use = TRUE;
    conn->ssl[sockindex].state = ssl_connection_negotiating;
    result = Curl_ssl->connect_blocking(conn, sockindex);
    if(!result) Curl_pgrsTime(conn->data, TIMER_APPCONNECT); /* SSL is connected */
    return result;
}
CURLcode Curl_ssl_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
    ...
    if(!result && *done) Curl_pgrsTime(conn->data, TIMER_APPCONNECT); /* SSL is connected */
    return result;
}
...
void Curl_ssl_sessionid_unlock(struct connectdata *conn) {
    if(SSLSESSION_SHARED(conn->data)) Curl_share_unlock(conn->data, CURL_LOCK_DATA_SSL_SESSION);
}
bool Curl_ssl_getsessionid(struct connectdata *conn, void **ssl_sessionid, size_t *idsize, int sockindex) {
    ...
    *ssl_sessionid = NULL;
    ...
    if(!SSL_SET_OPTION(primary.sessionid)) return TRUE;
    if(SSLSESSION_SHARED(data)) general_age = &data->share->sessionage;
    else general_age = &data->state.sessionage;
    for(i = 0; i < data->set.general_ssl.max_ssl_sessions; i++) {
        check = &data->state.session[i];
        if(!check->sessionid) continue;
        if(strcasecompare(name, check->name) && ((!conn->bits.conn_to_host && !check->conn_to_host) || (conn->bits.conn_to_host && check->conn_to_host &&
           strcasecompare(conn->conn_to_host.name, check->conn_to_host))) && ((!conn->bits.conn_to_port && check->conn_to_port == -1) ||
           (conn->bits.conn_to_port && check->conn_to_port != -1 && conn->conn_to_port == check->conn_to_port)) && (port == check->remote_port) &&
           strcasecompare(conn->handler->scheme, check->scheme) && Curl_ssl_config_matches(ssl_config, &check->ssl_config)) {
            (*general_age)++;
            check->age = *general_age;
            *ssl_sessionid = check->sessionid;
            if(idsize) *idsize = check->idsize;
            no_match = FALSE;
            break;
        }
    }
...
CURLcode Curl_ssl_addsessionid(struct connectdata *conn, void *ssl_sessionid, size_t idsize, int sockindex) {
    ...
    const bool isProxy = CONNECT_PROXY_SSL();
    struct ssl_primary_config * const ssl_config = isProxy ? &conn->proxy_ssl_config : &conn->ssl_config;
    const char *hostname = isProxy ? conn->http_proxy.host.name : conn->host.name;
#else
    ...
    if(!clone_host) return CURLE_OUT_OF_MEMORY; /* bail out */
    ...
    if(conn->bits.conn_to_port) conn_to_port = conn->conn_to_port;
    else conn_to_port = -1;
    if(SSLSESSION_SHARED(data)) general_age = &data->share->sessionage;
    else general_age = &data->state.sessionage;
    ...
    if(i == data->set.general_ssl.max_ssl_sessions) Curl_ssl_kill_session(store);
...
void Curl_ssl_close_all(struct Curl_easy *data) {
    if(data->state.session && !SSLSESSION_SHARED(data)) {
        size_t i;
        for(i = 0; i < data->set.general_ssl.max_ssl_sessions; i++) Curl_ssl_kill_session(&data->state.session[i]);
        Curl_safefree(data->state.session);
    }
    Curl_ssl->close_all(data);
}
#if defined(USE_OPENSSL) || defined(USE_GNUTLS) || defined(USE_SCHANNEL) || defined(USE_SECTRANSP) || defined(USE_NSS) || \
...
CURLcode Curl_ssl_shutdown(struct connectdata *conn, int sockindex) {
    if(Curl_ssl->shut_down(conn, sockindex)) return CURLE_SSL_SHUTDOWN_FAILED;
...
CURLcode Curl_ssl_initsessions(struct Curl_easy *data, size_t amount) {
    ...
    if(data->state.session) return CURLE_OK;
...
CURLcode Curl_ssl_init_certinfo(struct Curl_easy *data, int num) {
    ...
    if(!table) return CURLE_OUT_OF_MEMORY;
    ci->num_of_certs = num;
    ci->certinfo = table;
    return CURLE_OK;
}
...
static CURLcode pubkey_pem_to_der(const char *pem, unsigned char **der, size_t *der_len) {
    ...
    CURLcode result;
    if(!pem) return CURLE_BAD_CONTENT_ENCODING;
    begin_pos = strstr(pem, "-----BEGIN PUBLIC KEY-----");
    if(!begin_pos) return CURLE_BAD_CONTENT_ENCODING;
    pem_count = begin_pos - pem;
    if(0 != pem_count && '\n' != pem[pem_count - 1]) return CURLE_BAD_CONTENT_ENCODING;
    pem_count += 26;
    end_pos = strstr(pem + pem_count, "\n-----END PUBLIC KEY-----");
    if(!end_pos) return CURLE_BAD_CONTENT_ENCODING;
    pem_len = end_pos - pem;
    stripped_pem = malloc(pem_len - pem_count + 1);
    if(!stripped_pem) return CURLE_OUT_OF_MEMORY;
    while(pem_count < pem_len) {
        if('\n' != pem[pem_count] && '\r' != pem[pem_count]) stripped_pem[stripped_pem_count++] = pem[pem_count];
        ++pem_count;
    }
...
CURLcode Curl_pin_peer_pubkey(struct Curl_easy *data, const char *pinnedpubkey, const unsigned char *pubkey, size_t pubkeylen) {
    ...
    if(!pubkey || !pubkeylen) return result;
    if(strncmp(pinnedpubkey, "sha256//", 8) == 0) {
        CURLcode encode;
        size_t encodedlen, pinkeylen;
        char *encoded, *pinkeycopy, *begin_pos, *end_pos;
        unsigned char *sha256sumdigest;
        if(!Curl_ssl->sha256sum) return result;
        sha256sumdigest = malloc(CURL_SHA256_DIGEST_LENGTH);
        if(!sha256sumdigest) return CURLE_OUT_OF_MEMORY;
        encode = Curl_ssl->sha256sum(pubkey, pubkeylen, sha256sumdigest, CURL_SHA256_DIGEST_LENGTH);
        if(encode != CURLE_OK) return encode;
    ...
    do {
        ...
        if(filesize < 0 || filesize > MAX_PINNED_PUBKEY_SIZE) break;
        size = curlx_sotouz((curl_off_t) filesize);
        if(pubkeylen > size) break;
        buf = malloc(size + 1);
        if(!buf) break;
        if((int) fread(buf, size, 1, fp) != 1) break;
        if(pubkeylen == size) {
            if(!memcmp(pubkey, buf, pubkeylen)) result = CURLE_OK;
            break;
        }
        buf[size] = '\0';
        pem_read = pubkey_pem_to_der((const char *)buf, &pem_ptr, &pem_len);
        if(pem_read) break;
        if(pubkeylen == pem_len && !memcmp(pubkey, pem_ptr, pubkeylen)) result = CURLE_OK;
    } while(0);
    Curl_safefree(buf);
...
CURLcode Curl_none_md5sum(unsigned char *input, size_t inputlen, unsigned char *md5sum, size_t md5len UNUSED_PARAM) {
    ...
    if(!MD5pw) return CURLE_OUT_OF_MEMORY;
...
static int Curl_multissl_init(void) {
    if(multissl_init(NULL)) return 1;
    return Curl_ssl->init();
}
static CURLcode Curl_multissl_connect(struct connectdata *conn, int sockindex) {
    if(multissl_init(NULL)) return CURLE_FAILED_INIT;
...
static int multissl_init(const struct Curl_ssl *backend) {
    const char *env;
    char *env_tmp;
    if(Curl_ssl != &Curl_ssl_multi) return 1;
    if(backend) {
        Curl_ssl = backend;
        return 0;
    }
    if(!available_backends[0]) return 1;
    env = env_tmp = curl_getenv("CURL_SSL_BACKEND");
#ifdef CURL_DEFAULT_SSL_BACKEND
    if(!env) env = CURL_DEFAULT_SSL_BACKEND;
#endif
    if(env) {
    int i;
    for(i = 0; available_backends[i]; i++) {
...
CURLsslset curl_global_sslset(curl_sslbackend id, const char *name, const curl_ssl_backend ***avail) {
    int i;
    if(avail) *avail = (const curl_ssl_backend **)&available_backends;
