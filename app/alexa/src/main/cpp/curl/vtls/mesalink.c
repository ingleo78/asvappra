...
static CURLcode mesalink_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    switch(SSL_CONN_CONFIG(version)) {
        case CURL_SSLVERSION_SSLv3: case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1:
            failf(data, "MesaLink does not support SSL 3.0, TLS 1.0, or TLS 1.1");
            return CURLE_NOT_BUILT_IN;
        case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1_2: req_method = TLSv1_2_client_method(); break;
        case CURL_SSLVERSION_TLSv1_3: req_method = TLSv1_3_client_method(); break;
    ...
    if(SSL_SET_OPTION(cert) && SSL_SET_OPTION(key)) {
        ...
        if(SSL_CTX_use_certificate_chain_file(BACKEND->ctx, SSL_SET_OPTION(cert), file_type) != 1) {
...
static ssize_t mesalink_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    char error_buffer[MESALINK_MAX_ERROR_SZ];
        ...
        switch(err) {
            case SSL_ERROR_WANT_READ: case SSL_ERROR_WANT_WRITE:
                *curlcode = CURLE_AGAIN;
                return -1;
            default:
                failf(conn->data, "SSL write: %s, errno %d", ERR_error_string_n(err, error_buffer, sizeof(error_buffer)), SOCKERRNO);
...
static ssize_t mesalink_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
    ...
    if(nread <= 0) {
        ...
        switch(err) {
            case SSL_ERROR_ZERO_RETURN: case IO_ERROR_CONNECTION_ABORTED: break;
            case SSL_ERROR_WANT_READ: case SSL_ERROR_WANT_WRITE:
                *curlcode = CURLE_AGAIN;
                return -1;
            default:
                failf(conn->data, "SSL read: %s, errno %d", ERR_error_string_n(err, error_buffer, sizeof(error_buffer)), SOCKERRNO);
...
static CURLcode mesalink_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    CURLcode result;
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    curl_socket_t sockfd = conn->sock[sockindex];
...
static CURLcode Curl_mesalink_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
...
static void* Curl_mesalink_get_internals(struct ssl_connect_data *connssl, CURLINFO info UNUSED_PARAM) {
    (void)info;
