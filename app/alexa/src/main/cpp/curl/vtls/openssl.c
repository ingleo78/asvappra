...
static int ossl_get_ssl_conn_index(void) {
    ...
    if(ssl_ex_data_conn_index < 0) ssl_ex_data_conn_index = SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
    return ssl_ex_data_conn_index;
}
static int ossl_get_ssl_sockindex_index(void) {
    static int ssl_ex_data_sockindex_index = -1;
    if(ssl_ex_data_sockindex_index < 0) ssl_ex_data_sockindex_index = SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
...
#define RANDOM_FILE ""
#endif
    {
        ...
        if(rand_enough()) return CURLE_OK;
    }
...
#define EGD_SOCKET "" /* doesn't matter won't be used */
    ...
    return (rand_enough() ? CURLE_OK : CURLE_SSL_CONNECT_ERROR);
}
...
static int do_file_type(const char *type) {
    if(!type || !type[0]) return SSL_FILETYPE_PEM;
    if(strcasecompare(type, "PEM")) return SSL_FILETYPE_PEM;
    if(strcasecompare(type, "DER")) return SSL_FILETYPE_ASN1;
    if(strcasecompare(type, "ENG")) return SSL_FILETYPE_ENGINE;
    if(strcasecompare(type, "P12")) return SSL_FILETYPE_PKCS12;
    return -1;
}
#ifdef USE_OPENSSL_ENGINE
static int ssl_ui_reader(UI *ui, UI_STRING *uis) {
    ...
    switch(UI_get_string_type(uis)) {
        case UIT_PROMPT: case UIT_VERIFY:
...
static int ssl_ui_writer(UI *ui, UI_STRING *uis) {
    switch(UI_get_string_type(uis)) {
        case UIT_PROMPT: case UIT_VERIFY: if(UI_get0_user_data(ui) && (UI_get_input_flags(uis) & UI_INPUT_FLAG_DEFAULT_PWD)) return 1;
    }
...
SSL_CTX_use_certificate_bio(SSL_CTX *ctx, BIO *in, int type, const char *key_passwd) {
    int ret = 0;
    X509 *x = NULL;
    if(type == SSL_FILETYPE_ASN1) x = d2i_X509_bio(in, NULL);
    else if(type == SSL_FILETYPE_PEM) x = PEM_read_bio_X509(in, NULL, passwd_callback, (void *)key_passwd);
    else {
        ret = 0;
        goto end;
    }
...
static int SSL_CTX_use_PrivateKey_bio(SSL_CTX *ctx, BIO* in, int type, const char *key_passwd) {
    int ret = 0;
    EVP_PKEY *pkey = NULL;
    if(type == SSL_FILETYPE_PEM) pkey = PEM_read_bio_PrivateKey(in, NULL, passwd_callback, (void*)key_passwd);
    else if(type == SSL_FILETYPE_ASN1) pkey = d2i_PrivateKey_bio(in, NULL);
    else {
        ret = 0;
        goto end;
    }
...
static int SSL_CTX_use_certificate_chain_bio(SSL_CTX *ctx, BIO* in, const char *key_passwd) {
    ...
    if(ret) {
        ...
        if((ERR_GET_LIB(err) == ERR_LIB_PEM) && (ERR_GET_REASON(err) == PEM_R_NO_START_LINE)) ERR_clear_error();
        else ret = 0;
    }
    end:
    X509_free(x);
...
static int cert_stuff(struct connectdata *conn, SSL_CTX* ctx, char *cert_file, BIO *cert_bio, const char *cert_type, char *key_file, BIO* key_bio,
    ...
    if(cert_file || cert_bio || (file_type == SSL_FILETYPE_ENGINE)) {
        ...
        switch(file_type) {
            ...
            #if defined(USE_OPENSSL_ENGINE) && defined(ENGINE_CTRL_GET_CMD_FROM_NAME)
                {
                    if(!data->state.engine) {
                        if(is_pkcs11_uri(cert_file)) {
                            if(Curl_ossl_set_engine(data, "pkcs11") != CURLE_OK) return 0;
                        }
                    }
                    if(data->state.engine) {
                        ...
                        if(!ENGINE_ctrl_cmd(data->state.engine, cmd_name, 0, &params, NULL, 1)) {
                            failf(data, "ssl engine cannot load client cert with id '%s' [%s]", cert_file, ossl_strerror(ERR_get_error(), error_buffer,
                                  sizeof(error_buffer)));
                            return 0;
                        }
                        if(!params.cert) {
            ...
            case SSL_FILETYPE_PKCS12: {
                    ...
                    if(!PKCS12_parse(p12, key_passwd, &pri, &x509, &ca)) {
                        failf(data, "could not parse PKCS12 file, check password, " OSSL_PACKAGE " error %s", ossl_strerror(ERR_get_error(), error_buffer,
                              sizeof(error_buffer)));
                        PKCS12_free(p12);
                        return 0;
                    }
        ...
        if((!key_file) && (!key_bio)) {
            ...
            switch(file_type) {
                case SSL_FILETYPE_PEM: if(cert_done) break;
                case SSL_FILETYPE_ASN1:
        ...
        if(pktype == EVP_PKEY_RSA) {
            ...
            if(RSA_flags(rsa) & RSA_METHOD_FLAG_NO_CHECK) check_privkey = FALSE;
            RSA_free(rsa);
        }
    #endif
        SSL_free(ssl);
...
static int x509_name_oneline(X509_NAME *a, char *buf, size_t size) {
    ...
    if((size_t)biomem->length < size) size = biomem->length;
    else size--;
...
static int Curl_ossl_init(void) {
    ...
    if(!SSLeay_add_ssl_algorithms()) return 0;
    OpenSSL_add_all_algorithms();
#endif
    Curl_tls_keylog_open();
    if(ossl_get_ssl_conn_index() < 0 || ossl_get_ssl_sockindex_index() < 0) return 0;
    return 1;
}
...
#ifdef USE_OPENSSL_ENGINE
    if(data->state.engine) {
        if(ENGINE_set_default(data->state.engine, ENGINE_METHOD_ALL) > 0) infof(data, "set default crypto engine '%s'\n", ENGINE_get_id(data->state.engine));
        else {
...
static int Curl_ossl_shutdown(struct connectdata *conn, int sockindex) {
    ...
    if(data->set.ftp_ccc == CURLFTPSSL_CCC_ACTIVE) (void)SSL_shutdown(backend->handle);
#endif
    if(backend->handle) {
        ...
        while(!done) {
            ...
            if(what > 0) {
                ...
                switch(err) {
                    case SSL_ERROR_NONE:
                    case SSL_ERROR_ZERO_RETURN: done = TRUE; break;
                    case SSL_ERROR_WANT_READ: infof(data, "SSL_ERROR_WANT_READ\n"); break;
        ...
        if(data->set.verbose) {
            ...
            switch(SSL_get_shutdown(backend->handle)) {
                case SSL_SENT_SHUTDOWN: infof(data, "SSL_get_shutdown() returned SSL_SENT_SHUTDOWN\n"); break;
                case SSL_RECEIVED_SHUTDOWN: infof(data, "SSL_get_shutdown() returned SSL_RECEIVED_SHUTDOWN\n"); break;
                case SSL_SENT_SHUTDOWN|SSL_RECEIVED_SHUTDOWN: infof(data, "SSL_get_shutdown() returned SSL_SENT_SHUTDOWN|" "SSL_RECEIVED__SHUTDOWN\n"); break;
            }
        #endif
        }
        SSL_free(backend->handle);
...
static bool subj_alt_hostcheck(struct Curl_easy *data, const char *match_pattern, const char *hostname, const char *dispname) {
    ...
    free(match_pattern2);
    } else failf(data, "SSL: out of memory when allocating temporary for subjectAltName");
    return res;
#else
...
static CURLcode verifyhost(struct connectdata *conn, X509 *server_cert) {
    ...
    if(altnames) {
        ...
        for(i = 0; (i < numalts) && !dnsmatched; i++) {
            ...
            if(check->type == GEN_DNS) dNSName = TRUE;
            else if(check->type == GEN_IPADD) iPAddress = TRUE;
            if(check->type == target) {
                const char *altptr = (char *)ASN1_STRING_get0_data(check->d.ia5);
                size_t altlen = (size_t) ASN1_STRING_length(check->d.ia5);
                switch(target) {
                case GEN_DNS: if((altlen == strlen(altptr)) && subj_alt_hostcheck(data, altptr, hostname, dispname)) dnsmatched = TRUE; break;
                case GEN_IPADD:
    ...
    if(matched);
        ...
        if(name)
            while((j = X509_NAME_get_index_by_NID(name, NID_commonName, i)) >= 0) i = j;
        if(i >= 0) {
        ...
        if(peer_CN == nulstr) peer_CN = NULL;
        else {
        ...
        if(peer_CN) OPENSSL_free(peer_CN);
    }
    return result;
}
...
static void ssl_tls_trace(int direction, int ssl_ver, int content_type, const void *buf, size_t len, SSL *ssl, void *userp) {
    ...
    if(!conn || !conn->data || !conn->data->set.fdebug || (direction != 0 && direction != 1)) return;
    data = conn->data;
    switch(ssl_ver) {
    #ifdef SSL2_VERSION
        case SSL2_VERSION: verstr = "SSLv2"; break;
    #endif
    #ifdef SSL3_VERSION
        case SSL3_VERSION: verstr = "SSLv3"; break;
    #endif
        case TLS1_VERSION: verstr = "TLSv1.0"; break;
    #ifdef TLS1_1_VERSION
        case TLS1_1_VERSION: verstr = "TLSv1.1"; break;
    #endif
    #ifdef TLS1_2_VERSION
        case TLS1_2_VERSION: verstr = "TLSv1.2"; break;
    #endif
    #ifdef TLS1_3_VERSION
        case TLS1_3_VERSION: verstr = "TLSv1.3"; break;
    #endif
        case 0: break;
        default:
    ...
    if(ssl_ver
       ...
       && content_type != SSL3_RT_INNER_CONTENT_TYPE
        ...
        ssl_ver >>= 8;
        if(ssl_ver == SSL3_VERSION_MAJOR && content_type) tls_rt_name = tls_rt_type(content_type);
        else tls_rt_name = "";
        ...
        if(0 <= txt_len && (unsigned)txt_len < sizeof(ssl_buf)) Curl_debug(data, CURLINFO_TEXT, ssl_buf, (size_t)txt_len);
    }
...
static CURLcode set_ssl_version_min_max(SSL_CTX *ctx, struct connectdata *conn) {
    ...
    switch(curl_ssl_version_min) {
        case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_TLSv1_0: ossl_ssl_version_min = TLS1_VERSION; break;
        case CURL_SSLVERSION_TLSv1_1: ossl_ssl_version_min = TLS1_1_VERSION; break;
        case CURL_SSLVERSION_TLSv1_2: ossl_ssl_version_min = TLS1_2_VERSION; break;
    #ifdef TLS1_3_VERSION
        case CURL_SSLVERSION_TLSv1_3: ossl_ssl_version_min = TLS1_3_VERSION; break;
    #endif
    }
    if(curl_ssl_version_min != CURL_SSLVERSION_DEFAULT) {
        if(!SSL_CTX_set_min_proto_version(ctx, ossl_ssl_version_min)) return CURLE_SSL_CONNECT_ERROR;
    }
    curl_ssl_version_max = SSL_CONN_CONFIG(version_max);
    switch(curl_ssl_version_max) {
        case CURL_SSLVERSION_MAX_TLSv1_0: ossl_ssl_version_max = TLS1_VERSION; break;
        case CURL_SSLVERSION_MAX_TLSv1_1: ossl_ssl_version_max = TLS1_1_VERSION; break;
        case CURL_SSLVERSION_MAX_TLSv1_2: ossl_ssl_version_max = TLS1_2_VERSION; break;
    #ifdef TLS1_3_VERSION
        case CURL_SSLVERSION_MAX_TLSv1_3: ossl_ssl_version_max = TLS1_3_VERSION; break;
    #endif
        case CURL_SSLVERSION_MAX_NONE: case CURL_SSLVERSION_MAX_DEFAULT: default:
            ossl_ssl_version_max = 0;
            break;
    }
    if(!SSL_CTX_set_max_proto_version(ctx, ossl_ssl_version_max)) return CURLE_SSL_CONNECT_ERROR;
    return CURLE_OK;
}
#endif
#ifdef OPENSSL_IS_BORINGSSL
...
set_ssl_version_min_max_legacy(ctx_option_t *ctx_options, struct connectdata *conn, int sockindex) {
    ...
    switch(ssl_version) {
        ...
        case CURL_SSLVERSION_TLSv1: break;
    }
...
static int ossl_new_session_cb(SSL *ssl, SSL_SESSION *ssl_sessionid) {
    ...
    if(connectdata_idx < 0 || sockindex_idx < 0) return 0;
    ...
    if(SSL_SET_OPTION(primary.sessionid)) {
        ...
        if(!incache) {
            if(!Curl_ssl_addsessionid(conn, ssl_sessionid, 0, sockindex)) res = 1;
...
static CURLcode ossl_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    *certverifyresult = !X509_V_OK;
    switch(ssl_version) {
        case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1: case CURL_SSLVERSION_TLSv1_2:
        case CURL_SSLVERSION_TLSv1_3:
        #if (OPENSSL_VERSION_NUMBER >= 0x10100000L)
        ...
        #else
        #ifdef USE_TLS_SRP
            if(ssl_authtype == CURL_TLSAUTH_SRP) return CURLE_SSL_CONNECT_ERROR;
        #endif
    ...
    if(backend->ctx) SSL_CTX_free(backend->ctx);
    ...
    switch(ssl_version) {
        ...
        case CURL_SSLVERSION_SSLv3:
            ...
            break;
        case CURL_SSLVERSION_DEFAULT: case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_TLSv1_0: case CURL_SSLVERSION_TLSv1_1: case CURL_SSLVERSION_TLSv1_2:
        case CURL_SSLVERSION_TLSv1_3: ctx_options |= SSL_OP_NO_SSLv2; ctx_options |= SSL_OP_NO_SSLv3;
        #if (OPENSSL_VERSION_NUMBER >= 0x10100000L)
...
#ifdef HAS_NPN
    if(conn->bits.tls_enable_npn) SSL_CTX_set_next_proto_select_cb(backend->ctx, select_next_proto_cb, conn);
#endif
#ifdef HAS_ALPN
    ...
    if(ssl_cert || ssl_cert_blob || ssl_cert_type) {
        ...
        if(ssl_cert_blob) {
            ...
            if(!ssl_cert_bio) return CURLE_SSL_CERTPROBLEM;
        }
        if(SSL_SET_OPTION(key_blob)) {
            ssl_key_bio = BIO_new_mem_buf(SSL_SET_OPTION(key_blob)->data, (int)SSL_SET_OPTION(key_blob)->len);
            if(!ssl_key_bio) return CURLE_SSL_CERTPROBLEM;
        }
        result_cert_stuff = cert_stuff(conn, backend->ctx, ssl_cert, ssl_cert_bio, ssl_cert_type, SSL_SET_OPTION(key), ssl_key_bio,
                                       SSL_SET_OPTION(key_type), SSL_SET_OPTION(key_passwd));
        if(ssl_cert_bio) BIO_free(ssl_cert_bio);
        if(ssl_key_bio) BIO_free(ssl_key_bio);
        if(!result_cert_stuff) return CURLE_SSL_CERTPROBLEM;
    }
    ciphers = SSL_CONN_CONFIG(cipher_list);
    if(!ciphers) ciphers = (char *)DEFAULT_CIPHER_SELECTION;
    if(ciphers) {
    ...
    if((SSL_CONN_CONFIG(verifypeer) || SSL_CONN_CONFIG(verifyhost)) && (SSL_SET_OPTION(native_ca_store))) {
        ...
        if(hStore) {
            ...
            for( ; ; ) {
                ...
                if(!pContext) break;
            #if defined(DEBUGBUILD) && !defined(CURL_DISABLE_VERBOSE_STRINGS)
                if(!CertGetNameStringA(pContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, cert_name, sizeof(cert_name))) strcpy(cert_name, "Unknown");
                infof(data, "SSL: Checking cert \"%s\"\n", cert_name);
            #endif
                encoded_cert = (const unsigned char *)pContext->pbCertEncoded;
                if(!encoded_cert) continue;
                GetSystemTimeAsFileTime(&now);
                if(CompareFileTime(&pContext->pCertInfo->NotBefore, &now) > 0 || CompareFileTime(&now, &pContext->pCertInfo->NotAfter) > 0) continue;
                ...
                if(CertGetEnhancedKeyUsage(pContext, 0, NULL, &req_size)) {
                    ...
                    if(CertGetEnhancedKeyUsage(pContext, 0, enhkey_usage, &req_size)) {
                        if(!enhkey_usage->cUsageIdentifier) {
                            if(GetLastError() != CRYPT_E_NOT_FOUND) continue;
                        } else {
                            DWORD i;
                            bool found = false;
        ...
        if(imported_native_ca) infof(data, "successfully imported windows ca store\n");
    ...
    if(ssl_cafile || ssl_capath) {
        if(!SSL_CTX_load_verify_locations(backend->ctx, ssl_cafile, ssl_capath)) {
            if(verifypeer && !imported_native_ca) {
                failf(data, "error setting certificate verify locations:\n  CAfile: %s\n  CApath: %s",ssl_cafile ? ssl_cafile : "none",
    ...
    if(verifypeer && !ssl_cafile && !ssl_capath && !imported_native_ca) SSL_CTX_set_default_verify_paths(backend->ctx);
#endif
    if(ssl_crlfile) {
...
#ifdef HAVE_KEYLOG_CALLBACK
    if(Curl_tls_keylog_enabled()) SSL_CTX_set_keylog_callback(backend->ctx, ossl_keylog_callback);
#endif
    ...
    if(backend->handle) SSL_free(backend->handle);
...
#if (OPENSSL_VERSION_NUMBER >= 0x0090808fL) && !defined(OPENSSL_NO_TLSEXT) && !defined(OPENSSL_NO_OCSP)
    if(SSL_CONN_CONFIG(verifystatus)) SSL_set_tlsext_status_type(backend->handle, TLSEXT_STATUSTYPE_ocsp);
#endif
...
#endif
    struct ssl_backend_data *backend = connssl->backend;
    DEBUGASSERT(ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state
                || ssl_connect_2_writing == connssl->connecting_state);
    ERR_clear_error();
    err = SSL_connect(backend->handle);
#ifndef HAVE_KEYLOG_CALLBACK
    if(Curl_tls_keylog_enabled()) ossl_log_tls12_secret(backend->handle, &backend->keylog_done);
#endif
    if(1 != err) {
        ...
        if(SSL_ERROR_WANT_ASYNC == detail) {
            ...
            if(CURLE_SSL_CONNECT_ERROR == result && errdetail == 0) {
                ...
                if(sockerr && detail == SSL_ERROR_SYSCALL) Curl_strerror(sockerr, extramsg, sizeof(extramsg));
        ...
        if(conn->bits.tls_enable_alpn) {
            ...
            #ifdef USE_NGHTTP2
                if(len == NGHTTP2_PROTO_VERSION_ID_LEN && !memcmp(NGHTTP2_PROTO_VERSION_ID, neg_protocol, len)) conn->negnpn = CURL_HTTP_VERSION_2;
                else
            #endif
                if(len == ALPN_HTTP_1_1_LENGTH && !memcmp(ALPN_HTTP_1_1, neg_protocol, ALPN_HTTP_1_1_LENGTH)) conn->negnpn = CURL_HTTP_VERSION_1_1;
            } else infof(data, "ALPN, server did not agree to a protocol\n");
          Curl_multiuse_state(conn, conn->negnpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
        }
    #endif
        return CURLE_OK;
    }
...
#define print_pubkey_BN(_type, _name, _num)    \
    do {                              \
        if(_type->_name) pubkey_show(data, mem, _num, #_type, #_name, _type->_name); \
    } while(0);
#endif
static void X509V3_ext(struct Curl_easy *data, int certnum, CONST_EXTS STACK_OF(X509_EXTENSION) *exts) {
    int i;
    if((int)sk_X509_EXTENSION_num(exts) <= 0) return;
    for(i = 0; i < (int)sk_X509_EXTENSION_num(exts); i++) {
        ...
        if(!X509V3_EXT_print(bio_out, ext, 0, 0)) ASN1_STRING_print(bio_out, (ASN1_STRING *)X509_EXTENSION_get_data(ext));
        BIO_get_mem_ptr(bio_out, &biomem);
...
static CURLcode get_cert_chain(struct connectdata *conn, struct ssl_connect_data *connssl) {
    ...
    if(!sk) return CURLE_OUT_OF_MEMORY;
    ...
    for(i = 0; i < (int)numcerts; i++) {
        ...
        if(num->type == V_ASN1_NEG_INTEGER) BIO_puts(mem, "-");
        for(j = 0; j < num->length; j++) BIO_printf(mem, "%02x", num->data[j]);
        ...
        if(!pubkey) infof(data, "   Unable to load public key\n");
        else {
            int pktype;
        #ifdef HAVE_OPAQUE_EVP_PKEY
        ...
        if(psig) {
            for(j = 0; j < psig->length; j++) BIO_printf(mem, "%02x:", psig->data[j]);
...
static CURLcode pkp_pin_peer_pubkey(struct Curl_easy *data, X509* cert, const char *pinnedpubkey) {
    ...
    if(!pinnedpubkey) return CURLE_OK;
    ...
    do {
        ...
        if((len1 != len2) || !temp || ((temp - buff1) != len1)) break;
...
static CURLcode servercert(struct connectdata *conn, struct ssl_connect_data *connssl, bool strict) {
    ...
    struct ssl_backend_data *backend = connssl->backend;
    if(data->set.ssl.certinfo) (void)get_cert_chain(conn, connssl);
    backend->server_cert = SSL_get_peer_certificate(backend->handle);
    if(!backend->server_cert) {
        BIO_free(mem);
        if(!strict) return CURLE_OK;
    ...
    if(rc) {
        if(strict) failf(data, "SSL: couldn't get X509-issuer name!");
        result = CURLE_PEER_FAILED_VERIFICATION;
    } else {
        infof(data, " issuer: %s\n", buffer);
        if(SSL_SET_OPTION(issuercert) || SSL_SET_OPTION(issuercert_blob)) {
            if(SSL_SET_OPTION(issuercert_blob)) fp = BIO_new_mem_buf(SSL_SET_OPTION(issuercert_blob)->data, (int)SSL_SET_OPTION(issuercert_blob)->len);
            else {
                fp = BIO_new(BIO_s_file());
                if(fp == NULL) {
                    failf(data, "BIO_new return NULL, " OSSL_PACKAGE " error %s", ossl_strerror(ERR_get_error(), error_buffer, sizeof(error_buffer)) );
                ...
                if(BIO_read_filename(fp, SSL_SET_OPTION(issuercert)) <= 0) {
                    if(strict) failf(data, "SSL: Unable to open issuer cert (%s)", SSL_SET_OPTION(issuercert));
                    BIO_free(fp);
                    X509_free(backend->server_cert);
                    backend->server_cert = NULL;
            ...
            if(!issuer) {
                if(strict) failf(data, "SSL: Unable to read issuer cert (%s)", SSL_SET_OPTION(issuercert));
                BIO_free(fp);
                X509_free(issuer);
                X509_free(backend->server_cert);
                backend->server_cert = NULL;
            ...
            if(X509_check_issued(issuer, backend->server_cert) != X509_V_OK) {
                if(strict) failf(data, "SSL: Certificate issuer check failed (%s)", SSL_SET_OPTION(issuercert));
                BIO_free(fp);
                X509_free(issuer);
                X509_free(backend->server_cert);
                backend->server_cert = NULL;
        ...
        if(*certverifyresult != X509_V_OK) {
            if(SSL_CONN_CONFIG(verifypeer)) {
                if(strict) failf(data, "SSL certificate verify result: %s (%ld)", X509_verify_cert_error_string(lerr), lerr);
                result = CURLE_PEER_FAILED_VERIFICATION;
...
static CURLcode ossl_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    if(!result) connssl->connecting_state = ssl_connect_done;
    return result;
}
...
static CURLcode ossl_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    ...
    while(ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state || ssl_connect_2_writing == connssl->connecting_state) {
        ...
        if(result || (nonblocking && (ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state ||
           ssl_connect_2_writing == connssl->connecting_state))) {
            return result;
        }
    }
...
static bool Curl_ossl_data_pending(const struct connectdata *conn, int connindex) {
    ...
    if(connssl->backend->handle && SSL_pending(connssl->backend->handle)) return TRUE;
#ifndef CURL_DISABLE_PROXY
    const struct ssl_connect_data *proxyssl = &conn->proxy_ssl[connindex];
    if(proxyssl->backend->handle && SSL_pending(proxyssl->backend->handle)) return TRUE;
#endif
    return FALSE;
}
    ...
    if(rc <= 0) {
        ...
        switch(err) {
            ...
            case SSL_ERROR_SYSCALL: {
                    int sockerr = SOCKERRNO;
                    sslerror = ERR_get_error();
                    if(sslerror) ossl_strerror(sslerror, error_buffer, sizeof(error_buffer));
                    else if(sockerr) Curl_strerror(sockerr, error_buffer, sizeof(error_buffer));
                    else {
...
static ssize_t ossl_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
    ...
    if(nread <= 0) {
        ...
        switch(err) {
            case SSL_ERROR_NONE: break; case SSL_ERROR_ZERO_RETURN:
                if(num == FIRSTSOCKET) connclose(conn, "TLS close_notify");
                break;
            case SSL_ERROR_WANT_READ: case SSL_ERROR_WANT_WRITE:
                *curlcode = CURLE_AGAIN;
                return -1;
            default:
                ...
                if((nread < 0) || sslerror) {
                    int sockerr = SOCKERRNO;
                    if(sslerror) ossl_strerror(sslerror, error_buffer, sizeof(error_buffer));
                    else if(sockerr && err == SSL_ERROR_SYSCALL) Curl_strerror(sockerr, error_buffer, sizeof(error_buffer));
                    else {
                        strncpy(error_buffer, SSL_ERROR_to_str(err), sizeof(error_buffer));
                        error_buffer[sizeof(error_buffer) - 1] = '\0';
                    }
                    failf(conn->data, OSSL_PACKAGE " SSL_read: %s, errno %d", error_buffer, sockerr);
                ...
                if(err == SSL_ERROR_SYSCALL) {
                    int sockerr = SOCKERRNO;
                    if(sockerr) Curl_strerror(sockerr, error_buffer, sizeof(error_buffer));
                    else msnprintf(error_buffer, sizeof(error_buffer), "Connection closed abruptly");
                    failf(conn->data, OSSL_PACKAGE " SSL_read: %s, errno %d (Fatal because this is a curl debug build)", error_buffer, sockerr);
                    *curlcode = CURLE_RECV_ERROR;
                    return -1;
                }
            #endif
...
static size_t Curl_ossl_version(char *buffer, size_t size) {
    ...
    if(Curl_strncasecompare(ver, expected, sizeof(expected) - 1)) ver += sizeof(expected) - 1;
...
static CURLcode Curl_ossl_random(struct Curl_easy *data, unsigned char *entropy, size_t length) {
    int rc;
    if(data) {
        if(Curl_ossl_seed(data)) return CURLE_FAILED_INIT;
    } else {
        if(!rand_enough()) return CURLE_FAILED_INIT;
    }
    rc = RAND_bytes(entropy, curlx_uztosi(length));
    return (rc == 1 ? CURLE_OK : CURLE_FAILED_INIT);
}
static CURLcode Curl_ossl_md5sum(unsigned char *tmp, size_t tmplen, unsigned char *md5sum, size_t unused) {
    EVP_MD_CTX *mdctx;
    unsigned int len = 0;
    (void) unused;
...
static CURLcode Curl_ossl_sha256sum(const unsigned char *tmp, size_t tmplen, unsigned char *sha256sum, size_t unused) {
    EVP_MD_CTX *mdctx;
    unsigned int len = 0;
    (void) unused;
    mdctx = EVP_MD_CTX_create();
