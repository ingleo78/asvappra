...
wolfssl_tls13_secret_callback(SSL *ssl, int id, const unsigned char *secret, int secretSz, void *ctx) {
    ...
    (void)ctx;
    if(!ssl || !Curl_tls_keylog_enabled()) return 0;
    switch(id) {
    ...
    if(SSL_get_client_random(ssl, client_random, SSL3_RANDOM_SIZE) == 0) return 0;
...
static void wolfssl_log_tls12_secret(SSL *ssl) {
    ...
    switch(wolfSSL_GetVersion(ssl)) {
        case WOLFSSL_SSLV3: case WOLFSSL_TLSV1: case WOLFSSL_TLSV1_1: case WOLFSSL_TLSV1_2: break;
        default: return;
    }
#endif
    if(SSL_get_keys(ssl, &ms, &msLen, &sr, &srLen, &cr, &crLen) != SSL_SUCCESS) return;
    for(i = 0; i < msLen; i++) x |= ms[i];
    if(x == 0) return;
...
static CURLcode
wolfssl_connect_step1(struct connectdata *conn, int sockindex) {
    char *ciphers;
    ...
    if(1 != SSL_CTX_load_verify_locations(backend->ctx, SSL_CONN_CONFIG(CAfile), SSL_CONN_CONFIG(CApath))) {
        if(SSL_CONN_CONFIG(verifypeer)) {
            failf(data, "error setting certificate verify locations:\n  CAfile: %s\n  CApath: %s", SSL_CONN_CONFIG(CAfile) ? SSL_CONN_CONFIG(CAfile) : "none",
                  SSL_CONN_CONFIG(CApath) ? SSL_CONN_CONFIG(CApath) : "none");
            return CURLE_SSL_CACERT_BADFILE;
        } else infof(data, "error setting certificate verify locations, continuing anyway:\n");
    ...
    #endif
           (wolfSSL_CTX_UseSNI(backend->ctx, WOLFSSL_SNI_HOST_NAME, hostname, (unsigned short)hostname_len) != 1)) {
    ...
    if(data->set.ssl.fsslctx) {
        ...
        failf(data, "SSL: Certificates can't be loaded because wolfSSL was built with \"no filesystem\". Either disable peer verification"
              " (insecure) or if you are building an application with libcurl you can load certificates via CURLOPT_SSL_CTX_FUNCTION.");
        return CURLE_SSL_CONNECT_ERROR;
    }
#endif
...
static CURLcode wolfssl_connect_step2(struct connectdata *conn, int sockindex) {
    ...
    const char * const pinnedpubkey = data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG];
#endif
    ...
    if(ret != 1) {
        ...
        if(SSL_ERROR_WANT_READ == detail) {
            ...
            return CURLE_SSL_CACERT_BADFILE;
            } else infof(data, "CA signer not available for verification, continuing anyway\n");
        }
    #endif
        else {
    ...
    if(pinnedpubkey) {
        ...
        if(Curl_parseX509(&x509_parsed, x509_der, x509_der + x509_der_len)) return CURLE_SSL_PINNEDPUBKEYNOTMATCH;
    ...
    if(conn->bits.tls_enable_alpn) {
        ...
        if(rc == SSL_SUCCESS) {
            ...
            if(protocol_len == ALPN_HTTP_1_1_LENGTH && !memcmp(protocol, ALPN_HTTP_1_1, ALPN_HTTP_1_1_LENGTH)) conn->negnpn = CURL_HTTP_VERSION_1_1;
        #ifdef USE_NGHTTP2
        ...
        } else if(rc == SSL_ALPN_NOT_FOUND) infof(data, "ALPN, server did not agree to a protocol\n");
        else {
...
static ssize_t wolfssl_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
    char error_buffer[WOLFSSL_MAX_ERROR_SZ];
...
static ssize_t wolfssl_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
...
static bool Curl_wolfssl_data_pending(const struct connectdata *conn, int connindex) {
    ...
    struct ssl_backend_data *backend = connssl->backend;
    if(backend->handle) return (0 != SSL_pending(backend->handle)) ? TRUE : FALSE;
    else return FALSE;
}
...
static CURLcode wolfssl_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    CURLcode result;
    struct Curl_easy *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    curl_socket_t sockfd = conn->sock[sockindex];
...
static CURLcode Curl_wolfssl_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
...
static CURLcode Curl_wolfssl_random(struct Curl_easy *data, unsigned char *entropy, size_t length) {
    WC_RNG rng;
    (void)data;
    if(wc_InitRng(&rng)) return CURLE_FAILED_INIT;
    if(length > UINT_MAX) return CURLE_FAILED_INIT;
    if(wc_RNG_GenerateBlock(&rng, entropy, (unsigned)length)) return CURLE_FAILED_INIT;
    if(wc_FreeRng(&rng)) return CURLE_FAILED_INIT;
    return CURLE_OK;
}
static CURLcode Curl_wolfssl_sha256sum(const unsigned char *tmp, size_t tmplen, unsigned char *sha256sum, size_t unused) {
    wc_Sha256 SHA256pw;
    (void)unused;
...
static void *Curl_wolfssl_get_internals(struct ssl_connect_data *connssl, CURLINFO info UNUSED_PARAM) {
    struct ssl_backend_data *backend = connssl->backend;
    (void)info;
