...
static CURLcode gskit_status(struct Curl_easy *data, int rc, const char *procname, CURLcode defcode) {
    switch(rc) {
        case GSK_OK: case GSK_OS400_ASYNCHRONOUS_SOC_INIT: return CURLE_OK;
        case GSK_KEYRING_OPEN_ERROR: case GSK_OS400_ERROR_NO_ACCESS: return CURLE_SSL_CACERT_BADFILE;
        case GSK_INSUFFICIENT_STORAGE: return CURLE_OUT_OF_MEMORY;
        case GSK_ERROR_BAD_V2_CIPHER: case GSK_ERROR_BAD_V3_CIPHER: case GSK_ERROR_NO_CIPHERS: return CURLE_SSL_CIPHER;
        case GSK_OS400_ERROR_NOT_TRUSTED_ROOT: case GSK_ERROR_CERT_VALIDATION: return CURLE_PEER_FAILED_VERIFICATION;
        case GSK_OS400_ERROR_TIMED_OUT: return CURLE_OPERATION_TIMEDOUT;
        case GSK_WOULD_BLOCK: return CURLE_AGAIN;
        case GSK_OS400_ERROR_NOT_REGISTERED: break;
        case GSK_ERROR_IO:
            switch(errno) {
                case ENOMEM: return CURLE_OUT_OF_MEMORY;
                default: failf(data, "%s I/O error: %s", procname, strerror(errno));
            }
            break;
        default: failf(data, "%s: %s", procname, gsk_strerror(rc));
    }
    return defcode;
}
static CURLcode set_enum(struct Curl_easy *data, gsk_handle h, GSK_ENUM_ID id, GSK_ENUM_VALUE value, bool unsupported_ok) {
    ...
    switch(rc) {
        case GSK_OK: return CURLE_OK;
        case GSK_ERROR_IO: failf(data, "gsk_attribute_set_enum() I/O error: %s", strerror(errno)); break;
        case GSK_ATTRIBUTE_INVALID_ID: if(unsupported_ok) return CURLE_UNSUPPORTED_PROTOCOL;
        default: failf(data, "gsk_attribute_set_enum(): %s", gsk_strerror(rc));
    }
...
static CURLcode set_buffer(struct Curl_easy *data, gsk_handle h, GSK_BUF_ID id, const char *buffer, bool unsupported_ok) {
    ...
    switch(rc) {
        case GSK_OK: return CURLE_OK;
        case GSK_ERROR_IO: failf(data, "gsk_attribute_set_buffer() I/O error: %s", strerror(errno)); break;
        case GSK_ATTRIBUTE_INVALID_ID: if(unsupported_ok) return CURLE_UNSUPPORTED_PROTOCOL;
        default: failf(data, "gsk_attribute_set_buffer(): %s", gsk_strerror(rc));
    }
    return CURLE_SSL_CONNECT_ERROR;
}
static CURLcode set_numeric(struct Curl_easy *data, gsk_handle h, GSK_NUM_ID id, int value) {
    ...
    switch(rc) {
        case GSK_OK: return CURLE_OK;
        case GSK_ERROR_IO: failf(data, "gsk_attribute_set_numeric_value() I/O error: %s", strerror(errno)); break;
        default: failf(data, "gsk_attribute_set_numeric_value(): %s", gsk_strerror(rc));
    }
    return CURLE_SSL_CONNECT_ERROR;
}
static CURLcode set_callback(struct Curl_easy *data, gsk_handle h, GSK_CALLBACK_ID id, void *info) {
    ...
    switch(rc) {
        case GSK_OK: return CURLE_OK;
        case GSK_ERROR_IO: failf(data, "gsk_attribute_set_callback() I/O error: %s", strerror(errno)); break;
        default: failf(data, "gsk_attribute_set_callback(): %s", gsk_strerror(rc));
    }
    return CURLE_SSL_CONNECT_ERROR;
}
static CURLcode set_ciphers(struct connectdata *conn, gsk_handle h, unsigned int *protoflags) {
    ...
    if(!cipherlist) return CURLE_OK;
    while(is_separator(*cipherlist)) cipherlist++;
    if(!*cipherlist) return CURLE_OK;
    ...
    result = CURLE_OK;
    for( ; ; ) {
        for(clp = cipherlist; *cipherlist && !is_separator(*cipherlist); ) cipherlist++;
        l = cipherlist - clp;
        if(!l) break;
        for(ctp = ciphertable; ctp->name; ctp++)
            if(strncasecompare(ctp->name, clp, l) && !ctp->name[l]) break;
        if(!ctp->name) {
    ...
    if(!result && (*protoflags & CURL_GSKPROTO_SSLV3_MASK)) result = set_buffer(data, h, GSK_V3_CIPHER_SPECS, ciphers[CURL_GSKPROTO_SSLV3].buf, FALSE);
    if(!result && (*protoflags & CURL_GSKPROTO_SSLV2_MASK)) result = set_buffer(data, h, GSK_V2_CIPHER_SPECS, ciphers[CURL_GSKPROTO_SSLV2].buf, FALSE);
    for(i = 0; i < CURL_GSKPROTO_LAST; i++) free(ciphers[i].buf);
    return result;
}
...
static CURLcode init_environment(struct Curl_easy *data, gsk_handle *envir, const char *appid, const char *file, const char *label, const char *password) {
    ...
    if(!result && appid) result = set_buffer(data, h, GSK_OS400_APPLICATION_ID, appid, FALSE);
    if(!result && file) result = set_buffer(data, h, GSK_KEYRING_FILE, file, FALSE);
    if(!result && label) result = set_buffer(data, h, GSK_KEYRING_LABEL, label, FALSE);
    if(!result && password) result = set_buffer(data, h, GSK_KEYRING_PW, password, FALSE);
    if(!result) {
...
static void cancel_async_handshake(struct connectdata *conn, int sockindex) {
    ...
    if(QsoCancelOperation(conn->sock[sockindex], 0) > 0) QsoWaitForIOCompletion(BACKEND->iocport, &cstat, (struct timeval *) NULL);
}
...
static int pipe_ssloverssl(struct connectdata *conn, int sockindex, int directions) {
    ...
    if(directions & SOS_WRITE) {
        ...
        if(n < conn->sock[sockindex]) n = conn->sock[sockindex];
    }
    ...
    if(FD_ISSET(BACKEND->remotefd, &fds_write)) {
        ...
        switch(i) {
            case GSK_OK:
                ...
                break;
            case GSK_OS400_ERROR_TIMED_OUT: case GSK_WOULD_BLOCK: break;
            default: return -1;
        }
    }
...
static void close_one(struct ssl_connect_data *connssl, struct connectdata *conn, int sockindex) {
    ...
    if(BACKEND->iocport >= 0) close_async_handshake(connssl);
}
static ssize_t gskit_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    ...
    if(pipe_ssloverssl(conn, sockindex, SOS_WRITE) >= 0) {
        ...
        if(cc == CURLE_OK)
          if(pipe_ssloverssl(conn, sockindex, SOS_WRITE) < 0) cc = CURLE_SEND_ERROR;
    }
    if(cc != CURLE_OK) {
        *curlcode = cc;
        written = -1;
...
static ssize_t gskit_recv(struct connectdata *conn, int num, char *buf, size_t buffersize, CURLcode *curlcode) {
    ...
    switch(cc) {
        case CURLE_OK: break;
        case CURLE_OPERATION_TIMEDOUT: cc = CURLE_AGAIN;
        default:
            *curlcode = cc;
            nread = -1;
            break;
...
static CURLcode set_ssl_version_min_max(unsigned int *protoflags, struct connectdata *conn) {
    ...
    for(; i <= (ssl_version_max >> 16); ++i) {
        switch(i) {
            case CURL_SSLVERSION_TLSv1_0: *protoflags |= CURL_GSKPROTO_TLSV10_MASK; break;
            case CURL_SSLVERSION_TLSv1_1: *protoflags |= CURL_GSKPROTO_TLSV11_MASK; break;
            case CURL_SSLVERSION_TLSv1_2: *protoflags |= CURL_GSKPROTO_TLSV11_MASK; break;
...
static CURLcode gskit_connect_step1(struct connectdata *conn, int sockindex) {
    ...
    if(conn->proxy_ssl[sockindex].use) {
        if(Curl_socketpair(0, 0, 0, sockpair)) return CURLE_SSL_CONNECT_ERROR;
    ...
    if(sni) {
        ...
        if(result == CURLE_UNSUPPORTED_PROTOCOL) result = CURLE_OK;
    }
    if(!result) {
        timediff_t timeout = Curl_timeleft(data, NULL, TRUE);
        if(timeout < 0) result = CURLE_OPERATION_TIMEDOUT;
        else result = set_numeric(data, BACKEND->handle, GSK_HANDSHAKE_TIMEOUT, (timeout + 999) / 1000);
    }
    if(!result) result = set_numeric(data, BACKEND->handle, GSK_OS400_READ_TIMEOUT, 1);
    if(!result) result = set_numeric(data, BACKEND->handle, GSK_FD, BACKEND->localfd >= 0 ? BACKEND->localfd : conn->sock[sockindex]);
    if(!result) result = set_ciphers(conn, BACKEND->handle, &protoflags);
    if(!protoflags) {
    ...
    if(!result) result = set_enum(data, BACKEND->handle, GSK_SERVER_AUTH_TYPE, verifypeer ? GSK_SERVER_AUTH_FULL : GSK_SERVER_AUTH_PASSTHRU, FALSE);
    if(!result) {
        memset(&commarea, 0, sizeof(commarea));
        BACKEND->iocport = QsoCreateIOCompletionPort();
        if(BACKEND->iocpoort != -1) {
        ...
        } else if(errno != ENOBUFS) result = gskit_status(data, GSK_ERROR_IO, "QsoCreateIOCompletionPort()", 0);
        else if(conn->proxy_ssl[sockindex].use) result = CURLE_SSL_CONNECT_ERROR;
        else {
...
static CURLcode gskit_connect_step2(struct connectdata *conn, int sockindex, bool nonblocking) {
    ...
    for(;;) {
        ...
        switch(QsoWaitForIOCompletion(BACKEND->iocport, &cstat, &stmv)) {
            ...
            case -1:
                if(errno == EINTR) {
                    if(nonblocking) return CURLE_OK;
                    continue;
                }
                if(errno != ETIME) {
...
static CURLcode gskit_connect_step3(struct connectdata *conn, int sockindex) {
    ...
    const char *ptr;
    CURLcode result;
    if(gskit_status(data, gsk_attribute_get_cert_info(BACKEND->handle, GSK_PARTNER_CERT_INFO, &cdev, &cdec), "gsk_attribute_get_cert_info()",
        ...
        for(i = 0; i++ < cdec; p++)
            ...
            case CERT_BODY_DER:
                ...
                break;
            case CERT_DN_PRINTABLE: infof(data, "\t subject: %.*s\n", p->cert_data_l, p->cert_data_p); break;
            case CERT_ISSUER_DN_PRINTABLE: infof(data, "\t issuer: %.*s\n", p->cert_data_l, p->cert_data_p); break;
            case CERT_VALID_FROM: infof(data, "\t start date: %.*s\n", p->cert_data_l, p->cert_data_p); break;
            case CERT_VALID_TO: infof(data, "\t expire date: %.*s\n", p->cert_data_l, p->cert_data_p); break;
        }
    }
    ...
    if(!result && ptr) {
        ...
        curl_asn1Element *p;
        if(Curl_parseX509(&x509, cert, certend)) return CURLE_SSL_PINNEDPUBKEYNOTMATCH;
...
static CURLcode gskit_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool *done) {
    ...
    *done = connssl->state == ssl_connection_complete;
    if(*done) return CURLE_OK;
    if(connssl->connecting_state == ssl_connect_1) {
        timeout_ms = Curl_timeleft(data, NULL, TRUE);
        if(timeout_ms < 0) {
            failf(data, "SSL connection timeout");
            result = CURLE_OPERATION_TIMEDOUT;
        } else result = gskit_connect_step1(conn, sockindex);
    }
    if(!result)
        if(pipe_ssloverssl(conn, sockindex, SOS_READ | SOS_WRITE) < 0) result = CURLE_SSL_CONNECT_ERROR;
    if(!result && connssl->connecting_state == ssl_connect_2) {
        timeout_ms = Curl_timeleft(data, NULL, TRUE);
        if(timeout_ms < 0) {
            failf(data, "SSL connection timeout");
            result = CURLE_OPERATION_TIMEDOUT;
        } else result = gskit_connect_step2(conn, sockindex, nonblocking);
    }
    if(!result)
        if(pipe_ssloverssl(conn, sockindex, SOS_READ | SOS_WRITE) < 0) result = CURLE_SSL_CONNECT_ERROR;
    if(!result && connssl->connecting_state == ssl_connect_3) result = gskit_connect_step3(conn, sockindex);
    if(result) close_one(connssl, conn, sockindex);
...
static CURLcode Curl_gskit_connect_nonblocking(struct connectdata *conn, int sockindex, bool *done) {
    ...
    if(*done || result) conn->ssl[sockindex].connecting_state = ssl_connect_1;
    return result;
}
...
static int Curl_gskit_check_cxn(struct connectdata *cxn) {
    ...
    err = 0;
    errlen = sizeof(err);
    if(getsockopt(cxn->sock[FIRSTSOCKET], SOL_SOCKET, SO_ERROR, (unsigned char *) &err, &errlen) || errlen != sizeof(err) || err) return 0;
    return -1;
}
