...
static SECStatus set_ciphers(struct Curl_easy *data, PRFileDesc * model, char *cipher_list) {
    ...
    if(!implemented_ciphers) return SECFailure;
    for(i = 0; i < num_implemented_ciphers; i++) SSL_CipherPrefSet(model, implemented_ciphers[i], PR_FALSE);
    for(i = 0; i < NUM_OF_CIPHERS; i++) cipher_state[i] = PR_FALSE;
    cipher = cipher_list;
    while(cipher_list && (cipher_list[0])) {
        while((*cipher) && (ISSPACE(*cipher)) ++cipher;
        ...
        if(cipher_list) cipher = cipher_list;
    }
    for(i = 0; i<NUM_OF_CIPHERS; i++) {
        if(!cipher_state[i]) continue;
...
static int is_file(const char *filename) {
    ...
    if(stat(filename, &st) == 0)
        if(S_ISREG(st.st_mode) || S_ISFIFO(st.st_mode) || S_ISCHR(st.st_mode)) return 1;
    return 0;
}
static char *dup_nickname(struct Curl_easy *data, const char *str) {
    const char *n;
    if(!is_file(str)) return strdup(str);
    n = strchr(str, '/');
    if(!n) {
...
static CURLcode insert_wrapped_ptr(struct curl_llist *list, void *ptr) {
    ...
    if(!wrap) return CURLE_OUT_OF_MEMORY;
    wrap->ptr = ptr;
...
static CURLcode nss_create_object(struct ssl_connect_data *connssl, CK_OBJECT_CLASS obj_class, const char *filename, bool cacert) {
    ...
    if(!slot_name) return CURLE_OUT_OF_MEMORY;
    ...
    if(!cacert && CKO_CERTIFICATE == obj_class) backend->obj_clicert = obj;
    return CURLE_OK;
}
...
static CURLcode nss_load_cert(struct ssl_connect_data *ssl, const char *filename, PRBool cacert) {
    CURLcode result = (cacert) ? CURLE_SSL_CACERT_BADFILE : CURLE_SSL_CERTPROBLEM;
    if(is_file(filename)) result = nss_create_object(ssl, CKO_CERTIFICATE, filename, cacert);
    if(!result && !cacert) {
...
static CURLcode nss_cache_crl(SECItem *crl_der) {
    ...
    if(insert_wrapped_ptr(&nss_crl_list, crl_der) != CURLE_OK) {
        if(SECSuccess == CERT_UncacheCRL(db, crl_der)) SECITEM_FreeItem(crl_der, PR_TRUE);
        PR_Unlock(nss_crllock);
        return CURLE_OUT_OF_MEMORY;
    }
...
static CURLcode nss_load_crl(const char *crlfilename) {
    ...
    if(!infile) return CURLE_SSL_CRL_BADFILE;
    if(PR_SUCCESS != PR_GetOpenFileInfo(infile, &info)) goto fail;
    if(!SECITEM_AllocItem(NULL, &filedata, info.size + 1)) goto fail;
    if(info.size != PR_Read(infile, filedata.data, info.size)) goto fail;
    crl_der = SECITEM_AllocItem(NULL, NULL, 0U);
    if(!crl_der) goto fail;
    ...
    if(body) {
        ...
        if(!begin) begin = PORT_Strchr(body, '\r');
        if(!begin) goto fail;
        trailer = strstr(++begin, "-----END");
        if(!trailer) goto fail;
        *trailer = '\0';
        if(ATOB_ConvertAsciiToItem(crl_der, begin)) goto fail;
...
static CURLcode nss_load_key(struct connectdata *conn, int sockindex, char *key_file) {
    ...
    if(!slot) return CURLE_SSL_CERTPROBLEM;
    tmp = SECMOD_WaitForAnyTokenEvent(pem_module, 0, 0);
    if(tmp) PK11_FreeSlot(tmp);
...
static CURLcode cert_stuff(struct connectdata *conn, int sockindex, char *cert_file, char *key_file) {
    ...
    if(key_file || (is_file(cert_file))) {
        if(key_file) result = nss_load_key(conn, sockindex, key_file);
...
static void HandshakeCallback(PRFileDesc *sock, void *arg) {
    ...
    if(!conn->bits.tls_enable_npn && !conn->bits.tls_enable_alpn) return;
...
static SECStatus BadCertHandler(void *arg, PRFileDesc *sock) {
    ...
    CERTCertificate *cert;
    if(SSL_IS_PROXY()) data->set.proxy_ssl.certverifyresult = err;
    else data->set.ssl.certverifyresult = err;
    if(err == SSL_ERROR_BAD_CERT_DOMAIN && !SSL_CONN_CONFIG(verifyhost)) return SECSuccess;
    cert = SSL_PeerCertificate(sock);
    if(cert) {
...
static SECStatus check_issuer_cert(PRFileDesc *sock, char *issuer_nickname) {
    ...
    if((!cert_issuer) || (!issuer)) res = SECFailure;
    else if(SECITEM_CompareItem(&cert_issuer->derCert, &issuer->derCert) != SECEqual) res = SECFailure;
    CERT_DestroyCertificate(cert);
...
static CURLcode cmp_peer_pubkey(struct ssl_connect_data *connssl, const char *pinnedpubkey) {
    ...
    switch(result) {
        case CURLE_OK: infof(data, "pinned public key verified successfully!\n"); break;
        case CURLE_SSL_PINNEDPUBKEYNOTMATCH: failf(data, "failed to verify pinned public key"); break;
    }
    return result;
}
static SECStatus SelectClientCert(void *arg, PRFileDesc *sock, struct CERTDistNamesStr *caNames, struct CERTCertificateStr **pRetCert,
    ...
    if(SECSuccess != NSS_GetClientAuthData((void *)nickname, sock, caNames, pRetCert, pRetKey) || NULL == *pRetCert) {
        if(NULL == nickname) failf(data, "NSS: client certificate not found (nickname not specified)");
        else failf(data, "NSS: client certificate not found: %s", nickname);
        return SECFailure;
    }
    nickname = (*pRetCert)->nickname;
    if(NULL == nickname) nickname = "[unknown]";
...
static void nss_update_connecting_state(ssl_connect_state state, void *secret) {
    ...
    if(PR_GetError() != PR_WOULD_BLOCK_ERROR) return;
    switch(connssl->connecting_state) {
    case ssl_connect_2: case ssl_connect_2_reading: case ssl_connect_2_writing: break;
    default: return;
    }
...
static PRInt32 nspr_io_recv(PRFileDesc *fd, void *buf, PRInt32 amount, PRIntn flags, PRIntervalTime timeout) {
    ...
    if(rv < 0) nss_update_connecting_state(ssl_connect_2_reading, fd->secret);
    return rv;
}
static PRInt32 nspr_io_send(PRFileDesc *fd, const void *buf, PRInt32 amount, PRIntn flags, PRIntervalTime timeout) {
    ...
    if(rv < 0) nss_update_connecting_state(ssl_connect_2_writing, fd->secret);
    return rv;
}
...
static CURLcode nss_load_module(SECMODModule **pmod, const char *library, const char *name) {
    ...
    if(module) return CURLE_OK;
    config_string = aprintf("library=%s name=%s", library, name);
    if(!config_string) return CURLE_OUT_OF_MEMORY;
...
static void nss_unload_module(SECMODModule **pmod) {
    SECMODModule *module = *pmod;
    if(!module) return;
    if(SECMOD_UnloadUserModule(module) != SECSuccess) return;
...
static CURLcode nss_init_core(struct Curl_easy *data, const char *cert_dir) {
    ...
    PRErrorCode err;
    const char *err_name;
    if(nss_context != NULL) return CURLE_OK;
    memset((void *) &initparams, '\0', sizeof(initparams));
    initparams.length = sizeof(initparams);
    if(cert_dir) {
        char *certpath = aprintf("sql:%s", cert_dir);
        if(!certpath) return CURLE_OUT_OF_MEMORY;
    ...
    nss_context = NSS_InitContext("", "", "", "", &initparams, NSS_INIT_READONLY | NSS_INIT_NOCERTDB   | NSS_INIT_NOMODDB       | NSS_INIT_FORCEOPEN
...
static CURLcode nss_init(struct Curl_easy *data) {
    ...
    if(cert_dir) {
        if((stat(cert_dir, &st) != 0) || (!S_ISDIR(st.st_mode))) cert_dir = NULL;
    }
    if(!cert_dir) {
        if((stat(SSL_DIR, &st) == 0) && (S_ISDIR(st.st_mode))) cert_dir = (char *)SSL_DIR;
    }
    if(nspr_io_identity == PR_INVALID_IO_LAYER) {
        nspr_io_identity = PR_GetUniqueIdentity("libcurl");
        if(nspr_io_identity == PR_INVALID_IO_LAYER) return CURLE_OUT_OF_MEMORY;
    ...
    if(!any_cipher_enabled()) NSS_SetDomesticPolicy();
    initialized = 1;
    return CURLE_OK;
}
...
CURLcode Curl_nss_force_init(struct Curl_easy *data) {
    CURLcode result;
    if(!nss_initlock) {
        if(data) failf(data, "unable to initialize NSS, curl_global_init() should have been called with CURL_GLOBAL_SSL or CURL_GLOBAL_ALL");
        return CURLE_FAILED_INIT;
    }
...
static void nss_close(struct ssl_connect_data *connssl) {
    ...
    if(backend->handle) {
        if(client_cert) SSL_InvalidateSession(backend->handle);
...
static void Curl_nss_close(struct connectdata *conn, int sockindex) {
    ...
    if(backend->handle) connssl_proxy->backend->handle = NULL;
    nss_close(connssl);
    nss_close(connssl_proxy);
}
static bool is_nss_error(CURLcode err) {
    switch(err) {
        case CURLE_PEER_FAILED_VERIFICATION: case CURLE_SSL_CERTPROBLEM: case CURLE_SSL_CONNECT_ERROR: case CURLE_SSL_ISSUER_ERROR: return true;
        default: return false;
    }
}
static bool is_cc_error(PRInt32 err) {
    switch(err) {
        case SSL_ERROR_BAD_CERT_ALERT: case SSL_ERROR_EXPIRED_CERT_ALERT: case SSL_ERROR_REVOKED_CERT_ALERT: return true;
        default: return false;
    }
}
...
static CURLcode nss_load_ca_certificates(struct connectdata *conn, int sockindex) {
    ...
    CURLcode result = CURLE_OK;
    if(cafile && !cafile[0]) cafile = NULL;
    if(capath && !capath[0]) capath = NULL;
    ...
    if(use_trust_module && !trust_module) {
        ...
        if(result == CURLE_FAILED_INIT) result = CURLE_OK;
    } else if(!use_trust_module && trust_module) {
        infof(data, "unloading %s\n", trust_library);
        nss_unload_module(&trust_module);
    }
    PR_Unlock(nss_trustload_lock);
    if(cafile) result = nss_load_cert(&conn->ssl[sockindex], cafile, PR_TRUE);
    if(result) return result;
    if(capath) {
        struct_stat st;
        if(stat(capath, &st) == -1) return CURLE_SSL_CACERT_BADFILE;
        if(S_ISDIR(st.st_mode)) {
            PRDirEntry *entry;
            PRDir *dir = PR_OpenDir(capath);
            if(!dir) return CURLE_SSL_CACERT_BADFILE;
            while((entry = PR_ReadDir(dir, PR_SKIP_BOTH | PR_SKIP_HIDDEN))) {
                ...
                if(CURLE_OK != nss_load_cert(&conn->ssl[sockindex], fullpath, PR_TRUE)) infof(data, "failed to load '%s' from CURLOPT_CAPATH\n", fullpath);
                free(fullpath);
            }
            PR_CloseDir(dir);
...
static CURLcode nss_init_sslver(SSLVersionRange *sslver, struct Curl_easy *data, struct connectdata *conn) {
    ...
    switch(min) {
        case CURL_SSLVERSION_TLSv1: case CURL_SSLVERSION_DEFAULT:
            if(SSL_VersionRangeGetDefault(ssl_variant_stream, &vrange) != SECSuccess) return CURLE_SSL_CONNECT_ERROR;
            if(sslver->min < vrange.min) sslver->min = vrange.min;
            break;
        default:
    ...
    switch(max) {
        case CURL_SSLVERSION_MAX_NONE: case CURL_SSLVERSION_MAX_DEFAULT: break;
        default:
...
static CURLcode nss_fail_connect(struct ssl_connect_data *connssl, struct Curl_easy *data, CURLcode curlerr) {
    ...
    if(is_nss_error(curlerr)) {
        err = PR_GetError();
        if(is_cc_error(err)) curlerr = CURLE_SSL_CERTPROBLEM;
...
static CURLcode nss_set_blocking(struct ssl_connect_data *connssl, struct Curl_easy *data, bool blocking) {
    ...
    if(PR_SetSocketOption(backend->handle, &sock_opt) != PR_SUCCESS) return nss_fail_connect(connssl, data, CURLE_SSL_CONNECT_ERROR);
    return CURLE_OK;
}
static CURLcode nss_setup_connect(struct connectdata *conn, int sockindex) {
    ...
    PR_Unlock(nss_initlock);
    if(result == CURLE_FAILED_INIT) infof(data, "WARNING: failed to load NSS PEM library %s. Using OpenSSL PEM certificates will not work.\n", pem_library);
    else if(result) goto error;
    result = CURLE_SSL_CONNECT_ERROR;
    model = PR_NewTCPSocket();
    if(!model) goto error;
    model = SSL_ImportFD(NULL, model);
    if(SSL_OptionSet(model, SSL_SECURITY, PR_TRUE) != SECSuccess) goto error;
    if(SSL_OptionSet(model, SSL_HANDSHAKE_AS_SERVER, PR_FALSE) != SECSuccess) goto error;
    if(SSL_OptionSet(model, SSL_HANDSHAKE_AS_CLIENT, PR_TRUE) != SECSuccess) goto error;
    ssl_no_cache = (SSL_SET_OPTION(primary.sessionid) && SSL_CONN_CONFIG(verifypeer)) ? PR_FALSE : PR_TRUE;
    if(SSL_OptionSet(model, SSL_NO_CACHE, ssl_no_cache) != SECSuccess) goto error;
    if(nss_init_sslver(&sslver, data, conn) != CURLE_OK) goto error;
    if(SSL_VersionRangeGetSupported(ssl_variant_stream, &sslver_supported) != SECSuccess) goto error;
    if(sslver_supported.max < sslver.max && sslver_supported.max >= sslver.min) {
        ...
        if(sslver_req_str && sslver_supp_str) infof(data, "Falling back from %s to max supported SSL version (%s)\n", sslver_req_str, sslver_supp_str);
        free(sslver_req_str);
        free(sslver_supp_str);
        sslver.max = sslver_supported.max;
    }
    if(SSL_VersionRangeSet(model, &sslver) != SECSuccess) goto error;
    ssl_cbc_random_iv = !SSL_SET_OPTION(enable_beast);
#ifdef SSL_CBC_RANDOM_IV
    if(SSL_OptionSet(model, SSL_CBC_RANDOM_IV, ssl_cbc_random_iv) != SECSuccess) infof(data, "warning: failed to set SSL_CBC_RANDOM_IV = %d\n", ssl_cbc_random_iv);
#else
    if(ssl_cbc_random_iv) infof(data, "warning: support for SSL_CBC_RANDOM_IV not compiled in\n");
#endif
    ...
    if(!SSL_CONN_CONFIG(verifypeer) && SSL_CONN_CONFIG(verifyhost)) infof(data, "warning: ignoring value of ssl.verifyhost\n");
    if(SSL_AuthCertificateHook(model, nss_auth_cert_hook, conn) != SECSuccess) goto error;
    if(SSL_IS_PROXY()) data->set.proxy_ssl.certverifyresult = 0;
    else data->set.ssl.certverifyresult = 0;
    if(SSL_BadCertHook(model, BadCertHandler, conn) != SECSuccess) goto error;
    if(SSL_HandshakeCallback(model, HandshakeCallback, conn) != SECSuccess) goto error;
    const CURLcode rv = nss_load_ca_certificates(conn, sockindex);
    if((rv == CURLE_SSL_CACERT_BADFILE) && !SSL_CONN_CONFIG(verifypeer)) infof(data, "warning: CA certificates failed to load\n");
    else if(rv) {
        result = rv;
        goto error;
    }
    ...
    if(SSL_SET_OPTION(cert)) {
        ...
        if(nickname) backend->obj_clicert = NULL;
        else {
    ...
    if(conn->proxy_ssl[sockindex].use) {
        ...
        if(!nspr_io) goto error;
    }
    nspr_io_stub = PR_CreateIOLayerStub(nspr_io_identity, &nspr_io_methods);
    if(!nspr_io_stub) {
        if(!second_layer) PR_Close(nspr_io);
        goto error;
    }
    ...
    PR_Close(model);
    model = NULL;
    if(SSL_SET_OPTION(key_passwd)) SSL_SetPKCS11PinArg(backend->handle, SSL_SET_OPTION(key_passwd));
#ifdef SSL_ENABLE_OCSP_STAPLING
    if(SSL_CONN_CONFIG(verifystatus)) {
        if(SSL_OptionSet(backend->handle, SSL_ENABLE_OCSP_STAPLING, PR_TRUE) != SECSuccess) goto error;
    }
#endif
#ifdef SSL_ENABLE_NPN
    if(SSL_OptionSet(backend->handle, SSL_ENABLE_NPN, conn->bits.tls_enable_npn ? PR_TRUE : PR_FALSE) != SECSuccess) goto error;
#endif
#ifdef SSL_ENABLE_ALPN
    if(SSL_OptionSet(backend->handle, SSL_ENABLE_ALPN, conn->bits.tls_enable_alpn ? PR_TRUE : PR_FALSE) != SECSuccess) goto error;
#endif
#if NSSVERNUM >= 0x030f04
    if(data->set.ssl.falsestart) {
        if(SSL_OptionSet(backend->handle, SSL_ENABLE_FALSE_START, PR_TRUE) != SECSuccess) goto error;
        if(SSL_SetCanFalseStartCallback(backend->handle, CanFalseStartCallback, conn) != SECSuccess) goto error;
    }
#endif
    ...
    if(conn->bits.tls_enable_npn || conn->bits.tls_enable_alpn) {
        ...
        if(SSL_SetNextProtoNego(backend->handle, protocols, cur) != SECSuccess) goto error;
    }
#endif
    if(SSL_ResetHandshake(backend->handle, PR_FALSE) != SECSuccess) goto error;
    if(SSL_SetURL(backend->handle, SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name) != SECSuccess) goto error;
    if(SSL_SetSockPeerID(backend->handle, SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name) != SECSuccess) goto error;
    return CURLE_OK;
    error:
    if(model) PR_Close(model);
...
static CURLcode nss_do_connect(struct connectdata *conn, int sockindex) {
    ...
    if(SSL_ForceHandshakeWithTimeout(backend->handle, timeout) != SECSuccess) {
        if(PR_GetError() == PR_WOULD_BLOCK_ERROR) return CURLE_AGAIN;
        else if(*certverifyresult == SSL_ERROR_BAD_CERT_DOMAIN) result = CURLE_PEER_FAILED_VERIFICATION;
        else if(*certverifyresult != 0) result = CURLE_PEER_FAILED_VERIFICATION;
        goto error;
    }
    ...
    if(SSL_SET_OPTION(issuercert)) {
        ...
        if(SECFailure == ret) {
            ...
            goto error;
        } else  infof(data, "SSL certificate issuer check ok\n");
    }
...
static CURLcode nss_connect_common(struct connectdata *conn, int sockindex, bool *done) {
    ...
    switch(result) {
        case CURLE_OK: break;
        case CURLE_AGAIN: if(!blocking) return CURLE_OK;
...
static ssize_t nss_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    ...
    if(rc < 0) {
        ...
        if(err == PR_WOULD_BLOCK_ERROR) *curlcode = CURLE_AGAIN;
        else {
            const char *err_name = nss_error_to_name(err);
...
static ssize_t nss_recv(struct connectdata *conn, int sockindex, char *buf, size_t buffersize, CURLcode *curlcode) {
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    struct ssl_backend_data *backend = connssl->backend;
    ssize_t nread;
    backend->data = conn->data;
...
static CURLcode Curl_nss_random(struct Curl_easy *data, unsigned char *entropy, size_t length) {
  Curl_nss_seed(data);
  if(SECSuccess != PK11_GenerateRandom(entropy, curlx_uztosi(length))) return CURLE_FAILED_INIT;
  return CURLE_OK;
}
static CURLcode Curl_nss_md5sum(unsigned char *tmp, size_t tmplen, unsigned char *md5sum, size_t md5len) {
    PK11Context *MD5pw = PK11_CreateDigestContext(SEC_OID_MD5);
    unsigned int MD5out;
    if(!MD5pw) return CURLE_NOT_BUILT_IN;
    PK11_DigestOp(MD5pw, tmp, curlx_uztoui(tmplen));
    PK11_DigestFinal(MD5pw, md5sum, &MD5out, curlx_uztoui(md5len));
    PK11_DestroyContext(MD5pw, PR_TRUE);
    return CURLE_OK;
}
static CURLcode Curl_nss_sha256sum(const unsigned char *tmp, size_t tmplen, unsigned char *sha256sum, size_t sha256len) {
    PK11Context *SHA256pw = PK11_CreateDigestContext(SEC_OID_SHA256);
    unsigned int SHA256out;
    if(!SHA256pw) return CURLE_NOT_BUILT_IN;
