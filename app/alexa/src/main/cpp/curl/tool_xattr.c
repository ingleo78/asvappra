...
int fwrite_xattr(CURL *curl, int fd) {
    ...
    while(err == 0 && mappings[i].attr != NULL) {
        ...
        if(!result && value) {
            bool freeptr = FALSE;
            if(CURLINFO_EFFECTIVE_URL == mappings[i].info) freeptr = stripcredentials(&value);
            if(value) {
                ...
                ssize_t rc = extattr_set_fd(fd, EXTATTR_NAMESPACE_USER, mappings[i].attr, value, strlen(value));
                err = (rc < 0 ? -1 : 0);
            #endif
                if(freeptr) curl_free(value);
            }
        }
        i++;
    }
