...
void Curl_ftp_parselist_data_free(struct ftp_parselist_data **parserp) {
    ...
    if(parser) Curl_fileinfo_cleanup(parser->file_data);
    free(parser);
    *parserp = NULL;
}
...
static int ftp_pl_get_permission(const char *str) {
    int permissions = 0;
    if(str[0] == 'r') permissions |= 1 << 8;
    else if(str[0] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[1] == 'w') permissions |= 1 << 7;
    else if(str[1] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[2] == 'x') permissions |= 1 << 6;
    else if(str[2] == 's') {
        permissions |= 1 << 6;
        permissions |= 1 << 11;
    } else if(str[2] == 'S') permissions |= 1 << 11;
    else if(str[2] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[3] == 'r') permissions |= 1 << 5;
    else if(str[3] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[4] == 'w') permissions |= 1 << 4;
    else if(str[4] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[5] == 'x') permissions |= 1 << 3;
    else if(str[5] == 's') {
        permissions |= 1 << 3;
        permissions |= 1 << 10;
    } else if(str[5] == 'S') permissions |= 1 << 10;
    else if(str[5] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[6] == 'r') permissions |= 1 << 2;
    else if(str[6] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[7] == 'w') permissions |= 1 << 1;
    else if(str[7] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    if(str[8] == 'x') permissions |= 1;
    else if(str[8] == 't') {
        permissions |= 1;
        permissions |= 1 << 9;
    } else if(str[8] == 'T') permissions |= 1 << 9;
    else if(str[8] != '-') permissions |= FTP_LP_MALFORMATED_PERM;
    return permissions;
}
static CURLcode ftp_pl_insert_finfo(struct connectdata *conn, struct fileinfo *infop) {
    ...
    if (!compare) compare = Curl_fnmatch;
    Curl_set_in_callback(conn->data, true);
    if (compare(conn->data->set.fnmatch_data, wc->pattern, finfo->filename) == 0) {
        if ((finfo->filetype == CURLFILETYPE_SYMLINK) && finfo->strings.target && (strstr(finfo->strings.target, " -> "))) add = FALSE;
    } else add = FALSE;
    Curl_set_in_callback(conn->data, false);
    if (add) Curl_llist_insert_next(llist, llist->tail, finfo, &infop->list);
    else Curl_fileinfo_cleanup(infop);
...
size_t Curl_ftp_parselist(char *buffer, size_t size, size_t nmemb, void *connptr) {
    ...
    if (parser->error) goto fail;
    if (parser->os_type == OS_TYPE_UNKNOWN && bufflen > 0) parser->os_type = (buffer[0] >= '0' && buffer[0] <= '9') ? OS_TYPE_WIN_NT : OS_TYPE_UNIX;
    while(i < bufflen) {
        char c = buffer[i];
        if(!parser->file_data) {
                ...
                switch(parser->state.UNIX.main) {
                    ...
                    case PL_UNIX_TIME:
                        switch(parser->state.UNIX.sub.time) {
                            ...
                            case PL_UNIX_TIME_PART1:
                                parser->item_length++;
                                if(c == ' ') parser->state.UNIX.sub.time = PL_UNIX_TIME_PREPART2;
                                else if(!ISALNUM(c) && c != '.') {
                                    parser->error = CURLE_FTP_BAD_FILE_LIST;
                                    goto fail;
                                }
                            ...
                            case PL_UNIX_TIME_PREPART2:
                                parser->item_length++;
                                if(c != ' ') {
                                    if(ISALNUM(c)) parser->state.UNIX.sub.time = PL_UNIX_TIME_PART2;
                                    else {
                                        parser->error = CURLE_FTP_BAD_FILE_LIST;
                                        goto fail;
                                    }
                            ...
                            case PL_UNIX_TIME_PART2:
                                parser->item_length++;
                                if(c == ' ') parser->state.UNIX.sub.time = PL_UNIX_TIME_PREPART3;
                                else if(!ISALNUM(c) && c != '.') {
                                    parser->error = CURLE_FTP_BAD_FILE_LIST;
                                    goto fail;
                                }
                            ...
                            case PL_UNIX_TIME_PREPART3:
                                parser->item_length++;
                                if(c != ' ') {
                                    if(ISALNUM(c)) parser->state.UNIX.sub.time = PL_UNIX_TIME_PART3;
                                    else {
                                      parser->error = CURLE_FTP_BAD_FILE_LIST;
                                      goto fail;
                                    }
                    ...
                    case PL_UNIX_FILENAME:
                        switch(parser->state.UNIX.sub.filename) {
                            ...
                            case PL_UNIX_FILENAME_NAME:
                                parser->item_length++;
                                if(c == '\r') parser->state.UNIX.sub.filename = PL_UNIX_FILENAME_WINDOWSEOL;
                                else if(c == '\n') {
                                finfo->b_data[parser->item_offset + parser->item_length - 1] = 0;
                    ...
                    switch(parser->state.UNIX.sub.symlink) {
                        ...
                        case PL_UNIX_SYMLINK_NAME:
                            parser->item_length++;
                            if(c == ' ') parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_PRETARGET1;
                            else if(c == '\r' || c == '\n') {
                                parser->error = CURLE_FTP_BAD_FILE_LIST;
                                goto fail;
                            }
                        ...
                        case PL_UNIX_SYMLINK_PRETARGET1:
                            parser->item_length++;
                            if(c == '-') parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_PRETARGET2;
                            else if(c == '\r' || c == '\n') {
                                parser->error = CURLE_FTP_BAD_FILE_LIST;
                                goto fail;
                            } else parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_NAME;
                            break;
                        case PL_UNIX_SYMLINK_PRETARGET2:
                            parser->item_length++;
                            if(c == '>') parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_PRETARGET3;
                            else if(c == '\r' || c == '\n') {
                                parser->error = CURLE_FTP_BAD_FILE_LIST;
                                goto fail;
                            } else  parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_NAME;
                        ...
                        case PL_UNIX_SYMLINK_TARGET:
                            parser->item_length++;
                            if(c == '\r') parser->state.UNIX.sub.symlink = PL_UNIX_SYMLINK_WINDOWSEOL;
                            else if(c == '\n') {
                                finfo->b_data[parser->item_offset + parser->item_length - 1] = 0;
                ...
                switch(parser->state.NT.main) {
                    ...
                    case PL_WINNT_TIME:
                        ...
                        switch(parser->state.NT.sub.time) {
                            case PL_WINNT_TIME_PRESPACE: if(!ISSPACE(c)) parser->state.NT.sub.time = PL_WINNT_TIME_TIME; break;
