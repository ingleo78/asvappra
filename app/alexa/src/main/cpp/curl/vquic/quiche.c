...
static CURLcode process_ingress(struct connectdata *conn, curl_socket_t sockfd, struct quicsocket *qs);
...
static CURLcode qs_disconnect(struct quicsocket *qs) {
    if(qs->h3config) quiche_h3_config_free(qs->h3config);
    if(qs->h3c) quiche_h3_conn_free(qs->h3c);
...
CURLcode Curl_quic_connect(struct connectdata *conn, curl_socket_t sockfd, int sockindex, const struct sockaddr *addr, socklen_t addrlen) {
    ...
    quiche_config_set_application_protos(qs->cfg, (uint8_t*)QUICHE_H3_APPLICATION_PROTOCOL, sizeof(QUICHE_H3_APPLICATION_PROTOCOL) - 1);
    result = Curl_rand(data, qs->scid, sizeof(qs->scid));
    if(result) return result;
    keylog_file = getenv("SSLKEYLOGFILE");
    if(keylog_file) quiche_config_log_keys(qs->cfg);
    qs->conn = quiche_connect(conn->host.name, (const uint8_t *) qs->scid, sizeof(qs->scid), qs->cfg);
    if(!qs->conn) {
        failf(data, "can't create quiche connection");
        return CURLE_OUT_OF_MEMORY;
    }
    if(keylog_file) quiche_conn_set_keylog_path(qs->conn, keylog_file);
#if !defined(WIN32) && defined(HAVE_QUICHE_CONN_SET_QLOG_FD)
    int qfd;
    (void)Curl_qlogdir(data, qs->scid, sizeof(qs->scid), &qfd);
    if(qfd != -1) quiche_conn_set_qlog_fd(qs->conn, qfd, "qlog title", "curl qlog");
#endif
...
static CURLcode quiche_has_connected(struct connectdata *conn, int sockindex, int tempindex) {
    ...
    if(!qs->h3config) return CURLE_OUT_OF_MEMORY;
...
static CURLcode process_ingress(struct connectdata *conn, int sockfd, struct quicsocket *qs) {
    ...
    do {
        ...
        if((recvd < 0) && ((SOCKERRNO == EAGAIN) || (SOCKERRNO == EWOULDBLOCK))) break;
        if(recvd < 0) {
            failf(conn->data, "quiche: recv() unexpectedly returned %d (errno: %d, socket %d)", recvd, SOCKERRNO, sockfd);
            return CURLE_RECV_ERROR;
        }
        recvd = quiche_conn_recv(qs->conn, buf, recvd);
        if(recvd == QUICHE_ERR_DONE) break;
        if(recvd < 0) {
...
static CURLcode flush_egress(struct connectdata *conn, int sockfd, struct quicsocket *qs) {
    ...
    if(timeout_ns) Curl_expire(conn->data, (timeout_ns + 999999) / 1000000, EXPIRE_QUIC);
    return CURLE_OK;
}
struct h3h1header {
    char *dest;
...
static int cb_each_header(uint8_t *name, size_t name_len, uint8_t *value, size_t value_len, void *argp) {
    ...
    size_t olen = 0;
    if((name_len == 7) && !strncmp(":status", (char *)name, 7)) msnprintf(headers->dest, headers->destlen, "HTTP/3 %.*s\n", (int)value_len, value);
    else if(!headers->nlen) return CURLE_HTTP3;
    else msnprintf(headers->dest, headers->destlen, "%.*s: %.*s\n", (int)name_len, name, (int) value_len, value);
...
static ssize_t h3_stream_recv(struct connectdata *conn, int sockindex, char *buf, size_t buffersize, CURLcode *curlcode) {
    ssize_t recvd = -1;
    ssize_t rcode;
    struct quicsocket *qs = conn->quic;
    ...
    *curlcode = (-1 == recvd)? CURLE_AGAIN : CURLE_OK;
    if(recvd >= 0) Curl_expire(data, 0, EXPIRE_QUIC);
    data->state.drain = (recvd >= 0) ? 1 : 0;
    return recvd;
}
static ssize_t h3_stream_send(struct connectdata *conn, int sockindex, const void *mem, size_t len, CURLcode *curlcode) {
    ssize_t sent;
    struct quicsocket *qs = conn->quic;
    curl_socket_t sockfd = conn->sock[sockindex];
    struct HTTP *stream = conn->data->req.protop;
    if(!stream->h3req) {
        ...
        sent = quiche_h3_send_body(qs->h3c, qs->conn, stream->stream3_id, (uint8_t*)mem, len, FALSE);
        if(sent < 0) {
            *curlcode = CURLE_SEND_ERROR;
            return -1;
        }
...
static CURLcode http_request(struct connectdata *conn, const void *mem, size_t len) {
    ...
    for(i = 0; i < nheader; ++i) {
        ...
        H3BUGF(infof(data, "h3 [%.*s: %.*s]\n", nva[i].name_len, nva[i].name, nva[i].value_len, nva[i].value));
    }
    if(acc > MAX_ACC) {
    ...
    switch(data->state.httpreq) {
        case HTTPREQ_POST: case HTTPREQ_POST_FORM: case HTTPREQ_POST_MIME: case HTTPREQ_PUT:
            ...
            stream3_id = quiche_h3_send_request(qs->h3c, qs->conn, nva, nheader, stream->upload_left ? FALSE: TRUE);
            if((stream3_id >= 0) && data->set.postfields) {
                ssize_t sent = quiche_h3_send_body(qs->h3c, qs->conn, stream3_id, (uint8_t*)data->set.postfields, stream->upload_left, TRUE);
                if(sent <= 0) {
