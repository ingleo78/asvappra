...
static char *c_escape(const char *str, size_t len) {
    ...
    unsigned char c;
    char *escaped, *e;
    if(len == CURL_ZERO_TERMINATED) len = strlen(str);
    if(len > (~(size_t) 0) / 4) return NULL;
...
CURLcode tool_setopt_enum(CURL *curl, struct GlobalConfig *config, const char *name, CURLoption tag, const struct NameValue *nvlist, long lval) {
    ...
    if(config->libcurl && !skip && !ret) {
        ...
        if(! nv->name) CODE2("curl_easy_setopt(hnd, %s, %ldL);", name, lval);
        else CODE2("curl_easy_setopt(hnd, %s, (long)%s);", name, nv->name);
    }
    nomem:
    return ret;
}
CURLcode tool_setopt_flags(CURL *curl, struct GlobalConfig *config, const char *name, CURLoption tag, const struct NameValue *nvlist, long lval) {
    CURLcode ret = CURLE_OK;
    bool skip = FALSE;
    ret = curl_easy_setopt(curl, tag, lval);
    if(!lval) skip = TRUE;
    if(config->libcurl && !skip && !ret) {
        ...
        if(rest) CODE2("%s%ldL);", preamble, rest);
    }
    nomem:
    return ret;
}
CURLcode tool_setopt_bitmask(CURL *curl, struct GlobalConfig *config, const char *name, CURLoption tag, const struct NameValueUnsigned *nvlist, long lval) {
    CURLcode ret = CURLE_OK;
    bool skip = FALSE;
    ret = curl_easy_setopt(curl, tag, lval);
    if(!lval) skip = TRUE;
    if(config->libcurl && !skip && !ret) {
        ...
        if(rest) CODE2("%s%luUL);", preamble, rest);
    }
    nomem:
    return ret;
}
static CURLcode libcurl_generate_slist(struct curl_slist *slist, int *slistno) {
    ...
    *slistno = ++easysrc_slist_count;
    ...
    for(; slist; slist = slist->next) {
        ...
        if(!escaped) return CURLE_OUT_OF_MEMORY;
...
static CURLcode libcurl_generate_mime(CURL *curl, struct GlobalConfig *config, struct tool_mime *toolmime, int *mimeno);
static CURLcode libcurl_generate_mime_part(CURL *curl, struct GlobalConfig *config, struct tool_mime *part, int mimeno) {
    CURLcode ret = CURLE_OK;
    int submimeno = 0;
    char *escaped = NULL;
    ...
    switch(part->kind) {
        ...
        case TOOLMIME_FILE: case TOOLMIME_FILEDATA:
            ...
            if(part->kind == TOOLMIME_FILEDATA && !filename) CODE1("curl_mime_filename(part%d, NULL);", mimeno);
            break;
...
CURLcode tool_setopt_mimepost(CURL *curl, struct GlobalConfig *config, const char *name, CURLoption tag, curl_mime *mimepost) {
    ...
    if(!ret && config->libcurl) {
        ...
        if(!ret) CODE2("curl_easy_setopt(hnd, %s, mime%d);", name, mimeno);
    }
    nomem:
    return ret;
}
CURLcode tool_setopt_slist(CURL *curl, struct GlobalConfig *config, const char *name, CURLoption tag, struct curl_slist *list) {
    ...
    if(config->libcurl && list && !ret) {
        ...
        if(!ret) CODE2("curl_easy_setopt(hnd, %s, slist%d);", name, i);
    }
    nomem:
    return ret;
}
CURLcode tool_setopt(CURL *curl, bool str, struct GlobalConfig *config, const char *name, CURLoption tag, ...) {
    ...
    va_end(arg);
    if(config->libcurl && !skip && !ret) {
        if(remark) REM2("%s set to a %s", name, value);
        else {
            if(escape) {
                escaped = c_escape(value, CURL_ZERO_TERMINATED);
                NULL_CHECK(escaped);
                CODE2("curl_easy_setopt(hnd, %s, \"%s\");", name, escaped);
            } else CODE2("curl_easy_setopt(hnd, %s, %s);", name, value);
        }
    }
    nomem:
    Curl_safefree(escaped);
...
bool tool_setopt_skip(CURLoption tag) {
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_HAPROXYPROTOCOL: case CURLOPT_HTTPPROXYTUNNEL: case CURLOPT_NOPROXY: case CURLOPT_PRE_PROXY: case CURLOPT_PROXY: case CURLOPT_PROXYAUTH:
        case CURLOPT_PROXY_CAINFO: case CURLOPT_PROXY_CAPATH: case CURLOPT_PROXY_CRLFILE: case CURLOPT_PROXYHEADER: case CURLOPT_PROXY_KEYPASSWD:
        case CURLOPT_PROXYPASSWORD: case CURLOPT_PROXY_PINNEDPUBLICKEY: case CURLOPT_PROXYPORT: case CURLOPT_PROXY_SERVICE_NAME:
        case CURLOPT_PROXY_SSLCERT: case CURLOPT_PROXY_SSLCERTTYPE: case CURLOPT_PROXY_SSL_CIPHER_LIST: case CURLOPT_PROXY_SSLKEY:
        case CURLOPT_PROXY_SSLKEYTYPE: case CURLOPT_PROXY_SSL_OPTIONS: case CURLOPT_PROXY_SSL_VERIFYHOST: case CURLOPT_PROXY_SSL_VERIFYPEER:
        case CURLOPT_PROXY_SSLVERSION: case CURLOPT_PROXY_TLS13_CIPHERS: case CURLOPT_PROXY_TLSAUTH_PASSWORD: case CURLOPT_PROXY_TLSAUTH_TYPE:
        case CURLOPT_PROXY_TLSAUTH_USERNAME: case CURLOPT_PROXY_TRANSFER_MODE: case CURLOPT_PROXYTYPE: case CURLOPT_PROXYUSERNAME:
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_FTPPORT: case CURLOPT_FTP_ACCOUNT: case CURLOPT_FTP_ALTERNATIVE_TO_USER: case CURLOPT_FTP_FILEMETHOD: case CURLOPT_FTP_SKIP_PASV_IP:
        case CURLOPT_FTP_USE_EPRT: case CURLOPT_FTP_USE_EPSV: case CURLOPT_FTP_USE_PRET: case CURLOPT_KRBLEVEL:
            return TRUE;
        default: break;
    }
#endif
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_INTERLEAVEDATA: return TRUE;
        default: break;
    }
#endif
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_COOKIE: case CURLOPT_COOKIEFILE: case CURLOPT_COOKIEJAR: case CURLOPT_COOKIESESSION: return TRUE;
        default: break;
    }
#endif
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_TELNETOPTIONS: return TRUE;
        default: break;
    }
#endif
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_TFTP_BLKSIZE: case CURLOPT_TFTP_NO_OPTIONS: return TRUE;
        default: break;
    }
#endif
    ...
    #define USED_TAG
    switch(tag) {
        case CURLOPT_NETRC: case CURLOPT_NETRC_FILE: return TRUE;
        default: break;
    }
#endif
#ifndef USED_TAG
