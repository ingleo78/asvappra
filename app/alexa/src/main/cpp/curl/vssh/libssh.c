...
static CURLcode sftp_error_to_CURLE(int err) {
    switch(err) {
        case SSH_FX_OK: return CURLE_OK;
        case SSH_FX_NO_SUCH_FILE: case SSH_FX_NO_SUCH_PATH: return CURLE_REMOTE_FILE_NOT_FOUND;
        case SSH_FX_PERMISSION_DENIED: case SSH_FX_WRITE_PROTECT: return CURLE_REMOTE_ACCESS_DENIED;
        case SSH_FX_FILE_ALREADY_EXISTS: return CURLE_REMOTE_FILE_EXISTS;
    }
    return CURLE_SSH;
}
#ifndef DEBUGBUILD
...
static int myssh_is_known(struct connectdata *conn) {
    ...
    if(data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5]) {
        ...
        for(i = 0; i < 16; i++) msnprintf(&md5buffer[i*2], 3, "%02x", (unsigned char)hash[i]);
    ...
    if(knownhostsentry) {
        if(knownhostsentry->publickey) {
            ...
            switch(ssh_key_type(knownhostsentry->publickey)) {
                case SSH_KEYTYPE_RSA: knownkey.keytype = CURLKHTYPE_RSA; break;
                case SSH_KEYTYPE_RSA1: knownkey.keytype = CURLKHTYPE_RSA1; break;
                case SSH_KEYTYPE_ECDSA: case SSH_KEYTYPE_ECDSA_P256: case SSH_KEYTYPE_ECDSA_P384: case SSH_KEYTYPE_ECDSA_P521:
                    knownkey.keytype = CURLKHTYPE_ECDSA;
                    break;
                case SSH_KEYTYPE_ED25519: knownkey.keytype = CURLKHTYPE_ED25519; break;
                case SSH_KEYTYPE_DSS: knownkey.keytype = CURLKHTYPE_DSS; break;
                default:
                    rc = SSH_ERROR;
                    goto cleanup;
            }
    ...
    switch(vstate) {
        case SSH_KNOWN_HOSTS_OK: keymatch = CURLKHMATCH_OK; break;
        case SSH_KNOWN_HOSTS_OTHER: /* fallthrough */
        case SSH_KNOWN_HOSTS_NOT_FOUND: /* fallthrough */
        case SSH_KNOWN_HOSTS_UNKNOWN: /* fallthrough */
        case SSH_KNOWN_HOSTS_ERROR: keymatch = CURLKHMATCH_MISSING; break;
        default: keymatch = CURLKHMATCH_MISMATCH; break;
    }
#else
    vstate = ssh_is_server_known(sshc->ssh_session);
    switch(vstate) {
        case SSH_SERVER_KNOWN_OK: keymatch = CURLKHMATCH_OK; break;
        case SSH_SERVER_FILE_NOT_FOUND: /* fallthrough */
        case SSH_SERVER_NOT_KNOWN: keymatch = CURLKHMATCH_MISSING; break;
        default: keymatch = CURLKHMATCH_MISMATCH; break;
    }
#endif
    if(func) {
        ...
        switch(ssh_key_type(pubkey)) {
            case SSH_KEYTYPE_RSA: foundkey.keytype = CURLKHTYPE_RSA; break;
            case SSH_KEYTYPE_RSA1: foundkey.keytype = CURLKHTYPE_RSA1; break;
            ...
            case SSH_KEYTYPE_ED25519: foundkey.keytype = CURLKHTYPE_ED25519; break;
        #endif
            case SSH_KEYTYPE_DSS: foundkey.keytype = CURLKHTYPE_DSS; break;
            default: rc = SSH_ERROR; goto cleanup;
        }
    ...
    rc = SSH_OK;
    cleanup:
    if(found_base64) free(found_base64);
    if(known_base64) free(known_base64);
    if(hash) ssh_clean_pubkey_hash(&hash);
    ssh_key_free(pubkey);
#if LIBSSH_VERSION_INT >= SSH_VERSION_INT(0,9,0)
    if(knownhostsentry) ssh_knownhosts_entry_free(knownhostsentry);
#endif
    return rc;
}
...
restart:
    switch(sshc->kbd_state) {
        case 0:
            ...
            if(rc == SSH_AUTH_AGAIN) return SSH_AGAIN;
            if(rc != SSH_AUTH_INFO) return SSH_ERROR;
            nprompts = ssh_userauth_kbdint_getnprompts(sshc->ssh_session);
            if(nprompts != 1) return SSH_ERROR;
        ...
        case 1:
            ...
            rc = ssh_userauth_kbdint(sshc->ssh_session, NULL, NULL);
            if(rc == SSH_AUTH_AGAIN) return SSH_AGAIN;
            else if(rc == SSH_AUTH_SUCCESS) rc = SSH_OK;
            else if(rc == SSH_AUTH_INFO) {
                nprompts = ssh_userauth_kbdint_getnprompts(sshc->ssh_session);
                if(nprompts != 0) return SSH_ERROR;
                sshc->kbd_state = 2;
                goto restart;
            } else rc = SSH_ERROR;
            break;
    ...
    do {
        switch(sshc->state) {
            ...
            case SSH_HOSTKEY:
                ...
                if(rc != SSH_OK)   MOVE_TO_ERROR_STATE(CURLE_PEER_FAILED_VERIFICATION);
            ...
            case SSH_AUTHLIST:{
                    ...
                    if(rc == SSH_AUTH_SUCCESS) {
                        ...
                        break;
                    } else if(rc == SSH_AUTH_ERROR) MOVE_TO_ERROR_STATE(CURLE_LOGIN_DENIED);
                    sshc->auth_methods = ssh_userauth_list(sshc->ssh_session, NULL);
                    if(sshc->auth_methods & SSH_AUTH_METHOD_PUBLICKEY) {
                        state(conn, SSH_AUTH_PKEY_INIT);
                        infof(data, "Authentication using SSH public key file\n");
                    } else if(sshc->auth_methods & SSH_AUTH_METHOD_GSSAPI_MIC) state(conn, SSH_AUTH_GSSAPI);
                    else if(sshc->auth_methods & SSH_AUTH_METHOD_INTERACTIVE) state(conn, SSH_AUTH_KEY_INIT);
                    else if(sshc->auth_methods & SSH_AUTH_METHOD_PASSWORD) state(conn, SSH_AUTH_PASS_INIT);
                    else MOVE_TO_ERROR_STATE(CURLE_LOGIN_DENIED);
                    break;
                }
            case SSH_AUTH_PKEY_INIT:
                if(!(data->set.ssh_auth_types & CURLSSH_AUTH_PUBLICKEY)) MOVE_TO_SECONDARY_AUTH;
                if(data->set.str[STRING_SSH_PRIVATE_KEY]) {
                    if(sshc->pubkey && !data->set.ssl.key_passwd) {
                        ...
                        if(rc != SSH_OK) MOVE_TO_SECONDARY_AUTH;
                    }
            ...
            case SSH_AUTH_GSSAPI:
                if(!(data->set.ssh_auth_types & CURLSSH_AUTH_GSSAPI)) MOVE_TO_TERTIARY_AUTH;
            ...
            case SSH_AUTH_KEY_INIT:
                if(data->set.ssh_auth_types & CURLSSH_AUTH_KEYBOARD) state(conn, SSH_AUTH_KEY);
                else MOVE_TO_LAST_AUTH;
                break;
            case SSH_AUTH_KEY:
            ...
            case SSH_AUTH_PASS_INIT:
                if(!(data->set.ssh_auth_types & CURLSSH_AUTH_PASSWORD)) MOVE_TO_ERROR_STATE(CURLE_LOGIN_DENIED);
            ...
            case SSH_SFTP_REALPATH:
                ...
                if(sshc->homedir == NULL) MOVE_TO_ERROR_STATE(CURLE_COULDNT_CONNECT);
            ...
            case SSH_SFTP_POSTQUOTE_INIT:
                ...
                break;
            case SSH_SFTP_QUOTE: sftp_quote(conn); break;
            case SSH_SFTP_NEXT_QUOTE:
                ...
                break;
            case SSH_SFTP_QUOTE_STAT: sftp_quote_stat(conn); break;
            ...
            case SSH_SFTP_QUOTE_STATVFS: {
                    ...
                    if(!statvfs && !sshc->acceptfail) {
                        ...
                        char *tmp = aprintf("statvfs:\n" "f_bsize: %llu\n" "f_frsize: %llu\nf_blocks: %llu\n" "f_bfree: %llu\nf_bavail: %llu\n" "f_files: %llu\n"
                                            "f_ffree: %llu\n" "f_favail: %llu\nf_fsid: %llu\n" "f_flag: %llu\nf_namemax: %llu\n",
                                            statvfs->f_bsize, statvfs->f_frsize, statvfs->f_blocks, statvfs->f_bfree, statvfs->f_bavail, statvfs->f_files,
                                            statvfs->f_ffree, statvfs->f_favail, statvfs->f_fsid, statvfs->f_flag, statvfs->f_namemax);
            ...
            case SSH_SFTP_TRANS_INIT:
                if(data->set.upload) state(conn, SSH_SFTP_UPLOAD_INIT);
                    ...
                    if(protop->path[strlen(protop->path)-1] == '/') state(conn, SSH_SFTP_READDIR_INIT);
            ...
            case SSH_SFTP_UPLOAD_INIT: {
                    ...
                    if(data->set.ftp_append) flags = O_WRONLY|O_CREAT|O_APPEND;
                    else if(data->state.resume_from > 0) flags = O_WRONLY|O_APPEND;
                    elseflags = O_WRONLY|O_CREAT|O_TRUNC;
                    if(sshc->sftp_file) sftp_close(sshc->sftp_file);
                    ...
                    if(!sshc->sftp_file) {
                        ...
                        if(((err == SSH_FX_NO_SUCH_FILE || err == SSH_FX_FAILURE || err == SSH_FX_NO_SUCH_PATH)) && (data->set.ftp_create_missing_dirs &&
                           (strlen(protop->path) > 1))) {
                            rc = 0;
                            sshc->secondCreateDirs = 1;
                            state(conn, SSH_SFTP_CREATE_DIRS_INIT);
                            break;
                        } else MOVE_TO_SFTP_CLOSE_STATE();
                    }
                    if(data->state.resume_from > 0) {
                        if(conn->seek_func) {
                            Curl_set_in_callback(data, true);
                        ...
                        if(rc != 0) MOVE_TO_SFTP_CLOSE_STATE();
                    }
                    if(data->state.infilesize > 0) {
                    data->req.size = data->state.infilesize;
                    Curl_pgrsSetUploadSize(data, data->state.infilesize);
            ...
            case SSH_SFTP_CREATE_DIRS_MKDIR:
                ...
                *sshc->slash_pos = '/';
                ...
                if(rc < 0) {
                    err = sftp_get_error(sshc->sftp_session);
                    if((err != SSH_FX_FILE_ALREADY_EXISTS) && (err != SSH_FX_FAILURE) && (err != SSH_FX_PERMISSION_DENIED)) MOVE_TO_SFTP_CLOSE_STATE();
                    rc = 0;
                }
                state(conn, SSH_SFTP_CREATE_DIRS);
                break;
            ...
            case SSH_SFTP_READDIR:
                if(sshc->readdir_attrs) sftp_attributes_free(sshc->readdir_attrs);
                ...
                if(sshc->readdir_attrs) {
                    ...
                    if(data->set.ftp_list_only) {
                        ...
                        if(data->set.verbose) Curl_debug(data, CURLINFO_DATA_OUT, (char*)sshc->readdir_filename, sshc->readdir_len);
                    } else {
                        ...
                        if((sshc->readdir_attrs->flags & SSH_FILEXFER_ATTR_PERMISSIONS) && ((sshc->readdir_attrs->permissions & S_IFMT) == S_IFLNK)) {
                            ...
                            msnprintf(sshc->readdir_linkPath, PATH_MAX, "%s%s", protop->path, sshc->readdir_filename);
            ...
            case SSH_SFTP_READDIR_LINK:
                if(sshc->readdir_link_attrs) sftp_attributes_free(sshc->readdir_link_attrs);
                ...
                if(sshc->readdir_link_attrs->name == NULL) {
                    ...
                    if(sshc->readdir_filename == NULL) sshc->readdir_len = 0;
                    else sshc->readdir_len = strlen(sshc->readdir_tmp);
            ...
            case SSH_SFTP_READDIR_BOTTOM:
                ...
                if(!result) {
                if(data->set.verbose) Curl_debug(data, CURLINFO_DATA_OUT, sshc->readdir_line, sshc->readdir_currLen);
                data->req.bytecount += sshc->readdir_currLen;
                }
                Curl_safefree(sshc->readdir_line);
                ssh_string_free_char(sshc->readdir_tmp);
                sshc->readdir_tmp = NULL;
                if(result) state(conn, SSH_STOP);
                else state(conn, SSH_SFTP_READDIR);
                break;
            ...
            case SSH_SESSION_FREE:
                ...
                if(sshc->quote_attrs) sftp_attributes_free(sshc->quote_attrs);
                if(sshc->readdir_attrs) sftp_attributes_free(sshc->readdir_attrs);
                if(sshc->readdir_link_attrs) sftp_attributes_free(sshc->readdir_link_attrs);
                if(sshc->privkey) ssh_key_free(sshc->privkey);
                if(sshc->pubkey) ssh_key_free(sshc->pubkey);
                ...
                state(conn, SSH_STOP);
                break;
            case SSH_QUIT: /* fallthrough, just stop! */
            default:
                sshc->nextstate = SSH_NO_STATE;
                state(conn, SSH_STOP);
                break;
...
static void myssh_block2waitfor(struct connectdata *conn, bool block) {
    ...
    if(block) {
        ...
        if(dir & SSH_READ_PENDING)  conn->waitfor = KEEP_RECV;
        else if(dir & SSH_WRITE_PENDING) conn->waitfor = KEEP_SEND;
    }
}
static CURLcode myssh_multi_statemach(struct connectdata *conn, bool *done) {
    struct ssh_conn *sshc = &conn->proto.sshc;
...
static CURLcode myssh_connect(struct connectdata *conn, bool *done) {
    ...
    struct Curl_easy *data = conn->data;
    int rc;
    if(!data->req.protop) myssh_setup_connection(conn);
    ...
    if(rc != SSH_OK) infof(data, "Could not parse SSH configuration files");}
...
static CURLcode myssh_done(struct connectdata *conn, CURLcode status) {
    ...
    if(!status) result = myssh_block_statemach(conn, FALSE);
    else result = status;
...
static void sftp_quote(struct connectdata *conn) {
    ...
    if(result) {
        ...
        else failf(data, "Syntax error: Bad first parameter");
        state(conn, SSH_SFTP_CLOSE);
        sshc->nextstate = SSH_NO_STATE;
        sshc->actualcode = result;
        return;
    ...
    if(strncasecompare(cmd, "chgrp ", 6) || strncasecompare(cmd, "chmod ", 6) || strncasecompare(cmd, "chown ", 6)) {
        ...
        if(result) {
            if(result == CURLE_OUT_OF_MEMORY) failf(data, "Out of memory");
            else failf(data, "Syntax error in chgrp/chmod/chown: Bad second parameter");
            Curl_safefree(sshc->quote_path1);
            state(conn, SSH_SFTP_CLOSE);
            sshc->nextstate = SSH_NO_STATE;
            sshc->actualcode = result;
