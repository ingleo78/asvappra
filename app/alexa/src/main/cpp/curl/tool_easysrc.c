...
CURLcode easysrc_addf(struct slist_wc **plist, const char *fmt, ...) {
    ...
    va_end(ap);
    if(! bufp) ret = CURLE_OUT_OF_MEMORY;
    else {
...
CURLcode easysrc_perform(void) {
    if(easysrc_toohard) {
        ...
        for(i = 0; ((c = srchard[i]) != NULL); i++) CHKRET(easysrc_add(&easysrc_code, c));
        if(easysrc_toohard) {
            for(ptr = easysrc_toohard->first; ptr; ptr = ptr->next) CHKRET(easysrc_add(&easysrc_code, ptr->data));
        }
        CHKRET(easysrc_add(&easysrc_code, ""));
        CHKRET(easysrc_add(&easysrc_code, "*/"));
        slist_wc_free_all(easysrc_toohard);
...
void dumpeasysrc(struct GlobalConfig *config) {
    ...
    if(!out) warnf(config, "Failed to open %s to write libcurl code!\n", o);
        ...
        if(easysrc_decl) {
            for(ptr = easysrc_decl->first; ptr; ptr = ptr->next) fprintf(out, "  %s\n", ptr->data);
        }
        if(easysrc_data) {
            fprintf(out, "\n");
        ...
        if(easysrc_code) {
            for(ptr = easysrc_code->first; ptr; ptr = ptr->next) {
                if(ptr->data[0]) fprintf(out, "  %s\n", ptr->data);
                else fprintf(out, "\n");
            }
        }
        if(easysrc_clean) {
            for(ptr = easysrc_clean->first; ptr; ptr = ptr->next) fprintf(out, "  %s\n", ptr->data);
        }
        for(i = 0; ((c = srcend[i]) != NULL); i++) fprintf(out, "%s\n", c);
