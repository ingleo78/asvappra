...
static CURLcode ldap_setup_connection(struct connectdata *conn) {
    ...
    if(rc != LDAP_URL_SUCCESS) {
        ...
        if(rc > LDAP_URL_SUCCESS && rc <= LDAP_URL_ERR_BADEXTS) {
            if(rc == LDAP_URL_ERR_MEM) status = CURLE_OUT_OF_MEMORY;
            msg = url_errs[rc];
        }
    ...
    if(!li) return CURLE_OUT_OF_MEMORY;
    li->proto = proto;
    conn->proto.ldapc = li;
...
static CURLcode ldap_connect(struct connectdata *conn, bool *done) {
    ...
    strcpy(hosturl, "ldap");
    ptr = hosturl + 4;
    if(conn->handler->flags & PROTOPT_SSL) *ptr++ = 's';
    msnprintf(ptr, sizeof(hosturl)-(ptr-hosturl), "://%s:%d", conn->host.name, conn->remote_port);
#ifdef CURL_OPENLDAP_DEBUG
    static int do_trace = 0;
    const char *env = getenv("CURL_OPENLDAP_TRACE");
    do_trace = (env && strtol(env, NULL, 10) > 0);
    if(do_trace) ldap_set_option(li->ld, LDAP_OPT_DEBUG_LEVEL, &do_trace);
#endif
...
static CURLcode ldap_connecting(struct connectdata *conn, bool *done) {
    ...
    if(conn->handler->flags & PROTOPT_SSL) {
        if(!li->ssldone) {
            ...
            if(result || !li->ssldone) return result;
        }
        if(!li->sslinst) {
            Sockbuf *sb;
    ...
    if(!li->didbind) {
        ...
        if(rc) return CURLE_LDAP_CANNOT_BIND;
        li->didbind = TRUE;
        if(tvp) return CURLE_OK;
    }
...
static CURLcode ldap_do(struct connectdata *conn, bool *done) {
    ...
    if(!lr) return CURLE_OUT_OF_MEMORY;
    lr->msgid = msgid;
    data->req.protop = lr;
    ...
    *err = CURLE_AGAIN;
    ...
    for(ent = ldap_first_message(li->ld, msg); ent; ent = ldap_next_message(li->ld, ent)) {
        ...
        if(msgtype == LDAP_RES_SEARCH_RESULT) {
            ...
            if(rc) {
                ...
                *err = CURLE_LDAP_SEARCH_FAILED;
                ...
                if(code == LDAP_SIZELIMIT_EXCEEDED) infof(data, "There are more than %d entries\n", lr->nument);
            ...
            ldap_memfree(info);
            break;
        } else if(msgtype != LDAP_RES_SEARCH_ENTRY) continue;
        lr->nument++;
        ...
        for(rc = ldap_get_attribute_ber(li->ld, ent, ber, &bv, &bvals); rc == LDAP_SUCCESS; rc = ldap_get_attribute_ber(li->ld, ent, ber, &bv, &bvals)) {
            ...
            if(bv.bv_len > 7 && !strncmp(bv.bv_val + bv.bv_len - 7, ";binary", 7)) binary = 1;
            else binary = 0;
            if(bvals == NULL) {
            ...
            for(i = 0; bvals[i].bv_val != NULL; i++) {
                ...
                if(!binary) {
                    if(ISSPACE(bvals[i].bv_val[0]) || ISSPACE(bvals[i].bv_val[bvals[i].bv_len-1])) binval = 1;
                    else {
                    unsigned int j;
...
static ber_slen_t ldapsb_tls_read(Sockbuf_IO_Desc *sbiod, void *buf, ber_len_t len) {
    ...
    if(ret < 0 && err == CURLE_AGAIN) SET_SOCKERRNO(EWOULDBLOCK);
    return ret;
}
