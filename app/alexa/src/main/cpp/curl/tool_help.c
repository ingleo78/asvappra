...
void tool_help(void) {
    ...
    for(i = 0; helptext[i].opt; i++) {
        ...
        if(i && ((i % PRINT_LINES_PAUSE) == 0)) tool_pressanykey();
    #endif
    }
}
...
void tool_version_info(void) {
    ...
    if(curlinfo->features) {
        ...
        for(i = 0; i < sizeof(feats)/sizeof(feats[0]); i++) {
            if(curlinfo->features & feats[i].bitmask) featp[numfeat++] = (char *)feats[i].name;
        }
    #ifdef USE_METALINK
        featp[numfeat++] = (char *)"Metalink";
    #endif
        qsort(&featp[0], numfeat, sizeof(char *), featcomp);
        for(i = 0; i< numfeat; i++) printf(" %s", featp[i]);
        puts("");
    }
...
void tool_list_engines(void) {
    ...
    if(engines) {
        for(; engines; engines = engines->next) printf("  %s\n", engines->data);
    } else  puts("  <none>");
