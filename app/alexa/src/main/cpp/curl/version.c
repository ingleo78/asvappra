...
char *curl_version(void) {
    ...
    msnprintf(iconv_version, sizeof(iconv_version), "iconv/%d.%d", _LIBICONV_VERSION >> 8, _LIBICONV_VERSION & 255);
#else
#endif
    src[i++] = iconv_version;
#endif
...
curl_version_info_data *curl_version_info(CURLversion stamp) {
    ...
    if(Curl_ssl->supports & SSLSUPP_HTTPS_PROXY) version_info.features |= CURL_VERSION_HTTPS_PROXY;
    else version_info.features &= ~CURL_VERSION_HTTPS_PROXY;
#endif
#ifdef HAVE_LIBZ
    ...
    if(version_info.libidn) version_info.features |= CURL_VERSION_IDN;
#elif defined(USE_WIN32_IDN)
