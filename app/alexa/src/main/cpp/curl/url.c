...
CURLcode Curl_close(struct Curl_easy **datap) {
    ...
    if(!datap || !*datap) return CURLE_OK;
    data = *datap;
    *datap = NULL;
    Curl_expire_clear(data);
    m = data->multi;
    if(m) curl_multi_remove_handle(data->multi, data);
    if(data->multi_easy) {
    ...
    data->magic = 0;
    if(data->state.rangestringalloc) free(data->state.range);
...
static void conn_shutdown(struct connectdata *conn) {
    ...
    if(CURL_SOCKET_BAD != conn->sock[SECONDARYSOCKET]) Curl_closesocket(conn, conn->sock[SECONDARYSOCKET]);
    if(CURL_SOCKET_BAD != conn->sock[FIRSTSOCKET]) Curl_closesocket(conn, conn->sock[FIRSTSOCKET]);
    if(CURL_SOCKET_BAD != conn->tempsock[0]) Curl_closesocket(conn, conn->tempsock[0]);
    if(CURL_SOCKET_BAD != conn->tempsock[1]) Curl_closesocket(conn, conn->tempsock[1]);
}
static void conn_free(struct connectdata *conn) {
    DEBUGASSERT(conn);
...
CURLcode Curl_disconnect(struct Curl_easy *data, struct connectdata *conn, bool dead_connection) {
    ...
    conn->data = data;
    if(conn->bits.connect_only) dead_connection = TRUE;
    if(conn->handler->disconnect) conn->handler->disconnect(conn, dead_connection);
    conn_shutdown(conn);
    conn_free(conn);
    return CURLE_OK;
}
...
static int IsMultiplexingPossible(const struct Curl_easy *handle, const struct connectdata *conn) {
    ...
    if((conn->handler->protocol & PROTO_FAMILY_HTTP) && (!conn->bits.protoconnstart || !conn->bits.close)) {
        if(Curl_multiplex_wanted(handle->multi) && (handle->set.httpversion >= CURL_HTTP_VERSION_2)) avail |= CURLPIPE_MULTIPLEX;
    }
    return avail;
}
#ifndef CURL_DISABLE_PROXY
...
static bool socks_proxy_info_matches(const struct proxy_info *data, const struct proxy_info *needle) {
    if(!proxy_info_matches(data, needle)) return FALSE;
    if((data->user == NULL) != (needle->user == NULL)) return FALSE;
    if(data->user && needle->user && strcmp(data->user, needle->user) != 0) return FALSE;
    if((data->passwd == NULL) != (needle->passwd == NULL)) return FALSE;
    if(data->passwd && needle->passwd && strcmp(data->passwd, needle->passwd) != 0) return FALSE;
    return TRUE;
}
#else
...
static bool extract_if_dead(struct connectdata *conn, struct Curl_easy *data) {
    if(!CONN_INUSE(conn) && !conn->data) {
        ...
        if(conn_maxage(data, conn, now)) dead = TRUE;
...
#ifndef CURL_DISABLE_PROXY
    bool wantProxyNTLMhttp = (needle->bits.proxy_user_passwd && ((data->state.authproxy.want & (CURLAUTH_NTLM | CURLAUTH_NTLM_WB)) &&
    ...
    *waitpipe = FALSE;
    ...
    if(bundle) {
        ...
        while(curr) {
            ...
            check = curr->ptr;
            curr = curr->next;
            if(check->bits.connect_only || check->bits.close) continue;
            if(bundle->multiuse == BUNDLE_MULTIPLEX) multiplexed = CONN_INUSE(check);
            if(canmultiplex);
            else {
                if(multiplexed)  continue;
            ...
            if(needle->unix_domain_socket) {
                if(!check->unix_domain_socket) continue;
                if(strcmp(needle->unix_domain_socket, check->unix_domain_socket)) continue;
                if(needle->bits.abstract_unix_socket != check->bits.abstract_unix_socket) continue;
            } else if(check->unix_domain_socket) continue;
        #endif
            if((needle->handler->flags&PROTOPT_SSL) != (check->handler->flags&PROTOPT_SSL))
                if(get_protocol_family(check->handler->protocol) != needle->handler->protocol || !check->bits.tls_upgraded)
                    continue;
        #ifndef CURL_DISABLE_PROXY
            if(needle->bits.httpproxy != check->bits.httpproxy || needle->bits.socksproxy != check->bits.socksproxy) continue;
            if(needle->bits.socksproxy && !socks_proxy_info_matches(&needle->socks_proxy, &check->socks_proxy)) continue;
        #endif
            if(needle->bits.conn_to_host != check->bits.conn_to_host) continue;
            if(needle->bits.conn_to_port != check->bits.conn_to_port) continue;
        #ifndef CURL_DISABLE_PROXY
            if(needle->bits.httpproxy) {
                if(!proxy_info_matches(&needle->http_proxy, &check->http_proxy)) continue;
                if(needle->bits.tunnel_proxy != check->bits.tunnel_proxy) continue;
                if(needle->http_proxy.proxytype == CURLPROXY_HTTPS) {
                    if(needle->handler->flags&PROTOPT_SSL) {
                        if(!Curl_ssl_config_matches(&needle->proxy_ssl_config, &check->proxy_ssl_config)) continue;
                        if(check->proxy_ssl[FIRSTSOCKET].state != ssl_connection_complete) continue;
                    } else {
                        if(!Curl_ssl_config_matches(&needle->ssl_config, &check->ssl_config)) continue;
                        if(check->ssl[FIRSTSOCKET].state != ssl_connection_complete) continue;
                    }
                }
            }
        #endif
            DEBUGASSERT(!check->data || GOOD_EASY_HANDLE(check->data));
            if(!canmultiplex && check->data) continue;
            if(check->data && (check->data->multi != needle->data->multi)) continue;
            if(needle->localdev || needle->localport) {
                if((check->localport != needle->localport) || (check->localportrange != needle->localportrange) || (needle->localdev &&
                   (!check->localdev || strcmp(check->localdev, needle->localdev))))
                    continue;
            }
            if(!(needle->handler->flags & PROTOPT_CREDSPERREQUEST)) {
                if(strcmp(needle->user, check->user) || strcmp(needle->passwd, check->passwd)) continue;
            }
            if((needle->handler->flags&PROTOPT_SSL)
            #ifndef CURL_DISABLE_PROXY
                ...
                if((strcasecompare(needle->handler->scheme, check->handler->scheme) || (get_protocol_family(check->handler->protocol) ==
                    needle->handler->protocol && check->bits.tls_upgraded)) && (!needle->bits.conn_to_host ||
                    strcasecompare(needle->conn_to_host.name, check->conn_to_host.name)) && (!needle->bits.conn_to_port ||
                    needle->conn_to_port == check->conn_to_port) && strcasecompare(needle->host.name, check->host.name) &&
                ...
                if(match) {
                    ...
                    if(wantNTLMhttp) {
                        if(strcmp(needle->user, check->user) || strcmp(needle->passwd, check->passwd)) {
                            if(check->http_ntlm_state == NTLMSTATE_NONE) chosen = check;
                            continue;
                        }
                    } else if(check->http_ntlm_state != NTLMSTATE_NONE) continue;
                #ifndef CURL_DISABLE_PROXY
                    if(wantProxyNTLMhttp) {
                        if(!check->http_proxy.user || !check->http_proxy.passwd) continue;
                        if(strcmp(needle->http_proxy.user, check->http_proxy.user) || strcmp(needle->http_proxy.passwd, check->http_proxy.passwd)) continue;
                    } else if(check->proxy_ntlm_state != NTLMSTATE_NONE)  continue;
                #endif
...
void Curl_verboseconnect(struct connectdata *conn) {
    if(conn->data->set.verbose) infof(conn->data, "Connected to %s (%s) port %ld (#%ld)\n",
    #ifndef CURL_DISABLE_PROXY
       conn->bits.socksproxy ? conn->socks_proxy.host.dispname : conn->bits.httpproxy ? conn->http_proxy.host.dispname :
    #endif
       conn->bits.conn_to_host ? conn->conn_to_host.dispname : conn->host.dispname, conn->ip_addr_str, conn->port, conn->connection_id);
}
#endif
...
static void strip_trailing_dot(struct hostname *host) {
    ...
    if(len && (host->name[len-1] == '.')) host->name[len-1] = 0;
}
...
static struct connectdata *allocate_conn(struct Curl_easy *data) {
    ...
    conn->bits.httpproxy = (conn->bits.proxy && (conn->http_proxy.proxytype == CURLPROXY_HTTP || conn->http_proxy.proxytype == CURLPROXY_HTTP_1_0 ||
                            conn->http_proxy.proxytype == CURLPROXY_HTTPS)) ? TRUE : FALSE;
...
const struct Curl_handler *Curl_builtin_scheme(const char *scheme) {
    ...
    for(pp = protocols; (p = *pp) != NULL; pp++)
        if(strcasecompare(p->scheme, scheme)) return p;
    return NULL;
}
...
static void zonefrom_url(CURLU *uh, struct connectdata *conn) {
    ...
    if (!uc && zoneid) {
        ...
        if (!*endp && (scope < UINT_MAX)) conn->scope_id = (unsigned int) scope;
        ...
        #endif
            if(!scopeidx) infof(conn->data, "Invalid zoneid: %s; %s\n", zoneid, strerror(errno));
...
static CURLcode parseurlandfillconn(struct Curl_easy *data, struct connectdata *conn) {
    ...
    char *hostname;
    up_free(data);
    if(data->set.uh) uh = data->state.uh = curl_url_dup(data->set.uh);
    else  uh = data->state.uh = curl_url();
    if(!uh) return CURLE_OUT_OF_MEMORY;
    if(data->set.str[STRING_DEFAULT_PROTOCOL] && !Curl_is_absolute_url(data->change.url, NULL, MAX_SCHEME_LEN)) {
        ...
        if(!url) return CURLE_OUT_OF_MEMORY;
        data->change.url = url;
        data->change.url_alloc = TRUE;
    }
    if(!data->set.uh) {
        char *newurl;
        uc = curl_url_set(uh, CURLUPART_URL, data->change.url,CURLU_GUESS_SCHEME | CURLU_NON_SUPPORT_SCHEME | data->set.disallow_username_in_url ?
                          CURLU_DISALLOW_USER : 0 | (data->set.path_as_is ? CURLU_PATH_AS_IS : 0));
        if(uc) {
            DEBUGF(infof(data, "curl_url_set rejected %s\n", data->change.url));
            return Curl_uc_to_curlcode(uc);
        }
        uc = curl_url_get(uh, CURLUPART_URL, &newurl, 0);
        if(uc) return Curl_uc_to_curlcode(uc);
        if(data->change.url_alloc) free(data->change.url);
    ...
    } else if(uc != CURLUE_NO_USER) return Curl_uc_to_curlcode(uc);
    ...
    if(!uc) {
        ...
        conn->passwd = decoded;
        conn->bits.user_passwd = TRUE;
    } else if(uc != CURLUE_NO_PASSWORD) return Curl_uc_to_curlcode(uc);
    uc = curl_url_get(uh, CURLUPART_OPTIONS, &data->state.up.options,CURLU_URLDECODE);
    if(!uc) {
        conn->options = strdup(data->state.up.options);
        if(!conn->options) return CURLE_OUT_OF_MEMORY;
    } else if(uc != CURLUE_NO_OPTIONS) return Curl_uc_to_curlcode(uc);
    uc = curl_url_get(uh, CURLUPART_HOST, &data->state.up.hostname, 0);
    if(uc) {
        if(!strcasecompare("file", data->state.up.scheme)) return CURLE_OUT_OF_MEMORY;
    }
    ...
    if(!conn->host.rawalloc) return CURLE_OUT_OF_MEMORY;
    conn->host.name = conn->host.rawalloc;
    if(data->set.scope_id) conn->scope_id = data->set.scope_id;
    return CURLE_OK;
}
static CURLcode setup_range(struct Curl_easy *data) {
    ...
    if(s->resume_from || data->set.str[STRING_SET_RANGE]) {
        if(s->rangestringalloc) free(s->range);
        if(s->resume_from) s->range = aprintf("%" CURL_FORMAT_CURL_OFF_T "-", s->resume_from);
        else s->range = strdup(data->set.str[STRING_SET_RANGE]);
        s->rangestringalloc = (s->range) ? TRUE : FALSE;
        if(!s->range) return CURLE_OUT_OF_MEMORY;
        s->use_range = TRUE;
...
static CURLcode setup_connection_internals(struct connectdata *conn) {
    ...
    if(conn->port < 0) conn->port = p->defport;
    return CURLE_OK;
}
...
static bool check_noproxy(const char *name, const char *no_proxy) {
    if(no_proxy && no_proxy[0]) {
        ...
        size_t namelen;
        char *endptr;
        if(strcasecompare("*", no_proxy)) return TRUE;
        ...
        for(tok_start = 0; tok_start < no_proxy_len; tok_start = tok_end + 1) {
            while(tok_start < no_proxy_len && strchr(separator, no_proxy[tok_start]) != NULL) ++tok_start;
            if(tok_start == no_proxy_len)  break;
            for(tok_end = tok_start; tok_end < no_proxy_len && strchr(separator, no_proxy[tok_end]) == NULL; ++tok_end);
            if(no_proxy[tok_start] == '.') ++tok_start;
            if((tok_end - tok_start) <= namelen) {
                const char *checkn = name + namelen - (tok_end - tok_start);
                if(strncasecompare(no_proxy + tok_start, checkn,tok_end - tok_start)) {
                    if((tok_end - tok_start) == namelen || *(checkn - 1) == '.') return TRUE;
                }
            }
        }
    }
...
static char *detect_proxy(struct connectdata *conn) {
    ...
    char *envp = proxy_env;
    char *prox;
    while(*protop) *envp++ = (char)tolower((int)*protop++);
    strcpy(envp, "_proxy");
    ...
    if(proxy) infof(conn->data, "Uses proxy env variable %s == '%s'\n", envp, proxy);
    return proxy;
}
#endif
static CURLcode parse_proxy(struct Curl_easy *data, struct connectdata *conn, char *proxy, curl_proxytype proxytype) {
    ...
    if(!uc) {
        ...
        if(strcasecompare("https", scheme)) proxytype = CURLPROXY_HTTPS;
        else if(strcasecompare("socks5h", scheme)) proxytype = CURLPROXY_SOCKS5_HOSTNAME;
        else if(strcasecompare("socks5", scheme)) proxytype = CURLPROXY_SOCKS5;
        else if(strcasecompare("socks4a", scheme)) proxytype = CURLPROXY_SOCKS4A;
        else if(strcasecompare("socks4", scheme) || strcasecompare("socks", scheme)) proxytype = CURLPROXY_SOCKS4;
    ...
    if(portptr) {
        ...
        if(data->set.proxyport) port = data->set.proxyport;
        else {
            if(proxytype == CURLPROXY_HTTPS) port = CURL_DEFAULT_HTTPS_PROXY_PORT;
            else port = CURL_DEFAULT_PROXY_PORT;
        }
    }
    if(port >= 0) {
        proxyinfo->port = port;
        if(conn->port < 0 || sockstype || !conn->socks_proxy.host.rawalloc) conn->port = port;
    }
...
static CURLcode parse_proxy_auth(struct Curl_easy *data, struct connectdata *conn) {
    ...
    CURLcode result = CURLE_OK;
    if(proxyuser) result = Curl_urldecode(data, proxyuser, 0, &conn->http_proxy.user, NULL, REJECT_ZERO);
...
static CURLcode create_conn_helper_init_proxy(struct connectdata *conn) {
    ...
    if(!data->set.str[STRING_NOPROXY]) {
        ...
        if(no_proxy) infof(conn->data, "Uses proxy env variable %s == '%s'\n", p, no_proxy);
    }
...
#ifndef CURL_DISABLE_HTTP
    else if(!proxy && !socksproxy) proxy = detect_proxy(conn);
#endif
    ...
    if(proxy || socksproxy) {
        ...
        if(conn->http_proxy.host.rawalloc) {
            ...
            if((conn->handler->flags & PROTOPT_PROXY_AS_HTTP) && !conn->bits.tunnel_proxy) conn->handler = &Curl_handler_http;
...
CURLcode Curl_parse_login_details(const char *login, const size_t len, char **userp, char **passwdp, char **optionsp) {
    ...
    if(llen > CURL_MAX_INPUT_LENGTH) return CURLE_BAD_FUNCTION_ARGUMENT;
    if(passwdp) {
        psep = strchr(login, ':');
        if(psep >= login + len) psep = NULL;
    }
    if(optionsp) {
        osep = strchr(login, ';');
        if(osep >= login + len) osep = NULL;
    }
    ...
    if(userp && ulen) {
        ubuf = malloc(ulen + 1);
        if(!ubuf) result = CURLE_OUT_OF_MEMORY;
    }
...
static CURLcode parse_remote_port(struct Curl_easy *data, struct connectdata *conn) {
  if(data->set.use_port && data->state.allow_port) {
    ...
    if(uc) return CURLE_OUT_OF_MEMORY;
  }
  return CURLE_OK;
}
static CURLcode override_login(struct Curl_easy *data, struct connectdata *conn, char **userp, char **passwdp, char **optionsp) {
    ...
    if(data->set.str[STRING_PASSWORD]) {
        ...
        *passwdp = strdup(data->set.str[STRING_PASSWORD]);
        if(!*passwdp) return CURLE_OUT_OF_MEMORY;
        conn->bits.user_passwd = TRUE;
        passwd_changed = TRUE;
    }
    if(data->set.str[STRING_OPTIONS]) {
        ...
        *optionsp = strdup(data->set.str[STRING_OPTIONS]);
        if(!*optionsp) return CURLE_OUT_OF_MEMORY;
    }
    ...
    if(data->set.use_netrc != CURL_NETRC_IGNORED && (!*userp || !**userp || !*passwdp || !**passwdp)) {
        ...
        } else if(ret < 0) return CURLE_OUT_OF_MEMORY;
        else {
            conn->bits.netrc = TRUE;
            conn->bits.user_passwd = TRUE; /* enable user+password */
            if(netrc_user_changed) user_changed = TRUE;
            if(netrc_passwd_changed) passwd_changed = TRUE;
        }
    }
    if(user_changed) {
...
static CURLcode set_login(struct connectdata *conn) {
    ...
    if(!conn->passwd) {
        ...
        if(!conn->passwd) result = CURLE_OUT_OF_MEMORY;
    }
    return result;
}
static CURLcode parse_connect_to_host_port(struct Curl_easy *data, const char *host, char **hostname_result, int *port_result) {
    ...
    *port_result = -1;
    if(!host || !*host) return CURLE_OK;
    host_dup = strdup(host);
    if(!host_dup) return CURLE_OUT_OF_MEMORY;
    hostptr = host_dup;
    portptr = hostptr;
    if(*hostptr == '[') {
    #ifdef ENABLE_IPV6
        ...
        if(*ptr == '%') {
            if(strncmp("%25", ptr, 3)) infof(data, "Please URL encode %% as %%25, see RFC 6874.\n");
            ptr++;
            while(*ptr && (ISALPHA(*ptr) || ISXDIGIT(*ptr) || (*ptr == '-') || (*ptr == '.') || (*ptr == '_') || (*ptr == '~'))) ptr++;
        }
        if(*ptr == ']') *ptr++ = '\0';
...
static CURLcode parse_connect_to_string(struct Curl_easy *data, struct connectdata *conn, const char *conn_to_host, char **host_result, int *port_result) {
    ...
    *port_result = -1;
    if(*ptr == ':') {
        ...
        if(!hostname_to_match) return CURLE_OUT_OF_MEMORY;
    ...
    if(host_match && port_match) result = parse_connect_to_host_port(data, ptr, host_result, port_result);
    return result;
}
...
#ifdef USE_ALTSVC
    if(data->asi && !host && (port == -1) && ((conn->handler->protocol == CURLPROTO_HTTPS) ||
    #ifdef CURLDEBUG
       ...
       0
        ...
        if(hit) {
            ...
            if(!hostd) return CURLE_OUT_OF_MEMORY;
            ...
            if(srcalpnid != as->dst.alpnid) {
                ...
                case ALPN_h1: conn->httpversion = 11; break;
                case ALPN_h2: conn->httpversion = 20; break;
                case ALPN_h3:
...
static CURLcode resolve_server(struct Curl_easy *data, struct connectdata *conn, bool *async) {
    ...
    DEBUGASSERT(conn);
    DEBUGASSERT(data);
    if(conn->bits.reuse) *async = FALSE;
    else {
        int rc;
        ...
        if(!conn->bits.proxy) {
            ...
            if(conn->bits.conn_to_host) connhost = &conn->conn_to_host;
            else connhost = &conn->host;
            if(conn->bits.conn_to_port) conn->port = conn->conn_to_port;
            else conn->port = conn->remote_port;
            conn->hostname_resolve = strdup(connhost->name);
            if(!conn->hostname_resolve) return CURLE_OUT_OF_MEMORY;
            rc = Curl_resolv_timeout(conn, conn->hostname_resolve, (int)conn->port, &hostaddr, timeout_ms);
            if(rc == CURLRESOLV_PENDING) *async = TRUE;
            else if(rc == CURLRESOLV_TIMEDOUT) result = CURLE_OPERATION_TIMEDOUT;
            else if(!hostaddr) {
...
static CURLcode create_conn(struct Curl_easy *data, struct connectdata **in_connect, bool *async) {
    ...
    *in_connect = conn;
    ...
    if(result) goto out;
    if((conn->given->flags&PROTOPT_SSL) && conn->bits.httpproxy) conn->bits.tunnel_proxy = TRUE;
#endif
...
#endif
    if(conn->bits.conn_to_host && strcasecompare(conn->conn_to_host.name, conn->host.name)) conn->bits.conn_to_host = FALSE;
    if(conn->bits.conn_to_port && conn->conn_to_port == conn->remote_port) conn->bits.conn_to_port = FALSE;
#ifndef CURL_DISABLE_PROXY
    if((conn->bits.conn_to_host || conn->bits.conn_to_port) && conn->bits.httpproxy) conn->bits.tunnel_proxy = TRUE;
#endif
    ...
    if((data->set.reuse_fresh && !data->state.this_is_a_follow) || data->set.connect_only) reuse = FALSE;
    ...
    if(reuse) {
        ...
        *in_connect = conn;
        ...
        if(conn->handler->flags & PROTOPT_ALPN_NPN) {
            if(data->set.ssl_enable_alpn) conn->bits.tls_enable_alpn = TRUE;
            if(data->set.ssl_enable_npn) conn->bits.tls_enable_npn = TRUE;
        }
        if(waitpipe) connections_available = FALSE;
        else {
            ...
            CONNCACHE_UNLOCK(data);
            if(conn_candidate) (void)Curl_disconnect(data, conn_candidate,FALSE);
            else {
...
#ifndef CURL_DISABLE_PROXY
    if(conn->bits.httpproxy) strip_trailing_dot(&conn->http_proxy.host);
    if(conn->bits.socksproxy) strip_trailing_dot(&conn->socks_proxy.host);
#endif
    if(conn->bits.conn_to_host) strip_trailing_dot(&conn->conn_to_host);
    out:
    return result;
}
CURLcode Curl_setup_conn(struct connectdata *conn, bool *protocol_done) {
    ...
    *protocol_done = FALSE;
    ...
    if(data->set.str[STRING_USERAGENT]) {
        ...
        if(!data->state.aptr.uagent) return CURLE_OUT_OF_MEMORY;
    }
    ...
    if(CURL_SOCKET_BAD == conn->sock[FIRSTSOCKET]) {
        ...
        if(conn->ssl[FIRSTSOCKET].use || (conn->handler->protocol & PROTO_FAMILY_SSH)) Curl_pgrsTime(data, TIMER_APPCONNECT);
        conn->bits.tcpconnect[FIRSTSOCKET] = TRUE;
        *protocol_done = TRUE;
...
CURLcode Curl_connect(struct Curl_easy *data, bool *asyncp, bool *protocol_done) {
    ...
    *asyncp = FALSE;
    ...
    if(!result) {
        if(CONN_INUSE(conn) > 1) *protocol_done = TRUE;
        else if(!*asyncp) result = Curl_setup_conn(conn, protocol_done);
    }
    if(result == CURLE_NO_CONNECTION_AVAILABLE) return result;
    else if(result && conn) {
...
CURLcode Curl_init_do(struct Curl_easy *data, struct connectdata *conn) {
    ...
    if(conn) {
        ...
        if(data->state.wildcardmatch && !(conn->handler->flags & PROTOPT_WILDCARD)) data->state.wildcardmatch = FALSE;
    }
    data->state.done = FALSE;
    data->state.expect100header = FALSE;
    if(data->set.opt_no_body) data->state.httpreq = HTTPREQ_HEAD;
    k->start = Curl_now();
    k->now = k->start;
    k->header = TRUE;
    k->bytecount = 0;
...
static unsigned int get_protocol_family(unsigned int protocol) {
    unsigned int family;
    switch(protocol) {
        case CURLPROTO_HTTP: case CURLPROTO_HTTPS: family = CURLPROTO_HTTP; break;
        case CURLPROTO_FTP: case CURLPROTO_FTPS: family = CURLPROTO_FTP; break;
        case CURLPROTO_SCP: family = CURLPROTO_SCP; break;
        case CURLPROTO_SFTP: family = CURLPROTO_SFTP; break;
        case CURLPROTO_TELNET: family = CURLPROTO_TELNET; break;
        case CURLPROTO_LDAP:
        case CURLPROTO_LDAPS: family = CURLPROTO_LDAP; break;
        case CURLPROTO_DICT: family = CURLPROTO_DICT; break;
        case CURLPROTO_FILE: family = CURLPROTO_FILE; break;
        case CURLPROTO_TFTP: family = CURLPROTO_TFTP; break;
        case CURLPROTO_IMAP:
        case CURLPROTO_IMAPS: family = CURLPROTO_IMAP; break;
        case CURLPROTO_POP3: case CURLPROTO_POP3S: family = CURLPROTO_POP3; break;
        case CURLPROTO_SMTP: case CURLPROTO_SMTPS: family = CURLPROTO_SMTP; break;
        case CURLPROTO_RTSP: family = CURLPROTO_RTSP; break;
        case CURLPROTO_RTMP: case CURLPROTO_RTMPS: family = CURLPROTO_RTMP; break;
        case CURLPROTO_RTMPT: case CURLPROTO_RTMPTS: family = CURLPROTO_RTMPT; break;
        case CURLPROTO_RTMPE: family = CURLPROTO_RTMPE; break;
        case CURLPROTO_RTMPTE: family = CURLPROTO_RTMPTE; break;
        case CURLPROTO_GOPHER: family = CURLPROTO_GOPHER; break;
        case CURLPROTO_SMB:
        case CURLPROTO_SMBS: family = CURLPROTO_SMB; break;
        default: family = 0;
    }
    return family;
}