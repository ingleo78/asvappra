...
#define GetStr(str,val) do { \
    ...
    if((val)) *(str) = strdup((val)); \
    if(!(val)) return PARAM_NO_MEM; \
} while(0);
...
static int nss_hash_init(void **pctx, SECOidTag hash_alg) {
    ...
    if(!NSS_IsInitialized() && !nss_context) {
        ...
        nss_context = NSS_InitContext("", "", "", "", &params, NSS_INIT_READONLY | NSS_INIT_NOCERTDB   | NSS_INIT_NOMODDB       | NSS_INIT_FORCEOPEN
...
#elif defined(USE_WIN32_CRYPTO)
static void win32_crypto_final(struct win32_crypto_hash *ctx, unsigned char *digest, unsigned int digestLen) {
    unsigned long length;
    CryptGetHashParam(ctx->hHash, HP_HASHVAL, NULL, &length, 0);
    if(length == digestLen) CryptGetHashParam(ctx->hHash, HP_HASHVAL, digest, &length, 0);
...
static const struct metalink_digest_def SHA256_DIGEST_DEF[] = {"sha-256", SHA256_DIGEST_PARAMS};
static const struct metalink_digest_def SHA1_DIGEST_DEF[] = {"sha-1", SHA1_DIGEST_PARAMS};
static const struct metalink_digest_def MD5_DIGEST_DEF[] = {"md5", MD5_DIGEST_PARAMS};
...
static int digest_update(struct digest_context *context, const unsigned char *data, unsigned int len) {
...
static int digest_final(struct digest_context *context, unsigned char *result) {
    if(result) (*context->digest_hash->digest_final)(result, context->digest_hashctx);
...
static int check_hash(const char *filename, const struct metalink_digest_def *digest_def, const unsigned char *digest, FILE *error) {
    ...
    if(fd == -1) {
        fprintf(error, "Metalink: validating (%s) [%s] FAILED (%s)\n", filename, digest_def->hash_name, strerror(errno));
        return -1;
    }
    dctx = digest_init(digest_def->dparams);
    if(!dctx) {
        fprintf(error, "Metalink: validating (%s) [%s] FAILED (%s)\n", filename, digest_def->hash_name, "failed to initialize hash algorithm");
        close(fd);
        return -2;
    }
    ...
    if(check_ok) fprintf(error, "Metalink: validating (%s) [%s] OK\n", filename, digest_def->hash_name);
    else fprintf(error, "Metalink: validating (%s) [%s] FAILED (digest mismatch)\n", filename, digest_def->hash_name);
    free(result);
    close(fd);
    return check_ok;
}
int metalink_check_hash(struct GlobalConfig *config, struct metalinkfile *mlfile, const char *filename) {
    ...
    rv = check_hash(filename, mlfile->checksum->digest_def, mlfile->checksum->digest, config->errors);
    return rv;
}
static struct metalink_checksum * checksum_from_hex_digest(const struct metalink_digest_def *digest_def, const char *hex_digest) {
    ...
    digest = malloc(len/2);
    if(!digest) return 0;
    for(i = 0; i < len; i += 2) digest[i/2] = hex_to_uint(hex_digest + i);
...
static int check_hex_digest(const char *hex_digest, const struct metalink_digest_def *digest_def) {
    size_t i;
    for(i = 0; hex_digest[i]; ++i) {
        char c = hex_digest[i];
        if(!(('0' <= c && c <= '9') || ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z'))) return 0;
    }
...
static struct metalinkfile *new_metalinkfile(metalink_file_t *fileinfo) {
    ...
    if(fileinfo->checksums) {
        ...
        for(digest_alias = digest_aliases; digest_alias->alias_name; ++digest_alias) {
            ...
            for(p = fileinfo->checksums; *p; ++p) {
                if(curl_strequal(digest_alias->alias_name, (*p)->type) && check_hex_digest((*p)->hash, digest_alias->digest_def)) {
    ...
    if(fileinfo->resources) {
        ...
        for(p = fileinfo->resources; *p; ++p) {
            ...
            if((*p)->type == NULL || curl_strequal((*p)->type, "http") || curl_strequal((*p)->type, "https") || curl_strequal((*p)->type, "ftp") ||
...
int parse_metalink(struct OperationConfig *config, struct OutStruct *outs, const char *metalink_url) {
    ...
    if(metalink->files == NULL) {
        fprintf(config->global->errors, "Metalink: parsing (%s) WARNING (missing or invalid file name)\n", metalink_url);
    ...
    for(files = metalink->files; *files; ++files) {
        struct getout *url;
        if(!(*files)->resources) {
            fprintf(config->global->errors, "Metalink: parsing (%s) WARNING (missing or invalid resource)\n", metalink_url);
            continue;
        }
        ...
        if(url) {
            ...
            } else config->metalinkfile_list = config->metalinkfile_last = mlfile;
        }
    }
    metalink_delete(metalink);
    return (warnings) ? -2 : 0;
...
static int check_content_type(const char *content_type, const char *media_type) {
    ...
    for(; *ptr && (*ptr == ' ' || *ptr == '\t'); ++ptr);
    if(!*ptr) return 0;
    return curl_strnequal(ptr, media_type, media_type_len) && (*(ptr + media_type_len) == '\0' || *(ptr + media_type_len) == ' ' ||
