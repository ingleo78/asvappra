...
static char *vms_translate_path(const char *path) {
    ...
    if(test_str == NULL) return (char*)path;
    unix_path = decc$translate_vms(path);
    if((int)unix_path <= 0) return (char*)path;
}
#else
...
static struct passwd *vms_getpwuid(uid_t uid) {
    ...
    if(my_passwd == NULL) return my_passwd;
    unix_path = vms_translate_path(my_passwd->pw_dir);
    if((long)unix_path <= 0)  return my_passwd;
    if(unix_path == my_passwd->pw_dir) return my_passwd;
