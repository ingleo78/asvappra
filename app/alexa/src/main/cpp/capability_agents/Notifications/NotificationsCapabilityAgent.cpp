...
namespace alexaClientSDK {
    namespace acsdkNotifications {
        ...
        shared_ptr<NotificationsCapabilityAgent> NotificationsCapabilityAgent::create(shared_ptr<NotificationsStorageInterface> notificationsStorage,
            ...
            auto notificationsCapabilityAgent = shared_ptr<NotificationsCapabilityAgent>(new NotificationsCapabilityAgent(notificationsStorage,
                                                                                         renderer, contextManager, exceptionSender,
                                                                                         notificationsAudioFactory, dataManager,
                                                                                         metricRecorder));
        ...
        NotificationsCapabilityAgent::NotificationsCapabilityAgent(shared_ptr<NotificationsStorageInterface> notificationsStorage,
                                                                   shared_ptr<NotificationRendererInterface> renderer,
                                                                   shared_ptr<ContextManagerInterface> contextManager,
                                                                   shared_ptr<ExceptionEncounteredSenderInterface> exceptionSender,
                                                                   shared_ptr<NotificationsAudioFactoryInterface> notificationsAudioFactory,
                                                                   shared_ptr<CustomerDataManager> dataManager,
                                                                   shared_ptr<MetricRecorderInterface> metricRecorder) :
                                                                   CapabilityAgent{NAMESPACE, move(exceptionSender)},
                                                                   RequiresShutdown{"NotificationsCapabilityAgent"},
                                                                   CustomerDataHandler{move(dataManager)}, m_metricRecorder{move(metricRecorder)},
                                                                   m_notificationsStorage{move(notificationsStorage)},
                                                                   m_contextManager{move(contextManager)}, m_renderer{move(renderer)},
                                                                   m_notificationsAudioFactory{move(notificationsAudioFactory)},
        ...
        void NotificationsCapabilityAgent::executeInit() {
            ACSDK_DEBUG5(LX(__func__));
            avs::IndicatorState currentIndicatorState = avs::IndicatorState::OFF;
            ...
            m_isEnabled = (queueSize > 0 || (avs::IndicatorState::ON == currentIndicatorState));
            executeProvideState();
            if (queueSize > 0) {
        ...
        void NotificationsCapabilityAgent::notifyObserversOfIndicatorState(avs::IndicatorState state) {
        ...
        void NotificationsCapabilityAgent::provideState(const NamespaceAndName& stateProviderName, unsigned int stateRequestToken) {
        ...
        void NotificationsCapabilityAgent::executePossibleIndicatorStateChange(const avs::IndicatorState& nextIndicatorState) {
            avs::IndicatorState storedIndicatorState = avs::IndicatorState::OFF;
        ...
        void NotificationsCapabilityAgent::executeSetIndicator(const NotificationIndicator& nextNotificationIndicator, shared_ptr<DirectiveInfo> info) {
            ...
            switch (m_currentState) {
                ...
                case NotificationsCapabilityAgentState::IDLE: case NotificationsCapabilityAgentState::CANCELING_PLAY:
                    ...
                    executePossibleIndicatorStateChange(avs::intToIndicatorState(nextNotificationIndicator.persistVisualIndicator));
        ...
        void NotificationsCapabilityAgent::handleClearIndicatorDirective(shared_ptr<DirectiveInfo> info) {
            m_executor.submit([this, info] { executeClearIndicator(info); });
        }
        void NotificationsCapabilityAgent::executeClearIndicator(shared_ptr<DirectiveInfo> info) {
            ACSDK_DEBUG5(LX(__func__).d("currentState", m_currentState));
            int queueSize = 0;
            switch(m_currentState) {
            ...
            setHandlingCompleted(info);
            m_isEnabled = false;
            executePossibleIndicatorStateChange(avs::IndicatorState::OFF);
            executeProvideState();
        }
        void NotificationsCapabilityAgent::executeProvideState(bool sendToken, unsigned int stateRequestToken) {
            ...
            Value _IS_ENABLED_KEY{IS_ENABLED_KEY, strlen(IS_ENABLED_KEY)};
            state.AddMember(_IS_ENABLED_KEY, m_isEnabled, state.GetAllocator());
            avs::IndicatorState currentIndicatorState = avs::IndicatorState::OFF;
            if (!m_notificationsStorage->getIndicatorState(&currentIndicatorState)) {
            ...
            bool isVisualIndicatorPersisted = (currentIndicatorState == avs::IndicatorState::ON);
            Value _IS_VISUAL_INDICATOR_PERSISTED_KEY{IS_VISUAL_INDICATOR_PERSISTED_KEY, strlen(IS_VISUAL_INDICATOR_PERSISTED_KEY)};
            state.AddMember(_IS_VISUAL_INDICATOR_PERSISTED_KEY, isVisualIndicatorPersisted, state.GetAllocator());
        ...
        void NotificationsCapabilityAgent::setHandlingCompleted(shared_ptr<DirectiveInfo> info) {
            if (info) {
                if (info->result) info->result->setCompleted();
                removeDirective(info->directive->getMessageId());
            }
        }
        bool NotificationsCapabilityAgent::parseDirectivePayload(shared_ptr<DirectiveInfo> info, Document* document) {
        ...
        void NotificationsCapabilityAgent::doShutdown() {
            ...
            bool successfulShutdown = m_shutdownTrigger.wait_for(lock, SHUTDOWN_TIMEOUT, [this]() {
                                                                    return m_currentState == NotificationsCapabilityAgentState::SHUTDOWN;
                                                                 });
        ...
        void NotificationsCapabilityAgent::clearData() {
            ...
            auto result = m_executor.submit([this]() {
                ...
                m_notificationsStorage->setIndicatorState(avs::IndicatorState::OFF);
                notifyObserversOfIndicatorState(avs::IndicatorState::OFF);
            });
            result.wait();
        }
        unordered_set<shared_ptr<CapabilityConfiguration>> NotificationsCapabilityAgent::getCapabilityConfigurations() {
