...
namespace alexaClientSDK {
    namespace acsdkNotifications {
        ...
        void NotificationRenderer::onFocusChanged(FocusState newFocus, MixingBehavior behavior) {
            ...
            m_executor.submit([this]() {
                ...
                if (!setState(State::RENDERING_DEFAULT)) {
                    ...
                    if (!result.valid() || !result.get()) { ACSDK_ERROR(LX("renderNotification").m("UnableToReleaseChannel")); }
                    return;
                }
        ...
        shared_ptr<NotificationRenderer> NotificationRenderer::create(shared_ptr<MediaPlayerInterface> mediaPlayer,
        ...
        void NotificationRenderer::addObserver(shared_ptr<NotificationRendererObserverInterface> observer) {
            ACSDK_DEBUG5(LX(__func__));
            lock_guard<mutex> lock(m_mutex);
        ...
        bool NotificationRenderer::renderNotification(function<pair<unique_ptr<istream>, const MediaType>()> audioFactory, const string& url) {
            ...
            auto activity = FocusManagerInterface::Activity::create(NAMESPACE, shared_from_this(), milliseconds::zero(),
                                                         ContentType::MIXABLE);
        ...
        bool NotificationRenderer::cancelNotificationRendering() {
            ...
            if (!m_mediaPlayer->stop(m_sourceId)) { ACSDK_ERROR(LX("cancelNotificationRenderingFailed").d("reason", "stopFailed")); }
            return true;
        }
        ...
        void NotificationRenderer::onRenderingFinished(SourceId sourceId) {
            unordered_set<shared_ptr<NotificationRendererObserverInterface>> localObservers;
            auto result = m_focusManager->releaseChannel(CHANNEL_NAME, shared_from_this());
            if (!result.valid() || !result.get()) { ACSDK_ERROR(LX(__func__).m("UnableToReleaseChannel")); }
            {
                lock_guard<mutex> lock(m_mutex);
