...
#if HAVE_DECL_SETENV
static int save_set_env(const char *name, const char *value, const char **old_value) {
  *old_value = getenv(name);
  return setenv(name, value, 1);
}
static int restore_env(const char *name, const char *old_value) {
  int res;
  if (old_value == NULL) res = unsetenv(name);
  else res = setenv(name, old_value, 1);
  return res;
}
#endif
START_TEST(test_set_log) {
  ...
  ck_assert_msg(strcmp(srunner_log_fname(sr), "test_log") == 0, "Bad file name returned");
  srunner_free(sr);
}
END_TEST
#if HAVE_DECL_SETENV
START_TEST(test_set_log_env) {
  ...
  Suite *s = suite_create("Suite");
  SRunner *sr = srunner_create(s);
  ck_assert_msg(save_set_env("CK_LOG_FILE_NAME", "test_log", &old_val) == 0, "Failed to set environment variable");
  ck_assert_msg(srunner_has_log(sr), "SRunner not logging");
  ck_assert_msg(strcmp(srunner_log_fname(sr), "test_log") == 0, "Bad file name returned");
  srunner_set_log(sr, "test2_log");
  ck_assert_msg(srunner_has_log(sr), "SRunner not logging");
  ck_assert_msg(strcmp(srunner_log_fname(sr), "test2_log") == 0, "Bad file name returned");
  ck_assert_msg(restore_env("CK_LOG_FILE_NAME", old_val) == 0, "Failed to restore environment variable");
  srunner_free(sr);
}
END_TEST
#endif
...
START_TEST(test_double_set_log) {
  ...
  srunner_set_log(sr, "test_log");
  srunner_set_log(sr, "test2_log");
  ck_assert_msg(strcmp(srunner_log_fname(sr), "test_log") == 0, "Log file is initialize only and shouldn't be changeable once set");
  srunner_free(sr);
}
END_TEST
START_TEST(test_set_xml) {
  ...
  ck_assert_msg(strcmp(srunner_xml_fname(sr), "test_log.xml") == 0, "Bad file name returned");
  srunner_free(sr);
}
END_TEST
#if HAVE_DECL_SETENV
START_TEST(test_set_xml_env) {
  ...
  Suite *s = suite_create("Suite");
  SRunner *sr = srunner_create(s);
  ck_assert_msg(save_set_env("CK_XML_LOG_FILE_NAME", "test_log.xml", &old_val) == 0, "Failed to set environment variable");
  ck_assert_msg(srunner_has_xml (sr), "SRunner not logging XML");
  ck_assert_msg(strcmp(srunner_xml_fname(sr), "test_log.xml") == 0, "Bad file name returned");
  srunner_set_xml(sr, "test2_log.xml");
  ck_assert_msg(srunner_has_xml (sr), "SRunner not logging XML");
  ck_assert_msg(strcmp(srunner_xml_fname(sr), "test2_log.xml") == 0, "Bad file name returned");
  ck_assert_msg(restore_env("CK_XML_LOG_FILE_NAME", old_val) == 0, "Failed to restore environment variable");
  srunner_free(sr);
}
END_TEST
#endif
...
START_TEST(test_double_set_xml) {
  ...
  srunner_set_xml(sr, "test_log.xml");
  srunner_set_xml(sr, "test2_log.xml");
  ck_assert_msg(strcmp(srunner_xml_fname(sr), "test_log.xml") == 0, "XML Log file is initialize only and shouldn't be changeable once set");
  srunner_free(sr);
}
END_TEST
START_TEST(test_set_tap) {
  ...
  ck_assert_msg(strcmp(srunner_tap_fname(sr), "test_log.tap") == 0, "Bad file name returned");
  srunner_free(sr);
}
END_TEST
#if HAVE_DECL_SETENV
START_TEST(test_set_tap_env) {
  ...
  Suite *s = suite_create("Suite");
  SRunner *sr = srunner_create(s);
  ck_assert_msg(save_set_env("CK_TAP_LOG_FILE_NAME", "test_log.tap", &old_val) == 0, "Failed to set environment variable");
  ck_assert_msg(srunner_has_tap(sr), "SRunner not logging TAP");
  ck_assert_msg(strcmp(srunner_tap_fname(sr), "test_log.tap") == 0, "Bad file name returned");
  srunner_set_tap (sr, "test2_log.tap");
  ck_assert_msg(srunner_has_tap(sr), "SRunner not logging TAP");
  ck_assert_msg(strcmp(srunner_tap_fname(sr), "test2_log.tap") == 0, "Bad file name returned");
  ck_assert_msg(restore_env("CK_TAP_LOG_FILE_NAME", old_val) == 0, "Failed to restore environment variable");
  srunner_free(sr);
}
END_TEST
#endif
...
START_TEST(test_double_set_tap) {
  ...
  srunner_set_tap(sr, "test_log.tap");
  srunner_set_tap(sr, "test2_log.tap");
  ck_assert_msg(strcmp(srunner_tap_fname(sr), "test_log.tap") == 0, "TAP Log file is initialize only and shouldn't be changeable once set");
  srunner_free(sr);
}
END_TEST
