...
START_TEST(test_check_nfailures) {
  ...
  for (i = 0; i < nr_of_master_tests; i++) {
      if (master_tests[i].failure_type != CK_PASS) number_failed++;
  }
  ck_assert_msg (sub_nfailed == number_failed, "Unexpected number of failures received, %d, expected %d.", sub_nfailed, number_failed);
}
END_TEST
START_TEST(test_check_ntests_run) {
  ck_assert_msg (sub_ntests == nr_of_master_tests, "Unexpected number of tests run %d vs expected %d", sub_ntests, nr_of_master_tests);
}
END_TEST
START_TEST(test_check_failure_msgs) {
  ...
  for (i = 0; i < sub_ntests; i++) {
      ...
      switch(master_test->msg_type) {
          case CK_MSG_TEXT:
              if (strcmp(got_msg, expected_msg) != 0) not_equal = 1;
              break;
      #if ENABLE_REGEX
          case CK_MSG_REGEXP: {
                  ...
                  if (reg_err) {
                      ...
                      ck_assert_msg(reg_err == 0, "For test %d:%s:%s Expected regexp '%s', but regcomp returned error '%s'",
                                    i, master_test->tcname, master_test->test_name, expected_msg, err_text);
                  }
                  reg_err = regexec(&re, got_msg, 0, NULL, 0);
                  regfree(&re);
                  if (reg_err) not_equal = 1;
                  break;
              }
      #endif
          default: break;
      ...
      if (not_equal) {
          switch(master_test->msg_type) {
          #if ENABLE_REGEX
              case CK_MSG_REGEXP: msg_type_str = " regexp"; break;
          #endif
              default: msg_type_str = "";
          }
          ck_abort_msg("For test %d:%s:%s Expected%s '%s', got '%s'", i, master_test->tcname, master_test->test_name, msg_type_str,
...
START_TEST(test_check_failure_lnos) {
  ...
  for (i = 0; i < sub_ntests; i++) {
      ...
      if(line_no == -1) {
          ck_abort_msg("Did not find the %dth failure line number for suite %s, msg %s", (number_failed+1), tr_tcname(tr), tr_msg(tr));
      }
      if (line_no > 0 && tr_lno(tr) != line_no) {
          ck_abort_msg("For test %d (failure %d): Expected lno %ld, got %d for suite %s, msg %s", i, number_failed, line_no, tr_lno(tr),
                       tr_tcname(tr), tr_msg(tr));
      }
  }
  line_no = get_next_failure_line_num(line_num_failures);
  ck_assert_msg(line_no == -1, "No more failure line numbers expected, but found %ld", line_no);
}
END_TEST
START_TEST(test_check_failure_ftypes) {
  ...
  for (i = 0; i < sub_ntests; i++) {
      ...
      tr = tr_fail_array[i - passed];
      ck_assert_msg(tr != NULL, NULL);
      ck_assert_msg(master_tests[i].failure_type == tr_rtype(tr), "Failure type wrong for test %d:%s:%s", i, master_tests[i].tcname,
...
START_TEST(test_check_failure_lfiles) {
  ...
  for (i = 0; i < sub_nfailed; i++) {
      ...
      ck_assert_msg(strstr(tr_lfile(tr), "check_check_sub.c") != 0, "Bad file name for test %d:%s:%s", i, master_tests[i].tcname,
                    master_tests[i].test_name);
  }
}
END_TEST
START_TEST(test_check_tcnames) {
  ...
  if (strcmp(tcname, master_tests[_i].tcname) != 0) {
      ck_abort_msg("Expected '%s', got '%s' for test %d:%s", master_tests[_i].tcname, tcname, _i, master_tests[_i].test_name);
  } 
}
END_TEST
START_TEST(test_check_test_names) {
  ...
  for (i = 0; i < sub_ntests; i++) {
      ...
      if(test_name == NULL || strcmp(master_tests[i].test_name, test_name) != 0) {
          ck_abort_msg("Expected test name '%s' but found '%s' for test %d:%s", master_tests[i].test_name, (test_name == NULL ? "(null)" :
                       test_name), i, master_tests[i].tcname);
      }
      free(test_name);
  }
}
...
START_TEST(test_check_all_msgs) {
  ...
  if (not_equal) {    
    ...
    ck_abort_msg("For test %i:%s:%s expected%s '%s', got '%s'", _i, master_test->tcname, master_test->test_name, msg_type_str,
                 expected_msg, got_msg);
  }
}
END_TEST
START_TEST(test_check_all_ftypes) {
  ck_assert_msg(master_tests[_i].failure_type == tr_rtype(tr_all_array[_i]), "For test %d:%s:%s failure type wrong, expected %d but got %d",
                _i, master_tests[_i].tcname, master_tests[_i].test_name, master_tests[_i].failure_type, tr_rtype(tr_all_array[_i]));
}
END_TEST
...
START_TEST(test_setup) {
  ck_assert_msg (test_fixture_val == 1, "Value not setup or changed across tests correctly");
  test_fixture_val = 2;
}
END_TEST
...
START_TEST(test_teardown) {
  ck_assert_msg (test_fixture_val == 3, "Value not changed correctly in teardown");
}
END_TEST
Suite *make_master_suite (void) {
  ...
  tcase_add_unchecked_fixture(tc_fixture, test_fixture_setup, test_fixture_teardown);
