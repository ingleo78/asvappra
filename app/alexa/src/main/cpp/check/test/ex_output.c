...
START_TEST(test_xml_esc_fail_msg) {
    ck_abort_msg("fail \" ' < > & \0x09 \0x0A" "X""\x08"" message"); /* backspace char \x08 deletes the X */
}
END_TEST
...
static Suite *make_log2_suite(int include_exit_test) {
    ...
    tc = tcase_create("Core");
    suite_add_tcase(s, tc);
    if(include_exit_test == 1) { tcase_add_test(tc, test_abort); }
...
static Suite *make_xml_esc_suite(void) {
    Suite *s;
    TCase *tc;
    s = suite_create("XML escape \" ' < > & \0x09 \0x0A" "X""\x08"" tests");
    tc = tcase_create("description \" ' < > & \0x09 \0x0A" "X""\x08"" end");
    suite_add_tcase(s, tc);
...
static void run_tests(enum print_output printmode, char *log_type, int include_exit_test) {
    ...
    else if(strcmp(log_type, "STDOUT_DUMP") == 0) dump_everything_to_stdout = 1;
    else if(strcmp(log_type, "LOG") == 0) srunner_set_log(sr, "test.log");
    else if(strcmp(log_type, "LOG_STDOUT") == 0) srunner_set_log(sr, "-");
    else if(strcmp(log_type, "TAP") == 0) srunner_set_tap(sr, "test.tap");
    else if(strcmp(log_type, "TAP_STDOUT") == 0) srunner_set_tap(sr, "-");
    else if(strcmp(log_type, "XML") == 0) srunner_set_xml(sr, "test.xml");
    else if(strcmp(log_type, "XML_STDOUT") == 0) srunner_set_xml(sr, "-");
    else {
        print_usage();
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_SILENT") == 0) printmode = CK_SILENT;
    else if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_MINIMAL") == 0) printmode = CK_MINIMAL;
    else if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_NORMAL") == 0) printmode = CK_NORMAL;
    else if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_VERBOSE") == 0) printmode = CK_VERBOSE;
    else if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_ENV") == 0) printmode = CK_ENV;
#if ENABLE_SUBUNIT
    else if(strcmp(argv[OUTPUT_TYPE_ARG], "CK_SUBUNIT") == 0) printmode = CK_SUBUNIT;
#endif
    else {
        print_usage();
        return EXIT_FAILURE;
    }
    if(strcmp(argv[INCLUDE_EXIT_TEST_ARG], "NORMAL") == 0) include_exit_test = 0;
    else if(strcmp(argv[INCLUDE_EXIT_TEST_ARG], "EXIT_TEST") == 0) include_exit_test = 1;
    else {
        print_usage();
        return EXIT_FAILURE;
    }
