...
START_TEST(test_segv) {
  record_test_name(tcase_name());
  record_failure_line_num(__LINE__-4);
  raise (SIGSEGV);
}
END_TEST
...
START_TEST(test_mark_point) {
  ...
  i++;
  mark_point();
  record_failure_line_num(__LINE__-2);
  raise(SIGFPE);
...
START_TEST(test_fork1p_pass) {
  pid_t pid;
  record_test_name(tcase_name());
  if((pid = fork()) < 0) { ck_abort_msg("Failed to fork new process"); }
  else if (pid > 0) {
...
START_TEST(test_fork1c_pass) {
  pid_t pid;
  record_test_name(tcase_name());
  if((pid = check_fork()) < 0) { ck_abort_msg("Failed to fork new process"); }
  else if (pid > 0) check_waitpid_and_exit(pid);
  else {
      ck_assert_msg(1, NULL);
...
START_TEST(test_fork2_pass) {
  ...
  if((pid = check_fork()) < 0) { ck_abort_msg("Failed to fork new process"); }
      ...
      if((pid2 = check_fork()) < 0) { ck_abort_msg("Failed to fork new process"); }
      else if (pid2 == 0) {
          ck_assert_msg(1, NULL);
          check_waitpid_and_exit(0);
      }
...
START_TEST(test_invalid_set_fork_status) {
   ...
   SRunner *sr;
   record_test_name(tcase_name());
   record_failure_line_num(__LINE__-9);
