...
static void tcase_add_fixture(TCase *tc, SFun setup, SFun teardown, int ischecked);
static void tr_init(TestResult *tr);
static void suite_free(Suite *s);
static void tcase_free(TCase *tc);
Suite *suite_create(const char *name) {
    Suite *s;
    s = (Suite *)emalloc(sizeof(Suite));
    if(name == NULL) s->name = "";
    else s->name = name;
...
TCase *tcase_create(const char *name) {
    ...
    TCase *tc = (TCase *)emalloc(sizeof(TCase));
    if(name == NULL) tc->name = "";
    else tc->name = name;
    env = getenv("CK_DEFAULT_TIMEOUT");
    if(env != NULL) {
        char *endptr = NULL;
        double tmp = strtod(env, &endptr);
        if(tmp >= 0 && endptr != env && (*endptr) == '\0') timeout_sec = tmp;
    }
    env = getenv("CK_TIMEOUT_MULTIPLIER");
    if(env != NULL) {
        char *endptr = NULL;
        double tmp = strtod(env, &endptr);
        if(tmp >= 0 && endptr != env && (*endptr) == '\0') timeout_sec = timeout_sec * tmp;
    }
...
List *tag_string_to_list(const char *tags_string) {
    ...
    if (NULL == tags_string) return list;
...
unsigned int tcase_matching_tag(TCase *tc, List *check_for) {
    if (NULL == check_for) return 0;
    for(check_list_front(check_for); !check_list_at_end(check_for); check_list_advance(check_for)) {
        for(check_list_front(tc->tags); !check_list_at_end(tc->tags); check_list_advance(tc->tags)) {
            if (0 == strcmp((const char *)check_list_val(tc->tags), (const char *)check_list_val(check_for))) return 1;
        }
    }
    return 0;
}
void suite_add_tcase(Suite * s, TCase * tc) {
    if(s == NULL || tc == NULL || check_list_contains(s->tclst, tc)) return;
    check_list_add_end(s->tclst, tc);
}
void _tcase_add_test(TCase *tc, const TTest *ttest, int _signal, int allowed_exit_value, int start, int end) {
    TF *tf;
    if(tc == NULL || ttest == NULL) return;
    tf = (TF *)emalloc(sizeof(TF));
    tf->ttest = ttest;
    tf->loop_start = start;
    tf->loop_end = end;
    tf->signal = _signal;
    tf->allowed_exit_value = (WEXITSTATUS_MASK & allowed_exit_value);
...
static void tcase_add_fixture(TCase *tc, SFun setup, SFun teardown, int ischecked) {
    if(setup) {
        if(ischecked) check_list_add_end(tc->ch_sflst, fixture_create(setup, ischecked));
        else check_list_add_end(tc->unch_sflst, fixture_create(setup, ischecked));
    }
    if(teardown) {
        if(ischecked) check_list_add_front(tc->ch_tflst, fixture_create(teardown, ischecked));
        else check_list_add_front(tc->unch_tflst, fixture_create(teardown, ischecked));
    }
}
void tcase_set_timeout(TCase *tc, double timeout) {
    ...
    if(timeout >= 0) {
        ...
        if(env != NULL) {
            char *endptr = NULL;
            double tmp = strtod(env, &endptr);
            if(tmp >= 0 && endptr != env && (*endptr) == '\0') timeout = timeout * tmp;
        }
...
void _ck_assert_failed(const char *file, int line, const char *expr, const char *msg, ...) {
    char buf[BUFSIZ];
    const char *to_send;
    send_loc_info(file, line);
    if(msg != NULL) {
    ...
    if(cur_fork_status() == CK_FORK) {
        ...
        _exit(1);
    #endif
    } else longjmp(error_jmp_buffer, 1);
}
SRunner *srunner_create(Suite *s) {
    SRunner *sr = (SRunner*)emalloc(sizeof(SRunner));
    sr->slst = check_list_create();
    if(s != NULL) check_list_add_end(sr->slst, s);
    sr->stats = (TestStats*)emalloc(sizeof(TestStats));
...
TestResult **srunner_failures(SRunner *sr) {
    ...
    for(check_list_front(rlst); !check_list_at_end(rlst); check_list_advance(rlst)) {
        ...
        if(non_pass(tr->rtype)) trarray[i++] = tr;
    }
    return trarray;
}
...
void set_fork_status(enum fork_status fstat) {
    if(fstat == CK_FORK || fstat == CK_NOFORK || fstat == CK_FORK_GETENV) _fstat = fstat;
    else eprintf("Bad status in set_fork_status", __FILE__, __LINE__);
}
