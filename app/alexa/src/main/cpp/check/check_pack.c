...
static size_t get_max_msg_size(void) {
    ...
    if (env) value = (size_t)strtoul(env, NULL, 10);
    if (value == 0) value = ck_max_msg_size;
    if (value == 0) value = DEFAULT_MAX_MSG_SIZE;
    return value;
}
...
static void check_type(int type, const char *file, int line) {
    if(type < 0 || type >= CK_MSG_LAST) eprintf("Bad message type arg %d", file, line, type);
}
...
void ppack(FILE *fdes, enum ck_msg_type type, CheckMsg *msg) {
    ...
    size_t r;
    n = pack(type, &buf, msg);
    if(n < 0) eprintf("pack failed", __FILE__, __LINE__ - 2);
    if((size_t) n > get_max_msg_size()) eprintf("Message string too long", __FILE__, __LINE__ - 5);
    ...
    if(r != (size_t) n) eprintf("Error in call to fwrite:", __FILE__, __LINE__ - 5);
    free(buf);
}
static size_t read_buf(FILE * fdes, size_t size, char *buf) {
    size_t n;
    n = fread(buf, 1, size, fdes);
    if(ferror(fdes)) eprintf("Error in call to fread:", __FILE__, __LINE__ - 4);
    return n;
}
static size_t get_result(char *buf, RcvMsg *rmsg) {
    ...
    if(n < 0) eprintf("Error in call to upack", __FILE__, __LINE__ - 2);
    msgsz = (size_t) n;
    if(type == CK_MSG_CTX) {
        ...
        if(rmsg->failctx == CK_CTX_INVALID) rcvmsg_update_loc(rmsg, lmsg->file, lmsg->line);
        free(lmsg->file);
    ...
    } else check_type(type, __FILE__, __LINE__);
    return msgsz;
}
...
RcvMsg *punpack(FILE *fdes) {
    ...
    while(nparse > 0) {
        ...
        if ( ((ssize_t) nparse - (ssize_t) n) < 0) eprintf("Error in call to get_result", __FILE__, __LINE__ - 5);
        nparse -= n;
