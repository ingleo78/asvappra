...
static void srunner_fprint_summary(FILE *file, SRunner *sr, enum print_output print_mode);
static void srunner_fprint_results(FILE *file, SRunner *sr, enum print_output print_mode);
void srunner_print(SRunner *sr, enum print_output print_mode) {
    srunner_fprint(stdout, sr, print_mode);
}
void srunner_fprint(FILE *file, SRunner *sr, enum print_output print_mode) {
    if(print_mode == CK_ENV) print_mode = get_env_printmode();
    srunner_fprint_summary(file, sr, print_mode);
    srunner_fprint_results(file, sr, print_mode);
}
static void srunner_fprint_summary(FILE *file, SRunner *sr, enum print_output print_mode) {
#if ENABLE_SUBUNIT
    if(print_mode == CK_SUBUNIT) return;
#endif
...
void fprint_xml_esc(FILE *file, const char *str) {
    for(; *str != '\0'; str++) {
        ...
        } else if(next >= ' ' && next <= '~') fputc(next, file);
...
void tr_fprint(FILE * file, TestResult * tr, enum print_output print_mode) {
    if(print_mode == CK_ENV) print_mode = get_env_printmode();
    if((print_mode >= CK_VERBOSE && tr->rtype == CK_PASS) || (tr->rtype != CK_PASS && print_mode >= CK_NORMAL)) {
...
void tr_xmlprint(FILE * file, TestResult * tr, enum print_output print_mode CK_ATTRIBUTE_UNUSED) {
    ...
    char *slash = NULL;
    switch(tr->rtype) {
        case CK_PASS: snprintf(result, sizeof(result), "%s", "success"); break;
        case CK_FAILURE: snprintf(result, sizeof(result), "%s", "failure"); break;
        case CK_ERROR: snprintf(result, sizeof(result), "%s", "error"); break;
        default: abort(); break;
    }
    if(tr->file) {
        slash = strrchr(tr->file, '/');
        if(slash == NULL) slash = strrchr(tr->file, '\\');
        if(slash == NULL) {
            path_name = strdup(".");
            file_name = tr->file;
        } else {
            path_name = strdup(tr->file);
            path_name[slash - tr->file] = 0;
            file_name = slash + 1;
        }
    }
...
enum print_output get_env_printmode(void) {
    ...
    if(env == NULL) return CK_NORMAL;
    if(strcmp(env, "silent") == 0) return CK_SILENT;
    if(strcmp(env, "minimal") == 0) return CK_MINIMAL;
    if(strcmp(env, "verbose") == 0) return CK_VERBOSE;
    return CK_NORMAL;
}