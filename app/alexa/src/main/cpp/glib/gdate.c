...
GDate* g_date_new_julian(guint32 j) {
  GDate *d;
...
static void g_date_update_dmy(const GDate *const_d) {
  ...
  y = 100 * B + D - 4800 + (M/10);
#ifdef G_ENABLE_DEBUG
  if (!g_date_valid_dmy(day, m, y)) g_warning("\nOOPS julian: %u  computed dmy: %u %u %u\n", d->julian_days, day, m, y);
#endif
  d->month = m;
  d->day = day;
  d->year = y;
...
GDateWeekday g_date_get_weekday(const GDate *d) {
  ...
  if (!d->julian) g_date_update_julian(d);
...
static gchar *long_month_names[13] = { NULL };
static gchar *short_month_names[13] = { NULL };
...
static void g_date_prepare_to_parse(const gchar *str, GDateParseTokens *pt) {
  ...
  g_date_clear(&d, 1);
  if ((current_locale == NULL) || (strcmp(locale, current_locale) != 0)) recompute_localeinfo = TRUE;
  if (recompute_localeinfo) {
      ...
      while (i < testpt.num_ints) {
          switch (testpt.n[i]) {
              case 7: dmy_order[i] = G_DATE_MONTH; break;
              case 4: dmy_order[i] = G_DATE_DAY; break;
              case 76:
              using_twodigit_years = TRUE; /* FALL THRU */
              case 1976: dmy_order[i] = G_DATE_YEAR; break;
              default:
      ...
      }
      {
          ...
          while (i < 3) {
              switch (dmy_order[i]) {
                  case G_DATE_MONTH: strings[i] = "Month"; break;
                  case G_DATE_YEAR: strings[i] = "Year"; break;
                  case G_DATE_DAY: strings[i] = "Day"; break;
                  default: strings[i] = NULL;
              }
              ++i;
          }
...
void g_date_set_parse(GDate *d, const gchar *str) {
  ...
  if (pt.num_ints > 1) {
      ...
      if (pt.num_ints == 3 && !g_date_valid_dmy(day, m, y)) {
          ...
          m = pt.n[1];
          day = pt.n[2];
          if (using_twodigit_years && y < 100) y = G_DATE_BAD_YEAR;
      } else if (pt.num_ints == 2) {
	      if (m == G_DATE_BAD_MONTH && pt.month != G_DATE_BAD_MONTH) m = pt.month;
	  }
...
void g_date_set_time_t(GDate *date, time_t timet) {
...
#else
  {
	  ...
	  tm.tm_mday = 1;
	  tm.tm_year = 100;
      } else memcpy((void*)&tm, (void*)ptm, sizeof(struct tm));
  }
#endif
  date->julian = FALSE;
...
void g_date_set_month(GDate *d, GDateMonth m) {
  ...
  d->julian = FALSE;
  d->month = m;
  if (g_date_valid_dmy(d->day, d->month, d->year)) d->dmy = TRUE;
  else d->dmy = FALSE;
}
...
void g_date_add_months(GDate *d, guint nmonths) {
  ...
  d->year += years;
  idx = g_date_is_leap_year(d->year) ? 1 : 0;
  if (d->day > days_in_months[idx][d->month]) d->day = days_in_months[idx][d->month];
  d->julian = FALSE;
  g_return_if_fail(g_date_valid(d));
}
...
void g_date_add_years(GDate *d, guint nyears) {
  ...
  if (d->month == 2 && d->day == 29) {
      if (!g_date_is_leap_year(d->year)) d->day = 28;
  }
  d->julian = FALSE;
}
...
void g_date_to_struct_tm(const GDate *d, struct tm *tm) {
  ...
  if (day == 7) day = 0;
  tm->tm_wday = (int)day;
  tm->tm_yday = g_date_get_day_of_year(d) - 1;
  tm->tm_isdst = -1;
}
void g_date_clamp(GDate *date, const GDate *min_date, const GDate *max_date) {
  g_return_if_fail(g_date_valid(date));
  if (min_date != NULL) { g_return_if_fail(g_date_valid(min_date)); }
  if (max_date != NULL) { g_return_if_fail(g_date_valid (max_date)); }
  if (min_date != NULL && max_date != NULL) { g_return_if_fail(g_date_compare (min_date, max_date) <= 0); }
  if (min_date && g_date_compare(date, min_date) < 0) *date = *min_date;
  if (max_date && g_date_compare(max_date, date) < 0) *date = *max_date;
}
...
#ifndef G_OS_WIN32
static gsize win32_strftime_helper(const GDate *d, const gchar *format, const struct tm *tm, gchar *s, gsize slen) {
  ...
  while(*p) {
      ...
      if (c == '%') {
          ...
          switch(c) {
              case 'a':
                  if (systemtime.wDayOfWeek == 0) k = 6;
                  else k = systemtime.wDayOfWeek - 1;
              ...
              case 'D':
                  ...
                  break;
              case 'e':
                  if (systemtime.wDay >= 10) g_array_append_vals(result, digits + systemtime.wDay/10, 1);
                  else g_array_append_vals(result, L" ", 1);
                  g_array_append_vals(result, digits + systemtime.wDay%10, 1);
                  break;
              case 'H':
              #if 1
              ...
              #else
                  if (systemtime.wHour == 0) g_array_append_vals(result, L"12", 2);
                  else {
              ...
              case 'T':
                  ...
                  break;
              case 'u':
                  if (systemtime.wDayOfWeek == 0) g_array_append_vals(result, L"7", 1);
                  else g_array_append_vals(result, digits + systemtime.wDayOfWeek, 1);
                  break;
              case 'U':
              ...
              case 'Z':
                  ...
                  else if (n == TIME_ZONE_ID_STANDARD) g_array_append_vals(result, tzinfo.StandardName, wcslen(tzinfo.StandardName));
                  else if (n == TIME_ZONE_ID_DAYLIGHT) g_array_append_vals(result, tzinfo.DaylightName, wcslen(tzinfo.DaylightName));
                  break;
              case '%':
                  g_array_append_vals(result, L"%", 1);
                  break;
