...
void g_hook_free(GHookList *hook_list, GHook *hook) {
  ...
  if(hook_list->finalize_hook != NULL) hook_list->finalize_hook(hook_list, hook);
...
void g_hook_unref(GHookList *hook_list, GHook	*hook) {
  ...
  if (!hook->ref_count) {
      ...
      else hook_list->hooks = hook->next;
      if (hook->next) {
	  ...
	  hook_list->is_setup = FALSE;
	  if (!hook_list->hooks);
	  } else g_hook_free(hook_list, hook);
  }
}
...
void g_hook_list_invoke_check(GHookList *hook_list, gboolean may_recurse) {
  ...
  while(hook) {
      ...
      if (!was_in_call) hook->flags &= ~G_HOOK_FLAG_IN_CALL;
      if (need_destroy) g_hook_destroy_link(hook_list, hook);
      hook = g_hook_next_valid(hook_list, hook, may_recurse);
  }
}
...
GHook* g_hook_first_valid(GHookList *hook_list, gboolean may_be_in_call) {
  ...
  if (hook_list->is_setup) {
      ...
      if (hook) {
          g_hook_ref(hook_list, hook);
          if (G_HOOK_IS_VALID(hook) && (may_be_in_call || !G_HOOK_IN_CALL(hook))) return hook;
          else return g_hook_next_valid(hook_list, hook, may_be_in_call);
	  }
  }
  return NULL;
}
...
GHook* g_hook_get(GHookList *hook_list, gulong hook_id) {
  ...
  hook = hook_list->hooks;
  while(hook) {
      if (hook->hook_id == hook_id) return hook;
      hook = hook->next;
  }
  return NULL;
}
...
GHook* g_hook_find_data(GHookList *hook_list, gboolean need_valids, gpointer data) {
  ...
  hook = hook_list->hooks;
  while(hook) {
      if (hook->data == data && hook->hook_id && (!need_valids || G_HOOK_ACTIVE (hook))) return hook;
      hook = hook->next;
  }
  return NULL;
}
GHook* g_hook_find_func(GHookList *hook_list, gboolean need_valids, gpointer func) {
  ...
  hook = hook_list->hooks;
  while(hook) {
      if (hook->func == func && hook->hook_id && (!need_valids || G_HOOK_ACTIVE (hook))) return hook;
      hook = hook->next;
  }
  return NULL;
}
...
g_hook_find_func_data(GHookList *hook_list, gboolean	need_valids, gpointer	func, gpointer data) {
  ...
  hook = hook_list->hooks;
  while(hook) {
      if (hook->data == data && hook->func == func && hook->hook_id && (!need_valids || G_HOOK_ACTIVE (hook))) return hook;
      hook = hook->next;
  }
  return NULL;
}
void g_hook_insert_sorted(GHookList *hook_list, GHook *hook, GHookCompareFunc func) {
  ...
  sibling = hook_list->hooks;
  while(sibling && !sibling->hook_id) sibling = sibling->next;
  while(sibling) {
      GHook *tmp;
      ...
      tmp = sibling->next;
      while(tmp && !tmp->hook_id) tmp = tmp->next;
      g_hook_unref(hook_list, sibling);
      sibling = tmp;
  }
...
gint g_hook_compare_ids(GHook *new_hook, GHook *sibling) {
  if (new_hook->hook_id < sibling->hook_id) return -1;
  else if (new_hook->hook_id > sibling->hook_id) return 1;
  return 0;
}