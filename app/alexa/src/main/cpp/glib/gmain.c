...
struct _GSourceDisposeFunc {};
typedef struct _GSourceDisposeFunc GSourceDisposeFunc;
struct _GPollRec {
  GPollFD *fd;
  GPollRec *next;
  gint priority;
...
#define SOURCE_BLOCKED(source) (((source)->flags & G_HOOK_FLAG_IN_CALL) != 0 && ((source)->flags & G_SOURCE_CAN_RECURSE) == 0)
#define SOURCE_UNREF(source, context)                             \
   G_STMT_START {                                                 \
       if ((source)->ref_count > 1) (source)->ref_count--;        \
       else g_source_unref_internal ((source), (context), TRUE);  \
   } G_STMT_END
static void g_source_unref_internal(GSource *source, GMainContext *context, gboolean have_lock);
static void g_source_destroy_internal(GSource *source, GMainContext *context, gboolean have_lock);
static void g_source_set_priority_unlocked(GSource *source, GMainContext *context, gint priority);
static void g_main_context_poll(GMainContext *context, gint timeout, gint priority, GPollFD *fds, gint n_fds);
static void g_main_context_add_poll_unlocked(GMainContext *context, gint priority, GPollFD *fd);
static void g_main_context_remove_poll_unlocked(GMainContext *context, GPollFD *fd);
static void g_main_context_wakeup_unlocked(GMainContext *context);
static gboolean g_timeout_prepare(GSource *source, gint *timeout);
...
static gboolean g_child_watch_dispatch(GSource *source, GSourceFunc callback, gpointer user_data);
...
void g_main_context_unref(GMainContext *context) {
  ...
  if (!g_atomic_int_dec_and_test(&context->ref_count)) return;
...
#endif
  } else main_contexts_without_pipe = g_slist_remove(main_contexts_without_pipe, context);
  if (context->cond != NULL) g_cond_free(context->cond);
#endif
  g_free(context);
}
#ifdef G_THREADS_ENABLED
static void g_main_context_init_pipe(GMainContext *context) {
#ifdef G_OS_WIN32
  if (context->wake_up_pipe[0] != -1) return;
...
#else
  if (context->wake_up_semaphore != NULL) return;
  context->wake_up_semaphore = CreateSemaphore (NULL, 0, 100, NULL);
  if (context->wake_up_semaphore == NULL) g_error ("Cannot create wake-up semaphore: %s", g_win32_error_message (GetLastError ()));
  context->wake_up_rec.fd = (gintptr)context->wake_up_semaphore;
  context->wake_up_rec.events = G_IO_IN;
  if (_g_main_poll_debug) g_print("wake-up semaphore: %p\n", context->wake_up_semaphore);
#endif
...
GMainContext* g_main_context_new(void) {
...
#ifdef G_MAIN_POLL_DEBUG
  {
      ...
      if (!beenhere) {
          if (getenv("G_MAIN_POLL_DEBUG") != NULL) _g_main_poll_debug = TRUE;
          beenhere = TRUE;
      }
  }
#endif
...
#ifdef G_THREADS_ENABLED
  if (g_thread_supported()) g_main_context_init_pipe(context);
  else main_contexts_without_pipe = g_slist_prepend(main_contexts_without_pipe, context);
#endif
  G_LOCK (main_context_list);
  main_context_list = g_slist_append(main_context_list, context);
#ifndef G_MAIN_POLL_DEBUG
  if (_g_main_poll_debug) g_print("created context=%p\n", context);
#endif
  G_UNLOCK(main_context_list);
  return context;
}
GMainContext* g_main_context_default(void) {
  ...
  #ifndef G_MAIN_POLL_DEBUGn
      if (_g_main_poll_debug) g_print("default context=%p\n", default_main_context);
  #endif
  }
  G_UNLOCK(main_loop);
...
void g_main_context_push_thread_default(GMainContext *context) {
  ...
  if (context == g_main_context_default()) context = NULL;
  else if (context) g_main_context_ref(context);
  stack = g_static_private_get(&thread_context_stack);
  if (!stack) {
      stack = g_queue_new();
      g_static_private_set(&thread_context_stack, stack, free_context_stack);
  }
...
GMainContext* g_main_context_get_thread_default(void) {
  GQueue *stack;
  stack = g_static_private_get(&thread_context_stack);
  if (stack) return g_queue_peek_head(stack);
  else return NULL;
}
...
static void g_source_list_add(GSource *source, GMainContext *context) {
  ...
  if (tmp_source) tmp_source->prev = source;
  source->prev = last_source;
  if (last_source) last_source->next = source;
  else context->source_list = source;
}
static void g_source_list_remove(GSource *source, GMainContext *context) {
  if (source->prev) source->prev->next = source->next;
  else context->source_list = source->next;
...
static void g_source_destroy_internal(GSource *source, GMainContext *context, gboolean have_lock) {
  if (!have_lock) LOCK_CONTEXT(context);
  ...
  if (!have_lock) UNLOCK_CONTEXT(context);
}
void g_source_destroy(GSource *source) {
  GMainContext *context;
  g_return_if_fail(source != NULL);
  context = source->context;
  if (context) g_source_destroy_internal(source, context, FALSE);
  else source->flags &= ~G_HOOK_FLAG_ACTIVE;
}
...
void g_source_add_poll (GSource *source, GPollFD *fd) {
  ...
  context = source->context;
  if (context) LOCK_CONTEXT(context);
  source->poll_fds = g_slist_prepend(source->poll_fds, fd);
  if (context) {
      if (!SOURCE_BLOCKED(source)) g_main_context_add_poll_unlocked(context, source->priority, fd);
      UNLOCK_CONTEXT(context);
  }
}
void g_source_remove_poll(GSource *source, GPollFD *fd) {
  ...
  if (context) {
      if (!SOURCE_BLOCKED(source)) g_main_context_remove_poll_unlocked(context, fd);
      UNLOCK_CONTEXT(context);
  }
}
void g_source_add_child_source(GSource *source, GSource *child_source) {
  ...
  context = source->context;
  if (context) LOCK_CONTEXT(context);
  if (!source->priv) source->priv = g_slice_new0(GSourcePrivate);
  if (!child_source->priv) child_source->priv = g_slice_new0(GSourcePrivate);
...
void g_source_remove_child_source(GSource *source, GSource *child_source) {
  ...
  if (context) UNLOCK_CONTEXT(context);
}
void g_source_set_callback_indirect(GSource *source, gpointer callback_data, GSourceCallbackFuncs *callback_funcs) {
  ...
  if (context) UNLOCK_CONTEXT(context);
  if (old_cb_funcs) old_cb_funcs->unref(old_cb_data);
}
...
static void g_source_callback_unref(gpointer cb_data) {
  ...
  if (callback->ref_count == 0) {
      if (callback->notify) callback->notify(callback->data);
      g_free(callback);
  }
}
static void g_source_callback_get(gpointer cb_data, GSource *source, GSourceFunc *func, gpointer *data) {
  GSourceCallback *callback = cb_data;
  *func = callback->func;
  *data = callback->data;
}
...
void g_source_set_priority(GSource *source, gint priority) {
  ...
  if (context) UNLOCK_CONTEXT(source->context);
}
...
void g_source_set_can_recurse(GSource *source, gboolean can_recurse) {
  ...
  context = source->context;
  if (context) LOCK_CONTEXT(context);
  if (can_recurse) source->flags |= G_SOURCE_CAN_RECURSE;
  else source->flags &= ~G_SOURCE_CAN_RECURSE;
  if (context) UNLOCK_CONTEXT(context);
}
...
static void g_source_unref_internal(GSource *source, GMainContext *context, gboolean have_lock) {
  ...
  g_return_if_fail(source != NULL);
  if (!have_lock && context) LOCK_CONTEXT(context);
  source->ref_count--;
  if (source->ref_count == 0) {
      ...
      source->callback_funcs = NULL;
      if (context) {
          if (!SOURCE_DESTROYED (source)) g_warning(G_STRLOC ": ref_count == 0, but source was still attached to a context!");
  ...
  if (!have_lock && context) UNLOCK_CONTEXT(context);
  if (old_cb_funcs) {
      if (have_lock) UNLOCK_CONTEXT(context);
      old_cb_funcs->unref(old_cb_data);
      if (have_lock) LOCK_CONTEXT(context);
...
gpointer g_source_add_unix_fd(GSource *source, gint fd, GIOCondition events) {
    ...
    if (context) {
        if (!SOURCE_BLOCKED(source)) g_main_context_add_poll_unlocked(context, source->priority, poll_fd);
        UNLOCK_CONTEXT(context);
    }
    return poll_fd;
}
void g_source_modify_unix_fd(GSource *source, gpointer tag, GIOCondition new_events) {
    ...
    if (context) g_main_context_wakeup(context);
}
void g_source_remove_unix_fd(GSource *source, gpointer tag) {
    ...
    if (context) {
        if (!SOURCE_BLOCKED(source)) g_main_context_remove_poll_unlocked(context, poll_fd);
        UNLOCK_CONTEXT(context);
    }
    g_free(poll_fd);
}
...
GSource* g_main_context_find_source_by_id(GMainContext *context, guint source_id) {
  ...
  if (context == NULL) context = g_main_context_default();
  LOCK_CONTEXT(context);
  source = context->source_list;
  while (source) {
      if (!SOURCE_DESTROYED (source) && source->source_id == source_id) break;
      source = source->next;
  }
  UNLOCK_CONTEXT(context);
  return source;
...
GSource* g_main_context_find_source_by_funcs_user_data(GMainContext *context, GSourceFuncs *funcs, gpointer user_data) {
  ...
  while(source) {
      if (!SOURCE_DESTROYED (source) && source->source_funcs == funcs && source->callback_funcs) {
          ...
          if (callback_data == user_data) break;
	  }
      source = source->next;
  }
  UNLOCK_CONTEXT(context);
...
#ifdef HAVE_CLOCK_GETTIME
  {
...
#ifdef HAVE_MONOTONIC_CLOCK
      {
          ...
          if G_UNLIKELY (!checked) {
              if (sysconf (_SC_MONOTONIC_CLOCK) >= 0) clockid = CLOCK_MONOTONIC;
              checked = TRUE;
          }
      }
#endif
      clock_gettime(clockid, &ts);
      g_assert(G_GINT64_CONSTANT(-315569520000000000) < ts.tv_sec && ts.tv_sec < G_GINT64_CONSTANT (315569520000000000));
...
static void g_main_dispatch(GMainContext *context) {
  ...
  for (i = 0; i < context->pending_dispatches->len; i++) {
      ...
      if (!SOURCE_DESTROYED(source)) {
          ...
          if (cb_funcs) cb_funcs->ref(cb_data);
          if ((source->flags & G_SOURCE_CAN_RECURSE) == 0) block_source(source);
          ...
          current->depth--;
          if (cb_funcs) cb_funcs->unref(cb_data);
          LOCK_CONTEXT(context);
          if (!was_in_call) source->flags &= ~G_HOOK_FLAG_IN_CALL;
          if ((source->flags & G_SOURCE_CAN_RECURSE) == 0 && !SOURCE_DESTROYED(source)) unblock_source(source);
...
static inline GSource* next_valid_source(GMainContext *context, GSource *source) {
  ...
  if (source) SOURCE_UNREF(source, context);
  return new_source;
}
...
void g_main_context_release(GMainContext *context) {
  ...
  if (context->owner_count == 0) {
      ...
      if (context->waiters) {
          ...
          gboolean loop_internal_waiter = (waiter->mutex == g_static_mutex_get_mutex(&context->mutex));
          context->waiters = g_slist_delete_link (context->waiters, context->waiters);
          if (!loop_internal_waiter) g_mutex_lock(waiter->mutex);
          g_cond_signal(waiter->cond);
          if (!loop_internal_waiter) g_mutex_unlock(waiter->mutex);
	  }
  }
  UNLOCK_CONTEXT(context);
#endif
...
gboolean g_main_context_wait(GMainContext *context, GCond *cond, GMutex *mutex) {
  ...
  if (!loop_internal_waiter) LOCK_CONTEXT(context);
  if (context->owner && context->owner != self) {
      GMainWaiter waiter;
      waiter.cond = cond;
      waiter.mutex = mutex;
      context->waiters = g_slist_append(context->waiters, &waiter);
      if (!loop_internal_waiter) UNLOCK_CONTEXT(context);
...
gboolean g_main_context_prepare(GMainContext *context, gint *priority) {
  ...
  if (context->pending_dispatches) {
      if (dispatch) g_main_dispatch(context, &current_time);
      UNLOCK_CONTEXT(context);
      return TRUE;
  }
#endif
  for (i = 0; i < context->pending_dispatches->len; i++) {
      if (context->pending_dispatches->pdata[i]) SOURCE_UNREF((GSource*)context->pending_dispatches->pdata[i], context);
  }
  g_ptr_array_set_size(context->pending_dispatches, 0);
  context->timeout = -1;
  ...
  while(source) {
      ...
      if (SOURCE_BLOCKED(source)) goto next;
      ...
      if (source_timeout >= 0) {
          if (context->timeout < 0) context->timeout = source_timeout;
          else context->timeout = MIN(context->timeout, source_timeout);
	  }
      next:
      source = next_valid_source (context, source);
  }
  UNLOCK_CONTEXT(context);
  if (priority) *priority = current_priority;
  return (n_ready > 0);
}
...
gboolean g_main_context_check(GMainContext *context, gint max_priority, GPollFD *fds, gint n_fds) {
  ...
  #endif
  } else context->poll_waiting = FALSE;
  ...
  i = 0;
  while(i < n_fds) {
      if (pollrec->fd->events) pollrec->fd->revents = fds[i].revents;
      pollrec = pollrec->next;
      i++;
  }
  source = next_valid_source(context, NULL);
...
static gboolean g_main_context_iterate(GMainContext *context, gboolean block, gboolean dispatch, GThread *self) {
  ...
  if (!g_main_context_acquire (context)) {
      ...
      if (!context->cond) context->cond = g_cond_new();
      got_ownership = g_main_context_wait(context, context->cond, g_static_mutex_get_mutex(&context->mutex));
      if (!got_ownership) return FALSE;
  } else LOCK_CONTEXT(context);
#endif
  ...
  if (dispatch) g_main_context_dispatch(context);
#ifdef G_THREADS_ENABLED
  g_main_context_release(context);
#endif
  LOCK_CONTEXT(context);
...
void g_main_loop_unref(GMainLoop *loop) {
  ...
  if (!g_atomic_int_dec_and_test(&loop->ref_count)) return;
...
void g_main_loop_run(GMainLoop *loop) {
  ...
  if (!g_main_context_acquire(loop->context)) {
      ...
      if (!g_thread_supported()) {
          g_warning("g_main_loop_run() was called from second thread but g_thread_init() was never called.");
          return;
	  }
      LOCK_CONTEXT (loop->context);
      g_atomic_int_inc (&loop->ref_count);
      if (!loop->is_running) loop->is_running = TRUE;
      if (!loop->context->cond) loop->context->cond = g_cond_new ();
      while(loop->is_running && !got_ownership) {
          got_ownership = g_main_context_wait (loop->context, loop->context->cond, g_static_mutex_get_mutex (&loop->context->mutex));
      }
      if (!loop->is_running) {
          UNLOCK_CONTEXT(loop->context);
          if (got_ownership) g_main_context_release(loop->context);
          g_main_loop_unref(loop);
          return;
	  }
      g_assert(got_ownership);
  } else LOCK_CONTEXT(loop->context);
#endif
  if (loop->context->in_check_or_prepare) {
      g_warning ("g_main_loop_run(): called recursively from within a source's check() or prepare() member, iteration not possible.");
      return;
  }
  g_atomic_int_inc(&loop->ref_count);
  loop->is_running = TRUE;
  while (loop->is_running) g_main_context_iterate (loop->context, TRUE, TRUE, self);
  UNLOCK_CONTEXT(loop->context);
#ifdef G_THREADS_ENABLED
...
#ifdef G_THREADS_ENABLED
  if (loop->context->cond) { g_cond_broadcast(loop->context->cond); }
#endif
  UNLOCK_CONTEXT(loop->context);
}
...
static void g_main_context_poll(GMainContext *context, gint timeout, gint priority, GPollFD *fds, gint n_fds) {
  ...
  if (n_fds || timeout != 0) {
      ...
      if (_g_main_poll_debug) {
          LOCK_CONTEXT(context);
          g_print("g_main_poll(%d) timeout: %d - elapsed %12.10f seconds", n_fds,  timeout, g_timer_elapsed(poll_timer, NULL));
          g_timer_destroy(poll_timer);
          pollrec = context->poll_records;
          while (pollrec != NULL) {
              i = 0;
              while (i < n_fds) {
                  if (fds[i].fd == pollrec->fd->fd && pollrec->fd->events && fds[i].revents) {
                      g_print(" [" G_POLLFD_FORMAT " :", fds[i].fd);
                      if (fds[i].revents & G_IO_IN) g_print("i");
                      if (fds[i].revents & G_IO_OUT) g_print("o");
                      if (fds[i].revents & G_IO_PRI) g_print("p");
                      if (fds[i].revents & G_IO_ERR) g_print("e");
                      if (fds[i].revents & G_IO_HUP) g_print("h");
                      if (fds[i].revents & G_IO_NVAL) g_print("n");
                      g_print("]");
                  }
                  i++;
              }
...
static void g_main_context_add_poll_unlocked(GMainContext *context, gint priority, GPollFD *fd) {
  ...
  if (lastrec) lastrec->next = newrec;
  else context->poll_records = newrec;
  newrec->next = pollrec;
...
static void g_main_context_remove_poll_unlocked(GMainContext *context, GPollFD *fd) {
  ...
  while (pollrec) {
      if (pollrec->fd == fd) {
          if (lastrec != NULL) lastrec->next = pollrec->next;
          else context->poll_records = pollrec->next;
...
void g_main_context_set_poll_func(GMainContext *context, GPollFunc func) {
  ...
  g_return_if_fail(g_atomic_int_get(&context->ref_count) > 0);
  LOCK_CONTEXT(context);
  if (func) context->poll_func = func;
  else context->poll_func = g_poll;
  UNLOCK_CONTEXT(context);
}
...
static void g_timeout_set_expiration(GTimeoutSource *timeout_source, gint64 current_time) {
  timeout_source->expiration = current_time + (guint64)timeout_source->interval * 1000;
  if (timeout_source->seconds) {
      static gint timer_perturb = -1;
      if (timer_perturb == -1){
          const char *session_bus_address = g_getenv("DBUS_SESSION_BUS_ADDRESS");
          if (!session_bus_address) session_bus_address = g_getenv("HOSTNAME");
          if (session_bus_address) timer_perturb = ABS((gint)g_str_hash (session_bus_address)) % 1000000;
          else timer_perturb = 0;
      }
      if (timer_perturb < timeout_source->expiration % 1000000) timeout_source->expiration += 1000000;
      timeout_source->expiration = ((timeout_source->expiration / 1000000) * 1000000) + timer_perturb;
  }
}
static gboolean g_timeout_prepare(GSource *source, gint *timeout) {
...
static gboolean g_timeout_dispatch(GSource *source, GSourceFunc callback, gpointer user_data) {
  ...
  if (again) g_timeout_set_expiration(timeout_source, g_source_get_time(source));
  return again;
}
...
guint g_timeout_add_seconds_full(gint priority, guint32 interval, GSourceFunc function, gpointer data, GDestroyNotify notify) {
  GSource *source;
  guint id;
  g_return_val_if_fail(function != NULL, 0);
...
static gboolean g_child_watch_check(GSource  *source) {
  ...
  if (child_exited) {
	  ...
	  } else child_watch_source->child_status = child_status;
  }
  return child_exited;
}
#else
static gboolean check_for_child_exited(GSource *source) {
  GChildWatchSource *child_watch_source;
  gint count;
  count = child_watch_count;
  child_watch_source = (GChildWatchSource*)source;
  if (child_watch_source->child_exited) return TRUE;
...
#ifndef G_OS_WIN32
static void g_child_watch_signal_handler(int signum) {
  child_watch_count ++;
  if (child_watch_init_state == CHILD_WATCH_INITIALIZED_THREADED) {
      write(child_watch_wake_up_pipe[1], "B", 1);
  }
}
static void g_child_watch_source_init_single(void) {
...
G_GNUC_NORETURN static gpointer child_watch_helper_thread(gpointer data) {
  while(1) {
      gchar b[20];
      GSList *list;
      ...
      for (list = main_context_list; list; list = list->next) {
          GMainContext *context;
          context = list->data;
          if (g_atomic_int_get(&context->ref_count) > 0) g_main_context_wakeup(context);
	  }
      G_UNLOCK (main_context_list);
  }
}
static void g_child_watch_source_init_multi_threaded(void) {
  GError *error = NULL;
  struct sigaction action;
  g_assert(g_thread_supported());
  ...
  if (g_thread_create (child_watch_helper_thread, NULL, FALSE, &error) == NULL) g_error("Cannot create a thread to monitor child exit status: %s\n", error->message);
...
static void g_child_watch_source_init(void) {
  if (g_thread_supported()) {
      if (child_watch_init_state == CHILD_WATCH_UNINITIALIZED) g_child_watch_source_init_multi_threaded();
      else if (child_watch_init_state == CHILD_WATCH_INITIALIZED_SINGLE) g_child_watch_source_init_promote_single_to_threaded();
  } else {
      if (child_watch_init_state == CHILD_WATCH_UNINITIALIZED) g_child_watch_source_init_single();
  }
}
#endif
...
static gboolean g_idle_check(GSource *source) {
  return source->source_funcs->check(source);
  return TRUE;
}
static gboolean g_idle_dispatch(GSource *source, GSourceFunc callback, gpointer user_data) {
  if (!callback) {
...
void g_main_context_invoke_full(GMainContext *context, gint priority, GSourceFunc function, gpointer data, GDestroyNotify notify) {
  ...
  if (g_main_context_is_owner(context)) {
      ...
      if (!thread_default) thread_default = g_main_context_default();
