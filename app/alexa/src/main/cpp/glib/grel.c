...
static GHashFunc tuple_hash(gint fields) {
  switch (fields) {
      case 2: return tuple_hash_2;
      default: g_error("no tuple hash for %d", fields);
  }
  return NULL;
}
static GEqualFunc tuple_equal(gint fields) {
  switch (fields) {
      case 2: return tuple_equal_2;
      default: g_error("no tuple equal for %d", fields);
  }
  return NULL;
}
...
static void relation_delete_value_tuple(gpointer tuple_key, gpointer tuple_value, gpointer user_data) {
  GRelation *relation = user_data;
  gpointer *tuple = tuple_value;
  g_slice_free1(relation->fields * sizeof(gpointer), tuple);
}
static void g_relation_free_array(gpointer key, gpointer value, gpointer user_data) {
...
void g_relation_insert(GRelation *relation, ...) {
  ...
  for (i = 0; i < relation->fields; i += 1) tuple[i] = va_arg(args, gpointer);
  va_end(args);
...
static void g_relation_delete_tuple(gpointer tuple_key, gpointer tuple_value, gpointer user_data) {
  ...
  for (j = 0; j < relation->fields; j += 1) {
      ...
      gpointer one_key;
      GHashTable *per_key_table;
      if (one_table == NULL) continue;
      if (j == relation->current_field) continue;
      one_key = tuple[j];
      per_key_table = g_hash_table_lookup(one_table, one_key);
      g_hash_table_remove(per_key_table, tuple);
  }
  if (g_hash_table_remove(relation->all_tuples, tuple)) g_slice_free1(relation->fields * sizeof(gpointer), tuple);
  relation->count -= 1;
}
...
static void g_relation_select_tuple(gpointer tuple_key, gpointer tuple_value, gpointer user_data) {
  gpointer *tuple = (gpointer*)tuple_value;
...
GTuples* g_relation_select(GRelation *relation, gconstpointer key, gint field) {
  ...
  key_table = g_hash_table_lookup(table, key);
  if (!key_table) return (GTuples*)tuples;
...
static void g_relation_print_one(gpointer tuple_key, gpointer tuple_value, gpointer user_data) {
  gint i;
  GString *gstring;
  ...
  for (i = 0; i < rel->fields; i += 1) {
      ...
      if (i < (rel->fields - 1)) g_string_append (gstring, ",");
  }
  g_string_append(gstring, "]");
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_INFO, "%s", gstring->str);
  g_string_free(gstring, TRUE);
}
static void g_relation_print_index(gpointer tuple_key, gpointer tuple_value, gpointer user_data) {
  GRelation* rel = (GRelation*)user_data;
...
void g_relation_print(GRelation *relation) {
  ...
  for (i = 0; i < relation->fields; i += 1) {
      if (relation->hashed_tuple_tables[i] == NULL) continue;
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_INFO, "*** index %d", i);
      g_hash_table_foreach(relation->hashed_tuple_tables[i], g_relation_print_index, relation);
  }
}