...
static void testcase_close(long double duration, gint exit_status, guint n_forks) {
  ...
  testcase_open--;
  if (gtester_verbose) g_print("%s\n", exit_status ? "FAIL" : "OK");
  if (exit_status && subtest_last_seed) g_print("GTester: last random seed: %s\n", subtest_last_seed);
  if (exit_status) testcase_fail_count += 1;
  if (subtest_mode_fatal && testcase_fail_count) terminate();
}
static void test_log_msg(GTestLogMsg *msg) {
  guint i;
  gchar **strv;
  switch(msg->log_type) {
      case G_TEST_LOG_NONE: break;
      case G_TEST_LOG_ERROR:
          strv = g_strsplit(msg->strings[0], "\n", -1);
          for (i = 0; strv[i]; i++) test_log_printfe("%s<error>%s</error>\n", sindent(log_indent), strv[i]);
          g_strfreev(strv);
          break;
      case G_TEST_LOG_START_BINARY:
          ...
          break;
      case G_TEST_LOG_LIST_CASE: g_print("%s\n", msg->strings[0]); break;
      ...
      break;
      case G_TEST_LOG_STOP_CASE: testcase_close(msg->nums[2], (int)msg->nums[0], (int)msg->nums[1]); break;
      case G_TEST_LOG_MIN_RESULT: case G_TEST_LOG_MAX_RESULT:
...
static gboolean child_report_cb(GIOChannel *source, GIOCondition condition, gpointer data) {
  ...
  do {
      ...
      if (first_read && (condition & G_IO_IN)) first_read_eof = length == 0;
      first_read = FALSE;
      if (length) {
          GTestLogMsg *msg;
...
static void child_watch_cb(GPid pid, gint status, gpointer data) {
  ...
  if (WIFEXITED(status)) subtest_exitstatus = WEXITSTATUS (status);
  else subtest_exitstatus = 0xffffffff;
...
static gboolean launch_test_binary(const char *binary, guint skip_tests) {
  ...
  if (pipe (report_pipe) < 0) {
      if (subtest_mode_fatal) g_error("Failed to open pipe for test binary: %s: %s", binary, g_strerror(errno));
      else g_warning("Failed to open pipe for test binary: %s: %s", binary, g_strerror(errno));
      return FALSE;
  }
  for (slist = subtest_args; slist; slist = slist->next) argc++;
  if (subtest_quiet) argc++;
  if (subtest_verbose) argc++;
  if (!subtest_mode_fatal) argc++;
  if (subtest_mode_quick) argc++;
  else argc++;
  if (subtest_mode_perf) argc++;
  if (gtester_list_tests) argc++;
  if (subtest_seedstr) argc++;
  argc++;
  if (skip_tests) argc++;
  for (slist = subtest_paths; slist; slist = slist->next) argc++;
  argv = g_malloc((argc + 2) * sizeof(gchar *));
  argv[i++] = binary;
  for (slist = subtest_args; slist; slist = slist->next) argv[i++] = (gchar*)slist->data;
  if (subtest_quiet) argv[i++] = "--quiet";
  if (subtest_verbose) argv[i++] = "--verbose";
  if (!subtest_mode_fatal) argv[i++] = "--keep-going";
  if (subtest_mode_quick) argv[i++] = "-m=quick";
  else argv[i++] = "-m=slow";
  if (subtest_mode_perf) argv[i++] = "-m=perf";
  if (gtester_list_tests) argv[i++] = "-l";
  if (subtest_seedstr) argv[i++] = queue_gfree(&free_list, g_strdup_printf("--seed=%s", subtest_seedstr));
  argv[i++] = queue_gfree(&free_list, g_strdup_printf("--GTestLogFD=%u", report_pipe[1]));
  if (skip_tests) argv[i++] = queue_gfree(&free_list, g_strdup_printf("--GTestSkipCount=%u", skip_tests));
  for (slist = subtest_paths; slist; slist = slist->next) argv[i++] = queue_gfree(&free_list, g_strdup_printf("-p=%s", (gchar*)slist->data));
  argv[i++] = NULL;
  g_spawn_async_with_pipes(NULL, (gchar**)argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, unset_cloexec_fdp, &report_pipe[1], &pid, NULL, NULL, NULL, &error);
  g_slist_foreach(free_list, (void(*)(void*,void*))g_free,NULL);
  g_slist_free(free_list);
  free_list = NULL;
  close(report_pipe[1]);
  if (!gtester_quiet) g_print("(pid=%lu)\n", (unsigned long)pid);
  if (error) {
      close(report_pipe[0]);
      if (subtest_mode_fatal) g_error("Failed to execute test binary: %s: %s", argv[0], error->message);
      else g_warning("Failed to execute test binary: %s: %s", argv[0], error->message);
      g_clear_error(&error);
      g_free(argv);
      return FALSE;
  }
  ...
  while (subtest_running || subtest_io_pending || loop_pending) {
      if (!subtest_running && subtest_io_pending && !loop_pending) break;
...
static void launch_test(const char *binary) {
  ...
  if (!gtester_quiet) g_print("TEST: %s... ", binary);
 retry:
  ...
  if (testcase_open) testcase_close(0, -256, 0);
  g_timer_stop(btimer);
  ...
  if (need_restart) goto retry;
  if (!gtester_quiet) g_print("%s: %s\n", testcase_fail_count || !success ? "FAIL" : "PASS", binary);
  g_timer_destroy (btimer);
  if (subtest_mode_fatal && !success) terminate();
}
...
static void parse_args(gint *argc_p, gchar ***argv_p) {
  ...
  for (i = 1; i < argc; i++) {
      if (strcmp(argv[i], "--g-fatal-warnings") == 0) {
          ...
          if (*equal == '=') subtest_paths = g_slist_prepend(subtest_paths, equal + 1);
          else if (i + 1 < argc) {
              argv[i++] = NULL;
          ...
          if (*equal == '=') subtest_args = g_slist_prepend(subtest_args, equal + 1);
          else if (i + 1 < argc) {
              argv[i++] = NULL;
          ...
          gchar *equal = argv[i] + 2;
          if (*equal == '=') output_filename = equal + 1;
          else if (i + 1 < argc) {
              argv[i++] = NULL;
          ...
          if (strcmp(mode, "perf") == 0) subtest_mode_perf = TRUE;
          else if (strcmp(mode, "slow") == 0 || strcmp(mode, "thorough") == 0) subtest_mode_quick = FALSE;
          ...
          if (*equal == '=') subtest_seedstr = equal + 1;
          else if (i + 1 < argc) {
              argv[i++] = NULL;
...
static gboolean do_nothing(gpointer data) {
  return TRUE;
}
int main(int    argc, char **argv) {
  ...
  if (gtester_selftest) return main_selftest(argc, argv);
  if (argc <= 1) {
      usage(FALSE);
      return 1;
  }
  if (output_filename) {
      ...
      if (log_fd < 0) g_error("Failed to open log file '%s': %s", output_filename, g_strerror(errno));
  }
