...
typedef HRESULT (WINAPI *CONVERTINETSTRING)(LPDWORD lpdwMode, DWORD dwSrcEncoding, DWORD dwDstEncoding, LPCSTR lpSrcStr, LPINT lpnSrcSize, LPBYTE lpDstStr,
                                            LPINT lpnDstSize);
typedef HRESULT (WINAPI *CONVERTINETMULTIBYTETOUNICODE)(LPDWORD lpdwMode, DWORD dwSrcEncoding, LPCSTR lpSrcStr, LPINT lpnMultiCharCount, LPWSTR lpDstStr,
                                                        LPINT lpnWideCharCount);
typedef HRESULT (WINAPI *CONVERTINETUNICODETOMULTIBYTE)(LPDWORD lpdwMode, DWORD dwEncoding, LPCWSTR lpSrcStr, LPINT lpnWideCharCount, LPSTR lpDstStr,
...
static int load_mlang() {
    ...
    int n;
    if (ConvertINetString != NULL) return TRUE;
    n = GetSystemDirectory(mlang_dll, MAX_PATH);
    if (n > 0 && n < MAX_PATH) {
        if (mlang_dll[n-1] != '\\' && mlang_dll[n-1] != '/') strcat(mlang_dll, "\\");
...
#if defined(USE_LIBICONV_DLL)
    if (libiconv_iconv_open(cd, tocode, fromcode)) return (iconv_t)cd;
#endif
    if (win_iconv_open(cd, tocode, fromcode)) return (iconv_t)cd;
    free(cd);
    errno = EINVAL;
    return (iconv_t)(-1);
}
int iconv_close(iconv_t _cd) {
    ...
    int e = *(cd->_errno());
#if defined(USE_LIBICONV_DLL)
    if (cd->hlibiconv != NULL) FreeLibrary(cd->hlibiconv);
#endif
    free(cd);
    errno = e;
    return r;
...
static size_t win_iconv(iconv_t _cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft) {
    ...
    if (inbuf == NULL || *inbuf == NULL) {
        if (outbuf != NULL && *outbuf != NULL && cd->to.flush != NULL) {
            ...
            if (outsize == -1) return (size_t)(-1);
            *outbuf += outsize;
            *outbytesleft -= outsize;
        }
        /*if (is_unicode(cd->from.codepage) && (cd->from.mode & UNICODE_MODE_SWAPPED)) cd->from.codepage ^= 1;
        cd->from.mode = 0;
        cd->to.mode = 0;*/
        return 0;
    }
...
static csconv_t make_csconv(const char *_name) {
    ...
    while ((p = strrstr(name, "//")) != NULL) {
        /*if (_stricmp(p + 2, "nocompat") == 0) use_compat = FALSE;
        else if (_stricmp(p + 2, "translit") == 0) flag |= FLAG_TRANSLIT;
        else if (_stricmp(p + 2, "ignore") == 0) flag |= FLAG_IGNORE;*/
        *p = 0;
    }
    //cv.mode = 0;
    cv.flags = flag;
    ...
    if (cv.codepage == 1200 || cv.codepage == 1201) {
        ...
        cv.mbtowc = kernel_mbtowc;
        cv.wctomb = kernel_wctomb;
        if (cpinfoex.MaxCharSize == 1) cv.mblen = sbcs_mblen;
        else if (cpinfoex.MaxCharSize == 2) cv.mblen = dbcs_mblen;
	    else cv.mblen = mbcs_mblen;
    } */else cv.codepage = -1;
    if (use_compat) {
        switch (cv.codepage) {
...
static int name_to_codepage(const char *name) {
    int i;
    /*if (*name == '\0' || strcmp(name, "char") == 0) return GetACP();
    else */if (strcmp(name, "wchar_t") == 0) return 1200;
    //else if (_strnicmp(name, "cp", 2) == 0) return atoi(name + 2);
    else if ('0' <= name[0] && name[0] <= '9') return atoi(name);
    //else if (_strnicmp(name, "xx", 2) == 0) return atoi(name + 2);
    /*for (i = 0; codepage_alias[i].name != NULL; ++i)
        if (_stricmp(name, codepage_alias[i].name) == 0) return codepage_alias[i].codepage;*/
    return -1;
}
static uint utf16_to_ucs4(const ushort *wbuf) {
    uint wc = wbuf[0];
    if (0xD800 <= wbuf[0] && wbuf[0] <= 0xDBFF) wc = ((wbuf[0] & 0x3FF) << 10) + (wbuf[1] & 0x3FF) + 0x10000;
    return wc;
}
static void ucs4_to_utf16(uint wc, ushort *wbuf, int *wbufsize) {
    if (wc < 0x10000) {
...
static int is_unicode(int codepage) {
    return (codepage == 1200 || codepage == 1201 || codepage == 12000 || codepage == 12001 || codepage == 65000 || codepage == 65001);
}
static int mbtowc_flags(int codepage) {
    /*return (codepage == 50220 || codepage == 50221 || codepage == 50222 || codepage == 50225 || codepage == 50227 || codepage == 50229 ||
	        codepage == 52936 || codepage == 54936 || (codepage >= 57002 && codepage <= 57011) ||codepage == 65000 || codepage == 42) ? 0 :
	        MB_ERR_INVALID_CHARS;*/
    return 0;
}
static int must_use_null_useddefaultchar(int codepage) {
    return (codepage == 65000 || codepage == 65001 || codepage == 50220 || codepage == 50221 || codepage == 50222 || codepage == 50225 ||
            codepage == 50227 || codepage == 50229 || codepage == 52936 || codepage == 54936 ||(codepage >= 57002 && codepage <= 57011) ||
            codepage == 42);
}
static void check_utf_bom(rec_iconv_t *cd, ushort *wbuf, int *wbufsize) {
    ...
    if (wbuf[0] == 0xFEFF && (!is_unicode(cd->to.codepage) || cd->to.codepage == 65001)) *wbufsize = 0;
}
static char *strrstr(const char *str, const char *token) {
    int len = strlen(token);
    const char *p = str + strlen(str);
    while (str <= --p) if (p[0] == token[0] && strncmp(p, token, len) == 0) return (char *)p;
    return NULL;
}
#if defined(USE_LIBICONV_DLL)
static int libiconv_iconv_open(rec_iconv_t *cd, const char *fromcode, const char *tocode) {
    ...
    if (p == NULL) p = DEFAULT_LIBICONV_DLL;
    for ( ; *p != 0; p = (*e == ',') ? e + 1 : e) {
        e = strchr(p, ',');
        if (p == e) continue;
        else if (e == NULL) e = p + strlen(p);
    ...
    if (hlibiconv == NULL) goto failed;
    hmsvcrt = find_imported_module_by_funcname(hlibiconv, "_errno");
    if (hmsvcrt == NULL) goto failed;
    _iconv_open = (f_iconv_open)GetProcAddress(hlibiconv, "libiconv_open");
    if (_iconv_open == NULL) _iconv_open = (f_iconv_open)GetProcAddress(hlibiconv, "iconv_open");
    cd->iconv_close = (f_iconv_close)GetProcAddress(hlibiconv, "libiconv_close");
    if (cd->iconv_close == NULL) cd->iconv_close = (f_iconv_close)GetProcAddress(hlibiconv, "iconv_close");
    cd->iconv = (f_iconv)GetProcAddress(hlibiconv, "libiconv");
    if (cd->iconv == NULL) cd->iconv = (f_iconv)GetProcAddress(hlibiconv, "iconv");
    cd->_errno = (f_errno)GetProcAddress(hmsvcrt, "_errno");
    if (_iconv_open == NULL || cd->iconv_close == NULL || cd->iconv == NULL || cd->_errno == NULL) goto failed;
    hlastdll = LoadLibrary(dllname);
    cd->cd = _iconv_open(tocode, fromcode);
    if (cd->cd == (iconv_t)(-1)) goto failed;
    cd->hlibiconv = hlibiconv;
    return TRUE;
failed:
    if (hlibiconv != NULL) FreeLibrary(hlibiconv);
    return FALSE;
}
    ...
    for ( ; Imp->OriginalFirstThunk != 0; ++Imp) {
        ...
        for ( ; Name->u1.Ordinal != 0; ++Name) {
            if (!IMAGE_SNAP_BY_ORDINAL(Name->u1.Ordinal)) {
                ImpName = (PIMAGE_IMPORT_BY_NAME) (Base + (DWORD)Name->u1.AddressOfData);
                if (strcmp((char *)ImpName->Name, funcname) == 0) return GetModuleHandle((char *)(Base + Imp->Name));
            }
        }
    }
    return NULL;
...
static int dbcs_mblen(csconv_t *cv, const uchar *buf, int bufsize) {
    int len = 0;/*IsDBCSLeadByteEx(cv->codepage, buf[0]) ? 2 : 1;
    if (bufsize < len) return_error(EINVAL);*/
    return len;
}
static int mbcs_mblen(csconv_t *cv, const uchar *buf, int bufsize) {
    int len = 0;
    if (cv->codepage == 54936) {
        if (buf[0] <= 0x7F) len = 1;
        else if (buf[0] >= 0x81 && buf[0] <= 0xFE && bufsize >= 2 && ((buf[1] >= 0x40 && buf[1] <= 0x7E) || (buf[1] >= 0x80 && buf[1] <= 0xFE))) len = 2;
        else if (buf[0] >= 0x81 && buf[0] <= 0xFE && bufsize >= 4 && buf[1] >= 0x30 && buf[1] <= 0x39) len = 4;
        else return_error(EINVAL);
        return len;
    } else return_error(EINVAL);
}
static int utf8_mblen(csconv_t *cv, const uchar *buf, int bufsize) {
...
static int kernel_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize) {
    ...
    if (len == -1) return -1;
    if (cv->codepage == 20127 && buf[0] >= 0x80) return_error(EILSEQ);
    //*wbufsize = MultiByteToWideChar(cv->codepage, mbtowc_flags (cv->codepage), (const char *)buf, len, (wchar_t *)wbuf, *wbufsize);
    if (*wbufsize == 0) return_error(EILSEQ);
    return len;
}
static int kernel_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize) {
    ...
    int flags = 0;
    int len = 0;
    if (bufsize == 0) return_error(E2BIG);
    ...
    if (len == 0) {
        //if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) return_error(E2BIG);
        return_error(EILSEQ);
    }
...
static int mlang_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize) {
    ...
    //if (hr != S_OK || insize != len) return_error(EILSEQ);
    return len;
}
static int mlang_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize) {
...
static int utf16_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize) {
    ...
    if (cv->codepage == 1200)  wbuf[0] = (buf[1] << 8) | buf[0];
    else if (cv->codepage == 1201)  wbuf[0] = (buf[0] << 8) | buf[1];
    ...
    if (0xD800 <= wbuf[0] && wbuf[0] <= 0xDBFF) {
        ...
        if (cv->codepage == 1200) wbuf[1] = (buf[3] << 8) | buf[2];
        else if (cv->codepage == 1201) wbuf[1] = (buf[2] << 8) | buf[3];
...
static int utf32_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize) {
    ...
    if (cv->codepage == 12000)  wc = (buf[3] << 24) | (buf[2] << 16) | (buf[1] << 8) | buf[0];
    else if (cv->codepage == 12001)  wc = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
...
static int iso2022jp_mbtowc(csconv_t *cv, const uchar *buf, int bufsize, ushort *wbuf, int *wbufsize) {
    ...
    //if (hr != S_OK || insize != len + esc_len) return_error(EILSEQ);
    /*if (wbuf[0] == buf[0] && cv->mode != ISO2022_MODE(ISO2022JP_CS_ASCII, ISO2022_SI))
        return_error(EILSEQ);*/
    //if (cv->mode != ISO2022_MODE(cs, shift)) cv->mode = ISO2022_MODE(cs, shift);
    return len;
}
static int iso2022jp_wctomb(csconv_t *cv, ushort *wbuf, int wbufsize, uchar *buf, int bufsize) {
    iso2022_esc_t *iesc = iso2022jp_esc;
    ...
    /*if (cv->mode == ISO2022_MODE(cs, shift)) {
        if (esc_len != 0) memmove(tmp, tmp + esc_len, len);
        esc_len = 0;
    } else {
...
static int iso2022jp_flush(csconv_t *cv, uchar *buf, int bufsize) {
    ...
    /*if (cv->mode != ISO2022_MODE(ISO2022JP_CS_ASCII, ISO2022_SI)) {
        esc_len = 0;
        if (ISO2022_MODE_SHIFT(cv->mode) != ISO2022_SI) esc_len += 1;
        if (ISO2022_MODE_CS(cv->mode) != ISO2022JP_CS_ASCII) esc_len += iesc[ISO2022JP_CS_ASCII].esc_len;
        if (bufsize < esc_len) return_error(E2BIG);
        esc_len = 0;
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved) {
    switch( fdwReason ) {
        case DLL_PROCESS_ATTACH: hwiniconv = (HMODULE)hinstDLL; break;
...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-l") == 0) {
            for (i = 0; codepage_alias[i].name != NULL; ++i) printf("%s\n", codepage_alias[i].name);
            return 0;
        }
        if (strcmp(argv[i], "-f") == 0) fromcode = argv[++i];
        else if (strcmp(argv[i], "-t") == 0) tocode = argv[++i];
        else {
            in = fopen(argv[i], "rb");
            if (in == NULL) {
