...
static void construct_test(void) {
  GNode *root;
  GNode *node;
  GNode *node_B;
  GNode *node_D;
  ...
  for (i = 0; i < g_node_n_children(node_G); i++) g_assert_cmpint(g_node_child_position(node_G, g_node_nth_child(node_G, i)), ==, i);
}
static void allocation_test(void) {
  ...
  for (i = 0; i < 2048; i++) {
      ...
      if ((i % 5) == 4) node = node->children->next;
  }
...
static gboolean check_order(GNode *node, gpointer data) {
  gchar **expected = data;
  gchar d;
...
static gpointer copy_up(gconstpointer src, gpointer data) {
  gchar l, u;
