...
static void test_to_ascii(void) {
  gint i;
  gchar *ascii;
  ...
  for (i = 0; i < num_non_round_trip_names; i++) {
      if (non_round_trip_names[i].orig_is_unicode) { g_assert(g_hostname_is_non_ascii(non_round_trip_names[i].orig_name)); }
      else { g_assert(!g_hostname_is_non_ascii(non_round_trip_names[i].orig_name)); }
      if (non_round_trip_names[i].ascii_is_encoded) { g_assert (g_hostname_is_ascii_encoded(non_round_trip_names[i].ascii_name)); }
      else {g_assert(!g_hostname_is_ascii_encoded(non_round_trip_names[i].ascii_name)); }
...
static void test_to_unicode(void) {
  gint i;
  gchar *unicode;
  for (i = 0; i < num_idn_test_domains; i++) {
...
static void test_is_ip_addr(void) {
  gint i;
  for (i = 0; i < num_ip_addr_tests; i++) {
      if (g_hostname_is_ip_address(ip_addr_tests[i].addr) != ip_addr_tests[i].is_addr) {
          char *msg = g_strdup_printf("g_hostname_is_ip_address (\"%s\") == %s", ip_addr_tests[i].addr, ip_addr_tests[i].is_addr ? "TRUE" : "FALSE");
...
int main(int   argc, char *argv[]) {
  ...
  if (argc == 2 && argv[1][0] != '-') {
      ...
      if (g_hostname_is_non_ascii(hostname)) {
          ...
          printf("to_unicode: %s\n", converted);
          g_free(converted);
	  } else printf("hostname is neither unicode nor ACE encoded\n");
      return 0;
  }
