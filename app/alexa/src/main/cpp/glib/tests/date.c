...
static void test_dmy_constructor(void) {
  GDate *d;
  guint32 j;
...
static void test_julian_constructor(void) {
  GDate *d1;
  GDate *d2;
...
static void test_year(gconstpointer t) {
  ...
  while(m < 13) {
      ...
      while(day <= dim) {
          ...
          g_assert(g_date_valid(d));
          if (m == G_DATE_JANUARY && day == 1) first_day_of_year = g_date_get_julian(d);
          ...
          g_assert (g_date_get_julian (d) + 1 - first_day_of_year == g_date_get_day_of_year (d));
          if (m == G_DATE_DECEMBER && day == 31) g_assert_cmpint (g_date_get_day_of_year (d), ==, days_in_year);
          ...
          if (g_date_get_weekday(d) == G_DATE_MONDAY) {
              ...
              if ((m == G_DATE_JANUARY && day <= 4) || (m == G_DATE_DECEMBER && day >= 29)) {
                  g_assert_cmpint(g_date_get_iso8601_week_of_year(d), ==, 1);
              } else { g_assert_cmpint(g_date_get_iso8601_week_of_year(d) - iso8601_week_of_year, ==, 1); }
          } else {
              g_assert_cmpint(g_date_get_monday_week_of_year(d) - monday_week_of_year, ==, 0);
              if (!(day == 1 && m == G_DATE_JANUARY)) g_assert_cmpint(g_date_get_iso8601_week_of_year(d) - iso8601_week_of_year, ==, 0);
          }
          ...
          if (g_date_get_weekday(d) == G_DATE_SUNDAY) { g_assert_cmpint(g_date_get_sunday_week_of_year(d) - sunday_week_of_year, ==, 1); }
          else { g_assert_cmpint(g_date_get_sunday_week_of_year (d) - sunday_week_of_year, ==, 0); }
          ...
          while (i < 402) {
              ...
              g_assert_cmpint(g_date_get_month(d), ==, m);
              g_assert_cmpint(g_date_get_year(d), ==, y);
              if (day < 29) { g_assert_cmpint(g_date_get_day(d), ==, day); }
              else g_date_set_day(d, day);
              tmp = *d;
              ...
              g_assert_cmpint(g_date_get_month(d), ==, m);
              g_assert_cmpint(g_date_get_year(d), ==, y);
              if (m != 2 && day != 29) { g_assert_cmpint(g_date_get_day (d), ==, day); }
              else g_date_set_day(d, day);
              i += 10;
          }
          j = g_date_get_julian(d);
