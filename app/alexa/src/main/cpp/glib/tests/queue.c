...
static void check_integrity(GQueue *queue) {
  GList *list;
  GList *last;
  GList *links;
  GList *link;
  ...
  if (!queue->head) g_assert(!queue->tail);
  if (!queue->tail) g_assert(!queue->head);
  n = 0;
  last = NULL;
  for (list = queue->head; list != NULL; list = list->next) {
      if (!list->next) last = list;
      ++n;
  }
  ...
  for (list = queue->tail; list != NULL; list = list->prev) {
      if (!list->prev) last = list;
      ++n;
  }
  g_assert(n == queue->length);
  g_assert(last == queue->head);
  links = NULL;
  for (list = queue->head; list != NULL; list = list->next) links = g_list_prepend (links, list);
  link = links;
  for (list = queue->tail; list != NULL; list = list->prev) {
  ...
  g_list_free(links);
  links = NULL;
  for (list = queue->tail; list != NULL; list = list->prev) links = g_list_prepend (links, list);
  link = links;
  for (list = queue->head; list != NULL; list = list->next) {
      g_assert(list == link->data);
      link = link->next;
...
static gboolean rnd_bool(void) {
  return g_random_int_range(0, 2);
}
static void check_max(gpointer elm, gpointer user_data) {
  gint *best = user_data;
  gint element = GPOINTER_TO_INT (elm);
  if (element > *best) *best = element;
}
static void check_min(gpointer elm, gpointer user_data) {
  gint *best = user_data;
  gint element = GPOINTER_TO_INT(elm);
  if (element < *best) *best = element;
}
static gint find_min(GQueue *queue) {
  gint min = G_MAXINT;
  g_queue_foreach(queue, check_min, &min);
  return min;
}
static gint find_max(GQueue *queue) {
  gint max = G_MININT;
  g_queue_foreach(queue, check_max, &max);
  return max;
}
static void delete_elm(gpointer elm, gpointer user_data) {
  g_queue_remove((GQueue*)user_data, elm);
  check_integrity((GQueue*)user_data);
}
static void delete_all(GQueue *queue) {
  g_queue_foreach(queue, delete_elm, queue);
}
static int compare_int(gconstpointer a, gconstpointer b, gpointer data) {
...
static gint get_random_position(GQueue *queue, gboolean allow_offlist) {
  int n;
  enum { OFF_QUEUE, HEAD, TAIL, MIDDLE, LAST } where;
  if (allow_offlist) where = g_random_int_range(OFF_QUEUE, LAST);
  else where = g_random_int_range(HEAD, LAST);
  switch(where) {
      case OFF_QUEUE: n = g_random_int(); break;
      case HEAD: n = 0; break;
      case TAIL:
          if (allow_offlist) n = queue->length;
          else n = queue->length - 1;
          break;
      case MIDDLE:
          if (queue->length == 0) n = 0;
          else n = g_random_int_range (0, queue->length);
          break;
      default:
...
static void random_test(gconstpointer d) {
  ...
  for (i = 0; i < N_ITERATIONS; ++i) {
      ...
      switch(op) {
          ...
          case FIND: {
                ...
                if (q->length == 0) find_existing = FALSE;
                if (!find_existing) first++;
                if (!find_existing) second--;
                if (find_existing) {
          ...
          case SORT: {
                  if (!g_queue_is_empty(q)) {
                      ...
                      if (max != min) g_queue_push_head(q, GINT_TO_POINTER(min));
          ...
          case PUSH_HEAD: {
                  ...
                  if (!qinf->head) qinf->tail = qinf->head = q->head;
                  else qinf->head = qinf->head->prev;
                  qinf->length++;
              }
              break;
          case PUSH_TAIL: {
                  ...
                  if (!qinf->tail) qinf->tail = qinf->head = q->head;
                  else qinf->tail = qinf->tail->next;
                  qinf->length++;
              }
              break;
          case PUSH_NTH: {
                  ...
                  if (qinf->head && qinf->head->prev) qinf->head = qinf->head->prev;
                  else qinf->head = q->head;
                  if (qinf->tail && qinf->tail->next) qinf->tail = qinf->tail->next;
                  else qinf->tail = g_list_last(qinf->head);
                  qinf->length++;
              }
              break;
          case POP_HEAD:
              if (qinf->head) qinf->head = qinf->head->next;
              if (!qinf->head) qinf->tail = NULL;
              qinf->length = (qinf->length == 0) ? 0 : qinf->length - 1;
              g_queue_pop_head(q);
              break;
          case POP_TAIL:
              if (qinf->tail) qinf->tail = qinf->tail->prev;
              if (!qinf->tail) qinf->head = NULL;
              qinf->length = (qinf->length == 0)? 0 : qinf->length - 1;
              g_queue_pop_tail(q);
              break;
          case POP_NTH:
              if (!g_queue_is_empty(q)) {
                  ...
                  if (n == q->length - 1) qinf->tail = qinf->tail->prev;
                  if (n == 0) qinf->head = qinf->head->next;
                  if (n >= 0 && n < q->length) qinf->length--;
                  g_assert(elm == g_queue_pop_nth(q, n));
              }
              break;
          case PEEK_HEAD:
              if (qinf->head) { g_assert(qinf->head->data == g_queue_peek_head(q)); }
              else { g_assert(g_queue_peek_head(q) == NULL); }
              break;
              case PEEK_TAIL:
              if (qinf->head) { g_assert(qinf->tail->data == g_queue_peek_tail(q)); }
              else { g_assert(g_queue_peek_tail(q) == NULL); }
              break;
          case PEEK_NTH:
              if (g_queue_is_empty(q)) {
                  for (j = -10; j < 10; ++j) g_assert(g_queue_peek_nth(q, j) == NULL);
              } else {
                  GList *list;
                  int n = get_random_position(q, TRUE);
                  if (n < 0 || n >= q->length) { g_assert(g_queue_peek_nth(q, n) == NULL); }
                  else {
                      list = qinf->head;
                      for (j = 0; j < n; ++j)
                        list = list->next;
          ...
          case INDEX: case LINK_INDEX: {
                  ...
                  for (list = q->head; list != NULL; list = list->next) {
                      if (list->data == GINT_TO_POINTER(x)) break;
                      n++;
                  }
                  g_assert(list);
                  g_assert(g_queue_index(q, GINT_TO_POINTER(x)) == g_queue_link_index(q, list));
              ...
              break;
          case REMOVE:
              if (!g_queue_is_empty(q)) g_queue_remove(q, qinf->tail->data);
              if (!g_queue_is_empty(q)) g_queue_remove(q, qinf->head->data);
              if (!g_queue_is_empty(q)) g_queue_remove(q, g_queue_peek_nth(q, get_random_position(q, TRUE)));
              qinf->head = q->head;
              qinf->tail = q->tail;
              qinf->length = q->length;
              break;
          case REMOVE_ALL:
              if (!g_queue_is_empty(q)) g_queue_remove_all(q, qinf->tail->data);
              if (!g_queue_is_empty(q)) g_queue_remove_all(q, qinf->head->data);
              if (!g_queue_is_empty(q)) g_queue_remove_all(q, g_queue_peek_nth (q, get_random_position(q, TRUE)));
              qinf->head = q->head;
              qinf->tail = q->tail;
              qinf->length = q->length;
              break;
          ...
          case POP_TAIL_LINK:
              ...
              break;
          case POP_NTH_LINK:
              if (g_queue_is_empty(q)) { g_assert (g_queue_pop_nth_link(q, 200) == NULL); }
              else {
                  int n = get_random_position(q, FALSE);
                  if (n == g_queue_get_length(q) - 1) qinf->tail = qinf->tail->prev;
                  if (n == 0) qinf->head = qinf->head->next;
                  qinf->length--;
              ...
              break;
          case PEEK_HEAD_LINK:
              if (g_queue_is_empty(q)) { g_assert(g_queue_peek_head_link(q) == NULL); }
              else { g_assert(g_queue_peek_head_link(q) == qinf->head); }
              break;
          case PEEK_TAIL_LINK:
              if (g_queue_is_empty(q)) { g_assert(g_queue_peek_tail_link(q) == NULL); }
              else { g_assert(g_queue_peek_tail_link(q) == qinf->tail); }
              break;
          case PEEK_NTH_LINK:
              if (g_queue_is_empty(q)) { g_assert(g_queue_peek_nth_link(q, 1000) == NULL); }
              else {
                  gint n = get_random_position(q, FALSE);
                  GList *link;
                  link = q->head;
                  for (j = 0; j < n; ++j) link = link->next;
          ...
          break;
          case LAST_OP: default: g_assert_not_reached(); break;
      }
      if (qinf->head != q->head || qinf->tail != q->tail || qinf->length != q->length) g_print("op: %d\n", op);
      g_assert(qinf->head == q->head);
      g_assert(qinf->tail == q->tail);
      g_assert(qinf->length == q->length);
      for (j = 0; j < N_QUEUES; ++j) check_integrity(queues[j].queue);
  }
  for (i = 0; i < N_QUEUES; ++i) g_queue_free(queues[i].queue);
}
static void remove_item(gpointer data, gpointer q) {
  GQueue *queue = q;
...
static void test_off_by_one(void) {
  GQueue *q;
  GList *node;
  q = g_queue_new();
...
static gint find_custom(gconstpointer a, gconstpointer b) {
  return GPOINTER_TO_INT(a) - GPOINTER_TO_INT(b);
}
static void test_find_custom(void) {
  GQueue *q;
  GList *node;
  q = g_queue_new();
  g_queue_push_tail(q, GINT_TO_POINTER(1234));
