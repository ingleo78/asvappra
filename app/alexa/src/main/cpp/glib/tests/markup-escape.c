...
static void escape_test(gconstpointer d) {
...
static void unichar_test(gconstpointer d) {
  ...
  outbuf[len] = 0;
  if (test->entity) g_snprintf(expected, 12, "&#x%x;", test->c);
  else strcpy(expected, outbuf);
  t.original = outbuf;
  t.expected = expected;
  escape_test(&t);
}
static void test_format(const gchar *format, const gchar *expected, ...) {
  gchar *result;
  va_list args;
