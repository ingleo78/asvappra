...
static gboolean randomly(gdouble prob) {
  return g_test_rand_double_range(0, 1) < prob;
}
static GVariantType *append_tuple_type_string (GString *, GString *, gboolean, gint);
static GVariantType *append_type_string(GString *string, GString *description, gboolean definite, gint depth) {
  if (!depth-- || randomly (0.3)) {
      gchar b = BASIC[g_test_rand_int_range(0, N_BASIC - definite)];
      g_string_append_c(string, b);
      g_string_append_c(description, b);
      switch(b) {
          case 'b': return g_variant_type_copy(G_VARIANT_TYPE_BOOLEAN);
          case 'y': return g_variant_type_copy(G_VARIANT_TYPE_BYTE);
          case 'n': return g_variant_type_copy(G_VARIANT_TYPE_INT16);
          case 'q': return g_variant_type_copy(G_VARIANT_TYPE_UINT16);
          case 'i': return g_variant_type_copy(G_VARIANT_TYPE_INT32);
          case 'u': return g_variant_type_copy(G_VARIANT_TYPE_UINT32);
          case 'x': return g_variant_type_copy(G_VARIANT_TYPE_INT64);
          case 't': return g_variant_type_copy(G_VARIANT_TYPE_UINT64);
          case 'h': return g_variant_type_copy(G_VARIANT_TYPE_HANDLE);
          case 'd': return g_variant_type_copy(G_VARIANT_TYPE_DOUBLE);
          case 's': return g_variant_type_copy(G_VARIANT_TYPE_STRING);
          case 'o': return g_variant_type_copy(G_VARIANT_TYPE_OBJECT_PATH);
          case 'g': return g_variant_type_copy(G_VARIANT_TYPE_SIGNATURE);
          case '?': return g_variant_type_copy(G_VARIANT_TYPE_BASIC);
      ...
      switch(g_test_rand_int_range(0, definite ? 5 : 7)) {
          case 0: {
                  ...
                  element = append_type_string(string, description, definite, depth);
          ...
          case 1: {
              ...
              break;
          case 2:
              result = append_tuple_type_string(string, description, definite, depth);
...
static GVariantType *append_tuple_type_string(GString *string, GString *description, gboolean definite, gint depth) {
  ...
  for (i = 0; i < size; i++) {
      ...
      if (i < size - 1) g_string_append(description, ", ");
  }
  types[i] = NULL;
  g_string_append_c(description, ']');
  g_string_append_c(string, ')');
  ...
  for (i = 0; i < size; i++) g_variant_type_free(types[i]);
  g_free(types);
  return result;
}
static gchar *invalid_mutation (const gchar *type_string) {
  ...
  }
  {
      ...
      } else if (randomly(0.5)) p = INVALIDS[g_test_rand_int_range(0, N_INVALIDS)];
      else p = '\0';
...
static gchar *describe_type(const GVariantType *type) {
  gchar *result;
      ...
      if (g_variant_type_is_array(type)) {
          ...
          if (!g_variant_type_equal(type, G_VARIANT_TYPE_TUPLE)) {
              ...
              for (i = 0; i < length; i++) {
                  ...
                  g_free (subtype);
                  if ((sub = g_variant_type_next(sub))) g_string_append(string, ", ");
              }
              g_assert(sub == NULL);
              g_string_append_c(string, ']');
              result = g_string_free(string, FALSE);
          } else result = g_strdup("R");
          ...
          value2 = describe_type(g_variant_type_next(g_variant_type_first(type)));
          g_assert(g_variant_type_next(g_variant_type_next(g_variant_type_first (type))) == NULL);
          ...
          g_free(key);
          g_free(value);
      } else if (g_variant_type_equal(type, G_VARIANT_TYPE_VARIANT)) result = g_strdup("V");
      else g_assert_not_reached();
  } else {
      if (g_variant_type_is_definite(type)) {
          g_assert(g_variant_type_is_basic(type));
          if (g_variant_type_equal(type, G_VARIANT_TYPE_BOOLEAN)) result = g_strdup("b");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_BYTE)) result = g_strdup("y");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT16)) result = g_strdup("n");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_UINT16)) result = g_strdup("q");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT32)) result = g_strdup("i");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_UINT32)) result = g_strdup("u");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT64)) result = g_strdup("x");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_UINT64)) result = g_strdup("t");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_HANDLE)) result = g_strdup("h");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_DOUBLE)) result = g_strdup("d");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_STRING)) result = g_strdup("s");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_OBJECT_PATH)) result = g_strdup("o");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_SIGNATURE)) result = g_strdup("g");
          else g_assert_not_reached();
      } else {
          if (g_variant_type_equal(type, G_VARIANT_TYPE_ANY)) result = g_strdup("S");
          else if (g_variant_type_equal(type, G_VARIANT_TYPE_BASIC)) result = g_strdup("?");
          else g_assert_not_reached();
      }
  }
  return result;
}
static gchar *generate_subtype(const gchar *type_string) {
  ...
  for (length = 0; type_string[length]; length++)
     n += type_string[length] == 'r' || type_string[length] == '?' || type_string[length] == '*';
  n = g_test_rand_int_range(0, n) + 1;
  l = -1;
  while(n--) l += strcspn(type_string + l + 1, "r?*") + 1;
  g_assert (type_string[l] == 'r' || type_string[l] == '?' || type_string[l] == '*');
  g_string_append_len(result, type_string, l);
  if (type_string[l] == 'r') replacement = append_tuple_type_string(result, junk, FALSE, 3);
  else if (type_string[l] == '?') replacement = append_type_string(result, junk, FALSE, 0);
  else if (type_string[l] == '*') replacement = append_type_string(result, junk, FALSE, 3);
  else g_assert_not_reached ();
  g_assert(g_variant_type_is_subtype_of(replacement, (gpointer)&type_string[l]));
...
static void subtype_check(const gchar *type_string, struct typestack *parent_ts) {
  ...
  for (node = &ts; node; node = node->parent) {
      ...
      g_assert(g_variant_type_is_subtype_of (node->type, ts.type) == g_variant_type_equal(ts.type, node->type));
      depth++;
  }
  if (!g_variant_type_is_definite (ts.type) && depth < 5) subtype_check(subtype, &ts);
  g_free(subtype);
}
static void test_gvarianttype(void) {
  gint i;
  for (i = 0; i < 2000; i++) {
      ...
      } else g_assert(strcspn (type_string->str, "r?*") == type_string->len);
...
static void calculate_type_info(const GVariantType *type, gsize *fixed_size, guint *alignment) {
  if (g_variant_type_is_array(type) || g_variant_type_is_maybe(type)) {
      calculate_type_info(g_variant_type_element(type), NULL, alignment);
      if (fixed_size) *fixed_size = 0;
  } else if (g_variant_type_is_tuple(type) || g_variant_type_is_dict_entry(type)) {
      if (g_variant_type_n_items (type)) {
          ...
          if (alignment) *alignment = al;
          if (fixed_size) *fixed_size = size;
      } else {
          if (fixed_size) *fixed_size = 1;
          if (alignment) *alignment = 1;
      }
  } else {
      gint fs, al;
      if (g_variant_type_equal(type, G_VARIANT_TYPE_BOOLEAN) || g_variant_type_equal(type, G_VARIANT_TYPE_BYTE)) al = fs = 1;
      else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT16) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT16)) al = fs = 2;
      else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT32) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT32) ||
               g_variant_type_equal(type, G_VARIANT_TYPE_HANDLE)) {
          al = fs = 4;
      } else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT64) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT64) ||
                 g_variant_type_equal(type, G_VARIANT_TYPE_DOUBLE)) {
          al = fs = 8;
      } else if (g_variant_type_equal(type, G_VARIANT_TYPE_STRING) || g_variant_type_equal(type, G_VARIANT_TYPE_OBJECT_PATH) ||
      ...
      if (fixed_size) *fixed_size = fs;
      if (alignment) *alignment = al;
  }
}
static gchar *describe_info(GVariantTypeInfo *info) {
  gchar *result;
  switch(g_variant_type_info_get_type_char(info)) {
      ...
      case G_VARIANT_TYPE_INFO_CHAR_DICT_ENTRY: {
          ...
          break;
      case G_VARIANT_TYPE_INFO_CHAR_VARIANT: result = g_strdup("V"); break;
      default:
...
static void check_offsets(GVariantTypeInfo *info, const GVariantType *type) {
  gint flavour;
  gint length;
  length = g_variant_type_info_n_members (info);
  ...
  for (flavour = 0; flavour < 8; flavour++) {
      ...
      for (i = 0; i < length; i++) {
          ...
          if (fixed_size) position += fixed_size;
          else {
              while((position & 0x7) != flavour) position++;
...
static void test_gvarianttypeinfo(void) {
  gint i;
  for (i = 0; i < 2000; i++) {
      GString *type_string, *description;
      ...
      g_assert_cmpstr(g_variant_type_info_get_type_string (info), ==,type_string->str);
      ...
      } else if (g_variant_type_is_tuple(type) || g_variant_type_is_dict_entry(type)) check_offsets(info, type);
      g_string_free(type_string, TRUE);
      g_string_free(description, TRUE);
...
static gchar *random_type_string(void) {
  ...
  if (g_test_rand_bit()) {
      ...
      type_string[i++] = '(';
      while(multiplier--) type_string[i++] = base_type;
      type_string[i++] = ')';
...
static RandomInstance *random_instance(GVariantTypeInfo *type_info) {
  ...
  if (type_info == NULL) {
      ...
      g_free(str);
  } else instance->type_info = g_variant_type_info_ref(type_info);
  instance->seed = g_test_rand_int();
  g_variant_type_info_query(instance->type_info, &instance->alignment, &instance->size);
  instance->is_fixed_sized = instance->size != 0;
  if (!instance->is_fixed_sized) instance->size = g_test_rand_int_range(0, MAX_INSTANCE_SIZE);
  instance->magic = INSTANCE_MAGIC;
  return instance;
}
static void random_instance_free(RandomInstance *instance) {
  g_variant_type_info_unref(instance->type_info);
  g_slice_free(RandomInstance, instance);
}
static void append_instance_size(RandomInstance *instance, gsize *offset) {
  *offset += (-*offset)&instance->alignment;
  *offset += instance->size;
}
static void random_instance_write(RandomInstance *instance, guchar *buffer) {
  GRand *rand;
  gint i;
  g_assert_cmpint((gsize) buffer & ALIGN_BITS & instance->alignment, ==, 0);
  rand = g_rand_new_with_seed(instance->seed);
  for (i = 0; i < instance->size; i++) buffer[i] = g_rand_int(rand);
  g_rand_free(rand);
}
static void append_instance_data(RandomInstance *instance, guchar **buffer) {
  while(((gsize)*buffer)&instance->alignment) *(*buffer)++ = '\0';
  random_instance_write(instance, *buffer);
  *buffer += instance->size;
}
static gboolean random_instance_assert(RandomInstance *instance, guchar *buffer, gsize size) {
  GRand *rand;
  gint i;
  g_assert_cmpint((gsize) buffer & ALIGN_BITS & instance->alignment, ==, 0);
...
static gboolean random_instance_check(RandomInstance *instance, guchar *buffer, gsize size) {
  GRand *rand;
  gint i;
  g_assert_cmpint((gsize)buffer & ALIGN_BITS & instance->alignment, ==, 0);
  if (size != instance->size) return FALSE;
  rand = g_rand_new_with_seed(instance->seed);
  for (i = 0; i < instance->size; i++)
      if (buffer[i] != (guchar)g_rand_int(rand)) break;
  g_rand_free(rand);
  return i == instance->size;
}
static void random_instance_filler(GVariantSerialised *serialised, gpointer data) {
  RandomInstance *instance = data;
  g_assert(instance->magic == INSTANCE_MAGIC);
  if (serialised->type_info == NULL) serialised->type_info = instance->type_info;
  if (serialised->size == 0) serialised->size = instance->size;
  g_assert(serialised->type_info == instance->type_info);
  g_assert(serialised->size == instance->size);
  if (serialised->data) random_instance_write(instance, serialised->data);
}
static gsize calculate_offset_size(gsize body_size, gsize n_offsets) {
  if (body_size == 0) return 0;
  if (body_size + n_offsets <= G_MAXUINT8) return 1;
  if (body_size + 2 * n_offsets <= G_MAXUINT16) return 2;
  if (body_size + 4 * n_offsets <= G_MAXUINT32) return 4;
  g_assert_not_reached();
}
static gpointer flavoured_malloc(gsize size, gsize flavour) {
  g_assert(flavour < 8);
  if (size == 0) return NULL;
  return ((gchar*)g_malloc(size + flavour)) + flavour;
}
static void flavoured_free(gpointer data, gsize flavour) {
  if (!data) return;
  g_free(((gchar*)data) - flavour);
}
static gpointer align_malloc(gsize size) {
  gpointer mem;
#ifdef HAVE_POSIX_MEMALIGN
  if (posix_memalign(&mem, 8, size)) g_error("posix_memalign failed");
#else
  mem = malloc(size);
#endif
  return mem;
}
static void align_free(gpointer mem) {
  free(mem);
}
static void append_offset(guchar **offset_ptr, gsize offset, guint offset_size) {
  union {
...
static void prepend_offset(guchar **offset_ptr, gsize offset, guint offset_size) {
  union {
      guchar bytes[sizeof(gsize)];
      gsize integer;
  } tmpvalue;
...
static void test_maybe(void) {
  ...
  needed_size = g_variant_serialiser_needed_size(type_info, random_instance_filler,NULL, 0);
  g_assert_cmpint(needed_size, ==, 0);
  needed_size = g_variant_serialiser_needed_size(type_info, random_instance_filler, (gpointer*)&instance, 1);
  if (instance->is_fixed_sized) { g_assert_cmpint(needed_size, ==, instance->size); }
  else { g_assert_cmpint(needed_size, ==, instance->size + 1); }
  {
      guchar *ptr;
      ptr = data = align_malloc(needed_size);
      append_instance_data(instance, &ptr);
      if (!instance->is_fixed_sized) *ptr++ = '\0';
  ...
  }
  {
      ...
      for (flavour = 0; flavour < 8; flavour += alignment) {
          ...
          g_variant_serialiser_serialise(serialised, random_instance_filler, (gpointer*)&instance, 1);
...
static void test_maybes(void) {
  guint i;
  for (i = 0; i < 1000; i++) test_maybe();
  g_variant_type_info_assert_no_infos();
}
static void test_array(void) {
  ...
  }
  {
      ...
      for (i = 0; i < n_children; i++) instances[i] = random_instance(element_info);
  }
  needed_size = g_variant_serialiser_needed_size(array_info, random_instance_filler, (gpointer*)instances, n_children);
  {
      gsize element_fixed_size;
      gsize body_size = 0;
      guint i;
      for (i = 0; i < n_children; i++) append_instance_size(instances[i], &body_size);
  ...
  }
  {
      ...
      for (flavour = 0; flavour < 8; flavour += alignment) {
          ...
          serialised.data = flavoured_malloc(needed_size, flavour);
          serialised.size = needed_size;
          g_variant_serialiser_serialise(serialised, random_instance_filler, (gpointer*)instances, n_children);
  ...
  {
      guint i;
      for (i = 0; i < n_children; i++) random_instance_free(instances[i]);
      g_free(instances);
  }
...
static void test_tuple(void) {
  ...
  instances = g_new(RandomInstance*, n_children);
  {
      ...
      for (i = 0; i < n_children; i++) {
          ...
          if (!instances[i]->is_fixed_sized) fixed_size = FALSE;
  ...
  needed_size = g_variant_serialiser_needed_size(type_info, random_instance_filler, (gpointer*)instances, n_children);
  {
      gsize body_size = 0;
      gsize offsets = 0;
      guint i;
      for (i = 0; i < n_children; i++) {
          ...
          if (i != n_children - 1 && !instances[i]->is_fixed_sized) offsets++;
      }
      if (fixed_size) {
          body_size += (-body_size) & alignment;
          g_assert((body_size == 0) == (n_children == 0));
          if (n_children == 0) body_size = 1;
      }
  ...
  }
  {
      ...
      for (i = 0; i < n_children; i++) {
          append_instance_data(instances[i], &body_ptr);
          if (i != n_children - 1 && !instances[i]->is_fixed_sized) prepend_offset(&ofs_ptr, body_ptr - data, offset_size);
      }
      if (fixed_size) {
          while (((gsize)body_ptr) & alignment) *body_ptr++ = '\0';
          g_assert((body_ptr == data) == (n_children == 0));
          if (n_children == 0) *body_ptr++ = '\0';
      }
...
#define is_sig is_string | 4
  ...
  for (i = 0; i < G_N_ELEMENTS(test_cases); i++) {
      guint flags;
      flags = g_variant_serialiser_is_string(test_cases[i].data, test_cases[i].size) ? 1 : 0;
      flags |= g_variant_serialiser_is_object_path(test_cases[i].data, test_cases[i].size) ? 2 : 0;
      flags |= g_variant_serialiser_is_signature(test_cases[i].data, test_cases[i].size) ? 4 : 0;
...
static GVariantType *make_random_definite_type(int depth) {
  GString *description;
  GString *type_string;
  GVariantType *type;
  description = g_string_new(NULL);
...
static void make_random_string(gchar *string, gsize size, const GVariantType *type) {
  gint i;
#define good_chars "bynqiuxthdsog"
  for (i = 0; i < size - 1; i++) string[i] = good_chars[g_test_rand_int_range(0, strlen(good_chars))];
  string[i] = '\0';
  if (*g_variant_type_peek_string(type) == 'o') string[0] = '/';
#undef good_chars
}
static TreeInstance *tree_instance_new(const GVariantType *type, int depth) {
  const GVariantType *child_type = NULL;
  GVariantType *mytype = NULL;
  TreeInstance *instance;
  gboolean is_tuple_type;
  if (type == NULL) type = mytype = make_random_definite_type(depth);
  ...
  switch (*g_variant_type_peek_string (type)) {
      ...
      case G_VARIANT_TYPE_INFO_CHAR_ARRAY:
          ...
          child_type = g_variant_type_element(type);
          break;
      case G_VARIANT_TYPE_INFO_CHAR_DICT_ENTRY: case G_VARIANT_TYPE_INFO_CHAR_TUPLE:
  ...
  if (instance->data_size == 0) {
      ...
      for (i = 0; i < instance->n_children; i++) {
          ...
          if (is_tuple_type) child_type = g_variant_type_next(child_type);
      }
...
static void tree_instance_free(TreeInstance *instance) {
  gint i;
  g_variant_type_info_unref(instance->info);
  for (i = 0; i < instance->n_children; i++) tree_instance_free(instance->children[i]);
  g_free(instance->children);
  g_slice_free(TreeInstance, instance);
}
static gboolean i_am_writing_byteswapped;
static void tree_filler(GVariantSerialised *serialised, gpointer data) {
  TreeInstance *instance = data;
  if (serialised->type_info == NULL) serialised->type_info = instance->info;
  if (instance->data_size == 0) {
      if (serialised->size == 0)
          serialised->size = g_variant_serialiser_needed_size(instance->info, tree_filler, (gpointer*)instance->children, instance->n_children);
      if (serialised->data)
          g_variant_serialiser_serialise(*serialised, tree_filler, (gpointer*)instance->children, instance->n_children);
  } else {
      if (serialised->size == 0) serialised->size = instance->data_size;
      if (serialised->data) {
          switch(instance->data_size) {
              case 1: *serialised->data = instance->data.integer; break;
              case 2: {
                      guint16 value = instance->data.integer;
                      if (i_am_writing_byteswapped) value = GUINT16_SWAP_LE_BE(value);
                      *(guint16*)serialised->data = value;
                  }
                  break;
              case 4: {
                      guint32 value = instance->data.integer;
                      if (i_am_writing_byteswapped) value = GUINT32_SWAP_LE_BE(value);
                      *(guint32*) serialised->data = value;
                  }
                  break;
              case 8: {
                      guint64 value = instance->data.integer;
                      if (i_am_writing_byteswapped) value = GUINT64_SWAP_LE_BE(value);
                      *(guint64*)serialised->data = value;
                  }
                  break;
              default: memcpy(serialised->data, instance->data.string, instance->data_size); break;
          }
      }
  }
}
static gboolean check_tree(TreeInstance *instance, GVariantSerialised serialised) {
  if (instance->info != serialised.type_info) return FALSE;
  if (instance->data_size == 0) {
      gint i;
      if (g_variant_serialised_n_children (serialised) != instance->n_children) return FALSE;
      for (i = 0; i < instance->n_children; i++) {
          GVariantSerialised child;
          gpointer data = NULL;
          gboolean ok;
          child = g_variant_serialised_get_child(serialised, i);
          if (child.size && child.data == NULL) child.data = data = g_malloc0(child.size);
      ...
      switch (instance->data_size) {
          case 1:
              ...
              return *(guint8*)serialised.data == (guint8)instance->data.integer;
          case 2:
              g_assert(serialised.size == 2);
              return *(guint16*)serialised.data == (guint16)instance->data.integer;
          case 4:
              g_assert(serialised.size == 4);
              return *(guint32*)serialised.data == (guint32)instance->data.integer;
          case 8:
              g_assert(serialised.size == 8);
              return *(guint64*)serialised.data == (guint64)instance->data.integer;
          default:
              if (serialised.size != instance->data_size) return FALSE;
              return memcmp(serialised.data, instance->data.string, instance->data_size) == 0;
      }
  }
}
static void serialise_tree(TreeInstance *tree, GVariantSerialised *serialised) {
...
static void test_byteswaps(void) {
  int i;
  for (i = 0; i < 200; i++) test_byteswap();
  g_variant_type_info_assert_no_infos();
}
static void test_fuzz(gdouble *fuzziness) {
  GVariantSerialised serialised;
  TreeInstance *tree;
  tree = tree_instance_new(NULL, 3);
...
static void test_fuzzes(gpointer data) {
  gdouble fuzziness;
  int i;
  fuzziness = GPOINTER_TO_INT(data) / 100.;
  for (i = 0; i < 200; i++) test_fuzz(&fuzziness);
  g_variant_type_info_assert_no_infos();
}
static GVariant *tree_instance_get_gvariant(TreeInstance *tree) {
  const GVariantType *type;
  GVariant *result;
  ...
  switch(g_variant_type_info_get_type_char(tree->info)) {
      case G_VARIANT_TYPE_INFO_CHAR_MAYBE: {
              ...
              GVariant *child;
              if (tree->n_children) child = tree_instance_get_gvariant(tree->children[0]);
              else child = NULL;
              child_type = g_variant_type_element(type);
              if (child != NULL && randomly(0.5)) child_type = NULL;
      ...
      case G_VARIANT_TYPE_INFO_CHAR_ARRAY: {
              ...
              for (i = 0; i < tree->n_children; i++) children[i] = tree_instance_get_gvariant(tree->children[i]);
              child_type = g_variant_type_element(type);
              if (i > 0 && randomly(0.5)) child_type = NULL;
      ...
      break;
      case 'b': result = g_variant_new_boolean(tree->data.integer > 0); break;
      case 'y': result = g_variant_new_byte(tree->data.integer); break;
      case 'n': result = g_variant_new_int16(tree->data.integer); break;
      case 'q': result = g_variant_new_uint16(tree->data.integer); break;
      case 'i': result = g_variant_new_int32(tree->data.integer); break;
      case 'u': result = g_variant_new_uint32(tree->data.integer); break;
      case 'x': result = g_variant_new_int64(tree->data.integer); break;
      case 't': result = g_variant_new_uint64(tree->data.integer); break;
      case 'h': result = g_variant_new_handle(tree->data.integer); break;
      case 'd': result = g_variant_new_double(tree->data.floating); break;
      case 's': result = g_variant_new_string(tree->data.string); break;
      case 'o': result = g_variant_new_object_path(tree->data.string); break;
      case 'g': result = g_variant_new_signature(tree->data.string); break;
      default: g_assert_not_reached();
  }
  return result;
}
static gboolean tree_instance_check_gvariant(TreeInstance *tree, GVariant *value) {
  const GVariantType *type;
  type = (GVariantType*)g_variant_type_info_get_type_string(tree->info);
  ...
  switch(g_variant_type_info_get_type_char(tree->info)) {
      case G_VARIANT_TYPE_INFO_CHAR_MAYBE: {
              ...
              if (child != NULL && tree->n_children == 1) equal = tree_instance_check_gvariant (tree->children[0], child);
              else if (child == NULL && tree->n_children == 0) equal = TRUE;
              else equal = FALSE;
              if (child != NULL) g_variant_unref (child);
              return equal;
          }
          break;
      case G_VARIANT_TYPE_INFO_CHAR_ARRAY: case G_VARIANT_TYPE_INFO_CHAR_TUPLE: case G_VARIANT_TYPE_INFO_CHAR_DICT_ENTRY: {
              gsize i;
              if (g_variant_n_children (value) != tree->n_children) return FALSE;
      ...
      case G_VARIANT_TYPE_INFO_CHAR_VARIANT: {
              ...
              equal = str1 == str2 &&tree_instance_check_gvariant(tree->children[0], child);
              g_variant_unref(child);
              return equal;
          }
          break;
      case 'b': return g_variant_get_boolean(value) == tree->data.integer;
      case 'y': return g_variant_get_byte(value) == (guchar)tree->data.integer;
      case 'n': return g_variant_get_int16(value) == (gint16)tree->data.integer;
      case 'q': return g_variant_get_uint16(value) == (guint16)tree->data.integer;
      case 'i': return g_variant_get_int32(value) == (gint32)tree->data.integer;
      case 'u': return g_variant_get_uint32(value) == (guint32)tree->data.integer;
      case 'x': return g_variant_get_int64(value) == (gint64)tree->data.integer;
      case 't': return g_variant_get_uint64(value) == (guint64)tree->data.integer;
      case 'h': return g_variant_get_handle(value) == (gint32)tree->data.integer;
      case 'd': {
              gdouble floating = g_variant_get_double (value);
              return memcmp (&floating, &tree->data.floating, sizeof floating) == 0;
          }
      case 's': case 'o': case 'g': return strcmp(g_variant_get_string(value, NULL), tree->data.string) == 0;
      default: g_assert_not_reached();
  }
}
static void tree_instance_build_gvariant(TreeInstance *tree, GVariantBuilder *builder, gboolean guess_ok) {
  const GVariantType *type;
  type = (GVariantType*)g_variant_type_info_get_type_string(tree->info);
  if (g_variant_type_is_container(type)) {
      gsize i;
      if (guess_ok && randomly(0.5)) {
          if (g_variant_type_is_array(type) && tree->n_children) type = G_VARIANT_TYPE_ARRAY;
          if (g_variant_type_is_maybe(type) && tree->n_children) type = G_VARIANT_TYPE_MAYBE;
          if (g_variant_type_is_tuple(type)) type = G_VARIANT_TYPE_TUPLE;
          if (g_variant_type_is_dict_entry(type)) type = G_VARIANT_TYPE_DICT_ENTRY;
      } else guess_ok = FALSE;
      g_variant_builder_open(builder, type);
      for (i = 0; i < tree->n_children; i++) tree_instance_build_gvariant(tree->children[i], builder, guess_ok);
      g_variant_builder_close(builder);
  } else g_variant_builder_add_value(builder, tree_instance_get_gvariant(tree));
}
static gboolean tree_instance_check_iter(TreeInstance *tree, GVariantIter *iter) {
  GVariant *value;
...
static void test_utf8(void) {
  ...
  value = g_variant_new_from_data(G_VARIANT_TYPE_STRING, invalid, sizeof invalid,FALSE, NULL, NULL);
  g_assert(g_utf8_validate (g_variant_get_string (value, NULL), -1, NULL));
  g_variant_unref(value);
  value = g_variant_new_from_data(G_VARIANT_TYPE_STRING, invalid, sizeof invalid, TRUE, NULL, NULL);
  g_assert(g_variant_get_string(value, NULL) == invalid);
  g_variant_unref(value);
}
static void test_containers(void) {
  gint i;
  for (i = 0; i < 100; i++) test_container();
  g_variant_type_info_assert_no_infos();
}
static void test_format_strings(void) {
  ...
  g_assert(g_variant_format_string_scan("(^as)", NULL, &end) && *end == '\0');
  ...
  g_assert(!g_variant_format_string_scan("{@**}", NULL, &end));
  g_assert(g_variant_format_string_scan("{@y*}", NULL, &end) && *end == '\0');
  g_assert(g_variant_format_string_scan("{yv}", NULL, &end) && *end == '\0');
  g_assert(!g_variant_format_string_scan("{&?v}", NULL, &end));
  g_assert(g_variant_format_string_scan("{@?v}", NULL, &end) && *end == '\0');
  g_assert(!g_variant_format_string_scan("{&@sv}", NULL, &end));
  g_assert(!g_variant_format_string_scan("{@&sv}", NULL, &end));
  g_assert(g_variant_format_string_scan("{&sv}", NULL, &end) && *end == '\0');
...
static void exit_on_abort(int signal) {
  exit(signal);
}
static gboolean do_failed_test(const gchar *pattern) {
...
static void test_invalid_varargs(void) {
...
static void check_and_free(GVariant *value, const gchar *str) {
...
static void test_varargs(void) {
  {
      ...
      g_variant_builder_add(&array, "{sv}", "title", g_variant_new_string ("Test case"));
      g_variant_builder_add_value(&array,g_variant_new_dict_entry(g_variant_new_string("temperature"),
                                 g_variant_new_variant(g_variant_new_double(37.5))));
      check_and_free(g_variant_new("(ma{sv}m(a{sv})ma{sv}ii)", NULL, FALSE, NULL, &array, 7777, 8888),
                    "(nothing, nothing, {'size': <(800, 600)>, 'title': <'Test case'>, 'temperature': <37.5>}, 7777, 8888)");
      check_and_free(g_variant_new("(imimimmimmimmi)", 123, FALSE, 321, TRUE, 123, FALSE, TRUE, 321, TRUE, FALSE, 321, TRUE, TRUE, 123),
                    "(123, nothing, 123, nothing, just nothing, 123)");
      check_and_free(g_variant_new("(ybnixd)", 'a', 1, 22, 33, (guint64) 44, 5.5),"(0x61, true, 22, 33, 44, 5.5)");
      check_and_free(g_variant_new("(@y?*rv)", g_variant_new("y", 'a'), g_variant_new("y", 'b'), g_variant_new("y", 'c'),
                     g_variant_new("(y)", 'd'), g_variant_new("y", 'e')),"(0x61, 0x62, 0x63, (0x64,), <byte 0x65>)");
  }
  {
      GVariantBuilder array;
      GVariantIter iter;
      ...
      g_variant_unref(value);
      g_variant_builder_init(&array, G_VARIANT_TYPE_ARRAY);
      for (i = 0; i < 100; i++) g_variant_builder_add(&array, "mi", i % 2 == 0, i);
      value = g_variant_builder_end(&array);
      i = 0;
      g_variant_iter_init(&iter, value);
      while(g_variant_iter_loop(&iter, "mi", NULL, &val)) g_assert(val == i++ || val == 0);
      g_assert(i == 100);
      i = 0;
      g_variant_iter_init(&iter, value);
  ...
  }
  {
      ...
      i = 0;
      while(g_variant_iter_loop(array, "s", &str)) g_assert_cmpstr(str, ==, test_strs[i++]);
      g_assert(i == 3);
  ...
  }
  {
      ...
      for (i = 0; i < 6; i++)
          if (i & 1) g_variant_builder_add(&builder, "s", strvector[i]);
          else g_variant_builder_add(&builder, "&s", strvector[i]);
      g_variant_builder_close(&builder);
      ...
      while(g_variant_iter_loop(&iter, "^as", &strv))
          for (i = 0; i < 6; i++) g_assert_cmpstr(strv[i], ==, strvector[i]);
      g_variant_iter_init(&iter, value);
      ...
      while(g_variant_iter_loop(&iter, "as", &i2)) {
          gchar *str;
          i = 0;
          while(g_variant_iter_loop(i2, "s", &str)) g_assert_cmpstr(str, ==, strvector[i++]);
          g_assert(i == 6);
      }
      g_variant_iter_init(&iter, value);
      ...
      while (g_variant_iter_loop(&iter, "@as", &sub)) {
          ...
          g_assert_cmpstr(str, ==, "['i', 'ii', 'iii', 'iv', 'v', 'vi']");
          g_free(str);
      }
  ...
  }
  {
      ...
      gdouble dval;
      gint32 hval;
      value = g_variant_new("(mymbmnmqmimumxmtmhmdmv)", FALSE, 'a', FALSE, TRUE, FALSE, (gint16)123, FALSE, (guint16)123, FALSE, (gint32)123,
                            FALSE, (guint32)123, FALSE, (gint64)123, FALSE, (guint64)123, FALSE, (gint32)-1, FALSE, (gdouble)37.5, NULL);
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                    NULL, NULL, NULL, NULL, NULL);
      memset(justs, 1, sizeof justs);
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", &justs[0], NULL, &justs[1], NULL, &justs[2], NULL, &justs[3], NULL, &justs[4], NULL, &justs[5],
                    NULL, &justs[6], NULL, &justs[7], NULL, &justs[8], NULL, &justs[9], NULL, NULL);
      g_assert(!(justs[0] || justs[1] || justs[2] || justs[3] || justs[4] || justs[5] || justs[6] || justs[7] || justs[8] || justs[9]));
      memset(justs, 1, sizeof justs);
      byteval = i16val = u16val = i32val = u32val = i64val = u64val = hval = 88;
      vval = (void*)1;
      bval = TRUE;
      dval = 88.88;
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", &justs[0], &byteval, &justs[1], &bval, &justs[2], &i16val, &justs[3], &u16val, &justs[4],
                    &i32val, &justs[5], &u32val, &justs[6], &i64val, &justs[7], &u64val, &justs[8], &hval, &justs[9], &dval, &vval);
      g_assert(!(justs[0] || justs[1] || justs[2] || justs[3] || justs[4] || justs[5] || justs[6] || justs[7] || justs[8] || justs[9]));
      g_assert(byteval == '\0' && bval == FALSE);
      g_assert(i16val == 0 && u16val == 0 && i32val == 0 && u32val == 0 && i64val == 0 && u64val == 0 && hval == 0 && dval == 0.0);
      g_assert(vval == NULL);
      byteval = i16val = u16val = i32val = u32val = i64val = u64val = hval = 88;
      vval = (void*)1;
      bval = TRUE;
      dval = 88.88;
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", NULL, &byteval, NULL, &bval, NULL, &i16val, NULL, &u16val, NULL, &i32val, NULL, &u32val,
                    NULL, &i64val, NULL, &u64val, NULL, &hval, NULL, &dval, &vval);
      g_assert(byteval == '\0' && bval == FALSE);
      g_assert(i16val == 0 && u16val == 0 && i32val == 0 && u32val == 0 && i64val == 0 && u64val == 0 && hval == 0 && dval == 0.0);
      g_assert(vval == NULL);
      g_variant_unref(value);
      value = g_variant_new("(mymbmnmqmimumxmtmhmdmv)", TRUE, 'a', TRUE, TRUE, TRUE, (gint16)123, TRUE, (guint16)123, TRUE, (gint32)123,
                            TRUE, (guint32)123, TRUE, (gint64)123, TRUE, (guint64)123, TRUE, (gint32)-1, TRUE, (gdouble)37.5, g_variant_new ("()"));
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                    NULL, NULL, NULL, NULL, NULL, NULL, NULL);
      memset(justs, 0, sizeof justs);
      g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", &justs[0], NULL, &justs[1], NULL, &justs[2], NULL, &justs[3], NULL, &justs[4], NULL,
                    &justs[5], NULL, &justs[6], NULL, &justs[7], NULL, &justs[8], NULL, &justs[9], NULL, NULL);
      g_assert(justs[0] && justs[1] && justs[2] && justs[3] && justs[4] && justs[5] && justs[6] && justs[7] && justs[8] && justs[9]);
      memset(justs, 0, sizeof justs);
      byteval = i16val = u16val = i32val = u32val = i64val = u64val = hval = 88;
      vval = (void*)1;
      ...
      g_assert(i16val == 123 && u16val == 123 && i32val == 123 && u32val == 123 && i64val == 123 && u64val == 123 && hval == -1 && dval == 37.5);
  ...
  }
  {
      ...
      g_variant_unref(value);
      g_free(str);
      if (do_failed_test("*which type of empty array*")) g_variant_new("(a{s*})", NULL);
  }
  g_variant_type_info_assert_no_infos();
}
static void hash_get(GVariant *value, const gchar *format, ...) {
  ...
  va_end(ap);
  if (hash) g_assert(*endptr == '#');
}
static GVariant *hash_new(const gchar *format, ...) {
  const gchar *endptr = NULL;
  GVariant *value;
  gboolean hash;
  va_list ap;
...
static void test_builder_memory(void) {
  GVariantBuilder *hb;
  GVariantBuilder sb;
...
static void test_hashing(void) {
  ...
  GHashTable *table;
  gint i;
  table = g_hash_table_new_full(g_variant_hash, g_variant_equal, (GDestroyNotify)g_variant_unref,NULL);
  ...
  again:
      ...
      g_hash_table_insert(table, g_variant_ref_sink(items[i]), GINT_TO_POINTER(i));
  }
  for (i = 0; i < G_N_ELEMENTS(items); i++) {
      gpointer result;
...
# define swapped16(x) x, 0
#endif
  guchar validbytes[] = { 'a', '\0', swapped16(66), 2, 0, 'b', '\0', swapped16(77), 2, 5, 11 };
  guchar corruptbytes[] = { 'a', '\0', swapped16(66), 2, 0, 'b', '\0', swapped16(77), 2, 6, 11 };
  ...
  value = g_variant_new_from_data(G_VARIANT_TYPE("a(sn)"), valid_data, sizeof validbytes, TRUE,NULL, NULL);
  ...
  g_free(string);
  value = g_variant_new_from_data(G_VARIANT_TYPE("a(sn)"), valid_data, sizeof validbytes, FALSE,NULL, NULL);
  swapped = g_variant_byteswap(value);
  g_variant_unref(value);
  g_assert(g_variant_get_size(swapped) == 13);
  string = g_variant_print(swapped, FALSE);
  ...
  g_assert_cmpstr(string, ==, "[('a', 66), ('b', 77)]");
  g_free(string);
  value = g_variant_new_from_data(G_VARIANT_TYPE("a(sn)"), corrupt_data, sizeof corruptbytes, FALSE,NULL, NULL);
...
static void test_parser(void) {
  ...
  g_free(res);
  parsed = g_variant_parse(g_variant_get_type(value), p,NULL, NULL, NULL);
...
static void test_parses(void) {
  gint i;
  for (i = 0; i < 100; i++) test_parser (); {
      GError *error = NULL;
      gchar str[128];
      GVariant *val;
      gchar *p, *p2;
      for (i = 0; i < 127; i++) str[i] = i + 1;
      str[i] = 0;
...
static void test_parse_failures(void) {
  ...
  for (i = 0; i < G_N_ELEMENTS(test); i += 3) {
      ...
      g_assert(value == NULL);
      if (!strstr(error->message, test[i+2])) g_error("test %d: Can't find `%s' in `%s'", i / 3, test[i+2], error->message);
      if (!g_str_has_prefix(error->message, test[i+1])) g_error("test %d: Expected location `%s' in `%s'", i / 3, test[i+1], error->message);
      g_error_free(error);
    }
}
static void test_parse_positional(void) {
  GVariant *value;
  check_and_free(g_variant_new_parsed("[('one', 1), (%s, 2), ('three', %i)]", "two", 3),"[('one', 1), ('two', 2), ('three', 3)]");
  value = g_variant_new_parsed("[('one', 1), (%s, 2), ('three', %u)]", "two", 3);
...
static void test_bytestring(void) {
  ...
  g_free(str);
  strv = g_strsplit(test_string, ",", 0);
  value = g_variant_new("(^aay^a&ay^ay^&ay)", strv, strv, strv[0], strv[0]);
  g_strfreev(strv);
...
static void test_lookup_value(void) {
  struct {
  ...
  for (i = 0; i < G_N_ELEMENTS(cases); i++) {
      ...
      if (value == NULL && cases[i].value == NULL) continue;
...
static void test_lookup(void) {
  ...
  gboolean ok;
  gint num;
  dict = g_variant_parse(NULL,"{'a': <5>, 'b': <'c'>}",NULL, NULL, NULL);
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i <= 20; i += 4) {
      ...
      g_test_add_data_func(testname, GINT_TO_POINTER(i), (gpointer)test_fuzzes);
      g_free(testname);
  }
