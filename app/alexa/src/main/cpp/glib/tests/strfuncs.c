...
#define DEFINE_CALL_CTYPE(function)	    \
	static int call_##function(int c) {	\
		return function (c);		    \
	}
#define DEFINE_CALL_G_ASCII_CTYPE(function)	           \
	static gboolean	call_g_ascii_##function(gchar c) { \
		return g_ascii_##function(c);	               \
	}
FOR_ALL_CTYPE(DEFINE_CALL_CTYPE)
FOR_ALL_CTYPE(DEFINE_CALL_G_ASCII_CTYPE)
static void test_is_function(const char *name, gboolean (*ascii_function)(gchar), int (*c_library_function)(int), gboolean (*unicode_function)(gunichar)) {
  int c;
  for (c = 0; c <= 0x7F; c++) {
      ...
      if (ascii_result != c_library_result && c != '\v') {
	      g_error("g_ascii_%s returned %d and %s returned %d for 0x%X", name, ascii_result, name, c_library_result, c);
	  }
      if (ascii_result != unicode_result) {
	      g_error("g_ascii_%s returned %d and g_unichar_%s returned %d for 0x%X", name, ascii_result, name, unicode_result, c);
	  }
  }
  for (c = 0x80; c <= 0xFF; c++) {
      gboolean ascii_result = ascii_function((gchar)c);
      if (ascii_result) g_error("g_ascii_%s returned TRUE for 0x%X", name, c);
  }
}
static void test_to_function(const char *name, gchar (*ascii_function)(gchar), int (*c_library_function)(int), gunichar (*unicode_function)(gunichar)) {
  int c;
  for (c = 0; c <= 0x7F; c++) {
      ...
      if (ascii_result != c_library_result) {
	      g_error("g_ascii_%s returned 0x%X and %s returned 0x%X for 0x%X", name, ascii_result, name, c_library_result, c);
	  }
      if (ascii_result != unicode_result) {
	      g_error("g_ascii_%s returned 0x%X and g_unichar_%s returned 0x%X for 0x%X", name, ascii_result, name, unicode_result, c);
	  }
  }
  for (c = 0x80; c <= 0xFF; c++) {
      int ascii_result = (guchar)ascii_function((gchar) c);
      if (ascii_result != c) {
	      g_error("g_ascii_%s returned 0x%X for 0x%X", name, ascii_result, c);
	  }
  }
}
static void test_digit_function(const char *name, int (*ascii_function)(gchar), int (*unicode_function)(gunichar)) {
  int c;
  for (c = 0; c <= 0x7F; c++) {
      int ascii_result = ascii_function((gchar)c);
      int unicode_result = unicode_function((gunichar)c);
      if (ascii_result != unicode_result) {
	      g_error ("g_ascii_%s_value returned %d and g_unichar_%s_value returned %d for 0x%X", name, ascii_result, name, unicode_result, c);
	  }
  }
  for (c = 0x80; c <= 0xFF; c++) {
      int ascii_result = ascii_function((gchar)c);
      if (ascii_result != -1) g_error ("g_ascii_%s_value returned %d for 0x%X", name, ascii_result, c);
  }
}
static void test_is_to_digit(void) {
...
static void test_strnfill(void) {
  gchar *str;
  str = g_strnfill (0, 'a');
  g_assert (str != NULL);
  g_assert (*str == '\0');
...
static void test_strconcat(void) {
  ...
  g_assert_cmpstr(str, ==, GLIB_TEST_STRING);
  g_free(str);
  str = g_strconcat(GLIB_TEST_STRING, GLIB_TEST_STRING, GLIB_TEST_STRING, NULL);
  g_assert(str != NULL);
...
static void test_strjoin(void) {
  ...
  g_assert_cmpstr(str, ==, GLIB_TEST_STRING);
  g_free(str);
  str = g_strjoin(NULL, GLIB_TEST_STRING, GLIB_TEST_STRING, GLIB_TEST_STRING, NULL);
  g_assert(str != NULL);
  g_assert_cmpstr(str, ==, GLIB_TEST_STRING GLIB_TEST_STRING GLIB_TEST_STRING);
  g_free(str);
  str = g_strjoin(":", GLIB_TEST_STRING, GLIB_TEST_STRING, GLIB_TEST_STRING, NULL);
  g_assert(str != NULL);
  g_assert_cmpstr(str, ==, GLIB_TEST_STRING ":" GLIB_TEST_STRING ":" GLIB_TEST_STRING);
  g_free(str);
}
static void test_strcanon(void) {
  gchar *str;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) str = g_strcanon(NULL, "ab", 'y');
...
static void test_strcompress_strescape(void) {
  gchar *str;
  gchar *tmp;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) str = g_strcompress(NULL);
  g_test_trap_assert_failed();
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) str = g_strcompress("abc\\");
  g_test_trap_assert_failed();
  str = g_strcompress("abc\\\\\\\"\\b\\f\\n\\r\\t\\003\\177\\234\\313\\12345z");
  g_assert(str != NULL);
  g_assert_cmpstr(str, ==, "abc\\\"\b\f\n\r\t\003\177\234\313\12345z");
  g_free(str);
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) str = g_strescape (NULL, NULL);
  g_test_trap_assert_failed();
  str = g_strescape("abc\\\"\b\f\n\r\t\003\177\234\313", NULL);
  g_assert(str != NULL);
  g_assert_cmpstr(str, ==, "abc\\\\\\\"\\b\\f\\n\\r\\t\\003\\177\\234\\313");
  g_free(str);
  str = g_strescape("abc\\\"\b\f\n\r\t\003\177\234\313","\b\f\001\002\003\004");
  g_assert(str != NULL);
  g_assert_cmpstr(str, ==, "abc\\\\\\\"\b\f\\n\\r\\t\003\\177\\234\\313");
  g_free(str);
...
static void test_ascii_strcasecmp(void) {
  gboolean res;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_ascii_strcasecmp ("foo", NULL);
  g_test_trap_assert_failed();
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_ascii_strcasecmp (NULL, "foo");
  g_test_trap_assert_failed();
...
static void do_test_strchug(const gchar *str, const gchar *expected) {
  gchar *tmp;
  gboolean res;
  tmp = g_strdup(str);
  g_strchug(tmp);
...
static void test_strchug(void) {
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) g_strchug(NULL);
  g_test_trap_assert_failed();
  do_test_strchug("", "");
  do_test_strchug(" ", "");
  do_test_strchug("\t\r\n ", "");
...
static void do_test_strchomp(const gchar *str, const gchar *expected) {
  gchar *tmp;
  gboolean res;
  tmp = g_strdup(str);
  g_strchomp(tmp);
...
static void test_strchomp (void) {
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) g_strchomp (NULL);
  g_test_trap_assert_failed();
  do_test_strchomp("", "");
  do_test_strchomp(" ", "");
  do_test_strchomp(" \t\r\n", "");
...
static void test_strreverse(void) {
  gchar *str;
  gchar *p;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) str = g_strreverse (NULL);
  g_test_trap_assert_failed();
...
static void test_has_prefix(void) {
  gboolean res;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_str_has_prefix("foo", NULL);
  g_test_trap_assert_failed();
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_str_has_prefix(NULL, "foo");
  g_test_trap_assert_failed();
...
static void test_has_suffix(void) {
  gboolean res;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_str_has_suffix("foo", NULL);
  g_test_trap_assert_failed();
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) res = g_str_has_suffix(NULL, "foo");
  g_test_trap_assert_failed();
...
static void strv_check(gchar **strv, ...) {
  gboolean ok = TRUE;
  gint i = 0;
  va_list list;
  va_start(list, strv);
  while(ok) {
      ...
      if (str == NULL) ok = FALSE;
      else { g_assert_cmpstr (strv[i], ==, str); }
      i++;
  }
  va_end(list);
  g_strfreev(strv);
...
static void test_strsplit_set(void) {
...
static void test_strv_length(void) {
  guint l;
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) l = g_strv_length (NULL);
  g_test_trap_assert_failed();
  l = g_strv_length(g_strsplit("1,2,3,4", ",", -1));
  g_assert_cmpuint(l, ==, 4);
}
static char *locales[] = {"sv_SE", "en_US", "fa_IR", "C", "ru_RU"};
void check_strtod_string(gchar *number, double res, gboolean check_end, gint correct_len) {
  double d;
  gint l;
  gchar *dummy;
  ...
  for (l = 0; l < G_N_ELEMENTS(locales); l++) {
      ...
      if (!ok) {
          ...
          else g_error("g_ascii_strtod on \"%s\" for locale %s endptr was REALLY wrong (number=%p, end=%p)\n", number, locales[l], number, end);
      }
  }
  g_free(number);
}
static void check_strtod_number(gdouble num, gchar *fmt, gchar *str) {
  int l;
...
static void check_uint64(const gchar *str, const gchar *end, gint base, guint64 result, gint error) {
  guint64 actual;
  gchar *endptr = NULL;
  gint err;
  errno = 0;
...
static void check_int64(const gchar *str, const gchar *end, gint base, gint64 result, gint error) {
  gint64 actual;
  gchar *endptr = NULL;
  gint err;
  errno = 0;
...
static void test_strip_context(void) {
  const gchar *msgid;
  const gchar *msgval;
  const gchar *s;
  msgid = "blabla";
