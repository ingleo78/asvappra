...
static gboolean test_load(GBookmarkFile *bookmark, const gchar *filename) {
    GError *error = NULL;
    gboolean res;
...
static void test_query(GBookmarkFile *bookmark) {
    gint size;
    gchar **uris;
    gsize uris_len, i;
    gchar *mime;
...
static gboolean test_modify(GBookmarkFile *bookmark) {
    gchar *text;
    guint count;
    time_t stamp;
    time_t now;
    ...
    gchar *icon;
    gchar *mime;
    if (g_test_verbose()) g_print("\t=> check global title/description...");
    ...
    if (g_test_verbose ()) g_print("\t=> check bookmark title/description...");
    ...
    g_free(mime);
    if (g_test_verbose()) g_print("ok\n");
    if (g_test_verbose()) g_print("\t=> check non existing bookmark...");
    ...
    g_clear_error(&error);
    if (g_test_verbose()) g_print("ok\n");
    if (g_test_verbose()) g_print("\t=> check application...");
    ...
    g_assert(g_bookmark_file_has_application(bookmark, TEST_URI_0, TEST_APP_NAME, NULL));
    g_bookmark_file_get_app_info(bookmark, TEST_URI_0, TEST_APP_NAME, &text, &count, &stamp, &error);
    g_assert_no_error(error);
    g_assert(count == 1);
    ...
    g_strfreev(apps);
    g_bookmark_file_get_app_info(bookmark, TEST_URI_0, "fail", &text, &count, &stamp, &error);
    g_assert_error(error, G_BOOKMARK_FILE_ERROR, G_BOOKMARK_FILE_ERROR_APP_NOT_REGISTERED);
    g_clear_error (&error);
    if (g_test_verbose()) g_print("ok\n");
    if (g_test_verbose()) g_print("\t=> check groups...");
    ...
    g_assert_no_error(error);
    if (g_test_verbose()) g_print("ok\n");
    if (g_test_verbose()) g_print("\t=> check remove...");
...
static void test_file(gconstpointer d) {
