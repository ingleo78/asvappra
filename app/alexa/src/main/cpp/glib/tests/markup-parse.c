...
static void start_element_handler(GMarkupParseContext *context, const gchar *element_name, const gchar **attribute_names, const gchar **attribute_values,
...
static void passthrough_handler(GMarkupParseContext *context, const gchar *passthrough_text, gsize text_len, gpointer user_data, GError **error) {
  indent(0);
  g_string_append_printf(string, "PASS '%.*s'\n", (int)text_len, passthrough_text);
}
static void error_handler(GMarkupParseContext *context, GError *error, gpointer user_data) {
  g_string_append_printf (string, "ERROR %s\n", error->message);
}
static const GMarkupParser parser = { start_element_handler, end_element_handler, text_handler, passthrough_handler, error_handler };
...
static int test_in_chunks(const gchar *contents, gint length, gint chunk_size) {
  ...
  while(i < length) {
      ...
      if (!g_markup_parse_context_parse(context,contents + i, this_chunk,NULL)) {
...
static int test_file(const gchar *filename) {
  gchar *contents;
  gsize length;
  GError *error;
  GMarkupParseContext *context;
  ...
  g_markup_parse_context_free (context);
  if (test_in_chunks (contents, length, 1) != 0) return 1;
  if (test_in_chunks (contents, length, 2) != 0) return 1;
  if (test_in_chunks (contents, length, 5) != 0) return 1;
  if (test_in_chunks (contents, length, 12) != 0) return 1;
  if (test_in_chunks (contents, length, 1024) != 0) return 1;
  return 0;
}
static gchar *get_expected_filename(const gchar *filename) {
  gchar *f, *p, *expected;
  f = g_strdup(filename);
  p = strstr(f, ".gmarkup");
  *p = 0;
...
static void test_parse(gconstpointer d) {
  ...
  if (strstr(filename, "valid")) { g_assert_cmpint(res, ==, 0); }
  else { g_assert_cmpint(res, ==, 1); }
...
int main(int argc, char *argv[]) {
  ...
  while ((name = g_dir_read_name(dir)) != NULL) {
      if (strstr(name, "expected")) continue;
