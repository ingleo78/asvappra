...
static void run_to_uri_tests(void) {
  int i;
  gchar *res;
  GError *error;
  for (i = 0; i < G_N_ELEMENTS(to_uri_tests); i++) {
      error = NULL;
      res = g_filename_to_uri(to_uri_tests[i].filename, to_uri_tests[i].hostname, &error);
      if (res) { g_assert_cmpstr(res, ==, to_uri_tests[i].expected_result); }
      else { g_assert_error(error, G_CONVERT_ERROR, to_uri_tests[i].expected_error); }
      g_free(res);
  }
}
static void run_from_uri_tests(void) {
  int i;
  gchar *res;
  gchar *hostname;
  GError *error;
  for (i = 0; i < G_N_ELEMENTS(from_uri_tests); i++) {
      error = NULL;
      res = g_filename_from_uri(from_uri_tests[i].uri, &hostname, &error);
#ifdef G_OS_WIN32
      {
          gchar *p, *slash;
...
#endif
      if (res) { g_assert_cmpstr(res, ==, from_uri_tests[i].expected_filename); }
      else { g_assert_error(error, G_CONVERT_ERROR, from_uri_tests[i].expected_error); }
      g_assert_cmpstr(hostname, ==, from_uri_tests[i].expected_hostname);
    }
}
static gint safe_strcmp_filename(const gchar *a, const gchar *b) {
#ifndef G_OS_WIN32
  return g_strcmp0 (a, b);
#else
  if (!a || !b) return g_strcmp0(a, b);
  else {
      while(*a && *b) {
          if ((G_IS_DIR_SEPARATOR(*a) && G_IS_DIR_SEPARATOR(*b)) || *a == *b) a++, b++;
          else return (*a - *b);
      }
      return (*a - *b);
  }
...
static gint safe_strcmp_hostname (const gchar *a, const gchar *b) {
  if (a == NULL) a = "";
  if (b == NULL) b = "";
#ifndef G_OS_WIN32
  return g_strcmp0(a, b);
#else
  if (g_strcmp0(a, "localhost") == 0 && b == NULL) return 0;
  else return g_strcmp0(a, b);
#endif
}
static void run_roundtrip_tests(void) {
  ...
  GError *error;
  for (i = 0; i < G_N_ELEMENTS(to_uri_tests); i++) {
      if (to_uri_tests[i].expected_error != 0) continue;
      error = NULL;
      uri = g_filename_to_uri(to_uri_tests[i].filename, to_uri_tests[i].hostname, &error);
...
static void run_uri_list_tests(void) {
  gchar *list = "# urn:isbn:0-201-08372-8\r\nhttp://www.huh.org/books/foo.html\r\nhttp://www.huh.org/books/foo.pdf   \r\n"
  ...
  for (j = 0; j < 3; j++) g_assert_cmpstr(uris[j], ==, expected_uris[j]);
  g_strfreev(uris);
