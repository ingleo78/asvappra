...
static void test_string_chunks(void) {
...
static void test_string_chunk_insert(void) {
...
static void test_string_printf(void) {
  GString *string;
  string = g_string_new(NULL);
#ifndef G_OS_WIN32
  g_string_printf(string, "%s|%0100d|%s|%0*d|%*.*f|%10000.10000f", "this pete guy sure is a wuss, like he's the number ", 1, " wuss.  everyone agrees.\n",
		          10, 666, 15, 15, 666.666666666, 666.666666666);
#else
  g_string_printf(string, "%s|%0100d|%s|%0*d|%*.*f|%100.100f", "this pete guy sure is a wuss, like he's the number ", 1, " wuss.  everyone agrees.\n",
		          10, 666, 15, 15, 666.666666666, 666.666666666);
#endif
  g_string_free(string, TRUE);
}
static void test_string_assign(void) {
  GString *string;
  string = g_string_new(NULL);
...
static void test_string_append_c (void) {
  ...
  for (i = 0; i < 10000; i++)
      if (i % 2) g_string_append_c(string, 'a'+(i%26));
      else (g_string_append_c)(string, 'a'+(i%26));
...
static void test_string_prepend_c (void) {
  GString *string;
  gint i;
  string = g_string_new("hi pete!");
  for (i = 0; i < 10000; i++) g_string_prepend_c(string, 'a'+(i%26));
  g_assert((strlen("hi pete!") + 10000) == string->len);
  g_assert((strlen("hi pete!") + 10000) == strlen(string->str));
  g_string_free(string, TRUE);
}
...
static void test_string_insert_unichar(void) {
  GString *string;
  string = g_string_new("firsthalf");
...
static void test_string_overwrite(void) {
  GString *string;
  string = g_string_new("testing");
...
static void test_string_nul_handling(void) {
  GString *string1, *string2;
...
static void test_string_up_down (void) {
  GString *s;
