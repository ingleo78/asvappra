...
static void test_assertions(void) {
  ...
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) { g_assert_cmpint(4, !=, 4); }
  g_test_trap_assert_failed();
  g_test_trap_assert_stderr("*assertion failed*");
}
static void test_timer(void) {
  ...
  g_test_maximized_result(5, "bogus-quantity: %ddummies", 5);
}
static void test_fork_fail(void) {
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) { g_assert_not_reached(); }
  g_test_trap_assert_failed();
...
static void test_fork_timeout(void) {
  if (g_test_trap_fork(0.11 * 1000000, 0)) {
      while(TRUE) g_usleep(1000 * 1000);
  }
  g_test_trap_assert_failed();
...
static void test_data_test(gconstpointer test_data) {
  g_assert(test_data == (void*)0xc0c0baba);
}
static void test_random_conversions(void) {
...
static gboolean fatal_handler(const gchar *log_domain, GLogLevelFlags log_level, const gchar *message, gpointer user_data) {
  return FALSE;
}
static void test_fatal_log_handler(void) {
  ...
  if (g_test_trap_fork(0, G_TEST_TRAP_SILENCE_STDERR)) g_error("Test failing");
  g_test_trap_assert_failed();
  if (g_test_trap_fork (0, G_TEST_TRAP_SILENCE_STDERR))
    g_str_has_prefix (NULL, "file://");
  g_test_trap_assert_failed ();
...
int main(int argc, char *argv[]) {
  ...
  if (g_test_perf()) g_test_add_func("/misc/timer", test_timer);
  g_test_add_func("/forking/fail assertion", test_fork_fail);
  g_test_add_func("/forking/patterns", test_fork_patterns);
  if (g_test_slow()) g_test_add_func("/forking/timeout", test_fork_timeout);
