...
int array[10000];
static void fill_hash_table_and_array(GHashTable *hash_table) {
  int i;
  for (i = 0; i < 10000; i++) {
      array[i] = i;
...
static void init_result_array(int result_array[10000]) {
  int i;
  for (i = 0; i < 10000; i++) result_array[i] = -1;
}
static void verify_result_array(int array[10000]) {
  int i;
  for (i = 0; i < 10000; i++) g_assert(array[i] == i);
}
static void handle_pair(gpointer key, gpointer value, int result_array[10000]) {
  int n;
  g_assert(key == value);
  n = *((int*)value);
...
static gboolean my_hash_callback_remove(gpointer key, gpointer value, gpointer user_data) {
  int *d = value;
  if ((*d) % 2) return TRUE;
  return FALSE;
}
static void my_hash_callback_remove_test(gpointer key, gpointer value, gpointer user_data) {
  int *d = value;
  if ((*d) % 2) g_assert_not_reached();
}
static void my_hash_callback(gpointer key, gpointer value, gpointer user_data) {
  handle_pair(key, value, user_data);
}
static guint my_hash(gconstpointer key) {
  return (guint)*((const gint*)key);
}
static gboolean my_hash_equal(gconstpointer a, gconstpointer b) {
...
static guint honeyman_hash(gconstpointer key) {
  ...
  g_assert(*name != 0);
  size = strlen(name);
  while(size--) sum = (sum >> 7) ^ CrcTable[(sum ^ (*name++)) & 0x7f];
  return(sum);
}
...
static void not_even_foreach(gpointer key, gpointer value, gpointer user_data) {
...
static gboolean remove_even_foreach(gpointer key, gpointer value, gpointer user_data) {
  const char *_key = (const char*)key;
  const char *_value = (const char*)value;
  int i;
  char val [20];
...
static void second_hash_test (gconstpointer d) {
  ...
  gboolean found;
  crcinit();
  h = g_hash_table_new_full(simple_hash ? one_hash : honeyman_hash, second_hash_cmp, g_free, g_free);
  g_assert(h != NULL);
  for (i=0; i<20; i++) {
      sprintf(key, "%d", i);
      g_assert(atoi(key) == i);
  ...
  for (i=0; i<20; i++) {
      ...
      found = g_hash_table_lookup_extended(h, key, (gpointer)&orig_key, (gpointer)&orig_val);
      if ((i % 2) == 0 || i == 3) {
          g_assert(!found);
          continue;
      }
...
static gboolean find_first(gpointer key, gpointer value, gpointer user_data) {
  gint *v = value; 
  gint *test = user_data;
  return (*v == *test);
}
...
static void test_hash_misc(void) {
  ...
  if (!pvalue || *pvalue != value) g_assert_not_reached();
  keys = g_hash_table_get_keys(hash_table);
  if (!keys) g_assert_not_reached();
  values = g_hash_table_get_values(hash_table);
  if (!values) g_assert_not_reached();
  keys_len = g_list_length(keys);
  values_len = g_list_length(values);
  if (values_len != keys_len &&  keys_len != g_hash_table_size (hash_table)) g_assert_not_reached();
  g_list_free(keys);
  g_list_free(values);
  ...
  for (i = 0; i < 10000; i++) {
      ...
      if (i % 2) g_hash_table_iter_remove(&iter);
  }
  g_assert(! g_hash_table_iter_next(&iter, &ikey, &ivalue));
  ...
  verify_result_array(result_array);
  for (i = 0; i < 10000; i++) g_hash_table_remove(hash_table, &array[i]);
  fill_hash_table_and_array(hash_table);
  if (g_hash_table_foreach_remove(hash_table, my_hash_callback_remove, NULL) != 5000 || g_hash_table_size(hash_table) != 5000)
      g_assert_not_reached();
  g_hash_table_foreach(hash_table, my_hash_callback_remove_test, NULL);
  g_hash_table_destroy(hash_table);
}
static gint destroy_counter;
static void value_destroy(gpointer value) {
  destroy_counter++;
}
static void test_hash_ref(void) {
  GHashTable *h;
  GHashTableIter iter;
  gchar *key, *value;
