...
static gint my_compare_with_data(gconstpointer a, gconstpointer b, gpointer user_data) {
  const char *cha = a;
  const char *chb = b;
...
char chars2[] = "0123456789abcdefghijklmnopqrstuvwxyz";
...
static void test_tree_search(void) {
  ...
  for (i = 0; chars[i]; i++) g_tree_insert(tree, &chars[i], &chars[i]);
  ...
  p = chars2;
  g_tree_foreach(tree, check_order, &p);
  for (i = 25; i >= 0; i--) g_tree_insert(tree, &chars[i + 10], &chars[i + 10]);
  p = chars;
  g_tree_foreach(tree, check_order, &p);
  c = '0';
...
static void test_tree_traverse (void) {
  GTree *tree;
  gint i;
  gchar *p, *result;
  tree = g_tree_new(my_compare);
