...
static void check_node(GSequenceNode *node) {
  if (node) {
      g_assert(node->parent != node);
      if (node->parent) g_assert(node->parent->left == node || node->parent->right == node);
      g_assert(node->n_nodes == 1 + (node->left ? node->left->n_nodes : 0) + (node->right ? node->right->n_nodes : 0));
      if (node->left) g_assert(get_priority(node) >= get_priority(node->left));
      if (node->right) g_assert(get_priority(node) >= get_priority(node->right));
      check_node(node->left);
      check_node(node->right);
    }
}
void g_sequence_check (GSequence *seq) {
...
static Item *fix_pointer(gconstpointer data) {
  return (Item*)((char*)data - 1);
}
static Item *get_item(GSequenceIter *iter) {
  return fix_pointer(g_sequence_get(iter));
}
static void check_integrity(SequenceInfo *info) {
  GList *list;
  GSequenceIter *iter;
  int i;
  g_sequence_check(info->sequence);
#if 0
  if (g_sequence_get_length(info->sequence) != info->n_items) g_print("%d %d\n", g_sequence_get_length(info->sequence), info->n_items);
#endif
...
static gpointer new_item(SequenceInfo *seq) {
...
static void seq_foreach(gpointer data, gpointer user_data) {
  Item *item = fix_pointer(data);
...
static gint simple_items_cmp(gconstpointer a, gconstpointer b, gpointer data) {
  const Item *item_a = fix_pointer(a);
  const Item *item_b = fix_pointer(b);
  if (item_a->number > item_b->number) return +1;
  else if (item_a->number < item_b->number) return -1;
  else return 0;
}
static gint simple_iters_cmp(gconstpointer a, gconstpointer b, gpointer data) {
  GSequence *seq = data;
...
static gint compare_items(gconstpointer a, gconstpointer b, gpointer data) {
  const Item *item_a = fix_pointer(a);
  const Item *item_b = fix_pointer(b);
  if (item_a->number < item_b->number) return -1;
...
static void check_sorted(SequenceInfo *info) {
  GList *list;
  int last;
  ...
  for (list = info->queue->head; list != NULL; list = list->next) {
      ...
      if (last_iter) g_assert(iter == g_sequence_iter_next(last_iter));
      last = item->number;
      last_iter = iter;
  }
}
static gint compare_iters(gconstpointer a, gconstpointer b, gpointer data) {
  GSequence *seq = data;
  GSequenceIter *iter_a = (GSequenceIter*)a;
  GSequenceIter *iter_b = (GSequenceIter*)b;
...
static int queue_link_index(SequenceInfo *seq, GList *link) {
  if (link) return g_queue_link_index(seq->queue, link);
  else return g_queue_get_length(seq->queue);
}
static void get_random_range(SequenceInfo *seq, GSequenceIter **begin_iter, GSequenceIter **end_iter, GList **begin_link, GList **end_link) {
  ...
  if (begin_iter) *begin_iter = g_sequence_get_iter_at_pos(seq->sequence, b);
  if (end_iter) *end_iter = g_sequence_get_iter_at_pos(seq->sequence, e);
  if (begin_link) *begin_link = g_queue_peek_nth_link(seq->queue, b);
  if (end_link) *end_link = g_queue_peek_nth_link(seq->queue, e);
  if (begin_iter && begin_link) { g_assert(queue_link_index(seq, *begin_link) ==g_sequence_iter_get_position(*begin_iter)); }
  if (end_iter && end_link) { g_assert(queue_link_index(seq, *end_link) == g_sequence_iter_get_position(*end_iter)); }
}
static gint get_random_position (SequenceInfo *seq) {
  int length = g_queue_get_length(seq->queue);
  g_assert(length == g_sequence_get_length(seq->sequence));
  return g_random_int_range(-2, length + 5);
}
static GSequenceIter *get_random_iter(SequenceInfo  *seq, GList **link) {
  GSequenceIter *iter;
  int pos = get_random_position(seq);
  if (link) *link = g_queue_peek_nth_link(seq->queue, pos);
  iter = g_sequence_get_iter_at_pos(seq->sequence, pos);
  if (link) g_assert(queue_link_index(seq, *link) == g_sequence_iter_get_position(iter));
  return iter;
}
static void dump_info(SequenceInfo *seq) {
#if 0
  GSequenceIter *iter;
  GList *list;
...
static void queue_insert_before(SequenceInfo *seq, GList *link, gpointer data) {
  if (link) g_queue_insert_before(seq->queue, link, data);
  else g_queue_push_tail(seq->queue, data);
}
static void run_random_tests(gconstpointer d) {
...
#define RANDOM_SEQUENCE() &(sequences[g_random_int_range (0, N_SEQUENCES)])
  for (k = 0; k < N_ITERATIONS; ++k) {
      ...
      switch(op) {
          ...
          case SORT_ITER: {
              check_integrity(seq);
              g_sequence_sort_iter(seq->sequence, (GSequenceIterCompareFunc)compare_iters, seq->sequence);
              g_queue_sort(seq->queue, compare_iters, NULL);
              check_sorted(seq);
          }
          break;
          case GET_END_ITER: case GET_BEGIN_ITER: {
          ...
          case MOVE: {
                  ...
                  if (!g_sequence_iter_is_end(iter1)) {
                      ...
                      if (!link2) g_assert(g_sequence_iter_is_end(iter2));
                  ...
                  if (!g_sequence_iter_is_end(iter1)) g_sequence_move(iter1, iter1);
              }
              break;
          case SWAP: {
                  GList *link1, *link2;
                  ...
                  GSequenceIter *iter1 = get_random_iter(seq1, &link1);
                  GSequenceIter *iter2 = get_random_iter(seq2, &link2);
                  if (!g_sequence_iter_is_end(iter1) && !g_sequence_iter_is_end(iter2)) {
                      gpointer tmp;
          ...
          case INSERT_SORTED: {
                  ...
                  check_sorted(seq);
                  for (i = 0; i < N_TIMES; ++i) {
                      GSequenceIter *iter = g_sequence_insert_sorted(seq->sequence, new_item(seq), compare_items, NULL);
          ...
          case INSERT_SORTED_ITER: {
                  ...
                  for (i = 0; i < N_TIMES; ++i) {
                      GSequenceIter *iter;
                      iter = g_sequence_insert_sorted_iter(seq->sequence, new_item(seq), (GSequenceIterCompareFunc)compare_iters, seq->sequence);
                      g_queue_insert_sorted(seq->queue, iter, compare_iters, NULL);
                  }
                  check_sorted(seq);
                  dump_info(seq);
          ...
          case SORT_CHANGED_ITER: {
                  ...
                  for (i = 0; i < N_TIMES; ++i) {
                      ...
                      if (!g_sequence_iter_is_end(iter)) {
                          g_sequence_set(iter, new_item(seq));
                          g_sequence_sort_changed_iter(iter, (GSequenceIterCompareFunc)compare_iters, seq->sequence);
                          g_queue_delete_link(seq->queue, link);
                          g_queue_insert_sorted(seq->queue, iter, compare_iters, NULL);
                      }
                      check_sorted(seq);
          ...
          case MOVE_RANGE: {
                  ...
                  if (queue_link_index(src, begin_link) >= queue_link_index(src, end_link)) break;
                  if (src == dst && queue_link_index(src, dst_link) >= queue_link_index(src, begin_link) &&
          ...
          case SEARCH_ITER: {
                  ...
                  check_sorted(seq);
                  item = new_item(seq);
                  search_iter = g_sequence_search_iter(seq->sequence, item, (GSequenceIterCompareFunc)compare_iters, seq->sequence);
                  insert_iter = g_sequence_insert_sorted(seq->sequence, item, compare_items, NULL);
                  g_assert(search_iter == g_sequence_iter_next(insert_iter));
                  g_queue_insert_sorted(seq->queue, insert_iter, compare_iters, NULL);
              }
          ...
          case LOOKUP_ITER: {
                  ...
                  insert_iter = g_sequence_insert_sorted(seq->sequence, item, compare_items, NULL);
                  g_queue_insert_sorted(seq->queue, insert_iter, compare_iters, NULL);
                  lookup_iter = g_sequence_lookup_iter(seq->sequence, item, (GSequenceIterCompareFunc)simple_iters_cmp, NULL);
                  g_assert(simple_iters_cmp(insert_iter, lookup_iter, NULL) == 0);
              }
              break;
          case GET: case SET: {
                  ...
                  if (!g_sequence_iter_is_end(iter)) {
                      ...
                      for (i = 0; i < N_TIMES; ++i) g_sequence_set(iter, new_item(seq));
                      check_integrity(seq);
                      g_sequence_set(iter, new_item(seq));
                  }
              }
          ...
          case ITER_IS_BEGIN: {
                  ...
                  check_integrity(seq);
                  if (g_sequence_get_length(seq->sequence) > 0) { g_assert(!g_sequence_iter_is_begin(g_sequence_get_end_iter(seq->sequence))); }
                  else { g_assert(g_sequence_iter_is_begin (g_sequence_get_end_iter(seq->sequence))); }
          ...
          case ITER_IS_END: {
                  ...
                  if (len > 0) { g_assert(!g_sequence_iter_is_end(g_sequence_get_begin_iter(seq->sequence))); }
                  else { g_assert(g_sequence_iter_is_end(g_sequence_get_begin_iter(seq->sequence))); }
          ...
          case ITER_GET_POSITION: {
                  GList *link;
                  GSequenceIter *iter = get_random_iter(seq, &link);
                  g_assert(g_sequence_iter_get_position(iter) == queue_link_index(seq, link));
              }
              break;
          case ITER_MOVE: {
                  int len = g_sequence_get_length(seq->sequence);
          ...
          case ITER_COMPARE: {
                  ...
                  if (cmp == 0) { g_assert(pos1 == pos2); }
                  else if (cmp < 0) { g_assert(pos1 < pos2); }
                  else { g_assert(pos1 > pos2); }
              }
              break;
          case RANGE_GET_MIDPOINT: {
                  ...
                  g_assert(g_sequence_iter_get_position(iter3) >= g_sequence_iter_get_position(iter1));
                  g_assert(g_sequence_iter_get_position(iter2) >= g_sequence_iter_get_position(iter3));
              }
              break;
      }
      check_integrity (seq);
...
static void test_out_of_range_jump(void) {
...
static int compare(gconstpointer a, gconstpointer b, gpointer userdata) {
  int ai, bi;
...
static int compare_iter(GSequenceIter *a, GSequenceIter *b, gpointer data) {
  return compare(g_sequence_get(a), g_sequence_get(b), data);
}
static void test_insert_sorted_non_pointer(void) {
  int i;
  for (i = 0; i < 10; i++) {
      GSequence *seq = g_sequence_new(NULL);
      int j;
      for (j = 0; j < 10000; j++) {
          g_sequence_insert_sorted(seq, GINT_TO_POINTER(g_random_int()), compare, NULL);
          g_sequence_insert_sorted_iter(seq, GINT_TO_POINTER(g_random_int()), compare_iter, NULL);
      }
