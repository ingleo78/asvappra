...
class MultipartParser {
	...
	private:
		...
		void callback(Callback cb, const char *buffer = NULL, size_t start = UNMARKED, size_t end = UNMARKED, bool allowEmpty = false) {
			if (start != UNMARKED && start == end && !allowEmpty) return;
			if (cb != NULL) cb(buffer, start, end, userData);
		}
		void dataCallback(Callback cb, size_t &mark, const char *buffer, size_t i, size_t bufferLen, bool clear, bool allowEmpty = false) {
			if (mark == UNMARKED) return;
			if (!clear) {
		...
		inline bool isBoundaryChar(char c) const { return boundaryIndex[(unsigned char) c]; }
		bool isHeaderFieldCharacter(char c) const { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == HYPHEN; }
		void setError(const char *message) {
			state = ERROR;
			errorReason = message;
		}
		void processPartData(size_t &prevIndex, size_t &index, const char *buffer, size_t len, size_t boundaryEnd, size_t &i, char c, State &state, int &flags) {
			prevIndex = index;
			if (index == 0) {
				while (i + boundary.size() <= len) {
					if (isBoundaryChar(buffer[i + boundaryEnd])) break;
					i += boundary.size();
				}
				if (i == len) return;
				c = buffer[i];
			...
			if (index < boundary.size()) {
				if (boundary[index] == c) {
					if (index == 0) dataCallback(onPartData, partDataMark, buffer, i, len, true);
					index++;
				} else index = 0;
			} else if (index == boundary.size()) {
				index++;
				if (c == CR) flags |= PART_BOUNDARY;
				else if (c == HYPHEN) flags |= LAST_BOUNDARY;
				else index = 0;
	...
	public:
		...
		size_t feed(const char *buffer, size_t len) {
			...
			while (i < len) {
				...
				switch (state) {
					case ERROR: return i;
					case START: index = 0; state = START_BOUNDARY;
					case START_BOUNDARY:
						...
						index++;
						break;
					case DUPLICATE_BOUNDARY_START: case CRLF_DUPLICATE_BOUNDARY_START: {
						...
						if (c != duplicateBoundary[duplicateIndex]) {
							...
							saveLen = len;
							saveI = i;
							std::copy(duplicateBoundary.c_str(), duplicateBoundary.c_str() + duplicateIndex + 1,replayBuffer.begin());
							...
							headerFieldMark = i;
							index = 0;
						} else if (duplicateIndex + 1 < duplicateBoundary.size()) index++;
						else state = HEADER_START;
						break;
					}
					case HEADER_START:
						headerFieldMark = i;
					...
					case HEADERS_ALMOST_DONE:
						...
						break;
					case PART_DATA_START: state = PART_DATA; partDataMark = i;
					case PART_DATA: processPartData(prevIndex, index, buffer, len, boundaryEnd, i, c, state, flags); break;
					default: return i;
				}
				if (++i >= len) {
