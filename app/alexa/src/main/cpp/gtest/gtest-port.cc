...
namespace testing {
    namespace internal {
    ...
    #if GTEST_OS_LINUX
        namespace {
            template <typename T> T ReadProcFileField(const string& filename, int field) {
                std::string dummy;
        ...
        size_t GetThreadCount() {
            ...
            close(fd);
            if (status == EOK) return static_cast<size_t>(process_info.num_threads);
            else return 0;
        }
    #elif GTEST_OS_AIX
        size_t GetThreadCount() {
            ...
            if (status == 1) return entry.pi_thcount;
            else return 0;
        }
    #else
        size_t GetThreadCount() {
        ...
        void AutoHandle::Reset(HANDLE handle) {
            if (handle_ != handle) {
                if (IsCloseable()) ::CloseHandle(handle_);
                handle_ = handle;
            } else {
                GTEST_CHECK_(!IsCloseable()) << "Resetting a valid handle to itself is likely a programmer error and thus not allowed.";
            }
        }
        bool AutoHandle::IsCloseable() const {
            return handle_ != NULL && handle_ != INVALID_HANDLE_VALUE;
        }
        Notification::Notification() : event_(::CreateEvent(NULL, TRUE, FALSE, NULL)) {
            ...
            if (type_ == kStatic) {
                switch(::InterlockedCompareExchange(&critical_section_init_phase_, 1L, 0L)) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2: break;
                            default: GTEST_CHECK_(false) << "Unexpected value of critical_section_init_phase_ " << "while initializing a static mutex.";
                }
            }
        }
        namespace {
            ...
            public:
                static HANDLE CreateThread(Runnable* runnable, Notification* thread_can_start) {
                    ...
                    DWORD thread_id;
                    HANDLE thread_handle = ::CreateThread(NULL, 0, &ThreadWithParamSupport::ThreadMain, param, 0x0, &thread_id);
                    GTEST_CHECK_(thread_handle != NULL) << "CreateThread failed with error " << ::GetLastError() << ".";
                    if (thread_handle == NULL) delete param;
        ...
        ThreadWithParamBase::ThreadWithParamBase(Runnable *runnable, Notification* thread_can_start) : thread_(ThreadWithParamSupport::CreateThread(runnable,
                                                                                                               thread_can_start)) {}
        ...
        public:
            static ThreadLocalValueHolderBase* GetValueOnCurrentThread(const ThreadLocalBase* thread_local_instance) {
                ...
                if (value_pos == thread_local_values.end()) {
                    value_pos = thread_local_values.insert(std::make_pair(thread_local_instance, linked_ptr<ThreadLocalValueHolderBase>(
                                    thread_local_instance->NewValueForCurrentThread()))).first;
                }
        ...
        private:
            ...
            static void StartWatcherThreadFor(DWORD thread_id) {
                ...
                HANDLE watcher_thread = ::CreateThread(NULL, 0, &ThreadLocalRegistryImpl::WatcherThreadFunc, reinterpret_cast<LPVOID>(new ThreadIdAndHandle(thread_id,
                                                       thread)), CREATE_SUSPENDED, &watcher_thread_id);
        ...
        bool MatchRepetitionAndRegexAtHead(bool escaped, char c, char repeat, const char* regex, const char* str) {
            ...
            for (size_t i = 0; i <= max_count; ++i) {
                if (i >= min_count && MatchRegexAtHead(regex, str + i)) return true;
        ...
        bool MatchRegexAtHead(const char* regex, const char* str) {
            ...
            if (escaped) ++regex;
            if (IsRepeat(regex[1])) return MatchRepetitionAndRegexAtHead(escaped, regex[0], regex[1], regex + 2, str);
            else return (*str != '\0') && AtomMatchesChar(escaped, *regex, *str) && MatchRegexAtHead(regex + 1, str + 1);
        }
        ...
        GTEST_API_ ::std::string FormatCompilerIndependentFileLocation(const char* file, int line) {
            const std::string file_name(file == NULL ? kUnknownFile : file);
            if (line < 0) return file_name;
            else return file_name + ":" + StreamableToString(line);
        }
        GTestLog::GTestLog(GTestLogSeverity severity, const char* file, int line) : severity_(severity) {
            const char* const marker = severity == GTEST_INFO ?    "[  INFO ]" : severity == GTEST_WARNING ? "[WARNING]" :
        ...
        std::string TempDir() {
            ...
            if (temp_dir == NULL || temp_dir[0] == '\0') return "\\temp\\";
            else if (temp_dir[strlen(temp_dir) - 1] == '\\') return temp_dir;
            else return std::string(temp_dir) + "\\";
        ...
        static const ::std::vector<testing::internal::string>* g_injected_test_argvs = NULL;
        void SetInjectableArgvs(const ::std::vector<testing::internal::string>* argvs) {
            if (g_injected_test_argvs != argvs) delete g_injected_test_argvs;
            g_injected_test_argvs = argvs;
        }
        const ::std::vector<testing::internal::string>& GetInjectableArgvs() {
            if (g_injected_test_argvs != NULL) return *g_injected_test_argvs;
            return GetArgvs();
        }
    #endif
    #if GTEST_OS_WINDOWS_MOBILE
        ...
        bool ParseInt32(const Message& src_text, const char* str, Int32* value) {
            ...
            if (long_value == LONG_MAX || long_value == LONG_MIN || result != long_value) {
                Message msg;
        ...
        Int32 Int32FromGTestEnv(const char* flag, Int32 default_value) {
            ...
            const std::string env_var = FlagToEnvVar(flag);
            const char* const string_value = posix::GetEnv(env_var.c_str());
            if (string_value == NULL) return default_value;
        ...
        std::string StringFromGTestEnv(const char* flag, const char* default_value) {
            ...
            if (strcmp(flag, "output") == 0) {
                ...
                if (value != NULL) return std::string("xml:") + value;
            }
            return default_value;
        }
    }
