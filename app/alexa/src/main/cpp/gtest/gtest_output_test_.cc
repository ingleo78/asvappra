...
public:
  ...
  static void AddFailure(FailureMode failure) {
    if (failure == FATAL_FAILURE) { FAIL() << "Expected fatal failure."; }
    else { ADD_FAILURE() << "Expected non-fatal failure."; }
  }
};
TEST_F(ExpectFailureTest, ExpectFatalFailure) {
  ...
  printf("(expecting 1 failure)\n");
  EXPECT_FATAL_FAILURE(AddFailure(NONFATAL_FAILURE), "Expected non-fatal failure.");
  printf("(expecting 1 failure)\n");
  EXPECT_FATAL_FAILURE(AddFailure(FATAL_FAILURE), "Some other fatal failure expected.");
}
TEST_F(ExpectFailureTest, ExpectNonFatalFailure) {
  ...
  printf("(expecting 1 failure)\n");
  EXPECT_NONFATAL_FAILURE(AddFailure(NONFATAL_FAILURE), "Some other non-fatal failure.");
}
#if GTEST_IS_THREADSAFE
...
int main(int argc, char **argv) {
  ...
  bool internal_skip_environment_and_ad_hoc_tests = std::count(argv, argv + argc,std::string("internal_skip_environment_and_ad_hoc_tests")) > 0;
#if GTEST_HAS_DEATH_TEST
