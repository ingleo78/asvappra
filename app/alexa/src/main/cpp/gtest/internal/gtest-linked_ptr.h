    ...
    namespace internal {
        ...
        public:
            ...
            void join(linked_ptr_internal const* ptr) GTEST_LOCK_EXCLUDED_(g_linked_ptr_mutex) {
                ...
                while(p->next_ != ptr) {
                    assert(p->next_ != this && "Trying to join() a linked ring we are already in. Is GMock thread safety enabled?");
                    p = p->next_;
                }
                p->next_ = this;
                next_ = ptr;
            ...
            bool depart() GTEST_LOCK_EXCLUDED_(g_linked_ptr_mutex) {
                ...
                while(p->next_ != this) {
                    assert(p->next_ != next_ && "Trying to depart() a linked ring we are not in. Is GMock thread safety enabled?");
                    p = p->next_;
                }
                p->next_ = next_;
                return false;
        ...
        private:
            ...
            template <typename U> void copy(linked_ptr<U> const* ptr) {
                value_ = ptr->get();
                if (value_) link_.join(&ptr->link_);
                else link_.join_new();
            }
        };
