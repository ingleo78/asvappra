...
namespace {
    ...
    TEST(OnCallSyntaxTest, WithCanAppearAtMostOnce) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ ON_CALL(a, ReturnResult(_)).With(_).With(_).WillByDefault(Return(Result())); },
                                ".With() cannot appear more than once in an ON_CALL()");
    }
    TEST(OnCallSyntaxTest, WillByDefaultIsMandatory) {
        MockA a;
        EXPECT_DEATH_IF_SUPPORTED({ ON_CALL(a, DoA(5)); a.DoA(5); }, "");
    }
    TEST(OnCallSyntaxTest, WillByDefaultCanAppearAtMostOnce) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ ON_CALL(a, DoA(5)) .WillByDefault(Return()) .WillByDefault(Return()); },
    ...
    TEST(ExpectCallSyntaxTest, WithCanAppearAtMostOnce) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(6)).With(_).With(_); }, ".With() cannot appear more than once in an EXPECT_CALL()");
        a.DoA(6);
    }
    TEST(ExpectCallSyntaxTest, WithMustBeFirstClause) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).Times(1).With(_); }, ".With() must be the first clause in an EXPECT_CALL()");
        a.DoA(1);
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(2)).WillOnce(Return()).With(_); }, ".With() must be the first clause in an EXPECT_CALL()");
        a.DoA(2);
    }
    ...
    TEST(ExpectCallSyntaxTest, TimesCanAppearAtMostOnce) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).Times(1).Times(2); }, ".Times() cannot appear more than once in an EXPECT_CALL()");
        a.DoA(1);
        a.DoA(1);
    }
    TEST(ExpectCallSyntaxTest, TimesMustBeBeforeInSequence) {
        MockA a;
        Sequence s;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).InSequence(s).Times(1); }, ".Times() cannot appear after ");
        a.DoA(1);
    }
    ...
    TEST(ExpectCallSyntaxTest, InSequenceMustBeBeforeAfter) {
        ...
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(2)).After(e).InSequence(s); }, ".InSequence() cannot appear after ");
        a.DoA(2);
    }
    TEST(ExpectCallSyntaxTest, InSequenceMustBeBeforeWillOnce) {
        MockA a;
        Sequence s;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).WillOnce(Return()).InSequence(s); }, ".InSequence() cannot appear after ");
        a.DoA(1);
    }
    ...
    TEST(ExpectCallSyntaxTest, WillMustBeBeforeWillRepeatedly) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).WillRepeatedly(Return()).WillOnce(Return()); }, ".WillOnce() cannot appear after ");
        a.DoA(1);
    }
    ...
    TEST(ExpectCallSyntaxTest, WillRepeatedlyCannotAppearMultipleTimes) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).WillRepeatedly(Return()).WillRepeatedly(Return()); }, ".WillRepeatedly() "
                                "cannot appear more than once in an EXPECT_CALL()");
    }
    TEST(ExpectCallSyntaxTest, WillRepeatedlyMustBeBeforeRetiresOnSaturation) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).RetiresOnSaturation().WillRepeatedly(Return()); }, ".WillRepeatedly() "
                                "cannot appear after ");
    }
    ...
    TEST(ExpectCallSyntaxTest, RetiresOnSaturationCannotAppearMultipleTimes) {
        MockA a;
        EXPECT_NONFATAL_FAILURE({ EXPECT_CALL(a, DoA(1)).RetiresOnSaturation().RetiresOnSaturation(); }, ".RetiresOnSaturation() "
    ...
    TEST(ExpectCallSyntaxTest, DefaultCardinalityIsOnce) {
        ...
        EXPECT_NONFATAL_FAILURE({ MockA a; EXPECT_CALL(a, DoA(1)); }, "to be called once");
        EXPECT_NONFATAL_FAILURE({ MockA a; EXPECT_CALL(a, DoA(1)); a.DoA(1); a.DoA(1); }, "to be called once");
    }
#if GTEST_HAS_STREAM_REDIRECTION
    ...
    TEST(ExpectCallTest, CatchesTooFewCalls) {
        EXPECT_NONFATAL_FAILURE({ MockB b;EXPECT_CALL(b, DoB(5)).Times(AtLeast(2)); b.DoB(5); }, "Actual function call count doesn't "
                                "match EXPECT_CALL(b, DoB(5))...\n         Expected: to be called at least twice\n           Actual: called once - "
    ...
    TEST(ExpectCallTest, InfersCardinalityWhenThereIsNoWillRepeatedly) {
        ...
        EXPECT_NONFATAL_FAILURE({ MockB b; EXPECT_CALL(b, DoB()).WillOnce(Return(1)).WillOnce(Return(2)); EXPECT_EQ(1, b.DoB()); },
                         "to be called twice");
        {
            MockB b;
            EXPECT_CALL(b, DoB()).WillOnce(Return(1)).WillOnce(Return(2));
    ...
    TEST(ExpectCallTest, InfersCardinality1WhenThereIsWillRepeatedly) {
        ...
        MockB b;
        EXPECT_CALL(b, DoB()).WillOnce(Return(1)).WillRepeatedly(Return(2)); }, "to be called at least once");
    }
    ...
    TEST(UnexpectedCallTest, GeneratesFailureForVoidFunction) {
        ...
        EXPECT_NONFATAL_FAILURE(a1.DoA(9),"Unexpected mock function call - returning directly.\n    Function call: DoA(9)\nGoogle Mock tried "
                                "the following 1 expectation, but it didn't match:");
    ...
    TEST(InSequenceTest, NestedInSequence) {
        ...
        EXPECT_NONFATAL_FAILURE({ a.DoA(1); a.DoA(3); }, "Unexpected mock function call");
        a.DoA(2);
        a.DoA(3);
    }
    ...
    public:
        MockC() {}
        MOCK_METHOD6(VoidMethod, void(bool cond, int n, string s, void* p, const Printable& x, Unprintable y));
    ...
    TEST(FunctionCallMessageTest, UninterestingCallOnNaggyMockPrintsArgumentsAndReturnValue) {
        ...
        EXPECT_THAT(output2.c_str(),ContainsRegex("Uninteresting mock function call - returning directly\\.\n    Function call: VoidMethod"
                    "\\(false, 5, \"Hi\", NULL, @.+ Printable, 4-byte object <00-00 00-00>\\)"));
    }
    class GMockVerboseFlagTest : public VerboseFlagPreservingFixture {
    public:
        void VerifyOutput(const std::string& output, bool should_print, const string& expected_substring, const string& function_name) {
            if (should_print) {
            ...
            #endif
            } else { EXPECT_STREQ("", output.c_str()); }
        }
        ...
        void TestUninterestingCallOnNaggyMock(bool should_print) {
            NaggyMock<MockA> a;
            const string note = "NOTE: You can safely ignore the above warning unless this call should not happen.  Do not suppress it by blindly adding "
                                "an EXPECT_CALL() if you don't mean to enforce the call.  See https://github.com/google/googletest/blob/master/googlemock/docs/"
                                "CookBook.md#knowing-when-to-expect for details.";
            CaptureStdout();
            a.DoA(5);
