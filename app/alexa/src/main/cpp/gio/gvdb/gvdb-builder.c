...
static void gvdb_item_free(gpointer data) {
  GvdbItem *item = data;
  g_free(item->key);
  if (item->value) g_variant_unref(item->value);
  if (item->table) g_hash_table_unref(item->table);
  g_slice_free(GvdbItem, item);
}
GHashTable *gvdb_hash_table_new(GHashTable *parent, const gchar *name_in_parent) {
  GHashTable *table;
  table = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, gvdb_item_free);
  if (parent) {
      GvdbItem *item;
...
static guint32 djb_hash(const gchar *key) {
  ...
  while(*key) hash_value = hash_value * 33 + *key++;
  return hash_value;
}
...
void gvdb_hash_table_insert_string(GHashTable  *table, const gchar *key, const gchar *value) {
  GvdbItem *item;
...
void gvdb_item_set_parent(GvdbItem *item, GvdbItem *parent) {
  ...
  for (node = &parent->child; *node; node = &(*node)->sibling)
    if (strcmp((*node)->key, item->key) > 0) break;
  item->parent = parent;
  item->sibling = *node;
  *node = item;
}
...
static guint32_le item_to_index(GvdbItem *item) {
  if (item != NULL) return item->assigned_index;
...
static gpointer file_builder_allocate(FileBuilder *fb, guint alignment, gsize size, struct gvdb_pointer *pointer) {
  FileChunk *chunk;
...
static void file_builder_add_value(FileBuilder *fb, GVariant *value, struct gvdb_pointer *pointer) {
  ...
  } else variant = g_variant_new_variant(value);
...
static void file_builder_add_string(FileBuilder *fb, const gchar *string, guint32_le *start, guint16_le *size) {
  FileChunk *chunk;
  gsize length;
...
static void file_builder_allocate_for_hash(FileBuilder *fb, gsize n_buckets, gsize n_items, guint bloom_shift, gsize n_bloom_words, guint32_le **bloom_filter,
                                           guint32_le **hash_buckets, struct gvdb_hash_item **hash_items, struct gvdb_pointer *pointer) {
  ...
  size = sizeof bloom_hdr + sizeof table_hdr + n_bloom_words * sizeof(guint32_le) + n_buckets * sizeof(guint32_le) + n_items * sizeof(struct gvdb_hash_item);
  ...
  *hash_items = (struct gvdb_hash_item*)chunk(n_items * sizeof(struct gvdb_hash_item));
  g_assert(size == 0);
  #undef chunk
  memset(*bloom_filter, 0, n_bloom_words * sizeof(guint32_le));
}
static void file_builder_add_hash(FileBuilder *fb, GHashTable *table, struct gvdb_pointer *pointer) {
  ...
  file_builder_allocate_for_hash(fb, mytable->n_buckets, index, 5, 0, &bloom_filter, &buckets, &items, pointer);
  index = 0;
  for (bucket = 0; bucket < mytable->n_buckets; bucket++) {
      ...
      for (item = mytable->buckets[bucket]; item; item = item->next) {
          ...
          entry->unused = 0;
          if (item->parent != NULL) basename = item->key + strlen(item->parent->key);
          else basename = item->key;
          file_builder_add_string(fb, basename, &entry->key_start, &entry->key_size);
          if (item->value != NULL) {
          ...
          if (item->child != NULL) {
              ...
              for (child = item->child; child; child = child->sibling) children++;
              offsets = file_builder_allocate (fb, 4, 4 * children, &entry->value.pointer);
              entry->type = 'L';
              for (child = item->child; child; child = child->sibling) offsets[i++] = child->assigned_index;
              g_assert(children == i);
          }
          if (item->table != NULL) {
              entry->type = 'H';
...
static GString *file_builder_serialise(FileBuilder *fb, struct gvdb_pointer root) {
...
gboolean gvdb_table_write_contents(GHashTable *table, const gchar *filename, gboolean byteswap, GError **error) {
