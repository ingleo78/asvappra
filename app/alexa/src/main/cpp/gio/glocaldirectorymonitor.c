...
static void g_local_directory_monitor_finalize(GObject *object) {
...
static void g_local_directory_monitor_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec) {
  switch (property_id) {
      case PROP_DIRNAME: case PROP_FLAGS: break;
      default: G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec); break;
  }
}
static GObject *g_local_directory_monitor_constructor(GType type, guint n_construct_properties, GObjectConstructParam *construct_properties) {
  GObject *obj;
  ...
  if (!klass->mount_notify && (flags & G_FILE_MONITOR_WATCH_MOUNTS)) {
  #ifndef G_OS_WIN32
      ...
      if (mount) g_unix_mount_free(mount);
      local_monitor->mount_monitor = g_unix_mount_monitor_new();
      g_signal_connect_object(local_monitor->mount_monitor, "mounts-changed", G_CALLBACK(mounts_changed), local_monitor, 0);
  #endif
  }
  return obj;
}
static void g_local_directory_monitor_class_init(GLocalDirectoryMonitorClass* klass) {
  ...
  g_object_class_install_property(gobject_class,PROP_DIRNAME,g_param_spec_string ("dirname", P_("Directory name"),
                                  P_("Directory to monitor"),NULL,G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME |
...
static void g_local_directory_monitor_init(GLocalDirectoryMonitor *local_monitor) {}
static void mounts_changed(GUnixMountMonitor *mount_monitor, gpointer user_data) {
...
static gpointer get_default_local_directory_monitor(gpointer data) {
...
GFileMonitor* _g_local_directory_monitor_new(const char *dirname, GFileMonitorFlags flags, GError **error) {
  ...
  monitor = NULL;
  if (type != G_TYPE_INVALID) monitor = G_FILE_MONITOR(g_object_new(type, "dirname", dirname, "flags", flags, NULL));
  else g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unable to find default local directory monitor type"));
