...
#endif
    if (getrlimit(RLIMIT_NOFILE, &rl) > 0) maxfd = (int) rl.rlim_max;
    else maxfd = sysconf(_SC_OPEN_MAX);
...
static int reset_sigsv(const int except[]) {
    ...
    for (sig = 1; sig < NSIG; sig++) {
        int reset = 1;
        switch (sig) {
            case SIGKILL: case SIGSTOP: reset = 0; break;
            default: {
                int i;
...
static int ignore_sigsv(const int ignore[]) {
    ...
    for (i = 0; ignore[i] > 0; i++) {
        ...
        if ((sigaction(ignore[i], &sa, NULL) < 0)) return -1;
    }
    return 0;
}
#endif
static int fd_nonblock(int fd) {
    int i;
    assert(fd >= 0);
    if ((i = fcntl(fd, F_GETFL, 0)) < 0) return -1;
    if (i & O_NONBLOCK) return 0;
    return fcntl(fd, F_SETFL, i | O_NONBLOCK);
}
static int fd_cloexec(int fd) {
    int v;
    assert(fd >= 0);
    if ((v = fcntl(fd, F_GETFD, 0)) < 0) return -1;
    if (v & FD_CLOEXEC) return 0;
...
static void *serialize_addrinfo(void *p, const struct addrinfo *ai, size_t *length, size_t maxlength) {
    ...
    if (*length + l > maxlength) return NULL;
    ...
    if (ai->ai_canonname) strcpy((char*) p + sizeof(addrinfo_serialization_t) + ai->ai_addrlen, ai->ai_canonname);
    *length += l;
    return (uint8_t*) p + l;
}
...
static int send_nameinfo_reply(int out_fd, unsigned id, int ret, const char *host, const char *serv, int _errno, int _h_errno) {
    ...
    if (host) memcpy((uint8_t*)data + sizeof(nameinfo_response_t), host, hl);
    if (serv) memcpy((uint8_t*)data + sizeof(nameinfo_response_t) + hl, serv, sl);
    return send(out_fd, resp, resp->header.length, MSG_NOSIGNAL);
}
static int send_res_reply(int out_fd, unsigned id, const unsigned char *answer, int ret, int _errno, int _h_errno) {
    ...
    resp->_h_errno = _h_errno;
    assert(sizeof(data) >= resp->header.length);
    if (ret > 0) memcpy((uint8_t*)data + sizeof(res_response_t), answer, ret);
    return send(out_fd, resp, resp->header.length, MSG_NOSIGNAL);
}
static int handle_request(int out_fd, const packet_t *packet, size_t length) {
    ...
    switch (req->type) {
        case REQUEST_ADDRINFO: {
                ...
                ret = getaddrinfo(node, service,ai_req->hints_is_null ? NULL : &ai, &result);
        ...
        case REQUEST_NAMEINFO: {
                ...
                ret = getnameinfo((struct sockaddr *)&sa, ni_req->sockaddr_len,ni_req->gethost ? hostbuf : NULL, ni_req->gethost ? sizeof(hostbuf) : 0,
                                  ni_req->getserv ? servbuf : NULL, ni_req->getserv ? sizeof(servbuf) : 0, ni_req->flags);
                return send_nameinfo_reply(out_fd, req->id, ret,ret == 0 && ni_req->gethost ? hostbuf : NULL,ret == 0 && ni_req->getserv ? servbuf : NULL,
                                           errno, h_errno);
            }
        ...
        case REQUEST_RES_SEARCH: {
                ...
                if (req->type == REQUEST_RES_QUERY) ret = res_query(dname, res_req->class, res_req->type, (unsigned char*)answer, BUFSIZE);
                else ret = res_search(dname, res_req->class, res_req->type, (unsigned char*)answer, BUFSIZE);
                return send_res_reply(out_fd, req->id, (unsigned char*)answer, ret, errno, h_errno);
            }
        case REQUEST_TERMINATE: return -1;
    }
    return 0;
}
#ifndef HAVE_PTHREAD
static int process_worker(int in_fd, int out_fd) {
    ...
    close(1);
    close(2);
    if (open("/dev/null", O_RDONLY) != 0) goto fail;
    if (open("/dev/null", O_WRONLY) != 1) goto fail;
    if (open("/dev/null", O_WRONLY) != 2) goto fail;
    ...
    if (geteuid() == 0) {
        ...
        #else
            if ((r = setuid(pw->pw_uid)) >= 0) r = seteuid(pw->pw_uid);
        #endif
            if (r < 0) goto fail;
        }
    }
    if (reset_sigsv(ignore_sigs) < 0) goto fail;
    if (ignore_sigsv(ignore_sigs) < 0) goto fail;
    good_fds[0] = in_fd; good_fds[1] = out_fd; good_fds[2] = -1;
    if (close_allv(good_fds) < 0) goto fail;
#ifdef PR_SET_PDEATHSIG
    if (prctl(PR_SET_PDEATHSIG, SIGTERM) >= 0) have_death_sig = 1;
#endif
    if (!have_death_sig) fd_nonblock(in_fd);
    while (getppid() > 1) {
        ...
        if (!have_death_sig) {
            ...
            FD_ZERO(&fds);
            FD_SET(in_fd, &fds);
            if (select(in_fd+1, &fds, NULL, NULL, &tv) < 0) break;
            if (getppid() == 1) break;
        }
        ...
        if (handle_request(out_fd, buf, (size_t) length) < 0) break;
    }
    ret = 0;
fail:
    send_died(out_fd);
...
static void* thread_worker(void *p) {
    ...
    while(!asyncns->dead) {
        ...
        if (asyncns->dead) break;
        if (handle_request(asyncns->fds[RESPONSE_SEND_FD], buf, (size_t) length) < 0) break;
    }
...
_g_asyncns_t* _g_asyncns_new(unsigned n_proc) {
    ...
    for (i = 0; i < MESSAGE_FD_MAX; i++) asyncns->fds[i] = -1;
    memset(asyncns->queries, 0, sizeof(asyncns->queries));
    if (socketpair(PF_UNIX, SOCK_DGRAM, 0, asyncns->fds) < 0 || socketpair(PF_UNIX, SOCK_DGRAM, 0, asyncns->fds+2) < 0) goto fail;
    for (i = 0; i < MESSAGE_FD_MAX; i++) fd_cloexec(asyncns->fds[i]);
    for (asyncns->valid_workers = 0; asyncns->valid_workers < n_proc; asyncns->valid_workers++) {
    #ifndef HAVE_PTHREAD
        if ((asyncns->workers[asyncns->valid_workers] = fork()) < 0) goto fail;
    ...
    return asyncns;
fail:
    if (asyncns) _g_asyncns_free(asyncns);
    return NULL;
}
void _g_asyncns_free(_g_asyncns_t *asyncns) {
    ...
    if (asyncns->fds[REQUEST_SEND_FD] >= 0) {
        ...
        req.id = 0;
        for (p = 0; p < asyncns->valid_workers; p++) send(asyncns->fds[REQUEST_SEND_FD], &req, req.length, MSG_NOSIGNAL);
    }
    for (p = 0; p < asyncns->valid_workers; p++) {
    #ifndef HAVE_PTHREAD
        kill(asyncns->workers[p], SIGTERM);
        for (;;) {
            if (waitpid(asyncns->workers[p], NULL, 0) >= 0 || errno != EINTR) break;
        }
    #else
        for (;;) {
            if (pthread_join(asyncns->workers[p], NULL) != EINTR) break;
        }
    #endif
    }
    for (i = 0; i < MESSAGE_FD_MAX; i++)
        if (asyncns->fds[i] >= 0) close(asyncns->fds[i]);
    for (p = 0; p < MAX_QUERIES; p++)
        if (asyncns->queries[p]) _g_asyncns_cancel(asyncns, asyncns->queries[p]);
    free(asyncns);
    errno = saved_errno;
}
...
static void complete_query(_g_asyncns_t *asyncns, _g_asyncns_query_t *q) {
    ...
    assert(!q->done);
    q->done = 1;
    if ((q->done_prev = asyncns->done_tail)) asyncns->done_tail->done_next = q;
    else asyncns->done_head = q;
    asyncns->done_tail = q;
    q->done_next = NULL;
}
static const void *unserialize_addrinfo(const void *p, struct addrinfo **ret_ai, size_t *length) {
    ...
    assert(ret_ai);
    assert(length);
    if (*length < sizeof(addrinfo_serialization_t)) return NULL;
    memcpy(&s, p, sizeof(s));
    l = sizeof(addrinfo_serialization_t) + s.ai_addrlen + s.canonname_len;
    if (*length < l) return NULL;
    if (!(ai = malloc(sizeof(struct addrinfo)))) goto fail;
    ai->ai_addr = NULL;
    ai->ai_canonname = NULL;
    ai->ai_next = NULL;
    if (s.ai_addrlen && !(ai->ai_addr = malloc(s.ai_addrlen))) goto fail;
    if (s.canonname_len && !(ai->ai_canonname = malloc(s.canonname_len))) goto fail;
    ai->ai_flags = s.ai_flags;
    ai->ai_family = s.ai_family;
    ai->ai_socktype = s.ai_socktype;
    ai->ai_protocol = s.ai_protocol;
    ai->ai_addrlen = s.ai_addrlen;
    if (ai->ai_addr) memcpy(ai->ai_addr, (const uint8_t*) p + sizeof(addrinfo_serialization_t), s.ai_addrlen);
    if (ai->ai_canonname) memcpy(ai->ai_canonname, (const uint8_t*) p + sizeof(addrinfo_serialization_t) + s.ai_addrlen, s.canonname_len);
    *length -= l;
    *ret_ai = ai;
    return (const uint8_t*) p + l;
fail:
    if (ai) _g_asyncns_freeaddrinfo(ai);
    return NULL;
}
static int handle_response(_g_asyncns_t *asyncns, const packet_t *packet, size_t length) {
    ...
    switch (resp->type) {
        ...
        case RESPONSE_NAMEINFO: {
                ...
                if (ni_resp->hostlen)
                    if (!(q->host = strndup((const char*) ni_resp + sizeof(nameinfo_response_t), ni_resp->hostlen-1))) q->ret = EAI_MEMORY;
                if (ni_resp->servlen)
                    if (!(q->serv = strndup((const char*) ni_resp + sizeof(nameinfo_response_t) + ni_resp->hostlen, ni_resp->servlen-1))) q->ret = EAI_MEMORY;
                complete_query(asyncns, q);
                break;
            }
        case RESPONSE_RES: {
                ...
                if (res_resp->ret >= 0)  {
                    if (!(q->serv = malloc(res_resp->ret))) {
                        q->ret = -1;
                        q->_errno = ENOMEM;
                    } else memcpy(q->serv, (const char *)resp + sizeof(res_response_t), res_resp->ret);
                }
                complete_query(asyncns, q);
                break;
            }
...
int _g_asyncns_wait(_g_asyncns_t *asyncns, int block) {
    ...
    for (;;) {
        ...
        if (((l = recv(asyncns->fds[RESPONSE_RECV_FD], buf, sizeof(buf), 0)) < 0)) {
            ...
            if (select(asyncns->fds[RESPONSE_RECV_FD]+1, &fds, NULL, NULL, NULL) < 0) return -1;
            continue;
        }
        if (handle_response(asyncns, buf, (size_t) l) < 0) return -1;
        handled = 1;
    }
}
static _g_asyncns_query_t *alloc_query(_g_asyncns_t *asyncns) {
    ...
    while (asyncns->queries[asyncns->current_index]) {
        ...
        asyncns->current_id++;
        while (asyncns->current_index >= MAX_QUERIES) asyncns->current_index -= MAX_QUERIES;
    }
...
_g_asyncns_query_t* _g_asyncns_getaddrinfo(_g_asyncns_t *asyncns, const char *node, const char *service, const struct addrinfo *hints) {
    ...
    if (!(q = alloc_query(asyncns))) return NULL;
    ...
    if (node) strcpy((char*) req + sizeof(addrinfo_request_t), node);
    if (service) strcpy((char*) req + sizeof(addrinfo_request_t) + req->node_len, service);
    if (send(asyncns->fds[REQUEST_SEND_FD], req, req->header.length, MSG_NOSIGNAL) < 0) goto fail;
    return q;
fail:
    if (q) _g_asyncns_cancel(asyncns, q);
    return NULL;
}
int _g_asyncns_getaddrinfo_done(_g_asyncns_t *asyncns, _g_asyncns_query_t* q, struct addrinfo **ret_res) {
    ...
    if (!q->done) return EAI_AGAIN;
    *ret_res = q->addrinfo;
    q->addrinfo = NULL;
    ret = q->ret;
    if (ret == EAI_SYSTEM) errno = q->_errno;
    if (ret != 0) h_errno = q->_h_errno;
...
void _g_asyncns_cancel(_g_asyncns_t *asyncns, _g_asyncns_query_t* q) {
    ...
    if (q->done) {
        if (q->done_prev) q->done_prev->done_next = q->done_next;
        else asyncns->done_head = q->done_next;
        if (q->done_next) q->done_next->done_prev = q->done_prev;
