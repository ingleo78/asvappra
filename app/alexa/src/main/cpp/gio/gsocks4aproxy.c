...
G_DEFINE_TYPE_WITH_CODE(GSocks4aProxy, g_socks4a_proxy, G_TYPE_OBJECT,G_IMPLEMENT_INTERFACE (G_TYPE_PROXY, g_socks4a_proxy_iface_init)
...
static gint set_connect_msg(guint8 *msg, const gchar *hostname, guint16 port, const char *username, GError **error) {
  GInetAddress *addr;
  guint len = 0;
  gsize addr_len;
  gboolean is_ip;
  ...
  if (is_ip) ip = hostname;
  else ip = "0.0.0.1";
  addr = g_inet_address_new_from_string(ip);
  addr_len = g_inet_address_get_native_size(addr);
  if (addr_len != 4) {
      g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("SOCKSv4 does not support IPv6 address '%s'"), ip);
      g_object_unref(addr);
      return -1;
  }
  ...
  if (username) {
      ...
      if (user_len > SOCKS4_MAX_LEN) {
          g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("SOCKSv4 implementation limits username to %i characters"), SOCKS4_MAX_LEN);
          return -1;
	  }
  ...
  if (!is_ip) {
      ...
      if (host_len > SOCKS4_MAX_LEN) {
          g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("SOCKSv4a implementation limits hostname to %i characters"), SOCKS4_MAX_LEN);
          return -1;
	  }
...
static gboolean parse_connect_reply(const guint8 *data, GError **error) {
  if (data[0] != SOCKS4_REP_VERSION) {
      g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("The server is not a SOCKSv4 proxy server."));
      return FALSE;
  }
  if (data[1] != SOCKS4_REP_GRANTED) {
      g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("Connection through SOCKSv4 server was rejected"));
      return FALSE;
  }
  return TRUE;
}
static GIOStream *g_socks4a_proxy_connect(GProxy *proxy, GIOStream *io_stream, GProxyAddress *proxy_address, GCancellable *cancellable, GError **error) {
  GInputStream *in;
  GOutputStream *out;
  ...
  }
  {
      ...
      if (!g_input_stream_read_all(in, data, SOCKS4_CONN_REP_LEN, NULL, cancellable, error)) goto error;
      if (!parse_connect_reply(data, error)) goto error;
  }
...
static void free_connect_data(ConnectAsyncData *data) {
  ...
  if (data->proxy_address) g_object_unref(data->proxy_address);
...
static void g_socks4a_proxy_connect_async(GProxy *proxy, GIOStream *io_stream, GProxyAddress *proxy_address, GCancellable *cancellable, GAsyncReadyCallback callback,
			                              gpointer user_data) {
  GError *error = NULL;
  GSimpleAsyncResult *simple;
  ...
  guint16 port;
  const gchar *username;
  simple = g_simple_async_result_new(G_OBJECT(proxy), callback, user_data, g_socks4a_proxy_connect_async);
  ...
  data->length = set_connect_msg(data->buffer, hostname, port, username, &error);
  data->offset = 0;
  if (data->length < 0) {
      g_simple_async_result_take_error(data->simple, error);
      g_simple_async_result_set_op_res_gpointer(simple, NULL, NULL);
...
static void connect_reply_read_cb(GObject *source, GAsyncResult *result, gpointer user_data) {
  ...
  data->offset += read;
  if (data->offset == data->length) {
      if (!parse_connect_reply(data->buffer, &error)) complete_async_from_error(data, error);
      else {
          GSimpleAsyncResult *simple = data->simple;
          g_simple_async_result_complete(simple);
          g_object_unref(simple);
...
static GIOStream *g_socks4a_proxy_connect_finish(GProxy *proxy, GAsyncResult *result, GError **error);
static GIOStream *g_socks4a_proxy_connect_finish(GProxy *proxy, GAsyncResult *result, GError **error) {
