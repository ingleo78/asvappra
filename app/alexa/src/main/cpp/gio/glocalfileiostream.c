...
GFileIOStream *_g_local_file_io_stream_new(GLocalFileOutputStream *output_stream) {
  ...
  _g_local_file_input_stream_set_do_close(G_LOCAL_FILE_INPUT_STREAM(stream->input_stream), FALSE);
...
static GOutputStream *g_local_file_io_stream_get_output_stream(GIOStream *stream) {
  return G_LOCAL_FILE_IO_STREAM(stream)->output_stream;
}
static gboolean g_local_file_io_stream_close(GIOStream *stream, GCancellable *cancellable, GError **error) {
  ...
  return _g_local_file_output_stream_really_close(G_LOCAL_FILE_OUTPUT_STREAM(file->output_stream), cancellable, error);
}
