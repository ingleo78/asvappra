...
static void on_name_owner_notify(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
int main(int argc, char *argv[]) {
  ...
  g_option_context_set_summary(opt_context,"Example: to watch the object of gdbus-example-server, use:\n\n  ./gdbus-example-watch-proxy -n org.gtk.GDBus."
  ...
  if (opt_no_properties) flags |= G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES;
  if (opt_no_auto_start) flags |= G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START;
  loop = g_main_loop_new(NULL, FALSE);
  error = NULL;
  proxy = g_dbus_proxy_new_for_bus_sync(opt_system_bus ? G_BUS_TYPE_SYSTEM : G_BUS_TYPE_SESSION, flags, NULL, opt_name, opt_object_path, opt_interface, NULL,
                                        &error);
  if (proxy == NULL) {
  ...
  g_signal_connect(proxy,"notify::g-name-owner",G_CALLBACK (on_name_owner_notify), NULL);
  print_proxy(proxy);
