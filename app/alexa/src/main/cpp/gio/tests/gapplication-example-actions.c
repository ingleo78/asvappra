...
static void activate_action(GAction *action, GVariant *parameter, gpointer data) {
  g_print("action %s activated\n", g_action_get_name (action));
}
static void activate_toggle_action(GAction *action, GVariant *parameter, gpointer data) {
  GVariant *state;
  gboolean b;
  g_print("action %s activated\n", g_action_get_name(action));
...
static void add_actions(GApplication *app) {
  ...
  action = g_simple_action_new_stateful("toggle-action", NULL, g_variant_new_boolean(FALSE));
