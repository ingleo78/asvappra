...
static void do_mount_tests(GDrive *drive, GVolume *volume, GMount *mount) {
  GDrive *d;
  GVolume *v;
  gchar *name;
  gchar *uuid;
...
static void do_volume_tests(GDrive *drive, GVolume *volume) {
  GDrive *d;
  gchar *name;
  GMount *mount;
  gchar *uuid;
...
static void do_drive_tests(GDrive *drive) {
  GList *volumes, *l;
  gchar *name;
...
static void test_connected_drives(void) {
  GList *drives;
  GList *l;
...
static void test_volumes(void) {
  GList *volumes, *l;
  volumes = g_volume_monitor_get_volume(monitor);
  for (l = volumes; l; l = l->next) {
      GVolume *volume = l->data;
      GDrive *drive;
