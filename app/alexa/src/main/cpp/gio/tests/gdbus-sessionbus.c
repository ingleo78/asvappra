...
static void watch_parent(gint fd) {
  ...
  again:
      ...
      if (bytes_read < 0 && (errno == EAGAIN || errno == EINTR)) goto again;
      if (sscanf(buf, "add %d\n", &pid) == 1) { g_array_append_val(buses_to_kill_array, pid); }
          ...
          if (pid != 0) g_warning("unknown pid %d to remove", pid);
      } else g_warning("unknown command from parent '%s'", buf);
  }
  while(TRUE);
}
...
const gchar *session_bus_up_with_address(const gchar *given_address) {
  ...
  config_file_fd = -1;
  argv[2] = NULL;
  config_file_fd = g_file_open_tmp("g-dbus-tests-XXXXXX", &config_file_name, &error);
  if (config_file_fd < 0) {
  ...
  g_string_append(config_file_contents,"  <policy context=\"default\">\n    <!-- Allow everything to be sent -->\n    <allow send_destination=\"*\" "
  ...
  if (session_bus_address_to_pid == NULL) {
      ...
      switch(fork()) {
          ...
          case 0:
              ...
              break;
          default: close(pipe_fds[0]); break;
      }
  } else {
  ...
  if (!g_spawn_async_with_pipes(NULL, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, &pid, NULL, &stdout_fd, NULL, &error)) {
...
out:
  if (config_file_fd > 0) {
      if (close(config_file_fd) != 0) g_warning("Error closing fd for config file %s: %m", config_file_name);
      g_assert(config_file_name != NULL);
      if (unlink(config_file_name) != 0) g_warning("Error unlinking config file %s: %m", config_file_name);
  }
  g_free(argv[2]);
...
const gchar *session_bus_get_temporary_address(void) {
  if (temporary_address == NULL) temporary_address = g_strdup_printf ("unix:path=/tmp/g-dbus-tests-pid-%d", getpid ());
...
void session_bus_down(void) {
  if (temporary_address_used_by_bus == NULL) g_warning("There is not a session bus up");
  else {
