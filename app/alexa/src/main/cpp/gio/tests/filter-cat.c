...
static void decompressor_file_info_notify_cb (GZlibDecompressor *decompressor, GParamSpec *pspec, gpointer data) {
  GFileInfo *file_info;
  const gchar *filename;
  file_info = g_zlib_decompressor_get_file_info(decompressor);
  if (file_info == NULL) return;
  filename = g_file_info_get_name(file_info);
  if (filename) g_printerr("Decompressor filename: %s\n", filename);
}
static void cat(GFile *file) {
  GInputStream *in;
  ...
  if (from_charset && to_charset) {
      ...
      if (conv) {
          ...
          g_object_unref(conv);
          g_object_unref(old);
      } else g_printerr("%s: Can't convert between charsets: %s\n", g_get_prgname(), error->message);
  }
  if (compress) {
      GInputStream *old;
      GFileInfo *in_file_info;
      in_file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_TIME_MODIFIED,G_FILE_QUERY_INFO_NONE,NULL, &error);
      ...
      if (res > 0) {
          ...
          while(res > 0) {
              ...
              if (written == -1 && errno != EINTR) {
                  g_printerr("%s: %s, error writing to stdout", g_get_prgname(), g_file_get_uri(file));
                  goto out;
              }
              res -= written;
              p += written;
...
out:
  ...
  if (cconv != NULL && fallback) {
      ...
      if (num > 0) g_printerr("Number of fallback errors: %u\n", num);
  }
}
int main(int argc, char *argv[]) {
  ...
  int i;
  g_type_init();
  context = g_option_context_new("LOCATION... - concatenate LOCATIONS to standard output.");
