...
static void test_seek_to_start(GInputStream *stream) {
  GError *error = NULL;
...
static void test_read_lines(GDataStreamNewlineType newline_type) {
  ...
  const char* endl[4] = {"\n", "\r", "\r\n", "\n"};
  int i;
  for (i = 0; i < MAX_LINES; i++) lines[i] = "some_text";
  base_stream = g_memory_input_stream_new();
  ...
  for (i = 0; i < MAX_LINES; i++) 
      g_memory_input_stream_add_data(G_MEMORY_INPUT_STREAM(base_stream),g_strconcat(lines[i], endl[newline_type], NULL), -1, g_free);
...
static void test_read_lines_any(void) {
...
static void test_read_until(void) {
  ...
  #define DATA_PART_LEN  7
  #define DATA_SEP  "#$%^"
  #define DATA_SEP_LEN  4
  const int DATA_PARTS_NUM = DATA_SEP_LEN * REPEATS;
  base_stream = g_memory_input_stream_new();
  stream = G_INPUT_STREAM(g_data_input_stream_new(base_stream));
  for (i = 0; i < REPEATS; i++) g_memory_input_stream_add_data(G_MEMORY_INPUT_STREAM(base_stream), DATA_STRING, -1, NULL);
  error = NULL;
  data = (char*)1;
  line = 0;
  while(data) {
...
static void test_read_upto(void) {
  GInputStream *stream;
  GInputStream *base_stream;
  GError *error = NULL;
  char *data;
  ...
  base_stream = g_memory_input_stream_new();
  stream = G_INPUT_STREAM(g_data_input_stream_new(base_stream));
  for (i = 0; i < REPEATS; i++) g_memory_input_stream_add_data(G_MEMORY_INPUT_STREAM(base_stream), DATA_STRING, 32, NULL);
  error = NULL;
  data = (char*)1;
  line = 0;
  while(data) {
...
#define TEST_DATA_RETYPE_BUFF(a, t, v)	(a == TEST_DATA_BYTE ? (t)*(guchar*)v : (a == TEST_DATA_INT16 ? (t)*(gint16*)v : (a == TEST_DATA_UINT16 ? (t)*(guint16*)v : \
                                        (a == TEST_DATA_INT32 ? (t)*(gint32*)v : (a == TEST_DATA_UINT32	? (t)*(guint32*)v : (a == TEST_DATA_INT64 ? (t)*(gint64*)v :\
                                        (t)*(guint64*)v ))))))
static void test_data_array(GInputStream *stream, GInputStream *base_stream, gpointer buffer, int len, enum TestDataType data_type, GDataStreamByteOrder byte_order) {
  GError *error = NULL;
  int pos = 0;
  int data_size = 1;
  gint64 data;
  ...
  test_seek_to_start(base_stream);
  switch(data_type) {
      case TEST_DATA_BYTE: data_size = 1; break;
      case TEST_DATA_INT16: case TEST_DATA_UINT16: data_size = 2; break;
      case TEST_DATA_INT32: case TEST_DATA_UINT32: data_size = 4; break;
      case TEST_DATA_INT64: case TEST_DATA_UINT64: data_size = 8; break;
...
static void test_read_int(void) {
  ...
  for (i = 0; i < 3; i++) {
      ...
      for (j = 0; j <= TEST_DATA_UINT64; j++) test_data_array(stream, base_stream, buffer, MAX_BYTES, j, i);
  }
  g_object_unref(base_stream);
  g_object_unref(stream);
