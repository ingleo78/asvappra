...
static const struct StructureItem sample_struct[] = {
    ...
    {"UTF_pr\xcc\x8ci\xcc\x81lis\xcc\x8c z", NULL,	G_FILE_TYPE_REGULAR,	G_FILE_CREATE_NONE, 0, 0, TEST_COPY | TEST_CREATE | TEST_OPEN | TEST_OVERWRITE},
    {"dir_pr\xcc\x8ci\xcc\x81lis\xcc\x8c z", NULL,	G_FILE_TYPE_DIRECTORY,	G_FILE_CREATE_NONE, 0, 0, TEST_DELETE_NORMAL | TEST_CREATE},
...
#endif
static GFile *create_empty_file(GFile *parent, const char *filename, GFileCreateFlags create_flags) {
  GFile *child;
  gboolean res;
  GError *error;
...
static GFile *create_empty_dir(GFile * parent, const char *filename) {
  GFile *child;
  gboolean res;
  GError *error;
  child = g_file_get_child(parent, filename);
...
static GFile *create_symlink(GFile * parent, const char *filename, const char *points_to) {
  GFile *child;
  gboolean res;
  GError *error;
  child = g_file_get_child(parent, filename);
...
static void test_create_structure(gconstpointer test_data) {
  GFile *root;
  GFile *child;
  gboolean res;
  GError *error;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      ...
      if ((item.handle_special) || ((!posix_compat) && (item.file_type == G_FILE_TYPE_SYMBOLIC_LINK))) continue;
      child = NULL;
      ...
      if ((item.mode > 0) && (posix_compat)) {
          error = NULL;
          res = g_file_set_attribute_uint32(child, G_FILE_ATTRIBUTE_UNIX_MODE, item.mode,G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, &error);
          g_assert_cmpint(res, ==, TRUE);
          g_assert_no_error(error);
	  }
  ...
  g_assert(child != NULL);
  error = NULL;
  outs = g_file_replace(child, NULL, FALSE, G_FILE_CREATE_NONE, NULL, &error);
  g_assert_no_error(error);
  g_assert(outs != NULL);
...
static GFile *file_exists(GFile * parent, const char *filename, gboolean * result) {
  GFile *child;
  gboolean res;
...
static void test_attributes(struct StructureItem item, GFileInfo * info) {
  GFileType ftype;
  guint32 mode;
  ...
  if ((item.mode > 0) && (posix_compat)) {
      mode = g_file_info_get_attribute_uint32(info, G_FILE_ATTRIBUTE_UNIX_MODE) & 0xFFF;
  ...
  copy_name = g_file_info_get_attribute_string(info, G_FILE_ATTRIBUTE_STANDARD_COPY_NAME);
  if (copy_name) {
  ...
  if (posix_compat) {
      ...
      g_assert_cmpint(is_symlink, ==, item.file_type == G_FILE_TYPE_SYMBOLIC_LINK);
  }
...
static void test_initial_structure(gconstpointer test_data) {
  GFile *root;
  GFile *child;
  gboolean res;
  GError *error;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      item = sample_struct[i];
      if (((!posix_compat) && (item.file_type == G_FILE_TYPE_SYMBOLIC_LINK)) || (item.handle_special)) continue;
      log("    Testing file '%s'...\n", item.filename);
      child = file_exists(root, item.filename, &res);
      g_assert(child != NULL);
      g_assert_cmpint(res, ==, TRUE);
      error = NULL;
      info = g_file_query_info(child, "*", G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,NULL, &error);
      g_assert_no_error(error);
      g_assert(info != NULL);
  ...
  while(total_read < PATTERN_FILE_SIZE) {
      error = NULL;
      read = g_input_stream_read(G_INPUT_STREAM (ins), buffer + total_read, PATTERN_FILE_SIZE, NULL, &error);
      g_assert_no_error(error);
      total_read += read;
  ...
  g_assert_no_error(error);
  g_assert_cmpint(res, ==, TRUE);
  for (i = 0; i < PATTERN_FILE_SIZE; i++) g_assert_cmpint(*(buffer + i), ==, i % 256);
  g_object_unref(ins);
  g_object_unref(child);
  g_free(buffer);
  g_object_unref(root);
}
static void traverse_recurse_dirs(GFile * parent, GFile * root) {
  gboolean res;
  GError *error;
  ...
  g_assert(root != NULL);
  error = NULL;
  enumerator = g_file_enumerate_children(parent, "*",G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, &error);
  ...
  while((info) && (!error)) {
      ...
      if (g_file_info_get_file_type(info) == G_FILE_TYPE_DIRECTORY) traverse_recurse_dirs(descend, root);
...
static void test_traverse_structure(gconstpointer test_data) {
  GFile *root;
  gboolean res;
  g_assert(test_data != NULL);
...
static void test_enumerate(gconstpointer test_data) {
  GFile *root, *child;
  gboolean res;
  GError *error;
  GFileEnumerator *enumerator;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      item = sample_struct[i];
      if ((!posix_compat) && (item.file_type == G_FILE_TYPE_SYMBOLIC_LINK)) continue;
      if (((item.extra_flags & TEST_NOT_EXISTS) == TEST_NOT_EXISTS) || (((item.extra_flags & TEST_NO_ACCESS) == TEST_NO_ACCESS) && posix_compat) ||
          ((item.extra_flags & TEST_ENUMERATE_FILE) == TEST_ENUMERATE_FILE)) {
          log("    Testing file '%s'\n", item.filename);
          child = g_file_get_child(root, item.filename);
          g_assert(child != NULL);
          error = NULL;
          enumerator = g_file_enumerate_children(child, "*",G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,NULL, &error);
...
static void do_copy_move(GFile *root, struct StructureItem item, const char *target_dir, enum StructureExtraFlags extra_flags) {
  ...
  if ((item.extra_flags & TEST_COPY) == TEST_COPY) {
      res = g_file_copy(src_file, dst_file,G_FILE_COPY_NOFOLLOW_SYMLINKS | ((extra_flags == TEST_OVERWRITE) ? G_FILE_COPY_OVERWRITE : G_FILE_COPY_NONE),
                        NULL, NULL, NULL, &error);
  } else res = g_file_move(src_file, dst_file, G_FILE_COPY_NOFOLLOW_SYMLINKS, NULL,NULL, NULL, &error);
  if (error) log("       res = %d, error code %d = %s\n", res, error->code, error->message);
  if (((item.extra_flags & TEST_NOT_EXISTS) != TEST_NOT_EXISTS) && (extra_flags == TEST_ALREADY_EXISTS)) {
      g_assert_cmpint(res, ==, FALSE);
      g_assert_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS);
  } else if (((item.extra_flags & TEST_NOT_EXISTS) != TEST_NOT_EXISTS) && (extra_flags == TEST_TARGET_IS_FILE)) {
      g_assert_cmpint(res, ==, FALSE);
      g_assert_error(error, G_IO_ERROR, G_IO_ERROR_NOT_DIRECTORY);
  } else if ((item.extra_flags & TEST_COPY_ERROR_RECURSE) == TEST_COPY_ERROR_RECURSE) {
      g_assert_cmpint(res, ==, FALSE);
      g_assert_error(error, G_IO_ERROR, G_IO_ERROR_WOULD_RECURSE);
  } else if (((item.extra_flags & TEST_NOT_EXISTS) == TEST_NOT_EXISTS) || (extra_flags == TEST_NOT_EXISTS)) {
      g_assert_cmpint(res, ==, FALSE);
      g_assert_error(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND);
  } else if (((item.extra_flags & TEST_NO_ACCESS) == TEST_NO_ACCESS) || (extra_flags == TEST_NO_ACCESS)) {
...
static void test_copy_move(gconstpointer test_data) {
  GFile *root;
  gboolean res;
  guint i;
  struct StructureItem item;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      item = sample_struct[i];
      if ((!posix_compat) && (item.file_type == G_FILE_TYPE_SYMBOLIC_LINK)) continue;
      if (((item.extra_flags & TEST_COPY) == TEST_COPY) || ((item.extra_flags & TEST_MOVE) == TEST_MOVE)) {
          do_copy_move(root, item, TEST_DIR_TARGET, 0);
          if ((item.extra_flags & TEST_COPY) == TEST_COPY) {
              if ((item.extra_flags & TEST_OVERWRITE) == TEST_OVERWRITE) do_copy_move(root, item, TEST_DIR_TARGET, TEST_OVERWRITE);
              do_copy_move(root, item, ".", TEST_ALREADY_EXISTS);
              do_copy_move(root, item, TEST_TARGET_FILE,TEST_TARGET_IS_FILE);
              do_copy_move(root, item, TEST_NAME_NOT_EXISTS,TEST_NOT_EXISTS);
              if (posix_compat) {
                  do_copy_move(root, item, TEST_DIR_NO_ACCESS,TEST_NO_ACCESS);
                  do_copy_move(root, item, TEST_DIR_NO_WRITE,TEST_NO_ACCESS);
              }
          }
	  }
  }
...
static void test_create(gconstpointer test_data) {
  GFile *root, *child;
  gboolean res;
  GError *error;
  guint i;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      item = sample_struct[i];
      if (((item.extra_flags & TEST_CREATE) == TEST_CREATE) || ((item.extra_flags & TEST_REPLACE) == TEST_REPLACE) ||
          ...
          error = NULL;
          os = NULL;
          if ((item.extra_flags & TEST_CREATE) == TEST_CREATE) os = g_file_create(child, item.create_flags, NULL, &error);
          else if ((item.extra_flags & TEST_REPLACE) == TEST_REPLACE) os = g_file_replace(child, NULL, TRUE, item.create_flags, NULL, &error);
          else if ((item.extra_flags & TEST_APPEND) == TEST_APPEND) os = g_file_append_to(child, item.create_flags, NULL, &error);
          if (error) log("       error code %d = %s\n", error->code, error->message);
          if (((item.extra_flags & TEST_NOT_EXISTS) == 0) && ((item.extra_flags & TEST_CREATE) == TEST_CREATE)) {
              g_assert(os == NULL);
              g_assert_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS);
          } else if (item.file_type == G_FILE_TYPE_DIRECTORY) {
              g_assert(os == NULL);
              if ((item.extra_flags & TEST_CREATE) == TEST_CREATE) { g_assert_error(error, G_IO_ERROR, G_IO_ERROR_EXISTS); }
              else { g_assert_error(error, G_IO_ERROR, G_IO_ERROR_IS_DIRECTORY); }
          } else {
              g_assert(os != NULL);
              g_assert_no_error(error);
          }
          ...
          if (os) {
              error = NULL;
              res = g_output_stream_close(G_OUTPUT_STREAM(os), NULL, &error);
              if (error) log("         g_output_stream_close: error %d = %s\n", error->code, error->message);
              g_assert_cmpint(res, ==, TRUE);
              g_assert_no_error(error);
          }
          g_object_unref(child);
...
static void test_open(gconstpointer test_data) {
  GFile *root, *child;
  gboolean res;
  GError *error;
  guint i;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      ...
      if ((item.extra_flags & TEST_OPEN) == TEST_OPEN) {
          ...
          if (((item.extra_flags & TEST_NOT_EXISTS) == TEST_NOT_EXISTS) || ((item.extra_flags & TEST_INVALID_SYMLINK) == TEST_INVALID_SYMLINK)) {
          ...
          if (input_stream) {
              error = NULL;
              res = g_input_stream_close(G_INPUT_STREAM(input_stream), NULL, &error);
              g_assert_cmpint(res, ==, TRUE);
              g_assert_no_error(error);
          }
          g_object_unref (child);
...
static void test_delete(gconstpointer test_data) {
  GFile *root;
  GFile *child;
  gboolean res;
  GError *error;
  ...
  for (i = 0; i < G_N_ELEMENTS(sample_struct); i++) {
      item = sample_struct[i];
      if ((!posix_compat) && (item.file_type == G_FILE_TYPE_SYMBOLIC_LINK)) continue;
      if (((item.extra_flags & TEST_DELETE_NORMAL) == TEST_DELETE_NORMAL) || ((item.extra_flags & TEST_DELETE_TRASH) == TEST_DELETE_TRASH)) {
          child = file_exists(root, item.filename, &res);
          g_assert(child != NULL);
          log("  Deleting %s, path = %s\n", item.filename, g_file_get_path(child));
          error = NULL;
          if ((item.extra_flags & TEST_DELETE_NORMAL) == TEST_DELETE_NORMAL) res = g_file_delete(child, NULL, &error);
          else res = g_file_trash(child, NULL, &error);
          if ((item.extra_flags & TEST_DELETE_NON_EMPTY) == TEST_DELETE_NON_EMPTY) {
              g_assert_cmpint(res, ==, FALSE);
...
static void cleanup_dir_recurse(GFile *parent, GFile *root) {
  gboolean res;
  GError *error;
  GFileEnumerator *enumerator;
  GFileInfo *info;
  ...
  while((info) && (!error)) {
      ...
      if (g_file_info_get_file_type(info) == G_FILE_TYPE_DIRECTORY) cleanup_dir_recurse(descend, root);
      error = NULL;
...
static void prep_clean_structure(gconstpointer test_data) {
  GFile *root;
  g_assert(test_data != NULL);
...
int main(int argc, char *argv[]) {
  ...
  static GOptionEntry cmd_entries[] = {
    {"read-write", 'w', 0, G_OPTION_ARG_NONE, &write_test,"Perform write tests (incl. structure creation)", NULL},
    {"create-struct", 'c', 0, G_OPTION_ARG_NONE, &only_create_struct,"Only create testing structure (no tests)", NULL},
    {"verbose", 'v', 0, G_OPTION_ARG_NONE, &verbose, "Be verbose", NULL},
    {"posix", 'x', 0, G_OPTION_ARG_NONE, &posix_compat,"Test POSIX-specific features (unix permissions, symlinks)", NULL},
    {NULL}
  };
  test_suite = FALSE;
  verbose = FALSE;
  ...
  if (argc >= 2) target_path = strdup(argv[1]);
  if (! target_path) {
      g_print("error: target path was not specified\n");
      g_print("%s", g_option_context_get_help(context, TRUE, NULL));
      return g_test_run();
  }
  if (write_test || only_create_struct) g_test_add_data_func("/live-g-file/prep_clean_structure", target_path, prep_clean_structure);
  if (write_test || only_create_struct) g_test_add_data_func("/live-g-file/create_structure", target_path, test_create_structure);
  if (!only_create_struct) g_test_add_data_func("/live-g-file/test_initial_structure", target_path, test_initial_structure);
  if (!only_create_struct) g_test_add_data_func("/live-g-file/test_traverse_structure", target_path, test_traverse_structure);
  if (!only_create_struct) g_test_add_data_func("/live-g-file/test_enumerate", target_path, test_enumerate);
  if (!only_create_struct) g_test_add_data_func("/live-g-file/test_open", target_path, test_open);
  if (write_test && (!only_create_struct)) g_test_add_data_func("/live-g-file/test_create", target_path, test_create);
  if (write_test && (!only_create_struct)) g_test_add_data_func("/live-g-file/test_copy_move", target_path, test_copy_move);
  if (write_test && (!only_create_struct)) g_test_add_data_func("/live-g-file/test_delete", target_path, test_delete);
  if (write_test || only_create_struct) g_test_add_data_func("/live-g-file/final_clean", target_path, prep_clean_structure);
  return g_test_run();
}