...
static void on_name_appeared(GDBusConnection *connection, const gchar *name, const gchar *name_owner, gpointer user_data) {
  g_print("Name %s on %s is owned by %s\n", name, opt_system_bus ? "the system bus" : "the session bus", name_owner);
}
...
int main(int argc, char *argv[]) {
  ...
  g_option_context_set_summary(opt_context,"Example: to watch the power manager on the session bus, use:\n\n  ./example-watch-name -n org.gnome.PowerManager");
  ...
  if (opt_auto_start) flags |= G_BUS_NAME_WATCHER_FLAGS_AUTO_START;
  watcher_id = g_bus_watch_name(opt_system_bus ? G_BUS_TYPE_SYSTEM : G_BUS_TYPE_SESSION, opt_name, flags, on_name_appeared, on_name_vanished, NULL, NULL);
