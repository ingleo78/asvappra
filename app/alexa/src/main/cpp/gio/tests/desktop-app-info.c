...
static char *basedir;
static GAppInfo *create_app_info(const char *name) {
  GError *error;
  GAppInfo *info;
  error = NULL;
  info = g_app_info_create_from_commandline("/usr/bin/true blah", name,G_APP_INFO_CREATE_NONE, &error);
  g_assert(error == NULL);
...
static void test_last_used(void) {
  GList *applications;
...
static void cleanup_subdirs(const char *base_dir) {
  GFile *base, *file;
