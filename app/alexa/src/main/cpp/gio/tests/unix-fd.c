...
static void check_fd_list(const gint *fd_list) {
  gint i;
  for (i = 0; i < 40; i++) {
      int my_fd;
      my_fd = dup(0);
  ...
  for (i = 0; i < 40; i++) close(fd_list[i]);
}
static void create_fd_list(gint *fd_list) {
  gint i;
  for (i = 0; i < 40; i++) {
      fd_list[i] = dup(0);
...
static void test_fds(void) {
  ...
  ov.buffer = buffer;
  ov.size = 1;
  s = g_socket_send_message(sockets[0],NULL, &ov,1, (GSocketControlMessage**)&message,1,0,NULL, &err);
  g_assert_no_error(err);
  g_assert_cmpint(s, ==, 1);
  g_object_unref(message);
  message = NULL;
  ...
  iv.buffer = buffer;
  iv.size = 1;
  s = g_socket_receive_message(sockets[1],NULL, &iv,1, (GSocketControlMessage***)&mv, &nm, &flags,NULL, &err);
  g_assert_no_error(err);
  g_assert_cmpint(s, ==, 1);
  ...
  g_assert_cmpstr(buffer, ==,"failure to say failure to say 'i love gnome-panel!'.");
  g_object_unref(list);
