...
static gint get_server_stdout(GDBusConnection *connection, const gchar *name_owner, GError **error) {
  ...
  method_call_message = g_dbus_message_new_method_call(name_owner, "/org/gtk/GDBus/TestObject", "org.gtk.GDBus.TestInterface", "GimmeStdout");
  method_reply_message = g_dbus_connection_send_message_with_reply_sync(connection, method_call_message, G_DBUS_SEND_MESSAGE_FLAGS_NONE, -1, NULL, NULL, error);
  if (method_reply_message == NULL) goto out;
...
static void on_name_appeared(GDBusConnection *connection, const gchar *name, const gchar *name_owner, gpointer user_data) {
  ...
  if (fd == -1) {
      ...
      gchar *str;
      now = time(NULL);
      strftime(now_buf, sizeof now_buf,"%c", localtime(&now));
...
int main(int argc, char *argv[]) {
  ...
  GMainLoop *loop;
  g_type_init();
  watcher_id = g_bus_watch_name(G_BUS_TYPE_SESSION, "org.gtk.GDBus.TestServer", G_BUS_NAME_WATCHER_FLAGS_NONE, on_name_appeared, on_name_vanished, NULL, NULL);
