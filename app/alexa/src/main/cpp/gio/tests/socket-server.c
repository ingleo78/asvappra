...
int main(int argc, char *argv[]) {
  ...
  } else src_address = g_inet_socket_address_new (g_inet_address_new_any(G_SOCKET_FAMILY_IPV4), port);
  ...
  if (!use_udp) {
      ...
      if (non_blocking) g_socket_set_blocking(new_socket, FALSE);
      if (read_timeout) g_socket_set_timeout(new_socket, read_timeout);
  ...
  while(TRUE) {
      ...
      if (use_udp) {
          ...
          size = g_socket_receive_from(recv_socket, &address, buffer, sizeof buffer, cancellable, &error);
	  } else {
      ...
      if (use_udp) g_print(" from %s", socket_address_to_string (address));
      g_print("\n");
      if (verbose) g_print("-------------------------\n%.*s\n-------------------------\n", (int)size, buffer);
      to_send = size;
      if (delay) {
          if (verbose) g_print("delaying %d seconds before response\n", delay);
          g_usleep(1000 * 1000 * delay);
	  }
      while(to_send > 0) {
          if (use_udp) {
              ensure_socket_condition(recv_socket, G_IO_OUT, cancellable);
              size = g_socket_send_to(recv_socket, address, buffer, to_send, cancellable, &error);
          } else {
