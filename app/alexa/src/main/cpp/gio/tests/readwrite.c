...
static void verify_pos(GIOStream *iostream, goffset expected_pos) {
  goffset pos;
...
static void verify_iostream(GFileIOStream *file_iostream) {
  gboolean res;
  GIOStream *iostream;
  GError *error;
  GInputStream *in;
  ...
  res = g_seekable_seek(G_SEEKABLE (iostream),-10, G_SEEK_END,NULL, NULL);
  g_assert(res);
  ...
  res = g_seekable_seek(G_SEEKABLE(iostream),10, G_SEEK_SET,NULL, NULL);
  verify_pos(iostream, 10);
  res = g_output_stream_write_all(out, new_data, strlen(new_data), &n_bytes, NULL, NULL);
  g_assert(res);
  g_assert_cmpint(n_bytes, ==, strlen(new_data));
  verify_pos(iostream, 10 + strlen(new_data));
  res = g_seekable_seek(G_SEEKABLE(iostream),0, G_SEEK_SET,NULL, NULL);
  g_assert(res);
  verify_pos(iostream, 0);
  ...
  error = NULL;
  res = g_output_stream_write_all(out, new_data, strlen(new_data), &n_bytes, NULL, &error);
  g_assert(!res);
...
static void test_g_file_open_readwrite(void) {
  char *tmp_file;
  int fd;
  gboolean res;
  ...
  GFile *file;
  GError *error;
  fd = g_file_open_tmp("readwrite_XXXXXX", &tmp_file, NULL);
  g_assert(fd != -1);
  close(fd);
  res = g_file_set_contents(tmp_file, original_data, -1, NULL);
  g_assert(res);
...
static void test_g_file_create_readwrite(void) {
  char *tmp_file;
  int fd;
  gboolean res;
  GFileIOStream *file_iostream;
  ...
  res = g_output_stream_write_all(out, original_data, strlen (original_data), &n_bytes, NULL, NULL);
  g_assert(res);
  g_assert_cmpint(n_bytes, ==, strlen(original_data));
  res = g_seekable_seek(G_SEEKABLE(file_iostream),0, G_SEEK_SET,NULL, NULL);
  g_assert(res);
  verify_iostream(file_iostream);
  g_object_unref(file_iostream);
  g_object_unref(file);
...
static void test_g_file_replace_readwrite(void) {
  ...
  g_assert(fd != -1);
  close(fd);
  res = g_file_set_contents(tmp_file, new_data, -1, NULL);
  g_assert(res);
  file = g_file_new_for_path(tmp_file);
  error = NULL;
  file_iostream = g_file_replace_readwrite(file, NULL, TRUE, 0, NULL, &error);
  g_assert(file_iostream != NULL);
  ...
  res = g_file_get_contents(backup, &data, NULL, NULL);
  g_assert(res);
...
int main(int argc, char *argv[]) {
  ...
  g_test_add_func("/readwrite/test_g_file_open_readwrite", test_g_file_open_readwrite);
  g_test_add_func("/readwrite/test_g_file_create_readwrite", test_g_file_create_readwrite);
  g_test_add_func("/readwrite/test_g_file_replace_readwrite", test_g_file_replace_readwrite);
  return g_test_run();
}