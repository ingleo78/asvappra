...
static void test_locale(const char *locale) {
  GAppInfo *appinfo = NULL;
  const gchar *orig;
...
static void test_commandline(void) {
  ...
  GError *error;
  error = NULL;
  appinfo = g_app_info_create_from_commandline("./appinfo-test --option", "cmdline-app-test", G_APP_INFO_CREATE_SUPPORTS_URIS, &error);
  g_assert(appinfo != NULL);
  g_assert_no_error(error);
  ...
  g_object_unref(appinfo);
  error = NULL;
  appinfo = g_app_info_create_from_commandline("./appinfo-test --option", "cmdline-app-test", G_APP_INFO_CREATE_NONE, &error);
  g_assert(appinfo != NULL);
  g_assert_no_error(error);
  g_assert_cmpstr(g_app_info_get_name(appinfo), ==, "cmdline-app-test");
...
static void test_launch_context(void) {
  GAppLaunchContext *context;
  GAppInfo *appinfo;
  gchar *str;
  context = g_app_launch_context_new();
  appinfo = g_app_info_create_from_commandline("./appinfo-test --option", "cmdline-app-test", G_APP_INFO_CREATE_SUPPORTS_URIS, NULL);
