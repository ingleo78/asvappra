...
#define TEST_FILE "/home/"
char *test_file_buffer;
gsize test_file_size;
static char async_read_buffer[8192];
static void read_data(GObject *source, GAsyncResult *result, gpointer loop) {
...
static void opened_for_read(GObject *source, GAsyncResult *result, gpointer loop) {
  ...
  g_input_stream_read_async(G_INPUT_STREAM(in), async_read_buffer, sizeof(async_read_buffer), G_PRIORITY_DEFAULT, NULL, read_data, loop);
  g_object_unref(in);
}
...
static gpointer test1_thread(gpointer user_data) {
  ...
  g_file_read_async(file, G_PRIORITY_DEFAULT, NULL, opened_for_read, loop);
  g_object_unref(file);
  g_main_loop_run(loop);
  g_main_loop_unref(loop);
...
static void test_context_independence(void) {
  GMainContext *context;
  GMainLoop *loop;
  GFile *file = NULL;
  ...
  //file = g_file_new_for_path(TEST_FILE);
  g_assert(g_file_supports_thread_contexts(file));
  default_timeout = g_timeout_add_full(G_PRIORITY_HIGH, 0, test2_fail, NULL, NULL);
...
int main(int argc, char **argv) {
  ...
  g_file_get_contents(TEST_FILE, &test_file_buffer, &test_file_size, &error);
  g_assert_no_error(error);
