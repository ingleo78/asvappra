...
G_DEFINE_TYPE_WITH_CODE(GDummyFile, g_dummy_file, G_TYPE_OBJECT,G_IMPLEMENT_INTERFACE(G_TYPE_FILE, g_dummy_file_file_iface_init));
...
static void g_string_append_encoded(GString *string, const char *encoded, const char *reserved_chars_allowed);
static void g_dummy_file_finalize(GObject *object) {
  GDummyFile *dummy;
  dummy = G_DUMMY_FILE(object);
  if (dummy->decoded_uri) _g_decoded_uri_free(dummy->decoded_uri);
...
static char *g_dummy_file_get_basename(GFile *file) {
  ...
  if (dummy->decoded_uri) return g_path_get_basename(dummy->decoded_uri->path);
...
static GFile *g_dummy_file_get_parent(GFile *file) {
  ...
  if (dummy->decoded_uri == NULL || g_strcmp0(dummy->decoded_uri->path, "/") == 0) return NULL;
...
static intsafe_strcmp(const char *a, const char *b) {
  if (a == NULL) a = "";
  if (b == NULL) b = "";
  return strcmp(a, b);
}
static gboolean uri_same_except_path(GDecodedUri *a, GDecodedUri *b) {
  if (strcmp(a->scheme, b->scheme) != 0) return FALSE;
  if (strcmp(a->userinfo, b->userinfo) != 0) return FALSE;
  if (strcmp(a->host, b->host) != 0) return FALSE;
...
static gboolean g_dummy_file_prefix_matches(GFile *parent, GFile *descendant) {
  ...
  if (parent_dummy->decoded_uri != NULL && descendant_dummy->decoded_uri != NULL) {
      if (uri_same_except_path(parent_dummy->decoded_uri, descendant_dummy->decoded_uri)) {
          remainder = match_prefix(descendant_dummy->decoded_uri->path, parent_dummy->decoded_uri->path);
      ...
      remainder = match_prefix(descendant_dummy->text_uri, parent_dummy->text_uri);
      if (remainder != NULL && *remainder == '/') {
	      while(*remainder == '/') remainder++;
	      if (*remainder != 0) return TRUE;
	  }
...
static char *g_dummy_file_get_relative_path(GFile *parent, GFile *descendant) {
  ...
  if (parent_dummy->decoded_uri != NULL && descendant_dummy->decoded_uri != NULL) {
      if (uri_same_except_path(parent_dummy->decoded_uri, descendant_dummy->decoded_uri)) {
          ...
          if (remainder != NULL && *remainder == '/') {
              while (*remainder == '/') remainder++;
              if (*remainder != 0) return g_strdup(remainder);
	      }
      }
  } else {
      remainder = match_prefix(descendant_dummy->text_uri, parent_dummy->text_uri);
      if (remainder != NULL && *remainder == '/') {
	    while (*remainder == '/') remainder++;
	    if (*remainder != 0) return unescape_string(remainder, NULL, "/");
	  }
  }
  return NULL;
}
static GFile *g_dummy_file_resolve_relative_path(GFile *file, const char *relative_path) {
  GDummyFile *dummy = G_DUMMY_FILE(file);
  GFile *child;
  char *uri;
  GDecodedUri new_decoded_uri;
  ...
  if (dummy->decoded_uri == NULL) {
      ...
      new_decoded_uri = *dummy->decoded_uri;
      if (g_path_is_absolute(relative_path)) new_decoded_uri.path = g_strdup(relative_path);
      else new_decoded_uri.path = g_build_filename(new_decoded_uri.path, relative_path, NULL);
      uri = _g_encode_uri(&new_decoded_uri);
...
static GFile *g_dummy_file_get_child_for_display_name(GFile *file, const char *display_name, GError **error) {
  return g_file_get_child(file, display_name);
}
static gboolean g_dummy_file_has_uri_scheme(GFile *file, const char *uri_scheme) {
  GDummyFile *dummy = G_DUMMY_FILE(file);
  if (dummy->decoded_uri) return g_ascii_strcasecmp(uri_scheme, dummy->decoded_uri->scheme) == 0;
  return FALSE;
}
static char *g_dummy_file_get_uri_scheme (GFile *file) {
  GDummyFile *dummy = G_DUMMY_FILE(file);
  if (dummy->decoded_uri) return g_strdup(dummy->decoded_uri->scheme);
  return NULL;
}
static void g_dummy_file_file_iface_init(GFileIface *iface) {
...
GDecodedUri *_g_decode_uri(const char *uri) {
  ...
  *out = 0;
  ...
  if (hier_part_start[0] == '/' && hier_part_start[1] == '/') {
	  ...
	  } else host_start = authority_start;
