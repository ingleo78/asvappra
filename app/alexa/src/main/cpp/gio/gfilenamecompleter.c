...
static void g_filename_completer_finalize(GObject *object) {
  ...
  if (completer->basenames_dir) g_object_unref(completer->basenames_dir);
...
static void g_filename_completer_class_init(GFilenameCompleterClass *klass) {
  ...
  signals[GOT_COMPLETION_DATA] = g_signal_new(I_("got-completion-data"), G_TYPE_FILENAME_COMPLETER, G_SIGNAL_RUN_LAST,
                                              G_STRUCT_OFFSET(GFilenameCompleterClass, got_completion_data), NULL, NULL, g_cclosure_marshal_VOID__VOID,
                                              G_TYPE_NONE, 0);
}
...
static void got_more_files(GObject *source_object, GAsyncResult *res, gpointer user_data) {
  ...
  for (l = infos; l != NULL; l = l->next) {
      info = l->data;
      if (data->dirs_only && g_file_info_get_file_type(info) != G_FILE_TYPE_DIRECTORY) {
          g_object_unref(info);
          continue;
	  }
      ...
      if (data->should_escape) basename = g_uri_escape_string(name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
      else basename = g_filename_to_utf8(name, -1, NULL, NULL, NULL);
      if (basename) {
          if (append_slash) {
              t = basename;
  ...
  g_list_free(infos);
  if (infos) {
      g_file_enumerator_next_files_async(data->enumerator,100,0, data->cancellable, got_more_files, data);
  } else {
      data->completer->basename_loader = NULL;
      if (data->completer->basenames_dir) g_object_unref(data->completer->basenames_dir);
...
static void got_enum(GObject *source_object, GAsyncResult *res, gpointer user_data) {
  LoadBasenamesData *data = user_data;
  if (data->completer == NULL) {
      load_basenames_data_free(data);
      return;
...
static void schedule_load_basenames(GFilenameCompleter *completer, GFile *dir, gboolean should_escape) {
  ...
  g_file_enumerate_children_async(dir, G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_STANDARD_TYPE,0, 0, data->cancellable, got_enum, data);
}
static void cancel_load_basenames(GFilenameCompleter *completer) {
...
static GList *init_completion(GFilenameCompleter *completer, const char *initial_text, char **basename_out) {
  gboolean should_escape;
  GFile *file, *parent;
  char *basename;
  char *t;
  ...
  *basename_out = NULL;
  ...
  if (len > 0 && initial_text[len - 1] == '/') return NULL;
  ...
  if (completer->basenames_dir == NULL || completer->basenames_are_escaped != should_escape || !g_file_equal(parent, completer->basenames_dir)) {
...
char *g_filename_completer_get_completion_suffix (GFilenameCompleter *completer, const char *initial_text) {
  ...
  for (l = possible_matches; l != NULL; l = l->next) {
      ...
      if (g_str_has_prefix(possible_match, prefix)) {
          if (suffix == NULL) suffix = g_strdup(possible_match + strlen(prefix));
          else {
              lcp = longest_common_prefix(suffix,possible_match + strlen(prefix));
              g_free(suffix);
              suffix = lcp;
              if (*suffix == 0) break;
          }
...
char **g_filename_completer_get_completions(GFilenameCompleter *completer, const char *initial_text) {
  ...
  for (l = possible_matches; l != NULL; l = l->next) {
      possible_match = l->data;
      if (g_str_has_prefix(possible_match, prefix)) g_ptr_array_add(res, g_strconcat(initial_text, possible_match + strlen(prefix), NULL));
  }
  g_free (prefix);
