...
XdgMimeCache *_xdg_mime_cache_new_from_file(const char *file_name) {
  ...
  if (fd < 0) return NULL;
  if (fstat(fd, &st) < 0 || st.st_size < 4) goto done;
  buffer = (char*)mmap(NULL, st.st_size, PROT_READ, MAP_SHARED, fd, 0);
  if (buffer == MAP_FAILED) goto done;
  minor = GET_UINT16 (buffer, 2);
  if (GET_UINT16(buffer, 0) != MAJOR_VERSION || (minor < MINOR_VERSION_MIN || minor > MINOR_VERSION_MAX)) {
...
static int cache_magic_matchlet_compare_to_data(XdgMimeCache *cache, xdg_uint32_t offset, const void *data, size_t len) {
  ...
  for (i = range_start; i < range_start + range_length; i++) {
      ...
      if (i + data_length > len) return FALSE;
      if (mask_offset) {
          for (j = 0; j < data_length; j++) {
              if ((((unsigned char*)cache->buffer)[data_offset + j] & ((unsigned char*)cache->buffer)[mask_offset + j]) != ((((unsigned char*)data)[j + i]) &
      ...
      if (valid_matchlet) return TRUE;
  }
  return FALSE;  
}
static int cache_magic_matchlet_compare(XdgMimeCache *cache, xdg_uint32_t offset, const void *data, size_t len) {
  xdg_uint32_t n_children = GET_UINT32(cache->buffer, offset + 24);
  xdg_uint32_t child_offset = GET_UINT32(cache->buffer, offset + 28);
  int i;
  if (cache_magic_matchlet_compare_to_data (cache, offset, data, len)) {
      if (n_children == 0) return TRUE;
      for (i = 0; i < n_children; i++) {
	      if (cache_magic_matchlet_compare(cache, child_offset + 32 * i, data, len)) return TRUE;
	  }
  }
  return FALSE;  
}
static const char *cache_magic_compare_to_data(XdgMimeCache *cache, xdg_uint32_t offset, const void *data, size_t len, int *prio) {
...
static const char *cache_magic_lookup_data(XdgMimeCache *cache, const void *data, size_t len, int *prio, const char *mime_types[], int n_mime_types) {
  ...
  for (j = 0; j < n_entries; j++) {
      ...
      if (match) return match;
          ...
          for (n = 0; n < n_mime_types; n++) {
              if (mime_types[n] && _xdg_mime_mime_type_equal(mime_types[n], non_match)) mime_types[n] = NULL;
          }
	  }
  }
  return NULL;
...
static int cache_glob_lookup_literal(const char *file_name, const char *mime_types[], int n_mime_types, int case_sensitive_check) {
  const char *ptr;
  int i, min, max, mid, cmp;
  for (i = 0; _caches[i]; i++) {
      XdgMimeCache *cache = _caches[i];
...
static int cache_glob_lookup_fnmatch(const char *file_name, MimeWeight mime_types[], int n_mime_types) {
  ...
  for (i = 0; _caches[i]; i++) {
      ...
      if (n == n_mime_types) break;
  }
  return n;
}
static int cache_glob_node_lookup_suffix(XdgMimeCache  *cache, xdg_uint32_t n_entries, xdg_uint32_t offset, const char *file_name, int len, int case_sensitive_check,
  ...
  while(max >= min) {
      ...
      if (match_char < character) min = mid + 1;
      else if (match_char > character) max = mid - 1;
      else {
          len--;
          n = 0;
          n_children = GET_UINT32(cache->buffer, offset + 12 * mid + 4);
          child_offset = GET_UINT32(cache->buffer, offset + 12 * mid + 8);
          if (len > 0) n = cache_glob_node_lookup_suffix(cache, n_children, child_offset, file_name, len, case_sensitive_check, mime_types, n_mime_types);
          if (n == 0) {
              i = 0;
              while(n < n_mime_types && i < n_children) {
                  match_char = GET_UINT32(cache->buffer, child_offset + 12 * i);
                  if (match_char != 0) break;
...
static int cache_glob_lookup_suffix(const char *file_name, int len, int ignore_case, MimeWeight mime_types[], int n_mime_types) {
  int i, n;
  n = 0;
  for (i = 0; _caches[i]; i++) {
      XdgMimeCache *cache = _caches[i];
      ...
      n += cache_glob_node_lookup_suffix(cache, n_entries, offset, file_name, len, ignore_case, mime_types + n, n_mime_types - n);
      if (n == n_mime_types) break;
  }
  return n;
}
...
static int cache_glob_lookup_file_name(const char *file_name, const char *mime_types[], int n_mime_types) {
  int n;
  MimeWeight mimes[10];
  int n_mimes = 10;
  int i;
  ...
  if (n < 2) n += cache_glob_lookup_suffix(file_name, len, TRUE, mimes + n, n_mimes - n);
  free(lower_case);
  if (n < 2) n += cache_glob_lookup_fnmatch(file_name, mimes + n, n_mimes - n);
  n = filter_out_dupes(mimes, n);
  qsort(mimes, n, sizeof(MimeWeight), compare_mime_weight);
  if (n_mime_types < n) n = n_mime_types;
  for (i = 0; i < n; i++) mime_types[i] = mimes[i].mime;
  return n;
}
...
static const char *cache_get_mime_type_for_data(const void *data, size_t len, int *result_prio, const char *mime_types[], int n_mime_types) {
  const char *mime_type;
  int i, n, priority;
  priority = 0;
  mime_type = NULL;
  for (i = 0; _caches[i]; i++) {
      ...
      int prio;
      const char *match;
      match = cache_magic_lookup_data(cache, data, len, &prio, mime_types, n_mime_types);
      if (prio > priority) {
          priority = prio;
          mime_type = match;
	  }
  }
  if (result_prio) *result_prio = priority;
  if (priority > 0) return mime_type;
  for (n = 0; n < n_mime_types; n++) {
      if (mime_types[n]) return mime_types[n];
  }
...
const char *_xdg_mime_cache_get_mime_type_from_file_name(const char *file_name) {
  const char *mime_type;
  if (cache_glob_lookup_file_name(file_name, &mime_type, 1)) return mime_type;
  else return XDG_MIME_TYPE_UNKNOWN;
}
#endif
...
int _xdg_mime_cache_mime_type_subclass(const char *mime, const char *base) {
  ...
  if (strcmp(umime, ubase) == 0) return 1;
#if 1
  if (is_super_type(ubase) && xdg_mime_media_type_equal(umime, ubase)) return 1;
#endif
  if (strcmp(ubase, "text/plain") == 0 && strncmp(umime, "text/", 5) == 0) return 1;
  if (strcmp(ubase, "application/octet-stream") == 0) return 1;
  for (i = 0; _caches[i]; i++) {
      XdgMimeCache *cache = _caches[i];
      ...
      while(max >= min) {
          ...
          if (cmp < 0) min = med + 1;
              ...
              for (j = 0; j < n_parents; j++) {
                  ...
                  if (_xdg_mime_cache_mime_type_subclass(cache->buffer + parent_offset, ubase)) return 1;
              }
              break;
          }
	  }
      ...
      for (j = 0; j < n_entries; j++) {
          ...
          if (strcmp(cache->buffer + mimetype_offset, mime) == 0) {
              ...
              for (k = 0; k < n_parents && p < 127; k++) {
                  ...
                  for (l = 0; l < p; l++) {
                      if (strcmp(all_parents[l], cache->buffer + parent_mime_offset) == 0) break;
                  }
                  if (l == p) all_parents[p++] = cache->buffer + parent_mime_offset;
              }
              break;
          }
	  }
...
static void dump_glob_node(XdgMimeCache *cache, xdg_uint32_t offset, int depth) {
  ...
  printf("%c", character);
  if (mime_offset) printf(" - %s", cache->buffer + mime_offset);
  printf("\n");
  if (child_offset) {
      for (i = 0; i < n_children; i++) dump_glob_node(cache, child_offset + 20 * i, depth + 1);
  }
}
void _xdg_mime_cache_glob_dump(void) {
  ...
  for (i = 0; _caches[i]; i++) {
      ...
      for (j = 0; j < n_entries; j++) dump_glob_node(cache, offset + 20 * j, 0);
  }
}
#endif