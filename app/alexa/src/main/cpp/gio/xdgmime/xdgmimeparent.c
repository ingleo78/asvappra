...
void _xdg_mime_parent_list_free(XdgParentList *list) {
  ...
  if (list->parents) {
      for (i = 0; i < list->n_mimes; i++) {
          for (p = list->parents[i].parents; *p; p++) free(*p);
...
const char **_xdg_mime_parent_list_lookup(XdgParentList *list, const char *mime) {
  ...
  if (list->n_mimes > 0) {
      key.mime = (char*)mime;
      key.parents = NULL;
      entry = bsearch(&key, list->parents, list->n_mimes, sizeof(XdgMimeParents), &parent_entry_cmp);
      if (entry) return(const char**)entry->parents;
  }
  return NULL;
}
void _xdg_mime_parent_read_from_file(XdgParentList *list, const char *file_name) {
  ...
  fclose (file);
  if (list->n_mimes > 1) qsort(list->parents, list->n_mimes, sizeof(XdgMimeParents), &parent_entry_cmp);
}
