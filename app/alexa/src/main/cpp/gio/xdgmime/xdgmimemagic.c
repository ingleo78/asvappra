...
static void _xdg_mime_magic_matchlet_free(XdgMimeMagicMatchlet *mime_magic_matchlet) {
  if (mime_magic_matchlet) {
      if (mime_magic_matchlet->next) _xdg_mime_magic_matchlet_free (mime_magic_matchlet->next);
      if (mime_magic_matchlet->value) free (mime_magic_matchlet->value);
      if (mime_magic_matchlet->mask) free (mime_magic_matchlet->mask);
...
static void _xdg_mime_magic_match_free (XdgMimeMagicMatch *mime_magic_match) {
  ...
  while(ptr) {
      ...
      if (ptr->matchlet) _xdg_mime_magic_matchlet_free (ptr->matchlet);
      free (ptr);
      ptr = next;
  }
}
...
#define MAX_NUMBER_SIZE 30
  ...
  while(TRUE) {
	  ...
	  *end_of_file = TRUE;
      ...
      pos++;
      if (pos == MAX_NUMBER_SIZE) break;
  }
  if (pos > 0) {
      number_string[pos] = '\000';
      errno = 0;
      retval = strtol(number_string, NULL, 10);
      if ((retval < INT_MIN) || (retval > INT_MAX) || (errno != 0)) return -1;
  }
  return retval;
}
static XdgMimeMagicState _xdg_mime_magic_parse_header(FILE *magic_file, XdgMimeMagicMatch *match) {
  ...
  assert(match != NULL);
  c = getc_unlocked(magic_file);
  if (c == EOF) return XDG_MIME_MAGIC_EOF;
  if (c != '[') return XDG_MIME_MAGIC_ERROR;
  match->priority = _xdg_mime_magic_read_a_number(magic_file, &end_of_file);
  if (end_of_file) return XDG_MIME_MAGIC_EOF;
  if (match->priority == -1) return XDG_MIME_MAGIC_ERROR;
  c = getc_unlocked(magic_file);
  if (c == EOF) return XDG_MIME_MAGIC_EOF;
  if (c != ':') return XDG_MIME_MAGIC_ERROR;
...
static XdgMimeMagicState _xdg_mime_magic_parse_magic_line(FILE *magic_file, XdgMimeMagicMatch *match) {
  ...
  if (isdigit(c)) {
      ...
      if (end_of_file) return XDG_MIME_MAGIC_EOF;
      if (indent == -1) return XDG_MIME_MAGIC_ERROR;
      c = getc_unlocked(magic_file);
      if (c == EOF) return XDG_MIME_MAGIC_EOF;
  }
  if (c != '>') return XDG_MIME_MAGIC_ERROR;
  ...
  if (bytes_read != matchlet->value_length) {
      _xdg_mime_magic_matchlet_free(matchlet);
      if (feof(magic_file)) return XDG_MIME_MAGIC_EOF;
      else return XDG_MIME_MAGIC_ERROR;
  }
  c = getc_unlocked(magic_file);
  if (c == '&') {
  ...
  if (c == '\n') {
      if (matchlet->word_size > 1) {
          ...
          for (i = 0; i < matchlet->value_length; i = i + matchlet->word_size) {
              if (matchlet->word_size == 2) *((xdg_uint16_t*)matchlet->value + i) = SWAP_BE16_TO_LE16 (*((xdg_uint16_t*)(matchlet->value + i)));
              else if (matchlet->word_size == 4) *((xdg_uint32_t*)matchlet->value + i) = SWAP_BE32_TO_LE32(*((xdg_uint32_t*)(matchlet->value + i)));
              if (matchlet->mask) {
                  if (matchlet->word_size == 2) *((xdg_uint16_t*)matchlet->mask + i) = SWAP_BE16_TO_LE16(*((xdg_uint16_t*)(matchlet->mask + i)));
                  else if (matchlet->word_size == 4) *((xdg_uint32_t*)matchlet->mask + i) = SWAP_BE32_TO_LE32(*((xdg_uint32_t*)(matchlet->mask + i)));
              }
          }
      #endif
      }
...
static int _xdg_mime_magic_matchlet_compare_to_data(XdgMimeMagicMatchlet *matchlet, const void *data, size_t len) {
  ...
  for (i = matchlet->offset; i < matchlet->offset + matchlet->range_length; i++) {
      ...
      if (i + matchlet->value_length > len) return FALSE;
      if (matchlet->mask) {
          for (j = 0; j < matchlet->value_length; j++) {
              if ((matchlet->value[j] & matchlet->mask[j]) != ((((unsigned char*)data)[j + i]) & matchlet->mask[j])) {
...
static int _xdg_mime_magic_matchlet_compare_level(XdgMimeMagicMatchlet *matchlet, const void *data, size_t len, int indent) {
  while ((matchlet != NULL) && (matchlet->indent == indent)) {
      if (_xdg_mime_magic_matchlet_compare_to_data(matchlet, data, len)) {
          if ((matchlet->next == NULL) || (matchlet->next->indent <= indent)) return TRUE;
          if (_xdg_mime_magic_matchlet_compare_level(matchlet->next, data, len,indent + 1)) return TRUE;
	  }
      do {
  ...
  for (match = mime_magic->match_list; match; match = match->next) {
      if (_xdg_mime_magic_match_compare_to_data(match, data, len)) {
          ...
          for (n = 0; n < n_mime_types; n++) {
              if (mime_types[n] && _xdg_mime_mime_type_equal(mime_types[n], match->mime_type)) mime_types[n] = NULL;
          }
	  }
  }
  if (mime_type == NULL) {
      for (n = 0; n < n_mime_types; n++) {
	      if (mime_types[n]) mime_type = mime_types[n];
	  }
  }
  if (result_prio) *result_prio = prio;
  return mime_type;
}
static void _xdg_mime_update_mime_magic_extents(XdgMimeMagic *mime_magic) {
  ...
  for (match = mime_magic->match_list; match; match = match->next) {
	  ...
	  if (max_extent < extent) max_extent = extent;
	  }
  }
  mime_magic->max_extent = max_extent;
}
static XdgMimeMagicMatchlet *_xdg_mime_magic_matchlet_mirror(XdgMimeMagicMatchlet *matchlets) {
  XdgMimeMagicMatchlet *new_list;
  XdgMimeMagicMatchlet *tmp;
  if ((matchlets == NULL) || (matchlets->next == NULL)) return matchlets;
  new_list = NULL;
  tmp = matchlets;
  while (tmp != NULL) {
      XdgMimeMagicMatchlet *matchlet;
  ...
  while(state != XDG_MIME_MAGIC_EOF) {
      switch(state) {
          case XDG_MIME_MAGIC_SECTION:
              ...
              if (state == XDG_MIME_MAGIC_EOF || state == XDG_MIME_MAGIC_ERROR) _xdg_mime_magic_match_free(match);
              break;
          case XDG_MIME_MAGIC_MAGIC:
              ...
              } else if (state == XDG_MIME_MAGIC_EOF || state == XDG_MIME_MAGIC_ERROR) _xdg_mime_magic_match_free(match);
              break;
...
void _xdg_mime_magic_read_from_file(XdgMimeMagic *mime_magic, const char *file_name) {
  ...
  if (magic_file == NULL) return;
  if (fread(header, 1, 12, magic_file) == 12) {
      if (memcmp("MIME-Magic\0\n", header, 12) == 0) _xdg_mime_magic_read_magic_file(mime_magic, magic_file);
  }
  fclose(magic_file);
}