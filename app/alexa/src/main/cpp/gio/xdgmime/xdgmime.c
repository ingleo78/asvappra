...
static void xdg_run_command_on_dirs(XdgDirectoryFunc func, void *user_data) {
  ...
  while(*ptr != '\000') {
      ...
      if (*end_ptr == ':') len = end_ptr - ptr;
      else len = end_ptr - ptr + 1;
      dir = malloc(len + 1);
      strncpy(dir, ptr, len);
      dir[len] = '\0';
  ...
  if (stat(file_path, &st) == 0) {
      ...
      for (list = dir_time_list; list; list = list->next) {
          if (! strcmp(list->directory_name, file_path)) {
              if (st.st_mtime == list->mtime) list->checked = XDG_CHECKED_VALID;
              else list->checked = XDG_CHECKED_INVALID;
...
static int xdg_check_dirs(void) {
  ...
  for (list = dir_time_list; list; list = list->next) list->checked = XDG_CHECKED_UNCHECKED;
  xdg_run_command_on_dirs((XdgDirectoryFunc)xdg_check_dir, &invalid_dir_list);
  if (invalid_dir_list) return TRUE;
  for (list = dir_time_list; list; list = list->next) {
      if (list->checked != XDG_CHECKED_VALID) return TRUE;
  }
  return FALSE;
}
...
static void xdg_mime_init(void) {
  if (xdg_check_time_and_dirs()) xdg_mime_shutdown();
  if (need_reread) {
...
const char *xdg_mime_get_mime_type_for_data(const void *data, size_t len, int *result_prio) {
  ...
  xdg_mime_init();
  if (_caches) return _xdg_mime_cache_get_mime_type_for_data(data, len, result_prio);
...
const char * xdg_mime_get_mime_type_for_file(const char *file_name, struct stat *statbuf) {
  ...
  xdg_mime_init();
  if (_caches) return _xdg_mime_cache_get_mime_type_for_file(file_name, statbuf);
...
const char *xdg_mime_get_mime_type_from_file_name(const char *file_name) {
  const char *mime_type;
  xdg_mime_init ();
  if (_caches) return _xdg_mime_cache_get_mime_type_from_file_name (file_name);
  if (_xdg_glob_hash_lookup_file_name (global_hash, file_name, &mime_type, 1)) return mime_type;
...
int xdg_mime_get_mime_types_from_file_name(const char *file_name, const char *mime_types[], int n_mime_types) {
  xdg_mime_init();
  if (_caches) return _xdg_mime_cache_get_mime_types_from_file_name(file_name, mime_types, n_mime_types);
...
void xdg_mime_shutdown(void) {
  ...
  if (_caches) {
      int i;
      for (i = 0; i < n_caches; i++) _xdg_mime_cache_unref(_caches[i]);
      free(_caches);
      _caches = NULL;
      n_caches = 0;
  }
  for (list = callback_list; list; list = list->next) (list->callback)(list->data);
  need_reread = TRUE;
}
int xdg_mime_get_max_buffer_extents(void) {
  xdg_mime_init ();
  if (_caches) return _xdg_mime_cache_get_max_buffer_extents();
  return _xdg_mime_magic_get_buffer_extents(global_magic);
}
const char *_xdg_mime_unalias_mime_type(const char *mime_type) {
  const char *lookup;
  if (_caches) return _xdg_mime_cache_unalias_mime_type(mime_type);
  if ((lookup = _xdg_mime_alias_list_lookup(alias_list, mime_type)) != NULL) return lookup;
  return mime_type;
}
...
int _xdg_mime_mime_type_equal(const char *mime_a, const char *mime_b) {
  ...
  if (strcmp(unalias_a, unalias_b) == 0) return 1;
  return 0;
}
...
int xdg_mime_media_type_equal(const char *mime_a, const char *mime_b) {
  char *sep;
  sep = strchr(mime_a, '/');
  if (sep && strncmp(mime_a, mime_b, sep - mime_a + 1) == 0) return 1;
  return 0;
}
#if 1
...
int _xdg_mime_mime_type_subclass(const char *mime, const char *base) {
  ...
  const char **parents;
  if (_caches) return _xdg_mime_cache_mime_type_subclass(mime, base);
  umime = _xdg_mime_unalias_mime_type(mime);
  ubase = _xdg_mime_unalias_mime_type(base);
  if (strcmp(umime, ubase) == 0) return 1;
#if 1
  if (xdg_mime_is_super_type(ubase) && xdg_mime_media_type_equal(umime, ubase)) return 1;
#endif
  ...
  for (; parents && *parents; parents++) {
      if (_xdg_mime_mime_type_subclass(*parents, ubase)) return 1;
  }
  return 0;
}
...
char **xdg_mime_list_mime_parents(const char *mime) {
  ...
  int i, n;
  xdg_mime_init ();
  if (_caches) return _xdg_mime_cache_list_mime_parents(mime);
  umime = _xdg_mime_unalias_mime_type(mime);
  parents = _xdg_mime_parent_list_lookup(parent_list, umime);
...
int xdg_mime_register_reload_callback(XdgMimeCallback  callback, void *data, XdgMimeDestroy destroy) {
  ...
  if (list_el->next) list_el->next->prev = list_el;
...
const char *xdg_mime_get_icon(const char *mime) {
  xdg_mime_init();
  if (_caches) return _xdg_mime_cache_get_icon(mime);
  return _xdg_mime_icon_list_lookup(icon_list, mime);
}
const char *xdg_mime_get_generic_icon(const char *mime) {
  xdg_mime_init ();
  if (_caches) return _xdg_mime_cache_get_generic_icon(mime);
