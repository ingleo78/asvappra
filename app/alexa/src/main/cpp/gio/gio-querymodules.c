...
static void query_dir(const char *dirname) {
  ...
  while((name = g_dir_read_name(dir))) {
      ...
      if (!is_valid_module_name(name)) continue;
      ...
      if (module) {
          ...
          if (query) {
              ...
              if (extension_points) {
                  ...
                  for (i = 0; extension_points[i] != NULL; i++) g_string_append_printf(data, "%s%s", i == 0 ? "" : ",", extension_points[i]);
...
int main(gint argc, gchar *argv[]) {
  ...
  for (i = 1; i < argc; i++) query_dir(argv[i]);
  return 0;
}