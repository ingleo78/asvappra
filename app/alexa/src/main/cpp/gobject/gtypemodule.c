...
static void g_type_module_complete_type_info(GTypePlugin *plugin, GType g_type, GTypeInfo *info, GTypeValueTable *value_table);
static void g_type_module_complete_interface_info (GTypePlugin *plugin, GType instance_type, GType interface_type, GInterfaceInfo  *info);
...
GType g_type_module_get_type(void) {
  ...
  if (!type_module_type) {
      ...
      type_module_type = g_type_register_static(G_TYPE_OBJECT,g_intern_static_string("GTypeModule"), &type_module_info,
...
static ModuleTypeInfo* g_type_module_find_type_info(GTypeModule *module, GType type) {
  GSList *tmp_list = module->type_infos;
  while(tmp_list) {
      ModuleTypeInfo *type_info = tmp_list->data;
      if (type_info->type == type) return type_info;
      tmp_list = tmp_list->next;
  }
  return NULL;
}
static ModuleInterfaceInfo *g_type_module_find_interface_info(GTypeModule *module, GType instance_type, GType interface_type) {
  GSList *tmp_list = module->interface_infos;
  while(tmp_list) {
      ModuleInterfaceInfo *interface_info = tmp_list->data;
      if (interface_info->instance_type == instance_type && interface_info->interface_type == interface_type) return interface_info;
      tmp_list = tmp_list->next;
  }
  return NULL;
}
gboolean g_type_module_use(GTypeModule *module) {
  ...
  if (module->use_count == 1) {
      ...
      while(tmp_list) {
          ModuleTypeInfo *type_info = tmp_list->data;
          if (!type_info->loaded) {
              g_warning("plugin '%s' failed to register type '%s'\n", module->name ? module->name : "(unknown)", g_type_name(type_info->type));
              module->use_count--;
              return FALSE;
          }
          tmp_list = tmp_list->next;
...
static void g_type_module_complete_type_info(GTypePlugin *plugin, GType g_type, GTypeInfo *info, GTypeValueTable *value_table) {
  ...
  *info = module_type_info->info;
  if (module_type_info->info.value_table) *value_table = *module_type_info->info.value_table;
}
static void g_type_module_complete_interface_info(GTypePlugin *plugin, GType instance_type, GType interface_type, GInterfaceInfo *info) {
  GTypeModule *module = G_TYPE_MODULE(plugin);
...
GType g_type_module_register_type(GTypeModule *module, GType parent_type, const gchar *type_name, const GTypeInfo *type_info, GTypeFlags flags) {
  ...
  if (type) {
      ...
      if (module_type_info->parent_type != parent_type) {
          ...
          g_warning("Type '%s' recreated with different parent type.\n(was '%s', now '%s')", type_name, g_type_name (module_type_info->parent_type),
                    parent_type_name ? parent_type_name : "(unknown)");
          return 0;
	  }
      if (module_type_info->info.value_table) g_free ((GTypeValueTable *) module_type_info->info.value_table);
  } else {
  ...
  if (type_info->value_table) module_type_info->info.value_table = g_memdup(type_info->value_table, sizeof(GTypeValueTable));
...
void g_type_module_add_interface(GTypeModule *module, GType instance_type, GType interface_type, const GInterfaceInfo *interface_info) {
  ...
  if (g_type_is_a(instance_type, interface_type)) {
      GTypePlugin *old_plugin = g_type_interface_get_plugin(instance_type, interface_type);
      if (!old_plugin) {
          g_warning("Interface '%s' for '%s' was previously registered statically or for a parent type.", g_type_name(interface_type),
                    g_type_name(instance_type));
          return;
	  } else if (old_plugin != G_TYPE_PLUGIN(module)) {
          g_warning("Two different plugins tried to register interface '%s' for '%s'.", g_type_name(interface_type), g_type_name(instance_type));
          return;
	  }
...
GType g_type_module_register_enum(GTypeModule *module, const gchar *name, const GEnumValue *const_static_values) {
  ...
  g_enum_complete_type_info(G_TYPE_ENUM, &enum_type_info, const_static_values);
  return g_type_module_register_type(G_TYPE_MODULE(module), G_TYPE_ENUM, name, &enum_type_info, 0);
}
GType g_type_module_register_flags(GTypeModule *module, const gchar *name, const GFlagsValue *const_static_values) {
  ...
  g_return_val_if_fail(name != NULL, 0);
  g_return_val_if_fail(const_static_values != NULL, 0);
  g_flags_complete_type_info(G_TYPE_FLAGS, &flags_type_info, const_static_values);
  return g_type_module_register_type(G_TYPE_MODULE(module), G_TYPE_FLAGS, name, &flags_type_info, 0);
}