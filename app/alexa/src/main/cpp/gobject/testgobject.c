...
struct _TestIfaceClass {
  ...
  void	(*print_string)(TestIface *tiobj, const gchar *string);
};
static void	iface_base_init(TestIfaceClass *iface);
static void	iface_base_finalize(TestIfaceClass *iface);
static void	print_foo(TestIface	*tiobj, const gchar	*string);
static GType test_iface_get_type(void) {
  ...
  if (!test_iface_type) {
      static const GTypeInfo test_iface_info = {
          ...
          (GBaseInitFunc)iface_base_init,
          (GBaseFinalizeFunc)iface_base_finalize
      };
...
static void test_object_test_iface_init(gpointer giface, gpointer iface_data) {
...
static void iface_print_string(TestIface *tiobj, const gchar *string) {
  TestIfaceClass *iface;
  g_return_if_fail(TEST_IS_IFACE(tiobj));
  g_return_if_fail(G_IS_OBJECT(tiobj));
...
struct _TestObjectClass {
  ...
  gchar* (*test_signal)(TestObject *tobject, TestIface *iface_object, gpointer tdata);
};
...
static gboolean	test_signal_accumulator(GSignalInvocationHint *ihint, GValue *return_accu, const GValue *handler_return, gpointer data);
static gchar* test_object_test_signal(TestObject *tobject, TestIface *iface_object, gpointer tdata);
...
static void test_object_class_init(TestObjectClass *class) {
  ...
  g_signal_new("test-signal", G_OBJECT_CLASS_TYPE(class), G_SIGNAL_RUN_FIRST | G_SIGNAL_RUN_LAST | G_SIGNAL_RUN_CLEANUP,
		       G_STRUCT_OFFSET(TestObjectClass, test_signal), test_signal_accumulator, NULL, g_cclosure_marshal_STRING__OBJECT_POINTER,
...
static void test_object_check_private_init(TestObject *tobject) {
  TestObjectPrivate *priv;
  priv = TEST_OBJECT_GET_PRIVATE(tobject);
...
static gboolean test_signal_accumulator(GSignalInvocationHint *ihint, GValue *return_accu, const GValue *handler_return, gpointer data) {
  ...
  gchar *result_string;
  if (accu_string) result_string = g_strconcat(accu_string, new_string, NULL);
  else if (new_string) result_string = g_strdup(new_string);
  else result_string = NULL;
...
static void print_bar(TestIface *tiobj, const gchar *string) {
...
static void derived_object_test_iface_init(gpointer giface, gpointer iface_data) {
  TestIfaceClass *iface = giface;
...
GType derived_object_get_type(void) {
