...
static void	value_flags_enum_copy_value(const GValue *src_value, GValue *dest_value);
static gchar* value_flags_enum_collect_value(GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags);
static gchar* value_flags_enum_lcopy_value(const GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags);
void g_enum_types_init (void) {
    ...
    type = g_type_register_fundamental(G_TYPE_ENUM, g_intern_static_string("GEnum"), &info, &finfo,G_TYPE_FLAG_ABSTRACT |
                                       G_TYPE_FLAG_VALUE_ABSTRACT);
    g_assert (type == G_TYPE_ENUM);
    info.class_size = sizeof (GFlagsClass);
    type = g_type_register_fundamental(G_TYPE_FLAGS, g_intern_static_string("GFlags"), &info, &finfo,G_TYPE_FLAG_ABSTRACT |
                                       G_TYPE_FLAG_VALUE_ABSTRACT);
...
GEnumValue* g_enum_get_value_by_name(GEnumClass  *enum_class, const gchar *name) {
    ...
    if (enum_class->n_values) {
	    ...
	    if (strcmp(name, enum_value->value_name) == 0) return enum_value;
    }
    return NULL;
}
GFlagsValue* g_flags_get_value_by_name(GFlagsClass *flags_class, const gchar *name) {
    ...
    if (flags_class->n_values) {
	    ...
	    if (strcmp(name, flags_value->value_name) == 0) return flags_value;
    }
    return NULL;
}
GEnumValue* g_enum_get_value_by_nick(GEnumClass  *enum_class, const gchar *nick) {
    ...
    if (enum_class->n_values) {
        GEnumValue *enum_value;
        for (enum_value = enum_class->values; enum_value->value_name; enum_value++)
	    if (enum_value->value_nick && strcmp(nick, enum_value->value_nick) == 0) return enum_value;
    }
    return NULL;
}
GFlagsValue* g_flags_get_value_by_nick(GFlagsClass *flags_class, const gchar *nick) {
    ...
    if (flags_class->n_values) {
	    ...
	    if (flags_value->value_nick && strcmp(nick, flags_value->value_nick) == 0) return flags_value;
    }
    return NULL;
}
GEnumValue* g_enum_get_value(GEnumClass *enum_class, gint value) {
    ...
    if (enum_class->n_values) {
        GEnumValue *enum_value;
        for (enum_value = enum_class->values; enum_value->value_name; enum_value++)
	    if (enum_value->value == value) return enum_value;
    }
    return NULL;
}
GFlagsValue* g_flags_get_first_value(GFlagsClass *flags_class, guint value) {
    ...
    if (flags_class->n_values) {
        ...
        if (value == 0) {
            for (flags_value = flags_class->values; flags_value->value_name; flags_value++)
                if (flags_value->value == 0) return flags_value;
        } else {
            for (flags_value = flags_class->values; flags_value->value_name; flags_value++)
                if (flags_value->value != 0 && (flags_value->value & value) == flags_value->value) return flags_value;
        }      
    }
    return NULL;
}
