...
static void	value_param_transform_value(const GValue *src_value, GValue *dest_value);
static gpointer	value_param_peek_pointer(const GValue *value);
static gchar* value_param_collect_value(GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags);
static gchar* value_param_lcopy_value(const GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags);
...
static void g_param_spec_finalize(GParamSpec *pspec) {
    ...
    if (!(pspec->flags & G_PARAM_STATIC_NAME)) g_free(pspec->name);
    if (!(pspec->flags & G_PARAM_STATIC_NICK)) g_free(pspec->_nick);
    if (!(pspec->flags & G_PARAM_STATIC_BLURB)) g_free(pspec->_blurb);
...
void g_param_spec_sink (GParamSpec *pspec) {
    ...
    } while(!g_atomic_pointer_compare_and_exchange((void**)&pspec->qdata, oldvalue,(gpointer)((gsize)oldvalue & ~(gsize)PARAM_FLOATING_FLAG)));
    if ((gsize)oldvalue & PARAM_FLOATING_FLAG)g_param_spec_unref(pspec);
}
...
G_CONST_RETURN gchar* g_param_spec_get_nick (GParamSpec *pspec) {
    g_return_val_if_fail(G_IS_PARAM_SPEC(pspec), NULL);
    if (pspec->_nick) return pspec->_nick;
    else {
        GParamSpec *redirect_target;
        redirect_target = g_param_spec_get_redirect_target(pspec);
        if (redirect_target && redirect_target->_nick) return redirect_target->_nick;
    }
    return pspec->name;
}
G_CONST_RETURN gchar* g_param_spec_get_blurb(GParamSpec *pspec) {
    g_return_val_if_fail(G_IS_PARAM_SPEC(pspec), NULL);
    if (pspec->_blurb) return pspec->_blurb;
    else {
        GParamSpec *redirect_target;
        redirect_target = g_param_spec_get_redirect_target(pspec);
        if (redirect_target && redirect_target->_blurb) return redirect_target->_blurb;
    }
    return NULL;
}
static void canonicalize_key(gchar *key) {
    ...
    for (p = key; *p != 0; p++) {
        gchar c = *p;
        if (c != '-' && (c < '0' || c > '9') && (c < 'A' || c > 'Z') && (c < 'a' || c > 'z')) *p = '-';
    }
}
...
gpointer g_param_spec_internal(GType param_type, const gchar *name, const gchar *nick, const gchar *blurb, GParamFlags flags) {
  ...
  if (flags & G_PARAM_STATIC_NAME) {
      ...
      if (!is_canonical(pspec->name)) g_warning("G_PARAM_STATIC_NAME used with non-canonical pspec name: %s", pspec->name);
  } else {
      pspec->name = g_strdup(name);
      canonicalize_key(pspec->name);
      g_intern_string(pspec->name);
  }
  if (flags & G_PARAM_STATIC_NICK) pspec->_nick = (gchar*)nick;
  else pspec->_nick = g_strdup(nick);
  if (flags & G_PARAM_STATIC_BLURB) pspec->_blurb = (gchar*)blurb;
  else pspec->_blurb = g_strdup(blurb);
...
gboolean g_param_value_validate(GParamSpec *pspec, GValue *value) {
    ...
    if (G_PARAM_SPEC_GET_CLASS(pspec)->value_validate) {
        GValue oval = *value;
        if (G_PARAM_SPEC_GET_CLASS(pspec)->value_validate(pspec, value) || memcmp(&oval.data, &value->data, sizeof(oval.data))) return TRUE;
    }
    return FALSE;
}
gboolean g_param_value_convert(GParamSpec *pspec, const GValue *src_value, GValue *dest_value, gboolean	strict_validation) {
    ...
    if (g_value_transform(src_value, &tmp_value) && (!g_param_value_validate(pspec, &tmp_value) || !strict_validation)) {
...
static void value_param_free_value(GValue *value) {
    if (value->data[0].v_pointer) g_param_spec_unref(value->data[0].v_pointer);
}
static void value_param_copy_value(const GValue *src_value, GValue *dest_value) {
    if (src_value->data[0].v_pointer) dest_value->data[0].v_pointer = g_param_spec_ref(src_value->data[0].v_pointer);
    else dest_value->data[0].v_pointer = NULL;
}
static void value_param_transform_value(const GValue *src_value, GValue *dest_value) {
    if (src_value->data[0].v_pointer && g_type_is_a(G_PARAM_SPEC_TYPE(dest_value->data[0].v_pointer), G_VALUE_TYPE(dest_value))) {
...
static gchar* value_param_collect_value(GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags) {
    if (collect_values[0].v_pointer) {
        ...
        if (param->g_type_instance.g_class == NULL) {
            return g_strconcat("invalid unclassed param spec pointer for value type `", G_VALUE_TYPE_NAME(value), "'", NULL);
        } else if (!g_value_type_compatible(G_PARAM_SPEC_TYPE(param), G_VALUE_TYPE(value))) {
            return g_strconcat("invalid param spec type `", G_PARAM_SPEC_TYPE_NAME(param), "' for value type `", G_VALUE_TYPE_NAME(value), "'", NULL);
        }
...
static gchar* value_param_lcopy_value(const GValue *value, guint n_collect_values, GTypeCValue *collect_values, guint collect_flags) {
    ...
    if (!param_p) return g_strdup_printf("value location for `%s' passed as NULL", G_VALUE_TYPE_NAME(value));
    if (!value->data[0].v_pointer) *param_p = NULL;
    else if (collect_flags & G_VALUE_NOCOPY_CONTENTS) *param_p = value->data[0].v_pointer;
...
static guint param_spec_pool_hash(gconstpointer key_spec) {
    ...
    for (p = key->name; *p; p++) h = (h << 5) - h + *p;
    return h;
}
static gboolean param_spec_pool_equals(gconstpointer key_spec_1, gconstpointer key_spec_2) {
    const GParamSpec *key1 = key_spec_1;
    const GParamSpec *key2 = key_spec_2;
    return (key1->owner_type == key2->owner_type && strcmp(key1->name, key2->name) == 0);
}
...
void g_param_spec_pool_remove(GParamSpecPool *pool, GParamSpec *pspec) {
    if (pool && pspec) {
        G_SLOCK(&pool->smutex);
        if (g_hash_table_remove(pool->hash_table, pspec)) g_param_spec_unref(pspec);
        else g_warning(G_STRLOC ": attempt to remove unknown pspec `%s' from pool", pspec->name);
        G_SUNLOCK(&pool->smutex);
    } else {
        g_return_if_fail(pool != NULL);
        g_return_if_fail(pspec);
...
static inline GParamSpec* param_spec_ht_lookup(GHashTable *hash_table, const gchar *param_name, GType owner_type, gboolean walk_ancestors) {
    ...
    if (walk_ancestors)
        do {
            ...
            if (pspec) return pspec;
            key.owner_type = g_type_parent(key.owner_type);
        } while(key.owner_type);
    else pspec = g_hash_table_lookup(hash_table, &key);
...
static void pool_list(gpointer key, gpointer value, gpointer user_data) {
    ...
    if (owner_type == pspec->owner_type)data[0] = g_list_prepend(data[0], pspec);
}
...
static gint pspec_compare_id(gconstpointer a, gconstpointer b) {
    ...
    if (pspec1->param_id < pspec2->param_id) return -1;
    if (pspec1->param_id > pspec2->param_id) return 1;
    return strcmp(pspec1->name, pspec2->name);
}
static inline GSList* pspec_list_remove_overridden_and_redirected(GSList *plist, GHashTable *ht, GType owner_type, guint *n_p) {
    GSList *rlist = NULL;
    while(plist) {
...
static void pool_depth_list(gpointer key, gpointer value, gpointer user_data) {
    ...
    if (g_type_is_a(owner_type, pspec->owner_type)) {
        if (G_TYPE_IS_INTERFACE(pspec->owner_type))slists[0] = g_slist_prepend(slists[0], pspec);
        else {
...
static void pool_depth_list_for_interface(gpointer key, gpointer value, gpointer user_data) {
    GParamSpec *pspec = value;
    gpointer *data = user_data;
    GSList **slists = data[0];
    GType owner_type = (GType)data[1];
    if (pspec->owner_type == owner_type) slists[0] = g_slist_prepend(slists[0], pspec);
}
GParamSpec** g_param_spec_pool_list(GParamSpecPool *pool,GType owner_type, guint *n_pspecs_p) {
    ...
    *n_pspecs_p = 0;
    ...
    data[0] = slists;
    data[1] = (gpointer)owner_type;
    g_hash_table_foreach(pool->hash_table,G_TYPE_IS_INTERFACE(owner_type) ? pool_depth_list_for_interface : pool_depth_list, &data);
    for (i = 0; i < d; i++) slists[i] = pspec_list_remove_overridden_and_redirected(slists[i], pool->hash_table, owner_type, n_pspecs_p);
    pspecs = g_new(GParamSpec*, *n_pspecs_p + 1);
    p = pspecs;
    for (i = 0; i < d; i++) {
        slists[i] = g_slist_sort(slists[i], pspec_compare_id);
        for (node = slists[i]; node; node = node->next) *p++ = node->data;
...
static void param_spec_generic_class_init(gpointer g_class, gpointer class_data) {
    ...
    if (info->finalize) class->finalize = info->finalize;
    class->value_set_default = info->value_set_default;
    if (info->value_validate) class->value_validate = info->value_validate;
    class->values_cmp = info->values_cmp;
    g_free (class_data);
}
static void default_value_set_default(GParamSpec *pspec, GValue *value) {}
static gint default_values_cmp(GParamSpec *pspec, const GValue *value1, const GValue *value2) {
...
void g_value_set_param(GValue *value, GParamSpec *param) {
    ...
    if (param) g_return_if_fail(G_IS_PARAM_SPEC (param));
    if (value->data[0].v_pointer) g_param_spec_unref(value->data[0].v_pointer);
    value->data[0].v_pointer = param;
    if (value->data[0].v_pointer) g_param_spec_ref(value->data[0].v_pointer);
}
