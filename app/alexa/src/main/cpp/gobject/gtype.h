...
GType type_name##_get_type(void) { \
    ...
    if (g_once_init_enter(&g_define_type_id__volatile)) { \
        GType g_define_type_id = g_type_register_static_simple(TYPE_PARENT, g_intern_static_string(#TypeName), sizeof(TypeName##Class), \
                                                               (GClassInitFunc)type_name##_class_intern_init, sizeof(TypeName), \
                                                               (GInstanceInitFunc)type_name##_init, (GTypeFlags)flags); \
        {
...
GType type_name##_get_type(void) { \
    static volatile gsize g_define_type_id__volatile = 0; \
    if (g_once_init_enter(&g_define_type_id__volatile)) { \
        GType g_define_type_id = g_type_register_static_simple(G_TYPE_INTERFACE, g_intern_static_string(#TypeName), sizeof(TypeName##Interface), \
                                                               (GClassInitFunc)type_name##_default_init, 0, (GInstanceInitFunc)NULL, (GTypeFlags)0); \
        if (TYPE_PREREQ) g_type_interface_add_prerequisite(g_define_type_id, TYPE_PREREQ); \
        {
...
GType type_name##_get_type(void) { \
    ...
    if (g_once_init_enter(&g_define_type_id__volatile)) { \
        ...
        {
#else
#define _GDEFINE_BOXED_TYPE_BEGIN(TypeName, type_name, copy_func, free_func) \
GType type_name##get_type(void) { \
    static volatile gsize g_define_type_id__volatile = 0; \
    if (g_once_init_enter(&g_define_type_id__volatile)) { \
        GType g_define_type_id = g_boxed_type_register_static(g_intern_static_string(#TypeName), (GBoxedCopyFunc)copy_func, \
...
#define _G_TYPE_CVH(vl, gt)  (G_GNUC_EXTENSION ({ \
    ...
    if (!__val) __r = FALSE; \
    else if (__val->g_type == __t) __r = TRUE; \
    else  __r = g_type_check_value_holds (__val, __t); \
    __r; }))
#else
