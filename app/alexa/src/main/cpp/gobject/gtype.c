...
#define	INVALID_RECURSION(func, arg, type_name) G_STMT_START{ \
    ...
    if (_arg) g_error ("%s(%p)%s`%s'", _fname, _arg, _action, _tname); \
    else g_error ("%s()%s`%s'", _fname, _action, _tname); \
}G_STMT_END
#define g_return_val_if_type_system_uninitialized(return_value) G_STMT_START{ \
    if (G_UNLIKELY (!static_quark_type_flags)) { \
        g_log (G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL,"%s: You forgot to call g_type_init()", G_STRLOC); \
        return (return_value); \
    } \
}G_STMT_END
...
#endif
#define TYPE_FUNDAMENTAL_FLAG_MASK (G_TYPE_FLAG_CLASSED | G_TYPE_FLAG_INSTANTIATABLE | G_TYPE_FLAG_DERIVABLE | G_TYPE_FLAG_DEEP_DERIVABLE)
...
static IFaceHolder*	type_iface_peek_holder_L(TypeNode *iface, GType	instance_type);
...
#define NODE_IS_ANCESTOR(ancestor, node) ((ancestor)->n_supers <= (node)->n_supers && \
...
static inline TypeNode* lookup_type_node_I(register GType utype) {
  if (utype > G_TYPE_FUNDAMENTAL_MAX) return (TypeNode*) (utype & ~TYPE_ID_MASK);
  else return static_fundamental_type_nodes[utype >> G_TYPE_FUNDAMENTAL_SHIFT];
}
static TypeNode* type_node_any_new_W(TypeNode *pnode, GType ftype, const gchar *name, GTypePlugin *plugin, GTypeFundamentalFlags type_flags) {
  ...
  if (!pnode) node_size += SIZEOF_FUNDAMENTAL_INFO;
  ...
  if (!pnode) {
	  ...
	  } else _g_atomic_array_init (CLASSED_NODE_IFACES_ENTRIES (node));
  } else {
      node->supers[0] = type;
    ...
    g_hash_table_insert(static_type_nodes_ht, GUINT_TO_POINTER(node->qname), (gpointer)type);
    return node;
}
static inline GTypeFundamentalInfo*
type_node_fundamental_info_I (TypeNode *node) {
  GType ftype = NODE_FUNDAMENTAL_TYPE (node);
  if (ftype != NODE_TYPE (node)) node = lookup_type_node_I (ftype);
  return node ? G_STRUCT_MEMBER_P (node, -SIZEOF_FUNDAMENTAL_INFO) : NULL;
}
static TypeNode* type_node_fundamental_new_W (GType ftype, const gchar *name, GTypeFundamentalFlags type_flags) {
  GTypeFundamentalInfo *finfo;
  TypeNode *node;
  g_assert ((ftype & TYPE_ID_MASK) == 0);
  g_assert (ftype <= G_TYPE_FUNDAMENTAL_MAX);
  if (ftype >> G_TYPE_FUNDAMENTAL_SHIFT == static_fundamental_next) static_fundamental_next++;
...
static inline IFaceEntry* lookup_iface_entry_I(volatile IFaceEntries *entries, TypeNode *iface_node) {
  guint8 *offsets;
  guint offset_index;
  IFaceEntry *check;
  int index;
  ...
  G_ATOMIC_ARRAY_DO_TRANSACTION(
     &iface_node->_prot.offsets, guint8,entry = NULL;
	 ...
	 offset_index < G_ATOMIC_ARRAY_DATA_SIZE(offsets))
     {
         ...
         if (index > 0) {
             ...
             if (index < IFACE_ENTRIES_N_ENTRIES (entries)) {
                 ...
                 if (check->iface_type == NODE_TYPE(iface_node)) entry = check;
             }
         }
     });
 return entry;
...
static inline IFaceEntry* type_lookup_iface_entry_L(TypeNode *node, TypeNode *iface_node) {
  if (!NODE_IS_IFACE (iface_node)) return NULL;
  return lookup_iface_entry_I (CLASSED_NODE_IFACES_ENTRIES_LOCKED (node), iface_node);
}
static inline gboolean type_lookup_iface_vtable_I(TypeNode *node, TypeNode *iface_node, gpointer *vtable_ptr) {
  IFaceEntry *entry;
  gboolean res;
  if (!NODE_IS_IFACE (iface_node)) {
      if (vtable_ptr) *vtable_ptr = NULL;
      return FALSE;
  }
  G_ATOMIC_ARRAY_DO_TRANSACTION(
     CLASSED_NODE_IFACES_ENTRIES (node), IFaceEntries,entry = lookup_iface_entry_I (transaction_data, iface_node);
     res = entry != NULL;
     if (vtable_ptr) {
         if (entry) *vtable_ptr = entry->vtable;
         else *vtable_ptr = NULL;
     });
  return res;
}
static inline gboolean type_lookup_prerequisite_L (TypeNode *iface, GType prerequisite_type) {
  if (NODE_IS_IFACE (iface) && IFACE_NODE_N_PREREQUISITES (iface)) {
      ...
      do {
          ...
          if (prerequisite_type == *check) return TRUE;
          else if (prerequisite_type > *check)  {
              n_prerequisites -= i;
              prerequisites = check;
          } else n_prerequisites = i - 1;
...
static gboolean check_plugin_U(GTypePlugin *plugin, gboolean need_complete_type_info, gboolean need_complete_interface_info, const gchar *type_name) {
  if (!plugin) {
      g_warning ("plugin handle for type `%s' is NULL", type_name);
      return FALSE;
  }
  if (!G_IS_TYPE_PLUGIN (plugin)) {
      g_warning ("plugin pointer (%p) for type `%s' is invalid", plugin, type_name);
      return FALSE;
  }
  if (need_complete_type_info && !G_TYPE_PLUGIN_GET_CLASS (plugin)->complete_type_info) {
      g_warning ("plugin for type `%s' has no complete_type_info() implementation", type_name);
      return FALSE;
  }
  if (need_complete_interface_info && !G_TYPE_PLUGIN_GET_CLASS (plugin)->complete_interface_info) {
      g_warning ("plugin for type `%s' has no complete_interface_info() implementation", type_name);
      return FALSE;
  }
  return TRUE;
}
...
static gboolean check_derivation_I(GType parent_type, const gchar *type_name) {
  ...
  if (!pnode) {
      g_warning ("cannot derive type `%s' from invalid parent type `%s'", type_name, type_descriptive_name_I (parent_type));
      return FALSE;
  }
  finfo = type_node_fundamental_info_I (pnode);
  if (!(finfo->type_flags & G_TYPE_FLAG_DERIVABLE)) {
      g_warning ("cannot derive `%s' from non-derivable parent type `%s'", type_name, NODE_NAME (pnode));
      return FALSE;
  }
  if (parent_type != NODE_FUNDAMENTAL_TYPE (pnode) && !(finfo->type_flags & G_TYPE_FLAG_DEEP_DERIVABLE)) {
      g_warning ("cannot derive `%s' from non-fundamental parent type `%s'", type_name, NODE_NAME (pnode));
      return FALSE;
  }
  return TRUE;
}
static gboolean check_collect_format_I(const gchar *collect_format) {
  ...
  gchar valid_format[] = { G_VALUE_COLLECT_INT, G_VALUE_COLLECT_LONG, G_VALUE_COLLECT_INT64, G_VALUE_COLLECT_DOUBLE, G_VALUE_COLLECT_POINTER, 0 };
  while (*p) if (!strchr (valid_format, *p++)) return FALSE;
  return p - collect_format <= G_VALUE_COLLECT_FORMAT_MAX_LENGTH;
}
static gboolean check_value_table_I(const gchar *type_name, const GTypeValueTable *value_table) {
  if (!value_table) return FALSE;
  else if (value_table->value_init == NULL) {
      if (value_table->value_free || value_table->value_copy || value_table->value_peek_pointer || value_table->collect_format || value_table->collect_value ||
	      value_table->lcopy_format || value_table->lcopy_value) {
          g_warning ("cannot handle uninitializable values of type `%s'", type_name);
      }
      return FALSE;
  } else {
      if (!value_table->value_free) return FALSE;
      ...
      if ((value_table->collect_format || value_table->collect_value) && (!value_table->collect_format || !value_table->collect_value)) {
          g_warning ("one of `collect_format' and `collect_value()' is unspecified for type `%s'", type_name);
          return FALSE;
	  }
      if (value_table->collect_format && !check_collect_format_I (value_table->collect_format)) {
          g_warning ("the `%s' specification for type `%s' is too long or invalid", "collect_format",type_name);
          return FALSE;
	  }
      if ((value_table->lcopy_format || value_table->lcopy_value) && (!value_table->lcopy_format || !value_table->lcopy_value)) {
          g_warning ("one of `lcopy_format' and `lcopy_value()' is unspecified for type `%s'", type_name);
          return FALSE;
	  }
      if (value_table->lcopy_format && !check_collect_format_I (value_table->lcopy_format)) {
          g_warning ("the `%s' specification for type `%s' is too long or invalid", "lcopy_format", type_name);
          return FALSE;
	  }
  }
  return TRUE;
...
static gboolean check_type_info_I(TypeNode *pnode, GType ftype, const gchar *type_name, const GTypeInfo *info) {
  ...
  if (!(finfo->type_flags & G_TYPE_FLAG_INSTANTIATABLE) && (info->instance_size || info->n_preallocs || info->instance_init)) {
      if (pnode) g_warning ("cannot instantiate `%s', derived from non-instantiatable parent type `%s'", type_name, NODE_NAME (pnode));
      else g_warning ("cannot instantiate `%s' as non-instantiatable fundamental", type_name);
      return FALSE;
  }
  if (!((finfo->type_flags & G_TYPE_FLAG_CLASSED) || is_interface) && (info->class_init || info->class_finalize || info->class_data ||
       info->class_size || info->base_init || info->base_finalize)) {
      if (pnode) g_warning ("cannot create class for `%s', derived from non-classed parent type `%s'", type_name, NODE_NAME (pnode));
      else g_warning ("cannot create class for `%s' as non-classed fundamental", type_name);
      return FALSE;
  }
  if (is_interface && info->class_size < sizeof (GTypeInterface)) {
      g_warning ("specified interface size for type `%s' is smaller than `GTypeInterface' size", type_name);
      return FALSE;
  }
  if (finfo->type_flags & G_TYPE_FLAG_CLASSED) {
      if (info->class_size < sizeof (GTypeClass)) {
          g_warning ("specified class size for type `%s' is smaller than `GTypeClass' size", type_name);
          return FALSE;
	  }
      if (pnode && info->class_size < pnode->data->class.class_size) {
          g_warning ("specified class size for type `%s' is smaller " "than the parent type's `%s' class size", type_name, NODE_NAME (pnode));
          return FALSE;
	  }
  }
  if (finfo->type_flags & G_TYPE_FLAG_INSTANTIATABLE) {
      if (info->instance_size < sizeof (GTypeInstance)) {
          g_warning ("specified instance size for type `%s' is smaller than `GTypeInstance' size", type_name);
          return FALSE;
	  }
      if (pnode && info->instance_size < pnode->data->instance.instance_size) {
          g_warning ("specified instance size for type `%s' is smaller than the parent type's `%s' instance size", type_name, NODE_NAME (pnode));
          return FALSE;
	  }
  }
  return TRUE;
...
static TypeNode* find_conforming_child_type_L(TypeNode *pnode, TypeNode *iface) {
  TypeNode *node = NULL;
  guint i;
  if (type_lookup_iface_entry_L (pnode, iface)) return pnode;
...
static gboolean check_add_interface_L(GType instance_type, GType iface_type) {
  ...
  if (!node || !node->is_instantiatable) {
      g_warning ("cannot add interfaces to invalid (non-instantiatable) type `%s'", type_descriptive_name_I (instance_type));
      return FALSE;
  }
  if (!iface || !NODE_IS_IFACE (iface)) {
      g_warning ("cannot add invalid (non-interface) type `%s' to type `%s'", type_descriptive_name_I (iface_type), NODE_NAME (node));
      return FALSE;
  }
  tnode = lookup_type_node_I (NODE_PARENT_TYPE (iface));
  if (NODE_PARENT_TYPE (tnode) && !type_lookup_iface_entry_L (node, tnode)) {
      g_warning("cannot add sub-interface `%s' to type `%s' which does not conform to super-interface `%s'", NODE_NAME (iface), NODE_NAME (node),
		        NODE_NAME (tnode));
      return FALSE;
  }
  entry = type_lookup_iface_entry_L (node, iface);
  if (entry && entry->vtable == NULL && !type_iface_peek_holder_L (iface, NODE_TYPE (node))) return TRUE;
  tnode = find_conforming_child_type_L (node, iface);  /* tnode is_a node */
  if (tnode) {
      g_warning("cannot add interface type `%s' to type `%s', since type `%s' already conforms to interface", NODE_NAME (iface), NODE_NAME (node),
		        NODE_NAME (tnode));
      return FALSE;
  }
  ...
  for (i = 0; i < IFACE_NODE_N_PREREQUISITES (iface); i++) {
      ...
      if (!type_node_is_a_L (node, tnode)) {
          g_warning("cannot add interface type `%s' to type `%s' which does not conform to prerequisite `%s'", NODE_NAME (iface), NODE_NAME (node),
                    NODE_NAME (tnode));
          return FALSE;
	  }
  }
...
static gboolean check_interface_info_I(TypeNode *iface, GType instance_type, const GInterfaceInfo *info) {
  if ((info->interface_finalize || info->interface_data) && !info->interface_init) {
      g_warning ("interface type `%s' for type `%s' comes without initializer", NODE_NAME (iface), type_descriptive_name_I (instance_type));
      return FALSE;
  }
  return TRUE;
}
static void type_data_make_W (TypeNode *node, const GTypeInfo *info, const GTypeValueTable *value_table) {
  ...
  if (!value_table) {
      ...
      if (pnode) vtable = pnode->data->common.value_table;
      else {
  ...
  if (value_table) {
      ...
      if (value_table->collect_format) vtable_size += strlen (value_table->collect_format);
      if (value_table->lcopy_format) vtable_size += strlen (value_table->lcopy_format);
      vtable_size += 2;
  }
  if (node->is_instantiatable) {
      TypeNode *pnode = lookup_type_node_I (NODE_PARENT_TYPE (node));
      data = g_malloc0 (sizeof (InstanceData) + vtable_size);
      if (vtable_size) vtable = G_STRUCT_MEMBER_P (data, sizeof (InstanceData));
      ...
      if (pnode) data->instance.class_private_size = pnode->data->instance.class_private_size;
      ...
      if (vtable_size) vtable = G_STRUCT_MEMBER_P (data, sizeof (ClassData));
      ...
      if (pnode) data->class.class_private_size = pnode->data->class.class_private_size;
      data->class.init_state = UNINITIALIZED;
  } else if (NODE_IS_IFACE (node)) {
      data = g_malloc0 (sizeof (IFaceData) + vtable_size);
      if (vtable_size) vtable = G_STRUCT_MEMBER_P (data, sizeof (IFaceData));
      ...
      if (vtable_size) vtable = G_STRUCT_MEMBER_P (data, sizeof (BoxedData));
  } else {
      data = g_malloc0 (sizeof (CommonData) + vtable_size);
      if (vtable_size) vtable = G_STRUCT_MEMBER_P (data, sizeof (CommonData));
  }
  node->data = data;
  if (vtable_size) {
      gchar *p;
  ...
  node->mutatable_check_cache = (node->data->common.value_table->value_init != NULL && !((G_TYPE_FLAG_VALUE_ABSTRACT | G_TYPE_FLAG_ABSTRACT) &
				                GPOINTER_TO_UINT(type_get_qdata_L(node, static_quark_type_flags))));
  g_assert (node->data->common.value_table != NULL);
...
static inline void type_data_ref_Wm(TypeNode *node) {
  if (!node->data) {
      ...
      if (pnode) {
          ...
          if (node->data) INVALID_RECURSION("g_type_plugin_*", node->plugin, NODE_NAME (node));
	  }
...
static gboolean iface_node_has_available_offset_L(TypeNode *iface_node, int offset, int for_index) {
  guint8 *offsets;
  offsets = G_ATOMIC_ARRAY_GET_LOCKED (&iface_node->_prot.offsets, guint8);
  if (offsets == NULL) return TRUE;
  if (G_ATOMIC_ARRAY_DATA_SIZE (offsets) <= offset) return TRUE;
  if (offsets[offset] == 0 || offsets[offset] == for_index+1) return TRUE;
  return FALSE;
}
static int find_free_iface_offset_L(IFaceEntries *entries) {
  ...
  do {
      ...
      for (i = 0; i < n_entries; i++) {
          ...
          if (!iface_node_has_available_offset_L (iface_node, offset, i)) break;
	  }
  } while (i != n_entries);
  return offset;
}
static void iface_node_set_offset_L(TypeNode *iface_node, int offset, int index) {
  ...
  if (old_offsets == NULL) old_size = 0;
  else {
      old_size = G_ATOMIC_ARRAY_DATA_SIZE (old_offsets);
      if (offset < old_size && old_offsets[offset] == index + 1) return;
  }
  new_size = MAX (old_size, offset + 1);
  offsets = _g_atomic_array_copy (&iface_node->_prot.offsets,0, new_size - old_size);
  for (i = old_size; i < new_size; i++) offsets[i] = 0;
  offsets[offset] = index + 1;
  _g_atomic_array_update (&iface_node->_prot.offsets, offsets);
}
static void type_node_add_iface_entry_W(TypeNode *node, GType iface_type, IFaceEntry *parent_entry) {
  IFaceEntries *entries;
  IFaceEntry *entry;
  TypeNode *iface_node;
  guint i, j;
  ...
  if (entries != NULL) {
      ...
      for (i = 0; i < num_entries; i++) {
          ...
          if (entry->iface_type == iface_type) {
              if (!parent_entry) g_assert (entry->vtable == NULL && entry->init_state == UNINITIALIZED);
              return;
          }
	  }
  }
  entries = _g_atomic_array_copy(CLASSED_NODE_IFACES_ENTRIES(node),IFACE_ENTRIES_HEADER_SIZE, sizeof(IFaceEntry));
  num_entries = IFACE_ENTRIES_N_ENTRIES (entries);
  i = num_entries - 1;
  if (i == 0) entries->offset_index = 0;
  ...
  if (iface_node_has_available_offset_L (iface_node, entries->offset_index, i)) iface_node_set_offset_L(iface_node, entries->offset_index, i);
  else {
  ...
  _g_atomic_array_update (CLASSED_NODE_IFACES_ENTRIES (node), entries);
  if (parent_entry) {
      for (i = 0; i < node->n_children; i++) type_node_add_iface_entry_W (lookup_type_node_I (node->children[i]), iface_type, &entries->entry[i]);
  }
}
static void type_add_interface_Wm(TypeNode *node, TypeNode *iface, const GInterfaceInfo *info, GTypePlugin *plugin) {
  ...
  if (node->data) {
      ...
      if (class_state >= BASE_IFACE_INIT) type_iface_vtable_base_init_Wm (iface, node);
      if (class_state >= IFACE_INIT) type_iface_vtable_iface_init_Wm (iface, node);
  }
  entry = type_lookup_iface_entry_L (node, iface);
  for (i = 0; i < node->n_children; i++) type_node_add_iface_entry_W (lookup_type_node_I (node->children[i]), NODE_TYPE (iface), entry);
}
static void type_iface_add_prerequisite_W(TypeNode *iface, TypeNode *prerequisite_node) {
  ...
  g_assert(NODE_IS_IFACE (iface) && IFACE_NODE_N_PREREQUISITES (iface) < MAX_N_PREREQUISITES &&
	       (prerequisite_node->is_instantiatable || NODE_IS_IFACE (prerequisite_node)));
  prerequisites = IFACE_NODE_PREREQUISITES (iface);
  for (i = 0; i < IFACE_NODE_N_PREREQUISITES (iface); i++)
    if (prerequisites[i] == prerequisite_type) return;
    else if (prerequisites[i] > prerequisite_type) break;
  IFACE_NODE_N_PREREQUISITES(iface) += 1;
  IFACE_NODE_PREREQUISITES(iface) = g_renew(GType, IFACE_NODE_PREREQUISITES (iface), IFACE_NODE_N_PREREQUISITES (iface));
  prerequisites = IFACE_NODE_PREREQUISITES(iface);
  g_memmove(prerequisites + i + 1, prerequisites + i, sizeof(prerequisites[0]) * (IFACE_NODE_N_PREREQUISITES (iface) - i - 1));
  ...
  n_dependants = dependants ? dependants[0] : 0;
  for (i = 1; i <= n_dependants; i++) type_iface_add_prerequisite_W(lookup_type_node_I (dependants[i]), prerequisite_node);
}
void g_type_interface_add_prerequisite(GType interface_type, GType prerequisite_type) {
  ...
  if (!iface || !prerequisite_node || !NODE_IS_IFACE(iface)) {
      g_warning("interface type `%s' or prerequisite type `%s' invalid", type_descriptive_name_I(interface_type), type_descriptive_name_I(prerequisite_type));
      return;
  }
  G_WRITE_LOCK(&type_rw_lock);
  ...
  if (holders) {
      G_WRITE_UNLOCK(&type_rw_lock);
      g_warning("unable to add prerequisite `%s' to interface `%s' which is already in use for `%s'", type_descriptive_name_I (prerequisite_type),
		        type_descriptive_name_I (interface_type), type_descriptive_name_I (holders->instance_type));
      return;
  }
  if (prerequisite_node->is_instantiatable) {
      ...
      for (i = 0; i < IFACE_NODE_N_PREREQUISITES(iface); i++) {
          ...
          if (prnode->is_instantiatable) {
              G_WRITE_UNLOCK(&type_rw_lock);
              g_warning("adding prerequisite `%s' to interface `%s' conflicts with existing prerequisite `%s'", type_descriptive_name_I (prerequisite_type),
                        type_descriptive_name_I(interface_type), type_descriptive_name_I (NODE_TYPE (prnode)));
              return;
          }
	  }
      for (i = 0; i < prerequisite_node->n_supers + 1; i++) type_iface_add_prerequisite_W (iface, lookup_type_node_I (prerequisite_node->supers[i]));
      G_WRITE_UNLOCK(&type_rw_lock);
  } else if (NODE_IS_IFACE(prerequisite_node)) {
      GType *prerequisites;
      guint i;
      prerequisites = IFACE_NODE_PREREQUISITES(prerequisite_node);
      for (i = 0; i < IFACE_NODE_N_PREREQUISITES(prerequisite_node); i++) type_iface_add_prerequisite_W (iface, lookup_type_node_I (prerequisites[i]));
      type_iface_add_prerequisite_W (iface, prerequisite_node);
      G_WRITE_UNLOCK(&type_rw_lock);
  } else {
      G_WRITE_UNLOCK(&type_rw_lock);
      g_warning("prerequisite `%s' for interface `%s' is neither instantiatable nor interface", type_descriptive_name_I (prerequisite_type),
...
GType* g_type_interface_prerequisites(GType interface_type, guint *n_prerequisites) {
  ...
  if(iface) {
      ...
      for (i = 0; i < IFACE_NODE_N_PREREQUISITES(iface); i++) {
          ...
          if (node->is_instantiatable) {
              if (!inode || type_node_is_a_L (node, inode)) inode = node;
          } else types[n++] = NODE_TYPE (node);
	  }
      if (inode) types[n++] = NODE_TYPE (inode);
      if (n_prerequisites) *n_prerequisites = n;
      G_READ_UNLOCK (&type_rw_lock);
      return types;
  } else {
      if (n_prerequisites) *n_prerequisites = 0;
      return NULL;
  }
}
static IFaceHolder* type_iface_peek_holder_L(TypeNode *iface, GType instance_type) {
  ...
  while(iholder && iholder->instance_type != instance_type) iholder = iholder->next;
  return iholder;
}
static IFaceHolder* type_iface_retrieve_holder_info_Wm(TypeNode *iface, GType instance_type, gboolean need_info) {
  ...
  if (iholder && !iholder->info && need_info) {
      ...
      if (iholder->info) INVALID_RECURSION("g_type_plugin_*", iface->plugin, NODE_NAME(iface));
      memset (&tmp_info, 0, sizeof (tmp_info));
      G_WRITE_UNLOCK(&type_rw_lock);
      ...
      G_WRITE_LOCK(&type_rw_lock);
      if (iholder->info) INVALID_RECURSION("g_type_plugin_*", iholder->plugin, NODE_NAME(iface));
...
static void type_iface_blow_holder_info_Wm(TypeNode *iface, GType instance_type) {
  ...
  g_assert (NODE_IS_IFACE(iface));
  while (iholder->instance_type != instance_type) iholder = iholder->next;
...
static inline size_t type_total_instance_size_I(TypeNode *node) {
  gsize total_instance_size;
  total_instance_size = node->data->instance.instance_size;
  if (node->data->instance.private_size != 0) total_instance_size = ALIGN_STRUCT(total_instance_size) + node->data->instance.private_size;
...
static gint instance_real_class_cmp(gconstpointer p1, gconstpointer p2) {
...
static inline void instance_real_class_set(gpointer instance, GTypeClass *class) {
  InstanceRealClass key;
  key.instance = instance;
  key.class = class;
  G_LOCK (instance_real_class);
  if (!instance_real_class_bsa) instance_real_class_bsa = g_bsearch_array_create (&instance_real_class_bconfig);
...
static inline void instance_real_class_remove(gpointer instance) {
...
static inline GTypeClass* instance_real_class_get(gpointer instance) {
  InstanceRealClass key, *node;
  GTypeClass *class;
  key.instance = instance;
  G_LOCK (instance_real_class);
...
GTypeInstance* g_type_create_instance(GType type) {
  ...
  if (!node || !node->is_instantiatable) {
      g_warning ("cannot create new instance of invalid (non-instantiatable) type `%s'", type_descriptive_name_I (type));
      return NULL;
  }
  if (!node->mutatable_check_cache && G_TYPE_IS_ABSTRACT (type)) {
      g_warning ("cannot create instance of abstract (non-instantiatable) type `%s'", type_descriptive_name_I (type));
      return NULL;
  }
  class = g_type_class_ref(type);
  total_size = type_total_instance_size_I(node);
  instance = g_slice_alloc0(total_size);
  if (node->data->instance.private_size) instance_real_class_set(instance, class);
  ...
  if (node->data->instance.private_size) instance_real_class_remove(instance);
  instance->g_class = class;
  if (node->data->instance.instance_init) node->data->instance.instance_init(instance, class);
...
void g_type_free_instance(GTypeInstance *instance) {
  ...
  if (!node || !node->is_instantiatable || !node->data || node->data->class.class != (gpointer) class) {
      g_warning("cannot free instance of invalid (non-instantiatable) type `%s'", type_descriptive_name_I(class->g_type));
      return;
  }
  if (!node->mutatable_check_cache && G_TYPE_IS_ABSTRACT(NODE_TYPE (node))) {
      g_warning ("cannot free instance of abstract (non-instantiatable) type `%s'", NODE_NAME(node));
      return;
  }
...
static void type_iface_ensure_dflt_vtable_Wm(TypeNode *iface) {
  ...
  if (!iface->data->iface.dflt_vtable) {
      ...
      if (iface->data->iface.vtable_init_base || iface->data->iface.dflt_init) {
          ...
          if (iface->data->iface.dflt_init) iface->data->iface.dflt_init (vtable, (gpointer) iface->data->iface.dflt_data);
          G_WRITE_LOCK (&type_rw_lock);
      }
  }
}
static gboolean type_iface_vtable_base_init_Wm(TypeNode *iface, TypeNode *node) {
  ...
  if (pnode) {
      ...
      if (pentry) vtable = g_memdup(pentry->vtable, iface->data->iface.vtable_size);
  }
  if (!vtable) vtable = g_memdup(iface->data->iface.dflt_vtable, iface->data->iface.vtable_size);
  entry->vtable = vtable;
  vtable->g_type = NODE_TYPE(iface);
...
static void type_iface_vtable_iface_init_Wm(TypeNode *iface, TypeNode *node) {
  ...
  if (iholder->info->interface_init) {
      G_WRITE_UNLOCK(&type_rw_lock);
      if (iholder->info->interface_init) iholder->info->interface_init(vtable, iholder->info->interface_data);
      G_WRITE_LOCK(&type_rw_lock);
  }
...
static gboolean type_iface_vtable_finalize_Wm(TypeNode *iface, TypeNode *node, GTypeInterface *vtable) {
  IFaceEntry *entry = type_lookup_iface_entry_L(node, iface);
  IFaceHolder *iholder;
  ...
  if (iholder->info->interface_finalize || iface->data->iface.vtable_finalize_base) {
      G_WRITE_UNLOCK(&type_rw_lock);
      if (iholder->info->interface_finalize) iholder->info->interface_finalize(vtable, iholder->info->interface_data);
      if (iface->data->iface.vtable_finalize_base) iface->data->iface.vtable_finalize_base(vtable);
      G_WRITE_LOCK(&type_rw_lock);
  }
  vtable->g_type = 0;
  vtable->g_instance_type = 0;
...
static void type_class_init_Wm(TypeNode *node, GTypeClass *pclass) {
  ...
  guint i;
  g_assert(node->is_classed && node->data && node->data->class.class_size && !node->data->class.class && node->data->class.init_state == UNINITIALIZED);
  if (node->data->class.class_private_size) class = g_malloc0(ALIGN_STRUCT(node->data->class.class_size) + node->data->class.class_private_size);
  else class = g_malloc0(node->data->class.class_size);
  ...
  if (pclass) {
      ...
      memcpy(G_STRUCT_MEMBER_P(class, ALIGN_STRUCT(node->data->class.class_size)), G_STRUCT_MEMBER_P(pclass,
             ALIGN_STRUCT(pnode->data->class.class_size)), pnode->data->class.class_private_size);
      if (node->is_instantiatable) node->data->instance.private_size = pnode->data->instance.private_size;
  }
  class->g_type = NODE_TYPE(node);
  G_WRITE_UNLOCK(&type_rw_lock);
  for (bnode = node; bnode; bnode = lookup_type_node_I(NODE_PARENT_TYPE(bnode)))
      if (bnode->data->class.class_init_base) init_slist = g_slist_prepend(init_slist, (gpointer)bnode->data->class.class_init_base);
  ...
  while((entries = CLASSED_NODE_IFACES_ENTRIES_LOCKED(node)) != NULL && i < IFACE_ENTRIES_N_ENTRIES(entries)) {
      ...
      if (i == IFACE_ENTRIES_N_ENTRIES(entries)) break;
  ...
  G_WRITE_UNLOCK(&type_rw_lock);
  if (node->data->class.class_init) node->data->class.class_init(class, (gpointer)node->data->class.class_data);
  G_WRITE_LOCK(&type_rw_lock);
...
reiterate:
  ...
  for (i = 0; entries != NULL && i < IFACE_ENTRIES_N_ENTRIES(entries); i++) {
      ...
      if (entry->vtable) {
          if (type_iface_vtable_finalize_Wm(lookup_type_node_I(entry->iface_type), node, entry->vtable)) goto reiterate;
          else {
              entry->vtable = NULL;
              entry->init_state = UNINITIALIZED;
          }
...
static void type_data_finalize_class_U(TypeNode *node, ClassData *cdata) {
  ...
  if (cdata->class_finalize) cdata->class_finalize(class, (gpointer)cdata->class_data);
  if (cdata->class_finalize_base) cdata->class_finalize_base(class);
  for (bnode = lookup_type_node_I(NODE_PARENT_TYPE(node)); bnode; bnode = lookup_type_node_I(NODE_PARENT_TYPE(bnode)))
      if (bnode->data->class.class_finalize_base) bnode->data->class.class_finalize_base(class);
  g_free(cdata->class);
}
static void type_data_last_unref_Wm(TypeNode *node, gboolean uncached) {
  g_return_if_fail(node != NULL && node->plugin != NULL);
  if (!node->data || NODE_REFCOUNT(node) == 0) {
      g_warning("cannot drop last reference to unreferenced type `%s'", NODE_NAME(node));
      return;
  }
  if (node->is_classed && node->data && node->data->class.class && static_n_class_cache_funcs && !uncached) {
      ...
      for (i = 0; i < static_n_class_cache_funcs; i++) {
          ...
          G_READ_LOCK(&type_rw_lock);
          if (!node->data || NODE_REFCOUNT(node) == 0) INVALID_RECURSION("GType class cache function ", cache_func, NODE_NAME(node));
          if (need_break) break;
	  }
      G_READ_UNLOCK(&type_rw_lock);
      G_WRITE_LOCK(&type_rw_lock);
  ...
  if (g_atomic_int_dec_and_test((int*)&node->ref_count)) {
      ...
      if (node->is_classed && tdata->class.class) {
          if (CLASSED_NODE_IFACES_ENTRIES_LOCKED(node) != NULL) type_data_finalize_class_ifaces_Wm (node);
          ...
          if (tdata->iface.dflt_finalize || tdata->iface.vtable_finalize_base) {
              G_WRITE_UNLOCK(&type_rw_lock);
              if (tdata->iface.dflt_finalize) tdata->iface.dflt_finalize(tdata->iface.dflt_vtable, (gpointer)tdata->iface.dflt_data);
              if (tdata->iface.vtable_finalize_base) tdata->iface.vtable_finalize_base(tdata->iface.dflt_vtable);
              G_WRITE_LOCK(&type_rw_lock);
          }
      ...
      if (ptype) type_data_unref_U(lookup_type_node_I (ptype), FALSE);
      G_WRITE_LOCK(&type_rw_lock);
  }
}
static inline void type_data_unref_U(TypeNode *node, gboolean  uncached) {
  ...
  do {
      ...
      if (current <= 1) {
          if (!node->plugin) {
              g_warning("static type `%s' unreferenced too often", NODE_NAME(node));
              return;
	      }
          g_assert(current > 0);
...
void g_type_remove_class_cache_func(gpointer cache_data, GTypeClassCacheFunc cache_func) {
  ...
  for (i = 0; i < static_n_class_cache_funcs; i++)
      if (static_class_cache_funcs[i].cache_data == cache_data && static_class_cache_funcs[i].cache_func == cache_func) {
          ...
          g_memmove(static_class_cache_funcs + i,static_class_cache_funcs + i + 1,sizeof(static_class_cache_funcs[0]) * (static_n_class_cache_funcs - i));
          static_class_cache_funcs = g_renew (ClassCacheFunc, static_class_cache_funcs, static_n_class_cache_funcs);
          found_it = TRUE;
          break;
      }
  G_WRITE_UNLOCK(&type_rw_lock);
  if (!found_it) g_warning(G_STRLOC ": cannot remove unregistered class cache func %p with data %p", cache_func, cache_data);
}
...
void g_type_remove_interface_check(gpointer check_data, GTypeInterfaceCheckFunc check_func) {
  ...
  for (i = 0; i < static_n_iface_check_funcs; i++)
      if (static_iface_check_funcs[i].check_data == check_data && static_iface_check_funcs[i].check_func == check_func) {
          ...
          g_memmove(static_iface_check_funcs + i,static_iface_check_funcs + i + 1,sizeof(static_iface_check_funcs[0]) * (static_n_iface_check_funcs - i));
          static_iface_check_funcs = g_renew (IFaceCheckFunc, static_iface_check_funcs, static_n_iface_check_funcs);
          found_it = TRUE;
          break;
      }
  G_WRITE_UNLOCK(&type_rw_lock);
  if (!found_it) g_warning(G_STRLOC ": cannot remove unregistered class check func %p with data %p", check_func, check_data);
}
GType g_type_register_fundamental(GType type_id, const gchar *type_name, const GTypeInfo *info, const GTypeFundamentalInfo *finfo, GTypeFlags flags) {
  ...
  if (!check_type_name_I(type_name)) return 0;
  if ((type_id & TYPE_ID_MASK) || type_id > G_TYPE_FUNDAMENTAL_MAX) {
      g_warning("attempt to register fundamental type `%s' with invalid type id (%" G_GSIZE_FORMAT ")", type_name, type_id);
      return 0;
  }
  if ((finfo->type_flags & G_TYPE_FLAG_INSTANTIATABLE) && !(finfo->type_flags & G_TYPE_FLAG_CLASSED)) {
      g_warning("cannot register instantiatable fundamental type `%s' as non-classed", type_name);
      return 0;
  }
  if (lookup_type_node_I(type_id)) {
      g_warning("cannot register existing fundamental type `%s' (as `%s')", type_descriptive_name_I(type_id), type_name);
      return 0;
  }
  G_WRITE_LOCK(&type_rw_lock);
...
GType g_type_register_static(GType parent_type, const gchar *type_name, const GTypeInfo *info, GTypeFlags flags) {
  ...
  g_return_val_if_fail(type_name != NULL, 0);
  g_return_val_if_fail(info != NULL, 0);
  if (!check_type_name_I(type_name) || !check_derivation_I(parent_type, type_name)) return 0;
...
GType g_type_register_dynamic(GType parent_type, const gchar *type_name, GTypePlugin *plugin, GTypeFlags flags) {
  ...
  if (!check_type_name_I(type_name) || !check_derivation_I (parent_type, type_name) || !check_plugin_U(plugin, TRUE, FALSE, type_name)) return 0;
  G_WRITE_LOCK(&type_rw_lock);
  pnode = lookup_type_node_I(parent_type);
...
void g_type_add_interface_static(GType instance_type, GType interface_type, const GInterfaceInfo *info) {
  ...
  if (check_add_interface_L(instance_type, interface_type)) {
      ...
      if (check_interface_info_I(iface, NODE_TYPE(node), info)) type_add_interface_Wm(node, iface, info, NULL);
  }
  G_WRITE_UNLOCK(&type_rw_lock);
  g_static_rec_mutex_unlock(&class_init_rec_mutex);
}
void g_type_add_interface_dynamic(GType instance_type, GType interface_type, GTypePlugin *plugin) {
  ...
  if (!check_plugin_U(plugin, FALSE, TRUE, NODE_NAME(node))) return;
  g_static_rec_mutex_lock (&class_init_rec_mutex);
  G_WRITE_LOCK(&type_rw_lock);
  if (check_add_interface_L(instance_type, interface_type)) {
      TypeNode *iface = lookup_type_node_I(interface_type);
...
gpointer g_type_class_ref(GType type) {
  ...
  if (!node || !node->is_classed) {
      g_warning("cannot retrieve class for invalid (unclassed) type `%s'", type_descriptive_name_I(type));
      return NULL;
  }
  if (G_LIKELY(type_data_ref_U(node))) {
      if (G_LIKELY(g_atomic_int_get(&node->data->class.init_state) == INITIALIZED)) return node->data->class.class;
      holds_ref = TRUE;
  } else holds_ref = FALSE;
  g_static_rec_mutex_lock(&class_init_rec_mutex);
  ptype = NODE_PARENT_TYPE(node);
  pclass = ptype ? g_type_class_ref(ptype) : NULL;
  G_WRITE_LOCK(&type_rw_lock);
  if (!holds_ref) type_data_ref_Wm(node);
  if (!node->data->class.class) type_class_init_Wm(node, pclass);
  G_WRITE_UNLOCK(&type_rw_lock);
...
void g_type_class_unref(gpointer g_class) {
  ...
  node = lookup_type_node_I (class->g_type);
  if (node && node->is_classed && NODE_REFCOUNT(node)) type_data_unref_U(node, FALSE);
  else g_warning("cannot unreference class of invalid (unclassed) type `%s'", type_descriptive_name_I(class->g_type));
}
void g_type_class_unref_uncached(gpointer g_class) {
  ...
  g_return_if_fail(g_class != NULL);
  node = lookup_type_node_I(class->g_type);
  if (node && node->is_classed && NODE_REFCOUNT(node)) type_data_unref_U(node, TRUE);
  else g_warning("cannot unreference class of invalid (unclassed) type `%s'", type_descriptive_name_I(class->g_type));
}
gpointer g_type_class_peek(GType type) {
  ...
  gpointer class;
  node = lookup_type_node_I(type);
  if (node && node->is_classed && NODE_REFCOUNT(node) && g_atomic_int_get(&node->data->class.init_state) == INITIALIZED) class = node->data->class.class;
  else class = NULL;
  return class;
}
gpointer g_type_class_peek_static(GType type) {
  ...
  gpointer class;
  node = lookup_type_node_I(type);
  if (node && node->is_classed && NODE_REFCOUNT(node) && node->plugin == NULL && g_atomic_int_get(&node->data->class.init_state) == INITIALIZED) {
...
gpointer g_type_class_peek_parent(gpointer g_class) {
  ...
  if (node && node->is_classed && node->data && NODE_PARENT_TYPE(node)) {
      ...
      class = node->data->class.class;
  } else if (NODE_PARENT_TYPE(node)) g_warning(G_STRLOC ": invalid class pointer `%p'", g_class);
  return class;
}
gpointer g_type_interface_peek(gpointer instance_class, GType iface_type) {
  ...
  if (node && node->is_instantiatable && iface) type_lookup_iface_vtable_I(node, iface, &vtable);
  else g_warning(G_STRLOC ": invalid class pointer `%p'", class);
  return vtable;
}
gpointer g_type_interface_peek_parent(gpointer g_iface) {
  ...
  if (node) node = lookup_type_node_I(NODE_PARENT_TYPE(node));
  if (node && node->is_instantiatable && iface) type_lookup_iface_vtable_I(node, iface, &vtable);
  else if (node) g_warning(G_STRLOC ": invalid interface pointer `%p'", g_iface);
  return vtable;
}
gpointer g_type_default_interface_ref(GType g_type) {
  ...
  if (!node || !NODE_IS_IFACE(node) || (node->data && NODE_REFCOUNT(node) == 0)) {
      G_WRITE_UNLOCK(&type_rw_lock);
      g_warning("cannot retrieve default vtable for invalid or non-interface type '%s'", type_descriptive_name_I(g_type));
      return NULL;
  }
...
gpointer g_type_default_interface_peek(GType g_type) {
  ...
  gpointer vtable;
  node = lookup_type_node_I(g_type);
  if (node && NODE_IS_IFACE(node) && NODE_REFCOUNT(node)) vtable = node->data->iface.dflt_vtable;
  else vtable = NULL;
  return vtable;
}
void g_type_default_interface_unref(gpointer g_iface) {
  ...
  else g_warning("cannot unreference invalid interface default vtable for '%s'", type_descriptive_name_I(vtable->g_type));
}
...
GType g_type_next_base(GType type, GType base_type) {
  ...
  if (node) {
      ...
      if (base_node && base_node->n_supers < node->n_supers) {
          ...
          if (node->supers[n] == base_type) atype = node->supers[n - 1];
	  }
  }
  return atype;
}
static inline gboolean type_node_check_conformities_UorL(TypeNode *node, TypeNode *iface_node, gboolean support_interfaces, gboolean support_prerequisites,
                                                         gboolean have_lock) {
  gboolean match;
  if (NODE_IS_ANCESTOR(iface_node, node)) return TRUE;
  ...
  if (support_interfaces) {
      if (have_lock) {
	      if (type_lookup_iface_entry_L(node, iface_node)) match = TRUE;
	  } else {
	  if (type_lookup_iface_vtable_I(node, iface_node, NULL)) match = TRUE;
	  }
  }
  if (!match && support_prerequisites) {
      if (!have_lock) G_READ_LOCK(&type_rw_lock);
      if (support_prerequisites && type_lookup_prerequisite_L(node, NODE_TYPE(iface_node))) match = TRUE;
      if (!have_lock) G_READ_UNLOCK(&type_rw_lock);
  }
  return match;
}
static gboolean type_node_is_a_L(TypeNode *node, TypeNode *iface_node) {
...
static inline gboolean type_node_conforms_to_U(TypeNode *node, TypeNode *iface_node, gboolean support_interfaces, gboolean support_prerequisites) {
...
GType* g_type_children(GType type, guint *n_children) {
  ...
  if (node) {
      ...
      if (n_children) *n_children = node->n_children;
      G_READ_UNLOCK(&type_rw_lock);
      return children;
  } else {
...
GType* g_type_interfaces(GType type, guint *n_interfaces) {
  ...
  if (node && node->is_instantiatable) {
      ...
      if (entries) {
          ...
          for (i = 0; i < IFACE_ENTRIES_N_ENTRIES(entries); i++) ifaces[i] = entries->entry[i].iface_type;
      } else {
          ifaces = g_new(GType, 1);
          i = 0;
	  }
      ifaces[i] = 0;
      if (n_interfaces) *n_interfaces = i;
      G_READ_UNLOCK(&type_rw_lock);
      return ifaces;
  } else {
      if (n_interfaces) *n_interfaces = 0;
      return NULL;
  }
}
...
static inline gpointer type_get_qdata_L(TypeNode *node, GQuark quark) {
  ...
  if (quark && gdata && gdata->n_qdatas) {
      ...
      do {
          ...
          i = (n_qdatas + 1) / 2;
          check = qdatas + i;
          if (quark == check->quark) return check->data;
...
static inline void type_set_qdata_W(TypeNode *node, GQuark quark, gpointer data) {
  ...
  QData *qdata;
  guint i;
  if (!node->global_gdata) node->global_gdata = g_new0(GData, 1);
...
void g_type_set_qdata(GType type, GQuark quark, gpointer data) {
  ...
  if (node) {
      ...
      G_WRITE_UNLOCK(&type_rw_lock);
  } else g_return_if_fail(node != NULL);
}
static void type_add_flags_W(TypeNode *node, GTypeFlags flags) {
  guint dflags;
...
gboolean g_type_check_instance_is_a(GTypeInstance *type_instance, GType iface_type) {
  TypeNode *node, *iface;
  gboolean check;
  if (!type_instance || !type_instance->g_class) return FALSE;
...
GTypeInstance* g_type_check_instance_cast(GTypeInstance *type_instance, GType iface_type) {
  if (type_instance) {
      if (type_instance->g_class) {
          ...
          if (check) return type_instance;
          if (is_instantiatable) {
              g_warning("invalid cast from `%s' to `%s'", type_descriptive_name_I(type_instance->g_class->g_type), type_descriptive_name_I(iface_type));
          } else {
              g_warning("invalid uninstantiatable type `%s' in cast to `%s'", type_descriptive_name_I(type_instance->g_class->g_type),
                        type_descriptive_name_I(iface_type));
          }
	  } else g_warning("invalid unclassed pointer in cast to `%s'", type_descriptive_name_I(iface_type));
  }
...
GTypeClass* g_type_check_class_cast(GTypeClass *type_class, GType is_a_type) {
  if (type_class) {
      ...
      if (check) return type_class;
      if (is_classed) g_warning("invalid class cast from `%s' to `%s'", type_descriptive_name_I(type_class->g_type), type_descriptive_name_I(is_a_type));
      else g_warning("invalid unclassed type `%s' in class cast to `%s'", type_descriptive_name_I(type_class->g_type), type_descriptive_name_I(is_a_type));
  } else g_warning("invalid class cast from (NULL) pointer to `%s'", type_descriptive_name_I(is_a_type));
  return type_class;
}
gboolean g_type_check_instance(GTypeInstance *type_instance) {
  if (type_instance) {
	  ...
	  if (node && node->is_instantiatable) return TRUE;
	  g_warning("instance of invalid non-instantiatable type `%s'", type_descriptive_name_I (type_instance->g_class->g_type));
	  } else g_warning("instance with invalid (NULL) class pointer");
  } else g_warning("invalid (NULL) pointer instance");
  return FALSE;
}
static inline gboolean type_check_is_value_type_U(GType type) {
  ...
  TypeNode *node;
  node = lookup_type_node_I(type);
  if (node && node->mutatable_check_cache) return TRUE;
  G_READ_LOCK(&type_rw_lock);
restart_check:
  if (node) {
      if (node->data && NODE_REFCOUNT(node) > 0 && node->data->common.value_table->value_init) tflags = GPOINTER_TO_UINT(type_get_qdata_L(node, static_quark_type_flags));
      else if (NODE_IS_IFACE(node)) {
          guint i;
...
GTypeValueTable* g_type_value_table_peek(GType type) {
  ...
  if (node && NODE_REFCOUNT(node) && node->mutatable_check_cache) return node->data->common.value_table;
  G_READ_LOCK(&type_rw_lock);
restart_table_peek:
  has_refed_data = node && node->data && NODE_REFCOUNT(node) > 0;
  has_table = has_refed_data && node->data->common.value_table->value_init;
  if (has_refed_data) {
      if (has_table) vtable = node->data->common.value_table;
      else if (NODE_IS_IFACE(node)) {
          guint i;
          for (i = 0; i < IFACE_NODE_N_PREREQUISITES(node); i++) {
              GType prtype = IFACE_NODE_PREREQUISITES(node)[i];
  ...
  if (!node) g_warning(G_STRLOC ": type id `%" G_GSIZE_FORMAT "' is invalid", type);
  if (!has_refed_data) g_warning("can't peek value table for type `%s' which is not currently referenced", type_descriptive_name_I(type));
  return NULL;
}
G_CONST_RETURN gchar* g_type_name_from_instance(GTypeInstance *instance) {
  if (!instance) return "<NULL-instance>";
  else return g_type_name_from_class(instance->g_class);
}
G_CONST_RETURN gchar* g_type_name_from_class(GTypeClass *g_class) {
  if (!g_class) return "<NULL-class>";
  else return g_type_name(g_class->g_type);
}
...
void g_type_init_with_debug_flags(GTypeDebugFlags debug_flags) {
  ...
  if (env_string != NULL) {
      ...
      _g_type_debug_flags |= g_parse_debug_string(env_string, debug_keys, sizeof(debug_keys) / sizeof(debug_keys[0]));
      env_string = NULL;
  }
...
void g_type_class_add_private(gpointer g_class, gsize private_size) {
  ...
  if (!node || !node->is_instantiatable || !node->data || node->data->class.class != g_class) {
      g_warning("cannot add private field to invalid (non-instantiatable) type '%s'", type_descriptive_name_I(instance_type));
      return;
  }
...
gpointer g_type_instance_get_private(GTypeInstance *instance, GType private_type) {
  ...
  if (!class) class = instance->g_class;
  instance_node = lookup_type_node_I(class->g_type);
  if (G_UNLIKELY(!instance_node || !instance_node->is_instantiatable)) {
      g_warning("instance of invalid non-instantiatable type `%s'", type_descriptive_name_I(instance->g_class->g_type));
      return NULL;
  }
  private_node = lookup_type_node_I(private_type);
  if (G_UNLIKELY(!private_node || !NODE_IS_ANCESTOR(private_node, instance_node))) {
      g_warning("attempt to retrieve private data for invalid type '%s'", type_descriptive_name_I(private_type));
      return NULL;
  }
...
gpointer g_type_class_get_private(GTypeClass *klass, GType private_type) {
  ...
  if (G_UNLIKELY(!class_node || !class_node->is_classed)) {
      g_warning("class of invalid type `%s'", type_descriptive_name_I(klass->g_type));
      return NULL;
  }
  private_node = lookup_type_node_I(private_type);
