...
static void value_transform_memcpy_data0(const GValue *src_value, GValue *dest_value) {
...
static void value_transform_##func_name(const GValue *src_value, GValue *dest_value) { \
...
static void value_transform_##func_name(const GValue *src_value, GValue *dest_value) { \
  dest_value->data[0].v_pointer = g_strdup_printf((format), src_value->data[0].from_member); \
} \
extern void glib_dummy_decl(void)
...
static void value_transform_bool_string(const GValue *src_value, GValue *dest_value) {
  dest_value->data[0].v_pointer = g_strdup_printf("%s", src_value->data[0].v_int ? "TRUE" : "FALSE");
}
static void value_transform_string_string(const GValue *src_value, GValue *dest_value) {
  dest_value->data[0].v_pointer = g_strdup(src_value->data[0].v_pointer);
}
static void value_transform_enum_string(const GValue *src_value, GValue *dest_value) {
  ...
  if (enum_value) dest_value->data[0].v_pointer = g_strdup(enum_value->value_name);
  else dest_value->data[0].v_pointer = g_strdup_printf("%ld", src_value->data[0].v_long);
  g_type_class_unref(class);
}
static void value_transform_flags_string(const GValue *src_value, GValue *dest_value) {
  ...
  if (flags_value) {
      ...
      do {
          ...
          if (gstring->str[0]) g_string_append(gstring, " | ");
          g_string_append(gstring, flags_value->value_name);
          flags_value = g_flags_get_first_value (class, v_flags);
      } while(flags_value);
      if (v_flags) dest_value->data[0].v_pointer = g_strdup_printf("%s | %u", gstring->str, v_flags);
      else dest_value->data[0].v_pointer = g_strdup(gstring->str);
      g_string_free(gstring, TRUE);
  } else dest_value->data[0].v_pointer = g_strdup_printf("%lu", src_value->data[0].v_ulong);
  g_type_class_unref(class);
}
