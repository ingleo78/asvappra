...
static void put_marshal_value_getters(void) {
    fputs("\n", fout);
...
static gboolean complete_in_arg(InArgument *iarg) {
...
static gboolean complete_out_arg(OutArgument *oarg) {
...
static const gchar* pad(const gchar *string) {
    #define PAD_LENGTH	12
    static gchar *buffer = NULL;
    gint i;
    g_return_val_if_fail(string != NULL, NULL);
    if (!buffer)buffer = g_new(gchar, PAD_LENGTH + 1);
...
static const gchar* indent(guint n_spaces) {
    static gchar *buffer = NULL;
...
static void generate_marshal(const gchar *signame, Signature *sig) {
    guint ind, a;
    GList *node;
    ...
    if (gen_cbody && !have_std_marshaller) {
        ...
        for (a = 1, node = sig->args; node; node = node->next) {
            ...
            if (iarg->getter) g_fprintf (fout, "%s%s arg_%d,\n", indent (ind), pad (iarg->ctype), a++);
	    }
        ...
        for (a = 1, node = sig->args; node; node = node->next) {
            InArgument *iarg = node->data;
            if (iarg->getter) g_fprintf(fout, "%s%s (param_values + %d),\n", indent(ind), iarg->getter, a++);
	    }
...
static void process_signature(Signature *sig) {
    ...
    if (!skip_ploc) g_fprintf(fout, " (%s)", sig->ploc);
    g_fprintf(fout, " */\n");
...
static InArgument* new_in_arg(const gchar *pname) {
    InArgument *iarg = g_new0(InArgument, 1);
    iarg->keyword = g_strdup(pname);
    return iarg;
}
static OutArgument* new_out_arg(const gchar *pname) {
    OutArgument *oarg = g_new0(OutArgument,1);
    oarg->keyword = g_strdup(pname);
    return oarg;
}
static guint parse_line(GScanner *scanner, Signature *sig) {
    if (g_scanner_get_next_token(scanner) != G_TOKEN_IDENTIFIER) return G_TOKEN_IDENTIFIER;
    sig->rarg = new_out_arg(scanner->value.v_identifier);
    sig->ploc = g_strdup_printf("%s:%u", scanner->input_name, scanner->line);
    if (g_scanner_get_next_token(scanner) != ':') return ':';
    if (g_scanner_get_next_token(scanner) != G_TOKEN_IDENTIFIER) return G_TOKEN_IDENTIFIER;
...
static gboolean string_key_destroy(gpointer key, gpointer value, gpointer user_data) {
    g_free(key);
    return TRUE;
}
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) files = g_slist_prepend (files, argv[i]);
    if (files) files = g_slist_reverse(files);
    else files = g_slist_prepend(files, "/dev/stdin");
    ...
    if ((gen_cheader || gen_cbody) && std_includes) g_fprintf(fout, "#include\t<glib-object.h>\n\n");
    if (gen_cheader) g_fprintf(fout, "G_BEGIN_DECLS\n");
    if (gen_cbody) put_marshal_value_getters();
    for (slist = files; slist; slist = slist->next) {
        gchar *file = slist->data;
        gint fd;
        if (strcmp(file, "/dev/stdin") == 0) fd = dup(0);
        ...
        do {
            ...
            switch((guint) g_scanner_peek_next_token(scanner)) {
                case '\n': g_scanner_get_next_token(scanner); continue;
                case G_TOKEN_EOF: break;
                default: {
                        Signature signature = { NULL, NULL, NULL };
                        GList *node;
                        expected_token = parse_line(scanner, &signature);
                        if (expected_token == G_TOKEN_NONE) process_signature(&signature);
                        g_free (signature.ploc);
                        if (signature.rarg) g_free(signature.rarg->keyword);
...
static void parse_args(gint *argc_p, gchar ***argv_p) {
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp("--header", argv[i]) == 0) {
            ...
            gen_internal = TRUE;
            argv[i] = NULL;
        } else if ((strcmp("--prefix", argv[i]) == 0) || (strncmp("--prefix=", argv[i], 9) == 0)) {
            gchar *equal = argv[i] + 8;
            if (*equal == '=') marshaller_prefix = g_strdup(equal + 1);
            else if (i + 1 < argc) {
