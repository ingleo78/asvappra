...
namespace internal {
    inline double FastPath(double significand, int exp) {
        if (exp < -308) return 0.0;
        else if (exp >= 0) return significand * internal::Pow10(exp);
        else return significand / internal::Pow10(-exp);
    }
    ...
    inline int CheckWithinHalfULP(double b, const BigInteger& d, int dExp) {
        ...
        if (bExp >= 0) bS_Exp2 += bExp;
        else {
            dS_Exp2 -= bExp;
            hS_Exp2 -= bExp;
        }
        if (hExp >= 0) hS_Exp2 += hExp;
        else {
            dS_Exp2 -= hExp;
            bS_Exp2 -= hExp;
        }
    ...
    inline bool StrtodDiyFp(const char* decimals, int dLen, int dExp, double* result) {
        ...
        if (i < dLen && decimals[i] >= '5') significand++;
        ...
        if (actualExp != dExp) {
            ...
            if (dLen + adjustment > 19) error += kUlp / 2;
        }
        v = v * cachedPower;
    ...
    inline double StrtodBigInteger(double approx, const char* decimals, int dLen, int dExp) {
        ...
        if (cmp < 0) return a.Value();
            ...
            if (a.Significand() & 1) return a.NextPositiveDouble();
            else return a.Value();
