    ...
    template <typename Allocator> class Stack {
        public:
            Stack(Allocator* allocator, size_t stackCapacity) : allocator_(allocator), ownAllocator_(0), stack_(0), stackTop_(0), stackEnd_(0), initialCapacity_(stackCapacity) { }
                ...
                Stack(Stack&& rhs) : allocator_(rhs.allocator_), ownAllocator_(rhs.ownAllocator_), stack_(rhs.stack_), stackTop_(rhs.stackTop_),
            ...
            void ShrinkToFit() {
                if (Empty()) {
                    ...
                    stack_ = stackTop_ = stackEnd_ = NULL;
                } else Resize(GetSize());
            }
            template<typename T> RAPIDJSON_FORCEINLINE void Reserve(size_t count = 1) {
                if (RAPIDJSON_UNLIKELY(static_cast<std::ptrdiff_t>(sizeof(T) * count) > (stackEnd_ - stackTop_))) Expand<T>(count);
            }
            ...
            bool Empty() const { return stackTop_ == NULL && stack_ == NULL; }
        ...
        private:
            ...
            Stack(const Stack& s) : allocator_(s.allocator_), ownAllocator_(s.ownAllocator_), stack_(s.stack_), stackTop_(s.stackTop_), stackEnd_(s.stackEndk_) { };
            Stack& operator=(const Stack& s) { *this = s; return *this; };
