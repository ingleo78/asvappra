...
RAPIDJSON_NAMESPACE_BEGIN
    ...
    template<typename OutputStream, typename SourceEncoding=UTF8<>, typename TargetEncoding=UTF8<>, typename StackAllocator=CrtAllocator, unsigned writeFlags=RAPIDJSON_WRITE_DEFAULT_FLAGS>
    class Writer {
        public:
            typedef typename SourceEncoding::Ch Ch;
            static const int kDefaultMaxDecimalPlaces = 324;
            explicit Writer(OutputStream os) : os_(&os), level_stack_(NULL, kDefaultLevelDepth * sizeof(Level)), maxDecimalPlaces_(kDefaultMaxDecimalPlaces),
                                               hasRoot_(false) {}
            explicit Writer(OutputStream os, StackAllocator stackAllocator, size_t levelDepth = kDefaultLevelDepth) :
                os_(&os), level_stack_(&stackAllocator, levelDepth * sizeof(Level)), maxDecimalPlaces_(kDefaultMaxDecimalPlaces), hasRoot_(false) {}
            explicit Writer(StackAllocator* allocator = NULL, size_t levelDepth = kDefaultLevelDepth) :
            ...
            #endif
            ~Writer() { os_->Clear(); level_stack_.Clear(); free(os_); }
        ...
        protected:
            ...
            bool WriteDouble(double d) {
                if (internal::Double(d).IsNanOrInf()) {
                    ...
                    if (this->internal::Double(d).IsNan()) {
                        PutReserve(*os_, 3);
            ...
            bool WriteString(const Ch* str, SizeType length)  {
                ...
                if (TargetEncoding::supportUnicode) PutReserve(*os_, 2 + length * 6);
                else PutReserve(*os_, 2 + length * 12);
                PutUnsafe(*os_, '\"');
            ...
            OutputStream operator =(Writer<OutputStream> w) { return *w.os_; }
            ...
            bool WriteRawValue(const Ch* json, size_t length) {
                ...
                while (RAPIDJSON_LIKELY(is.Tell() < length)) {
                    ...
                    if (RAPIDJSON_UNLIKELY(!(writeFlags & kWriteValidateEncodingFlag ? Transcoder<SourceEncoding, TargetEncoding>::Validate(is, *os_) :
                        Transcoder<SourceEncoding, TargetEncoding>::TranscodeUnsafe(is, *os_))))
                        return false;
                }
                return true;
            ...
            void Prefix(Type type) {
                ...
                if (RAPIDJSON_LIKELY(level_stack_.GetSize() != 0)) {
                    ...
                    if (level->valueCount > 0) {
                        if (level->inArray) os_->Put(',');
                        else os_->Put((level->valueCount % 2 == 0) ? ',' : ':');
                    }
                    if (!level->inArray && level->valueCount % 2 == 0) RAPIDJSON_ASSERT(type == kStringType);
                    level->valueCount++;
                } else {
            ...
            bool EndValue(bool ret) {
                if (RAPIDJSON_UNLIKELY(level_stack_.Empty())) Flush();
                return ret;
            }
            OutputStream* os_;
            ...
            bool hasRoot_;
        private:
            Writer(const Writer& writer) : os_(writer->os_), maxDecimalPlaces_(writer.maxDecimalPlaces_), hasRoot_(writer.hasRoot_), Ch(writer->Ch) {
                level_stack_ = writer.level_stack_;
            }
            Writer& operator=(const Writer& writer) { os_->Clear(); level_stack_.Clear(); os_ = NULL; level_stack_ = NULL; *this = writer; return *this; };
    };
