    ...
    namespace avsCommon {
        namespace utils {
            namespace sds {
                ...
                template <typename T> SharedDataStream<T>::Writer::Writer(Policy policy, shared_ptr<BufferLayout> bufferLayout) : m_policy{policy}, m_bufferLayout{bufferLayout},
                                                                                                                m_closed{false} {
                    auto header = m_bufferLayout->getHeader();
                    header->isWriterEnabled = true;
                    header->writeEndCursor = header->writeStartCursor.load();
                }
                template <typename T> SharedDataStream<T>::Writer::~Writer() { close(); }
                template <typename T> ssize_t SharedDataStream<T>::Writer::write(const void* buf, size_t nWords, milliseconds timeout) {
                    ...
                    switch (m_policy) {
                        ...
                        case Policy::ALL_OR_NOTHING:
                            ...
                            if ((writeEnd >= header->oldestUnconsumedCursor) && ((writeEnd - header->oldestUnconsumedCursor) > m_bufferLayout->getDataSize())) {
                        ...
                        case Policy::BLOCKING:
                            ...
                            backwardSeekLock.lock();
                            if (milliseconds::zero() == timeout) header->spaceAvailableConditionVariable.wait(backwardSeekLock, predicate);
                            else if (!header->spaceAvailableConditionVariable.wait_for(backwardSeekLock, timeout, predicate)) return Error::TIMEDOUT;
                            ...
                            if (header->writeStartCursor >= header->oldestUnconsumedCursor) {
                                ...
                                if (wordsToOverrun < spaceAvailable) spaceAvailable = wordsToOverrun;
                            }
                    ...
                    if (beforeWrap > wordsToCopy) beforeWrap = wordsToCopy;
                    ...
                    unique_lock<Mutex> dataAvailableLock(header->dataAvailableMutex, defer_lock);
                    if (Policy::NONBLOCKABLE != m_policy) dataAvailableLock.lock();
                    header->writeStartCursor = header->writeEndCursor.load();
                    if (Policy::NONBLOCKABLE != m_policy) dataAvailableLock.unlock();
                    header->dataAvailableConditionVariable.notify_all();
                    return nWords;
                }
                template <typename T> typename SharedDataStream<T>::Index SharedDataStream<T>::Writer::tell() const { return m_bufferLayout->getHeader()->writeStartCursor; }
                ...
                template <typename T> size_t SharedDataStream<T>::Writer::getWordSize() const { return m_bufferLayout->getHeader()->wordSize; }
