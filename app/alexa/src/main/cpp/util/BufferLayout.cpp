...
namespace alexaClientSDK {
    namespace avsCommon {
        namespace utils {
            namespace sds {
                template<typename T> const std::string SharedDataStream<T>::BufferLayout::TAG = "SdsBufferLayout";
                template<typename T> SharedDataStream<T>::BufferLayout::BufferLayout(std::shared_ptr<Buffer> buffer) : m_buffer{buffer}, m_readerEnabledArray{nullptr},
                        m_readerCursorArray{nullptr}, m_readerCloseIndexArray{nullptr}, m_dataSize{0}, m_data{nullptr} {}
                template<typename T> SharedDataStream<T>::BufferLayout::~BufferLayout() { detach(); }
                ...
                template<typename FieldType, typename ClassType> auto inline max_field_limit(FieldType(ClassType::*)) -> decltype(std::numeric_limits<FieldType>::max()) {
                ...
                template<typename T> bool SharedDataStream<T>::BufferLayout::init(size_t wordSize, size_t maxReaders, size_t maxEphemeralReaders) {
                    if (wordSize > max_field_limit(&Header::wordSize)) {
                        logger::acsdkError(logger::LogEntry(TAG, "initFailed")
                                                   ...
                                                   .d("wordSize", wordSize)
                                                   .d("wordSizeLimit",
                                                      max_field_limit(&Header::wordSize)));
                        return false;
                    }
                    if (maxReaders > max_field_limit(&Header::maxReaders)) {
                        logger::acsdkError(logger::LogEntry(TAG, "initFailed")
                                                   ...
                                                   .d("maxReadersLimit",
                                                      max_field_limit(&Header::maxReaders)));
                        return false;
                    }
                ...
                bool SharedDataStream<T>::BufferLayout::attach() {
                    ...
                    if (0 == header->referenceCount) {
                        logger::acsdkError(
                                logger::LogEntry(TAG, "attachFailed").d("reason", "zeroUsers"));
                        return false;
                    }
                    if (std::numeric_limits<decltype(header->referenceCount)>::max() ==
                        header->referenceCount) {
                        logger::acsdkError(logger::LogEntry(TAG, "attachFailed")
                                                   .d("reason", "bufferMaxUsersExceeded")
                                                   .d("numUsers", header->referenceCount)
                                                   .d("maxNumUsers",
                                                      std::numeric_limits<decltype(header->referenceCount)>::max()));
                        return false;
                    }
                    ++header->referenceCount;
