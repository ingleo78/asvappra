...
namespace alexaClientSDK {
    namespace avsCommon {
        namespace utils {
            namespace http2 {
                using namespace std;
                static const string BOUNDARY_HEADER_PREFIX = "Content-Type: multipart/form-data; boundary=";
                static const string CRLF = "\r\n";
                static const string TWO_DASHES = "--";
                ostream& operator<<(ostream& stream, HTTP2MimeRequestEncoder::State state) {
                    switch (state) {
                ...
                HTTP2MimeRequestEncoder::HTTP2MimeRequestEncoder(const string& boundary, shared_ptr<HTTP2MimeRequestSourceInterface> source) :
                                                                 m_state{State::NEW}, m_rawBoundary{boundary}, m_prefixedBoundary{CRLF + TWO_DASHES + boundary},
                                                                 m_source{source}, m_getMimeHeaderLinesResult{HTTP2GetMimeHeadersResult::ABORT},
                                                                 m_stringIndex{0} {
                    ...
                    while (true) {
                        switch (m_state) {
                            ...
                            case State::SENDING_TERMINATING_DASHES:
                                ...
                                return continueResult();
                            case State::DONE: return HTTP2SendDataResult::COMPLETE;
