    ...
    namespace avsCommon {
        namespace utils {
            namespace sds {
                ...
                template <typename T> const string SharedDataStream<T>::Reader::TAG = "SdsReader";
                template <typename T> SharedDataStream<T>::Reader::Reader(Policy policy, shared_ptr<BufferLayout> bufferLayout, uint8_t id) : m_policy{policy},
                    ...
                    *m_readerCloseIndex = numeric_limits<Index>::max();
                    m_bufferLayout->enableReaderLocked(m_id);
                }
                template <typename T> SharedDataStream<T>::Reader::~Reader() {
                    seek(0, Reference::BEFORE_WRITER);
                    lock_guard<Mutex> lock(m_bufferLayout->getHeader()->readerEnableMutex);
                    m_bufferLayout->disableReaderLocked(m_id);
                    m_bufferLayout->updateOldestUnconsumedCursor();
                }
                template <typename T> ssize_t SharedDataStream<T>::Reader::read(void* buf, size_t nWords, milliseconds timeout) {
                    if (nullptr == buf) {
                        acsdkError(LogEntry(TAG, "readFailed").d("reason", "nullBuffer"));
                        return Error::INVALID;
                    }
                    if (0 == nWords) {
                        acsdkError(LogEntry(TAG, "readFailed").d("reason", "invalidNumWords").d("numWords", nWords));
                        return Error::INVALID;
                    }
                    ...
                    if ((header->writeEndCursor >= *m_readerCursor) && (header->writeEndCursor - *m_readerCursor) > m_bufferLayout->getDataSize()) {
                        return Error::OVERRUN;
                    }
                    unique_lock<Mutex> lock(header->dataAvailableMutex, defer_lock);
                    ...
                    switch (reference) {
                        ...
                        case Reference::BEFORE_READER:
                            if (offset > *m_readerCursor) {
                                acsdkError(LogEntry(TAG, "seekFailed").d("reason", "seekBeforeStreamStart")
                                                   .d("reference", "BEFORE_READER").d("seekOffset", offset).d("readerCursor", m_readerCursor->load()));
                                return false;
                            }
                        ...
                        case Reference::BEFORE_WRITER:
                            if (offset > *writeStartCursor) {
                                acsdkError(LogEntry(TAG, "seekFailed").d("reason", "seekBeforeStreamStart")
                                                   .d("reference", "BEFORE_WRITER").d("seekOffset", offset).d("writeStartCursor", writeStartCursor->load()));
                                return false;
                            }
                            absolute = *writeStartCursor - offset;
                            break;
                        case Reference::ABSOLUTE: absolute = offset;
                    }
                    if (absolute > *m_readerCloseIndex) {
                        acsdkError(LogEntry(TAG, "seekFailed").d("reason", "seekBeyondCloseIndex").d("position", absolute)
                ...
                template <typename T> typename SharedDataStream<T>::Index SharedDataStream<T>::Reader::tell(Reference reference) const {
                    ...
                    acsdkError(LogEntry(TAG, "tellFailed").d("reason", "invalidReference"));
                ...
                template <typename T> void SharedDataStream<T>::Reader::close(Index offset, Reference reference) {
                    ...
                    switch (reference) {
                        ...
                        case Reference::BEFORE_WRITER:
                            if (*writeStartCursor < offset) {
                                acsdkError(LogEntry(TAG, "closeFailed").d("reason", "invalidIndex").d("reference", "BEFORE_WRITER")
                                                   .d("offset", offset).d("writeStartCursor", writeStartCursor->load()));
                    ...
                    if (!validReference) acsdkError(LogEntry(TAG, "closeFailed").d("reason", "invalidReference"));
                    *m_readerCloseIndex = absolute;
                }
                template <typename T> size_t SharedDataStream<T>::Reader::getId() const {
                    return m_id;
                }
                template <typename T> size_t SharedDataStream<T>::Reader::getWordSize() const {
