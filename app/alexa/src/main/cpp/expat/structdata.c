...
void StructData_AddItem(StructData *storage, const XML_Char *s, int data0, int data1, int data2) {
  ...
  if (storage->count == storage->max_count) {
      ...
      new = realloc(storage->entries,storage->max_count * sizeof(StructDataEntry));
      assert(new != NULL);
      storage->entries = new;
  }
...
void StructData_CheckItems(StructData *storage, const StructDataEntry *expected, int count) {
  ...
  if (count != storage->count) {
      sprintf(buffer, "wrong number of entries: got %d, expected %d", storage->count, count);
      ...
      for (i = 0; i < count; i++) {
          ...
          if (xcstrcmp(got->str, want->str) != 0) {
              ...
              if (got->data0 != want->data0 || got->data1 != want->data1 || got->data2 != want->data2) {
                  sprintf(buffer,"struct '%" XML_FMT_STR "' expected (%d,%d,%d), got (%d,%d,%d)", got->str, want->data0, want->data1, want->data2, got->data0,
                          got->data1, got->data2);
                  StructData_Dispose(storage);
                  fail(buffer);
              }
...
void StructData_Dispose(StructData *storage) {
  int i;
  assert(storage != NULL);
  for (i = 0; i < storage->count; i++) free((void *)storage->entries[i].str);
