...
void CharData_AppendXMLChars(CharData *storage, const XML_Char *s, int len) {
  ...
  if (storage->count < 0) storage->count = 0;
  if (len < 0) len = xmlstrlen(s);
  if ((len + storage->count) > maxchars) len = (maxchars - storage->count);
...
int CharData_CheckXMLChars(CharData *storage, const XML_Char *expected) {
  ...
  if (len != count) {
      sprintf(buffer, "wrong number of data characters: got %d, expected %d", count, len);
      fail(buffer);
      return 0;
  }
