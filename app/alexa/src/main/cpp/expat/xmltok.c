...
#endif
#define VTABLE1 { PREFIX(prologTok), PREFIX(contentTok), PREFIX(cdataSectionTok) IGNORE_SECTION_TOK_VTABLE }, { PREFIX(attributeValueTok), PREFIX(entityValueTok) }, \
                PREFIX(nameMatchesAscii), PREFIX(nameLength), PREFIX(skipS), PREFIX(getAtts), PREFIX(charRefNumber), PREFIX(predefinedEntityName), \
...
#define UTF8_GET_NAMING2(pages, byte)  (namingBitmap[((pages)[(((byte)[0]) >> 2) & 7] << 3) + ((((byte)[0]) & 3) << 1) + ((((byte)[1]) >> 5) & 1)] & \
                                        (1u << (((byte)[1]) & 0x1F)))
#define UTF8_GET_NAMING3(pages, byte)  (namingBitmap[((pages)[((((byte)[0]) & 0xF) << 4) + ((((byte)[1]) >> 2) & 0xF)] << 3) + ((((byte)[1]) & 3) << 1) + \
                                        ((((byte)[2]) >> 5) & 1)] & (1u << (((byte)[2]) & 0x1F)))
#define UTF8_GET_NAMING(pages, p, n)  ((n) == 2 ? UTF8_GET_NAMING2(pages, (const unsigned char *)(p)) : ((n) == 3 ? UTF8_GET_NAMING3(pages, \
                                       (const unsigned char *)(p)) : 0))
#define UTF8_INVALID2(p)  ((*p) < 0xC2 || ((p)[1] & 0x80) == 0 || ((p)[1] & 0xC0) == 0xC0)
#define UTF8_INVALID3(p)  (((p)[2] & 0x80) == 0 || ((*p) == 0xEF && (p)[1] == 0xBF ? (p)[2] > 0xBD : ((p)[2] & 0xC0) == 0xC0) || ((*p) == 0xE0 ? (p)[1] < 0xA0 || \
                           ((p)[1] & 0xC0) == 0xC0 : ((p)[1] & 0x80) == 0 || ((*p) == 0xED ? (p)[1] > 0x9F : ((p)[1] & 0xC0) == 0xC0)))
#define UTF8_INVALID4(p)  (((p)[3] & 0x80) == 0 || ((p)[3] & 0xC0) == 0xC0 || ((p)[2] & 0x80) == 0 || ((p)[2] & 0xC0) == 0xC0 || ((*p) == 0xF0 ? (p)[1] < 0x90 || \
                           ((p)[1] & 0xC0) == 0xC0 : ((p)[1] & 0x80) == 0 || ((*p) == 0xF4 ? (p)[1] > 0x8F : ((p)[1] & 0xC0) == 0xC0)))
...
void _INTERNAL_trim_to_complete_utf8_characters(const char *from, const char **fromLimRef) {
  ...
  for (; fromLim > from; fromLim--, walked++) {
      ...
      } else if ((prev & 0x80u) == 0x00u) break;
  }
  *fromLimRef = fromLim;
}
static enum XML_Convert_Result PTRCALL utf8_toUtf8(const ENCODING *enc, const char **fromP, const char *fromLim, char **toP, const char *toLim) {
  ...
  }
  {
      ...
      if (fromLim < fromLimBefore) input_incomplete = true;
  }
  {
  ...
  if (output_exhausted) return XML_CONVERT_OUTPUT_EXHAUSTED;
  else if (input_incomplete) return XML_CONVERT_INPUT_INCOMPLETE;
  else return XML_CONVERT_COMPLETED;
}
static enum XML_Convert_Result PTRCALL utf8_toUtf16(const ENCODING *enc, const char **fromP, const char *fromLim, unsigned short **toP, const unsigned short *toLim) {
...
static enum XML_Convert_Result PTRCALL latin1_toUtf8(const ENCODING *enc, const char **fromP, const char *fromLim, char **toP, const char *toLim) {
  UNUSED_P(enc);
  for (;;) {
      unsigned char c;
...
static enum XML_Convert_Result PTRCALL latin1_toUtf16(const ENCODING *enc, const char **fromP, const char *fromLim, unsigned short **toP,
  ...
  if ((*toP == toLim) && (*fromP < fromLim)) return XML_CONVERT_OUTPUT_EXHAUSTED;
  else return XML_CONVERT_COMPLETED;
}
#ifdef XML_NS
...
static enum XML_Convert_Result PTRCALL ascii_toUtf8(const ENCODING *enc, const char **fromP, const char *fromLim, char **toP, const char *toLim) {
  UNUSED_P(enc);
...
static int PTRFASTCALL unicode_byte_type(char hi, char lo) {
  switch((unsigned char)hi) {
      case 0xD8: case 0xD9: case 0xDA: case 0xDB: return BT_LEAD4;
      case 0xDC: case 0xDD: case 0xDE: case 0xDF: return BT_TRAIL;
      case 0xFF:
          switch ((unsigned char)lo) {
              case 0xFF: case 0xFE: return BT_NONXML;
          }
          break;
  }
  return BT_NONASCII;
...
#define DEFINE_UTF16_TO_UTF8(E) \
  static enum XML_Convert_Result PTRCALL E##toUtf8(const ENCODING *enc, const char **fromP, const char *fromLim, char **toP, const char *toLim) {                                         \
      const char *from = *fromP; \
      UNUSED_P(enc); \
      fromLim = from + (((fromLim - from) >> 1) << 1); \
      ...
      *fromP = from; \
      if (from < fromLim) return XML_CONVERT_INPUT_INCOMPLETE; \
      else return XML_CONVERT_COMPLETED; \
  }
#define DEFINE_UTF16_TO_UTF16(E) \
  static enum XML_Convert_Result PTRCALL E##toUtf16(const ENCODING *enc, const char **fromP, const char *fromLim, unsigned short **toP, \
      ...
      enum XML_Convert_Result res = XML_CONVERT_COMPLETED; \
      UNUSED_P(enc); \
      fromLim = *fromP + (((fromLim - *fromP) >> 1) << 1); \
...
static int FASTCALL streqci(const char *s1, const char *s2) {
  for (;;) {
      ...
      if (ASCII_a <= c2 && c2 <= ASCII_z) c2 += ASCII_A - ASCII_a;
      if (c1 != c2) return 0;
      if (!c1) break;
  }
  return 1;
...
static int FASTCALL isSpace(int c) {
  switch (c) {
      case 0x20: case 0xD: case 0xA: case 0x9: return 1;
  }
  return 0;
}
static int parsePseudoAttribute(const ENCODING *enc, const char *ptr, const char *end, const char **namePtr, const char **nameEndPtr, const char **valPtr,
  ...
  *valPtr = ptr;
  for (;; ptr += enc->minBytesPerChar) {
      c = toAscii(enc, ptr, end);
      if (c == open) break;
      if (! (ASCII_a <= c && c <= ASCII_z) && ! (ASCII_A <= c && c <= ASCII_Z) && ! (ASCII_0 <= c && c <= ASCII_9) && c != ASCII_PERIOD && c != ASCII_MINUS &&
          c != ASCII_UNDERSCORE) {
          *nextTokPtr = ptr;
          return 0;
      }
...
static int doParseXmlDecl(const ENCODING *(*encodingFinder)(const ENCODING *, const char *, const char *), int isGeneralTextEntity, const ENCODING *enc,
                          const char *ptr, const char *end, const char **badPtr, const char **versionPtr, const char **versionEndPtr, const char **encodingName,
...
static int FASTCALL checkCharRefNumber(int result) {
  switch(result >> 8) {
      case 0xD8: case 0xD9: case 0xDA: case 0xDB: case 0xDC: case 0xDD: case 0xDE: case 0xDF: return -1;
      case 0:
...
static enum XML_Convert_Result PTRCALL unknown_toUtf8(const ENCODING *enc, const char **fromP, const char *fromLim, char **toP, const char *toLim) {
  const struct unknown_encoding *uenc = AS_UNKNOWN_ENCODING(enc);
...
ENCODING *XmlInitUnknownEncoding(void *mem, int *table, CONVERTER convert, void *userData) {
  ...
  for (i = 0; i < 128; i++) if (latin1_encoding.type[i] != BT_OTHER && latin1_encoding.type[i] != BT_NONXML && table[i] != i) return 0;
  for (i = 0; i < 256; i++) {
      int c = table[i];
      if (c == -1) {
          ...
          if (c > 0xFFFF) return 0;
          if (UCS2_GET_NAMING(nmstrtPages, c >> 8, c & 0xff)) e->normal.type[i] = BT_NMSTRT;
          else if (UCS2_GET_NAMING(namePages, c >> 8, c & 0xff)) e->normal.type[i] = BT_NAME;
          else e->normal.type[i] = BT_OTHER;
...
static const char KW_ISO_8859_1[] = { ASCII_I, ASCII_S, ASCII_O, ASCII_MINUS, ASCII_8, ASCII_8, ASCII_5, ASCII_9, ASCII_MINUS, ASCII_1, '\0' };
...
getEncodingIndex(const char *name) {
  ...
  for (i = 0; i < (int)(sizeof(encodingNames) / sizeof(encodingNames[0])); i++) if (streqci(name, encodingNames[i])) return i;
  return UNKNOWN_ENC;
}
...
static int initScan(const ENCODING *const *encodingTable, const INIT_ENCODING *enc, int state, const char *ptr, const char *end, const char **nextTokPtr) {
  ...
  if (ptr + 1 == end) {
      ...
      switch(INIT_ENC_INDEX(enc)) {
          case UTF_16_ENC: case UTF_16LE_ENC: case UTF_16BE_ENC: return XML_TOK_PARTIAL;
      }
      switch ((unsigned char)*ptr) {
          case 0xFE: case 0xFF: case 0xEF: if (INIT_ENC_INDEX(enc) == ISO_8859_1_ENC && state == XML_CONTENT_STATE) break;
          case 0x00: case 0x3C: return XML_TOK_PARTIAL;
      }
  } else {
