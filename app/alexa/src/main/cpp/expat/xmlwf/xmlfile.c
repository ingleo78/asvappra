...
static void reportError(XML_Parser parser, const XML_Char *filename) {
  ...
  if (message) {
      ftprintf(stdout, T("%s") T(":%") T(XML_FMT_INT_MOD) T("u") T(":%") T(XML_FMT_INT_MOD) T("u") T(": %s\n"), filename,
               XML_GetErrorLineNumber(parser), XML_GetErrorColumnNumber(parser), message);
  } else */ftprintf(stderr, T("%s: (unknown message %d)\n"), filename, code);
}
...
static const XML_Char * resolveSystemId(const XML_Char *base, const XML_Char *systemId, XML_Char **toFree) {
  ...
  *toFree = 0;
  if (!base || *systemId == T('/')
  #if defined(_WIN32)
      || *systemId == T('\\') || (isAsciiLetter(systemId[0]) && systemId[1] == T(':'))
  #endif
     )
      return systemId;
...
static int externalEntityRefFilemap(XML_Parser parser, const XML_Char *context, const XML_Char *base, const XML_Char *systemId, const XML_Char *publicId) {
  ...
  switch(filemapRes) {
      ...
      case 2:
          ftprintf(stderr, T("%s: file too large for memory-mapping") T(", switching to streaming\n"), filename);
...
int XML_ProcessFile(XML_Parser parser, const XML_Char *filename, unsigned flags) {
  ...
  if (flags & XML_EXTERNAL_ENTITIES) XML_SetExternalEntityRefHandler(parser, (flags & XML_MAP_FILE) ? externalEntityRefFilemap : externalEntityRefStream);
  ...
  } else result = processStream(filename, parser);
  return result;
}