...
static void XMLCALL characterData(void *userData, const XML_Char *s, int len) {
  ...
  for (; len > 0; --len, ++s) {
      ...
      #else
          case T('"'): fputts(T("&quot;"), fp); break;
          case 9: case 10: case 13: ftprintf(fp, T("&#%d;"), *s); break;
      #endif
          default: puttc(*s, fp); break;
      }
  }
}
...
static void XMLCALL startElement(void *userData, const XML_Char *name, const XML_Char **atts) {
  ...
  while(*p) ++p;
  nAtts = (int)((p - atts) >> 1);
  if (nAtts > 1) qsort((void *)atts, nAtts, sizeof(XML_Char *) * 2, attcmp);
  while(*atts) {
      puttc(T(' '), fp);
      fputts(*atts++, fp);
...
static void XMLCALL startElementNS(void *userData, const XML_Char *name, const XML_Char **atts) {
  ...
  while(*p) ++p;
  nAtts = (int)((p - atts) >> 1);
  if (nAtts > 1) qsort((void *)atts, nAtts, sizeof(XML_Char *) * 2, nsattcmp);
  while(*atts) {
      name = *atts++;
      sep = tcsrchr(name, NSSEP);
      puttc(T(' '), fp);
...
static void XMLCALL endDoctypeDecl(void *userData) {
  ...
  NotationList *p;
  int i;
  for (p = data->notationListHead; p != NULL; p = p->next) notationCount++;
  if (notationCount == 0) {
  ...
  for (p = data->notationListHead, i = 0; i < notationCount; p = p->next, i++) notations[i] = p;
...
static void metaLocation(XML_Parser parser) {
  ...
  if (uri) ftprintf(fp, T(" uri=\"%s\""), uri);
  ftprintf(fp, T(" byte=\"%") T(XML_FMT_INT_MOD) T("d\"") T(" nbytes=\"%d\"") T(" line=\"%") T(XML_FMT_INT_MOD) T("u\"") T(" col=\"%")
           T(XML_FMT_INT_MOD) T("u\""), XML_GetCurrentByteIndex(parser), XML_GetCurrentByteCount(parser), XML_GetCurrentLineNumber(parser),
...
static void XMLCALL metaStartElement(void *userData, const XML_Char *name, const XML_Char **atts) {
  ...
  FILE *fp = data->fp;
  const XML_Char **specifiedAttsEnd = atts + XML_GetSpecifiedAttributeCount(parser);
  const XML_Char **idAttPtr;
  int idAttIndex = XML_GetIdAttributeIndex(parser);
  if (idAttIndex < 0) idAttPtr = 0;
  else idAttPtr = atts + idAttIndex;
  ...
  if (*atts) {
      ...
      do {
          ...
          if (atts >= specifiedAttsEnd) fputts(T("\" defaulted=\"yes\"/>\n"), fp);
          else if (atts == idAttPtr) fputts(T("\" id=\"yes\"/>\n"), fp);
          else fputts(T("\"/>\n"), fp);
      } while (*(atts += 2));
      fputts(T("</starttag>\n"), fp);
  } else fputts(T("/>\n"), fp);
...
static void XMLCALL metaNotationDecl(void *userData, const XML_Char *notationName, const XML_Char *base, const XML_Char *systemId, const XML_Char *publicId) {
  ...
  if (publicId) ftprintf(fp, T(" public=\"%s\""), publicId);
  if (systemId) {
      fputts(T(" system=\""), fp);
...
static void XMLCALL metaEntityDecl(void *userData, const XML_Char *entityName, int is_param, const XML_Char *value, int value_length, const XML_Char *base,
...
static void XMLCALL metaStartNamespaceDecl(void *userData, const XML_Char *prefix, const XML_Char *uri) {
  ...
  FILE *fp = data->fp;
  fputts(T("<startns"), fp);
  if (prefix) ftprintf(fp, T(" prefix=\"%s\""), prefix);
  if (uri) {
      fputts(T(" ns=\""), fp);
...
static void XMLCALL metaEndNamespaceDecl(void *userData, const XML_Char *prefix) {
  ...
  XmlwfUserData *data = (XmlwfUserData*)XML_GetUserData(parser);
  FILE *fp = data->fp;
  if (! prefix) fputts(T("<endns/>\n"), fp);
  else ftprintf(fp, T("<endns prefix=\"%s\"/>\n"), prefix);
}
...
static int XMLCALL unknownEncoding(void *userData, const XML_Char *name, XML_Encoding *info) {
  ...
  int i;
  UNUSED_P(userData);
  for (i = 0; prefixU[i]; i++) if (name[i] != prefixU[i] && name[i] != prefixL[i]) return 0;
  cp = 0;
  for (; name[i]; i++) {
...
static void showVersion(XML_Char *prog) {
  ...
  if (features != NULL && features[0].feature != XML_FEATURE_END) {
      ...
      if (features[0].value) ftprintf(stdout, T("=%ld"), features[0].value);
      while (features[i].feature != XML_FEATURE_END) {
          ftprintf(stdout, T(", %s"), features[i].name);
          if (features[i].value) ftprintf(stdout, T("=%ld"), features[i].value);
          ++i;
      }
      ftprintf(stdout, T("\n"));
  }
...
static void usage(const XML_Char *prog, int rc) {
  ftprintf(stderr, T("usage: %s [-s] [-n] [-p] [-x] [-e ENCODING] [-w] [-r] [-d DIRECTORY]\n")
                   T("             [-c | -m | -t] [-N]\n")
                   T("             [FILE [FILE ...]]\n")
                   T("\n")
...
int wmain(int argc, XML_Char **argv) {
  ...
  while(i < argc) {
      ...
      switch(argv[i][j]) {
          ...
          case T('n'):
              ...
              j++;
              break;
          case T('p'): paramEntityParsing = XML_PARAM_ENTITY_PARSING_ALWAYS;
          case T('x'):
  ...
  for (; i < argc; i++) {
      ...
      int result;
      XML_Parser parser;
      if (useNamespaces) parser = XML_ParserCreateNS(encoding, NSSEP);
      else parser = XML_ParserCreate(encoding);
      if (!parser) {
          tperror(T("Could not instantiate parser"));
          exit(1);
      }
      if (requireStandalone) XML_SetNotStandaloneHandler(parser, notStandalone);
      ...
      if (outputType == 't') {
          ...
          switch(outputType) {
              ...
              case 'c':
                  ...
                  break;
              default:
                  if (useNamespaces) XML_SetElementHandler(parser, startElementNS, endElementNS);
                  else XML_SetElementHandler(parser, startElement, endElement);
      ...
      if (windowsCodePages) XML_SetUnknownEncodingHandler(parser, unknownEncoding, 0);
      result = XML_ProcessFile(parser, useStdin ? NULL : argv[i], processFlags);
      if (outputDir) {
          if (outputType == 'm') metaEndDocument(parser);
          fclose(userData.fp);
          if (!result) { tremove(outName); }
          free(outName);
      }
