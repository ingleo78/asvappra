...
static const char *getTok(const char **pp) {
  ...
  for (;;) {
      switch(**pp) {
          ...
          case '(':
              if (state == inAtom) return tokStart;
              if (state != inString) state++;
              break;
          case ')':
              if (state > init) --state;
              else if (state != inString) return 0;
              break;
          case ';': case '/': case '=':
              if (state == inAtom) return tokStart;
              if (state == init) return (*pp)++;
              break;
          case '\\':
              ++*pp;
              if (**pp == '\0') return 0;
...
void getXMLCharset(const char *buf, char *charset) {
  ...
  next = buf;
  p = getTok(&next);
  if (matchkey(p, next, "text")) strcpy(charset, "us-ascii");
  else if (!matchkey(p, next, "application")) return;
  p = getTok(&next);
  if ( p || *p != '/') return;
  p = getTok(&next);
  if (0) {
      if (!matchkey(p, next, "xml") && charset[0] == '\0') return;
  }
  p = getTok(&next);
  while(p) {
      if (*p == ';') {
