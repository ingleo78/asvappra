...
static inline GModule* g_module_find_by_handle(gpointer handle) {
    GModule *module;
    GModule *retval = NULL;
    if (main_module && main_module->handle == handle) retval = main_module;
    else
...
#define	SUPPORT_OR_RETURN(rv)	{ g_module_set_error ("dynamic modules are not supported by this system"); return rv; }
static gpointer _g_module_open(const gchar	*file_name, gboolean bind_lazy, gboolean bind_local) {
    return NULL;
}
static void _g_module_close(gpointer handle, gboolean is_unref) {}
...
static gchar* parse_libtool_archive (const gchar* libtool_name) {
    ...
    g_scanner_scope_add_symbol(scanner, 0, "dlname", GUINT_TO_POINTER(TOKEN_DLNAME));
    g_scanner_scope_add_symbol(scanner, 0, "installed", GUINT_TO_POINTER(TOKEN_INSTALLED));
    g_scanner_scope_add_symbol(scanner, 0, "libdir", GUINT_TO_POINTER(TOKEN_LIBDIR));
    while(!g_scanner_eof(scanner)) {
        token = g_scanner_get_next_token(scanner);
        if (token == TOKEN_DLNAME || token == TOKEN_INSTALLED || token == TOKEN_LIBDIR) {
            if (g_scanner_get_next_token(scanner) != '=' || g_scanner_get_next_token(scanner) != (token == TOKEN_INSTALLED ? G_TOKEN_IDENTIFIER : G_TOKEN_STRING)) {
                gchar *display_libtool_name = g_filename_display_name(libtool_name);
                ...
                } else if (token == TOKEN_INSTALLED) lt_installed = strcmp(scanner->value.v_identifier, "yes") == 0;
                else {
                    g_free(lt_libdir);
...
static inline gboolean str_check_suffix(const gchar* string, const gchar* suffix) {
...
static void _g_module_debug_init(void) {
    ...
    module_debug_flags = !env ? 0 : g_parse_debug_string(env, keys, G_N_ELEMENTS(keys));
...
GModule* g_module_open(const gchar *file_name, GModuleFlags flags) {
    ...
    if (G_UNLIKELY(!module_debug_initialized)) _g_module_debug_init();
    if (module_debug_flags & G_MODULE_DEBUG_BIND_NOW_MODULES) flags &= ~G_MODULE_BIND_LAZY;
    if (!file_name) {
        if (!main_module){
    ...
    if (g_file_test(file_name, G_FILE_TEST_IS_REGULAR)) name = g_strdup (file_name);
    if (!name) {
    ...
    if (!name) {
        ...
        if (!dot || dot < slash) name = g_strconcat(file_name, "." G_MODULE_SUFFIX, NULL);
        else name = g_strdup(file_name);
    }
    if (name) {
        if (str_check_suffix(name, ".la")) {
        ...
        if (name) handle = _g_module_open(name, (flags & G_MODULE_BIND_LAZY) != 0, (flags & G_MODULE_BIND_LOCAL) != 0);
    } else {
    ...
    if (handle) {
    ...
    #if defined(G_OS_WIN32) && !defined(_WIN64)
        module->cp_file_name = g_locale_from_utf8 (file_name, -1,NULL, NULL, NULL);
    #endif
        ...
        module->next = modules;
        modules = module;
        if (g_module_symbol(module, "g_module_check_init", (gpointer) &check_init) && check_init != NULL) check_failed = check_init (module);
        if (!check_failed) g_module_symbol(module, "g_module_unload", (gpointer) &module->unload);
        if (check_failed) {
            gchar *error;
            error = g_strconcat("GModule(", file_name ? file_name : "NULL", ") initialization check failed: ", check_failed, NULL);
            g_module_close(module);
            module = NULL;
            g_module_set_error(error);
            g_free(error);
	    } else g_module_set_error (saved_error);
        g_free (saved_error);
    }
    if (module != NULL && (module_debug_flags & G_MODULE_DEBUG_RESIDENT_MODULES)) g_module_make_resident(module);
    g_static_rec_mutex_unlock (&g_module_global_lock);
    return module;
}
#if defined (G_OS_WIN32) && !defined(_WIN64)
...
G_CONST_RETURN gchar* g_module_name(GModule *module) {
    ...
    if (module == main_module) return "main";
