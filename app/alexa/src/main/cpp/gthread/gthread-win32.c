...
#define win32_check_for_error(what) \
G_STMT_START{ \
  if (!(what)) g_error("file %s: line %d (%s): error %s during %s",__FILE__, __LINE__, G_STRFUNC, g_win32_error_message(GetLastError ()), #what); \
} G_STMT_END
...
static void g_mutex_free_win32_cs_impl(GMutex *mutex) {
...
static void g_mutex_lock_win32_cs_impl(GMutex *mutex) {
  //EnterCriticalSection(*(CRITICAL_SECTION**)mutex);
}
static gboolean g_mutex_trylock_win32_cs_impl(GMutex *mutex) {
  //return try_enter_critical_section(*(CRITICAL_SECTION**)mutex);
  return 0;
}
static void g_mutex_unlock_win32_cs_impl(GMutex *mutex) {
  //LeaveCriticalSection(*(CRITICAL_SECTION**)mutex);
}
static GMutex *g_mutex_new_win32_impl(void) {
  /*HANDLE handle;
  HANDLE *retval;
...
static void g_mutex_free_win32_impl(GMutex *mutex) {
  //win32_check_for_error(CloseHandle(*(HANDLE*)mutex));
  g_free(mutex);
}
static void g_mutex_lock_win32_impl(GMutex *mutex) {
  //WaitForSingleObject(*(HANDLE*)mutex, INFINITE);
}
static gboolean g_mutex_trylock_win32_impl(GMutex * mutex) {
  /*DWORD result;
  win32_check_for_error(WAIT_FAILED != (result = WaitForSingleObject(*(HANDLE*)mutex, 0)));
  return result != WAIT_TIMEOUT;*/
  return 0;
}
static void g_mutex_unlock_win32_impl(GMutex *mutex) {
  //ReleaseMutex(*(HANDLE*)mutex);
}
static GCond *g_cond_new_win32_impl(void) {
  GCond *retval = g_new(GCond, 1);
  //retval->array = g_ptr_array_new();
  //InitializeCriticalSection(&retval->lock);
  return retval;
}
static void g_cond_signal_win32_impl(GCond * cond) {
...
static void g_cond_broadcast_win32_impl(GCond * cond) {
  /*guint i;
  EnterCriticalSection(&cond->lock);
  for (i = 0; i < cond->array->len; i++) SetEvent(g_ptr_array_index(cond->array, i));
  g_ptr_array_set_size(cond->array, 0);
  LeaveCriticalSection(&cond->lock);*/
}
static gboolean g_cond_wait_internal(GCond *cond, GMutex *entered_mutex, gulong milliseconds) {
  /*gulong retval;
  ...
  win32_check_for_error (WAIT_FAILED != (retval = WaitForSingleObject (event, milliseconds)));
  g_thread_functions_for_glib_use_default.mutex_lock (entered_mutex);
  if (retval == WAIT_TIMEOUT) {
      EnterCriticalSection (&cond->lock);
      g_ptr_array_remove (cond->array, event);
      win32_check_for_error (WAIT_FAILED != (retval = WaitForSingleObject (event, 0)));
      LeaveCriticalSection (&cond->lock);
  }
...
static void g_cond_wait_win32_impl(GCond *cond, GMutex *entered_mutex) {
  /*g_return_if_fail (cond != NULL);
  g_return_if_fail (entered_mutex != NULL);
  g_cond_wait_internal (cond, entered_mutex, INFINITE);*/
}
static gboolean g_cond_timed_wait_win32_impl (GCond *cond, GMutex *entered_mutex, GTimeVal *abs_time) {
  GTimeVal current_time;
  gulong to_wait;
  g_return_val_if_fail (cond != NULL, FALSE);
  g_return_val_if_fail (entered_mutex != NULL, FALSE);
  if (!abs_time);//to_wait = INFINITE;
  else {
      g_get_current_time (&current_time);
      if (abs_time->tv_sec < current_time.tv_sec || (abs_time->tv_sec == current_time.tv_sec && abs_time->tv_usec <= current_time.tv_usec)) to_wait = 0;
      else to_wait = (abs_time->tv_sec - current_time.tv_sec) * 1000 + (abs_time->tv_usec - current_time.tv_usec) / 1000;
   }
  return g_cond_wait_internal(cond, entered_mutex, to_wait);
}
static void g_cond_free_win32_impl(GCond * cond) {
  /*DeleteCriticalSection (&cond->lock);
  g_ptr_array_free (cond->array, TRUE);
  g_free (cond);*/
}
static GPrivate *g_private_new_win32_impl(GDestroyNotify destructor) {
  GPrivate *result = NULL;
  /*EnterCriticalSection (&g_thread_global_spinlock);
  if (g_private_next >= G_PRIVATE_MAX) {
      char buf[100];
      sprintf (buf,"Too many GPrivate allocated. Their number is limited to %d.", G_PRIVATE_MAX);
      MessageBox (NULL, buf, NULL, MB_ICONERROR|MB_SETFOREGROUND);
      if (IsDebuggerPresent ()) G_BREAKPOINT ();
      abort ();
  }
...
static void g_private_set_win32_impl (GPrivate * private_key, gpointer value) {
  /*gpointer* array = TlsGetValue (g_private_tls);
  guint index = GPOINTER_TO_UINT (private_key);
  if (index >= G_PRIVATE_MAX) return;
  if (!array) {
...
static gpointer g_private_get_win32_impl(GPrivate * private_key) {
  /*gpointer* array = TlsGetValue (g_private_tls);
  guint index = GPOINTER_TO_UINT (private_key);
  if (index >= G_PRIVATE_MAX || !array) return NULL;
  return array[index];*/
  return NULL;
}
static void g_thread_set_priority_win32_impl(gpointer thread, GThreadPriority priority) {
  /*GThreadData *target = *(GThreadData **)thread;
  g_return_if_fail (priority >= G_THREAD_PRIORITY_LOW);
  g_return_if_fail (priority <= G_THREAD_PRIORITY_URGENT);
  win32_check_for_error (SetThreadPriority (target->thread, g_thread_priority_map [priority]));*/
}
static void g_thread_self_win32_impl(gpointer thread) {
  /*GThreadData *self = TlsGetValue (g_thread_self_tls);
  if (!self) {
      HANDLE handle = GetCurrentThread ();
      HANDLE process = GetCurrentProcess ();
      self = g_new (GThreadData, 1);
      win32_check_for_error (DuplicateHandle (process, handle, process, &self->thread, 0, FALSE, DUPLICATE_SAME_ACCESS));
  ...
  if (array) {
      ...
      do {
          ...
          for (i = 0; i < private_max; i++) {
              ...
              if (data) some_data_non_null = TRUE;
              array[i] = NULL;
              if (destructor && data) destructor (data);
          }
...
static guint __stdcall g_thread_proxy (gpointer data) {
...
static void g_thread_create_win32_impl(GThreadFunc func, gpointer data, gulong stack_size, gboolean joinable, gboolean bound, GThreadPriority priority,
  ...
  retval->thread = (HANDLE) _beginthreadex (NULL, stack_size, g_thread_proxy, retval, 0, &ignore);
...
static void g_thread_join_win32_impl(gpointer thread) {
  /*GThreadData *target = *(GThreadData **)thread;
  g_return_if_fail (target->joinable);
  win32_check_for_error (WAIT_FAILED != WaitForSingleObject (target->thread, INFINITE));
  win32_check_for_error (CloseHandle (target->thread));
  g_free (target);*/
}
static guint64 g_thread_gettime_impl(void) {
  guint64 v = 0;
...
static void g_thread_impl_init() {
  ...
  win32_check_for_error (TLS_OUT_OF_INDEXES != (g_cond_event_tls = TlsAlloc ()));
  InitializeCriticalSection (&g_thread_global_spinlock);
  kernel32 = GetModuleHandle ("KERNEL32.DLL");
  if (kernel32) {
      try_enter_critical_section = (GTryEnterCriticalSectionFunc)GetProcAddress(kernel32, "TryEnterCriticalSection");
      if (try_enter_critical_section && try_enter_critical_section (&g_thread_global_spinlock)) {
          LeaveCriticalSection (&g_thread_global_spinlock);
          g_thread_functions_for_glib_use_default.mutex_new = g_mutex_new_win32_cs_impl;
          g_thread_functions_for_glib_use_default.mutex_lock = g_mutex_lock_win32_cs_impl;
          g_thread_functions_for_glib_use_default.mutex_trylock = g_mutex_trylock_win32_cs_impl;
          g_thread_functions_for_glib_use_default.mutex_unlock = g_mutex_unlock_win32_cs_impl;
          g_thread_functions_for_glib_use_default.mutex_free = g_mutex_free_win32_cs_impl;
	  }
  }*/
}