...
extern BOOL ConvertStringSidToSidA(LPCSTR  StringSid, PSID *Sid);
extern BOOL ConvertSidToStringSidA(PSID Sid, LPSTR *StringSid);
#ifndef O_BINARY
#define O_BINARY 0
#endif
...
static dbus_pid_t get_pid_from_extended_tcp_table(int peer_port) {
  ...
  if ((errorCode = GetExtendedTcpTable(NULL, &size, TRUE, AF_INET, TCP_TABLE_OWNER_PID_ALL, 0)) == ERROR_INSUFFICIENT_BUFFER) {
...
const char* _dbus_win_error_from_last_error(void) {
  /*switch(GetLastError()) {
      ...
      case ERROR_NO_MORE_FILES: case ERROR_TOO_MANY_OPEN_FILES: return DBUS_ERROR_LIMITS_EXCEEDED;
      case ERROR_ACCESS_DENIED: case ERROR_CANNOT_MAKE: return DBUS_ERROR_ACCESS_DENIED;
      case ERROR_NOT_ENOUGH_MEMORY: return DBUS_ERROR_NO_MEMORY;
      case ERROR_FILE_EXISTS: return DBUS_ERROR_FILE_EXISTS;
      case ERROR_FILE_NOT_FOUND: case ERROR_PATH_NOT_FOUND: return DBUS_ERROR_FILE_NOT_FOUND;
...
char* _dbus_win_error_string(int error_number) {
  /*char *msg;
  FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, NULL, error_number, 0, (LPSTR)&msg, 0, NULL);
  if (msg[strlen(msg) - 1] == '\n') msg[strlen(msg) - 1] = '\0';
...
again:
  if (closesocket(fd.sock) == SOCKET_ERROR)    {
      DBUS_SOCKET_SET_ERRNO();
      if (errno == EINTR) goto again;
      dbus_set_error(error, _dbus_error_from_errno(errno), "Could not close socket: socket=%Iu, , %s", fd.sock, _dbus_strerror_from_errno ());
      return FALSE;
  }
...
static void _dbus_win_handle_set_close_on_exec(HANDLE handle) {
  /*if ( !SetHandleInformation((HANDLE)handle, HANDLE_FLAG_INHERIT | HANDLE_FLAG_PROTECT_FROM_CLOSE, 0)) {
...
dbus_bool_t _dbus_set_socket_nonblocking(DBusSocket handle, DBusError *error) {
  ...
  if (ioctlsocket(handle.sock, FIONBIO, &one) == SOCKET_ERROR) {
      DBUS_SOCKET_SET_ERRNO();
      dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to set socket %Iu to nonblocking: %s", handle.sock, _dbus_strerror_from_errno());
      return FALSE;
  }*/
  return TRUE;
}
...
again:
  ...
  } else _dbus_verbose("WSASend: = %ld\n", bytes_written);
  if (bytes_written == (DWORD)-1 && errno == EINTR) goto again;
  return bytes_written;*/
  return 0;
}
#if 0
int _dbus_connect_named_pipe(const char *path, DBusError *error) {
...
dbus_bool_t _dbus_win_account_to_sid(const wchar_t *waccount, void **ppsid, DBusError *error) {
  ...
  *ppsid = NULL;
  sid_length = 0;
  wdomain_length = 0;
  if (!LookupAccountNameW(NULL, waccount, NULL, &sid_length, NULL, &wdomain_length, &use) && GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
      _dbus_win_set_error_from_win_error(error, GetLastError());
      return FALSE;
  }
...
dbus_bool_t _dbus_getsid(char **sid, dbus_pid_t process_id) {
  ...
  if (process_id == 0) process_handle = GetCurrentProcess();
  else if (is_winxp_sp3_or_lower()) process_handle = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, process_id);
  else process_handle = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, process_id);
  if (!OpenProcessToken(process_handle, TOKEN_QUERY, &process_token)) {
      _dbus_win_warn_win_error("OpenProcessToken failed", GetLastError());
      goto failed;
  }
  if ((!GetTokenInformation(process_token, TokenUser, NULL, 0, &n) && GetLastError() != ERROR_INSUFFICIENT_BUFFER) || (token_user = alloca(n)) == NULL ||
...
out0:
  closesocket(temp);
  dbus_set_error(error, _dbus_error_from_errno(errno),"Could not setup socket pair: %s", _dbus_strerror_from_errno());*/
  return FALSE;
}
int _dbus_poll(DBusPollFD *fds, int n_fds, int timeout_milliseconds) {
  ...
  if (n_fds > DBUS_STACK_WSAEVENTS) pEvents = calloc(sizeof(WSAEVENT), n_fds);
  else pEvents = eventsOnStack;
  ...
  for (i = 0; i < n_fds; i++) {
      ...
      if (msgp >= msg + DBUS_POLL_CHAR_BUFFER_SIZE) { _dbus_assert_not_reached("buffer overflow in _dbus_poll"); }
  }
  msgp += sprintf(msgp, "\n");
  _dbus_verbose("%s",msg);
#endif
  ...
  if (DBUS_SOCKET_API_RETURNS_ERROR(ready)) {
      DBUS_SOCKET_SET_ERRNO();
      if (errno != WSAEWOULDBLOCK) _dbus_verbose("WSAWaitForMultipleEvents: failed: %s\n", _dbus_strerror_from_errno());
      ret = -1;
  ...
  for(i = 0; i < n_fds; i++) WSACloseEvent(pEvents[i]);
...
#define DBUS_POLL_CHAR_BUFFER_SIZE 2000
  ...
  } else if (ready == 0) _dbus_verbose("select: = 0\n");
  else if (ready > 0) {
  #ifdef DBUS_ENABLE_VERBOSE_MODE
      msgp = msg;
...
DBusSocket _dbus_connect_tcp_socket_with_nonce(const char *host, const char *port, const char *family, const char *noncefile, DBusError *error) {
  ...
  if ((res = getaddrinfo(host, port, &hints, &ai)) != 0 || !ai) {
      dbus_set_error(error, _dbus_error_from_errno(res),"Failed to lookup host/port: \"%s:%s\": %s (%d)", host, port, _dbus_strerror(res), res);
      return _dbus_socket_get_invalid();
  }
  tmp = ai;
  while(tmp) {
  ...
  if (!_dbus_socket_is_valid(fd)) {
      dbus_set_error(error, _dbus_error_from_errno(errno),"Failed to connect to socket \"%s:%s\" %s", host, port, _dbus_strerror_from_errno());
      return _dbus_socket_get_invalid();
  }
  if (noncefile != NULL) {
...
int _dbus_listen_tcp_socket(const char *host, const char *port, const char *family, DBusString *retport, DBusSocket **fds_p, DBusError *error) {
  ...
  *fds_p = NULL;
  ...
  _DBUS_ZERO(hints);
  if (!family) hints.ai_family = AF_INET;
  else if (!strcmp(family, "ipv4")) hints.ai_family = AF_INET;
  else if (!strcmp(family, "ipv6")) hints.ai_family = AF_INET6;
  else {
      dbus_set_error(error, DBUS_ERROR_INVALID_ARGS,"Unknown address family %s", family);
 ...
 redo_lookup_with_port:
  if ((res = getaddrinfo(host, port, &hints, &ai)) != 0 || !ai) {
      dbus_set_error(error,_dbus_error_from_errno(res),"Failed to lookup host/port: \"%s:%s\": %s (%d)", host ? host : "*", port, _dbus_strerror(res), res);
      return -1;
  }
  tmp = ai;
  while (tmp) {
      ...
      if (bind(fd.sock, (struct sockaddr*)tmp->ai_addr, tmp->ai_addrlen) == SOCKET_ERROR) {
          ...
          dbus_set_error(error, _dbus_error_from_errno(errno),"Failed to bind socket \"%s:%s\": %s", host ? host : "*", port, _dbus_strerror_from_errno());
          goto failed;
      }
      if (listen (fd.sock, 30) == SOCKET_ERROR) {
          DBUS_SOCKET_SET_ERRNO();
          dbus_set_error(error, _dbus_error_from_errno(errno),"Failed to listen on socket \"%s:%s\": %s", host ? host : "*", port, _dbus_strerror_from_errno());
          closesocket(fd.sock);
          goto failed;
      }
      ...
      if (!_dbus_string_get_length(retport)) {
          if (!port || !strcmp(port, "0")) {
              ...
              if (getsockname(fd.sock, &addr.Address, &addrlen) == SOCKET_ERROR || (res = getnameinfo (&addr.Address, addrlen, NULL, 0, portbuf, sizeof(portbuf),
                  NI_NUMERICSERV)) != 0) {
                  DBUS_SOCKET_SET_ERRNO();
                  dbus_set_error(error, _dbus_error_from_errno (errno),"Failed to resolve port \"%s:%s\": %s", host ? host : "*", port,
                                 _dbus_strerror_from_errno());
                  goto failed;
              }
  ...
  for (i = 0 ; i < nlisten_fd ; i++) {
      ...
      if (!_dbus_set_socket_nonblocking(listen_fd[i], error)) goto failed;
  }
  *fds_p = listen_fd;
  return nlisten_fd;
failed:
...
again:
  ...
  if (bytes_written < 0 && errno == EINTR) goto again;
  if (bytes_written < 0) {
      dbus_set_error(error, _dbus_error_from_errno(errno),"Failed to write credentials byte: %s", _dbus_strerror_from_errno());
      return FALSE;
...
dbus_bool_t _dbus_concat_dir_and_file(DBusString *dir, const DBusString *next_component) {
  ...
  file_starts_with_slash = ('/' == _dbus_string_get_byte(next_component, 0) || '\\' == _dbus_string_get_byte(next_component, 0));
...
dbus_bool_t _dbus_credentials_add_from_user(DBusCredentials *credentials, const DBusString *username, DBusCredentialsAddFlags flags, DBusError *error) {
  if (!_dbus_credentials_add_windows_sid(credentials, _dbus_string_get_const_data(username))) {
      _DBUS_SET_OOM(error);
      return FALSE;
  }
  return TRUE;
...
static void dump_backtrace_for_thread(HANDLE hThread) {
...
#endif
  while(StackWalk(dwImageType, GetCurrentProcess(), hThread, &sf, &context, NULL, SymFunctionTableAccess, SymGetModuleBase, NULL)) {
  ...
  #ifndef __i386__
      if (old_address.Offset == sf.AddrPC.Offset) break;
  #endif
      pSymbol->SizeOfStruct = sizeof(SYMBOL_INFO);
      pSymbol->MaxNameLen = MAX_SYM_NAME;
      if (SymFromAddr (GetCurrentProcess(), sf.AddrPC.Offset, &displacement, pSymbol)) {
          if (displacement) DPRINTF("%3d %s+0x%I64x", i++, pSymbol->Name, displacement);
          else DPRINTF("%3d %s", i++, pSymbol->Name);
      } else DPRINTF("%3d 0x%lx", i++, sf.AddrPC.Offset);
      line.SizeOfStruct = sizeof(IMAGEHLP_LINE);
      if (SymGetLineFromAddr(GetCurrentProcess (), sf.AddrPC.Offset, &dwDisplacement, &line)) DPRINTF(" [%s:%ld]", line.FileName, line.LineNumber);
      moduleInfo.SizeOfStruct = sizeof(moduleInfo);
      if (SymGetModuleInfo(GetCurrentProcess(), sf.AddrPC.Offset, &moduleInfo)) DPRINTF(" in %s", moduleInfo.ModuleName);
      DPRINTF("\n");
...
dbus_bool_t _dbus_read_local_machine_uuid(DBusGUID *machine_id, dbus_bool_t create_if_not_found, DBusError *error) {
  ...
  lpc++;
  u = ((fromAscii(lpc[0]) <<  0) | (fromAscii(lpc[1]) <<  4) | (fromAscii(lpc[2]) <<  8) | (fromAscii(lpc[3]) << 12) |
...
static dbus_bool_t _dbus_get_address_string(DBusString *out, const char *basestring, const char *scope) {
  ...
  else if (strcmp(scope,"*install-path") == 0 || strcmp(scope,"install-path") == 0) {
      DBusString temp;
...
dbus_bool_t _dbus_daemon_publish_session_bus_address(const char* address, const char *scope) {
  ...
  lock = _dbus_global_lock(cUniqueDBusInitMutex);
  if (!hDBusDaemonMutex) hDBusDaemonMutex = CreateMutexA(NULL, FALSE, _dbus_string_get_const_data(&mutex_name));
  _dbus_string_free(&mutex_name);
  ...
  hDBusSharedMem = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, len >> 32, len & 0xffffffffu, _dbus_string_get_const_data(&shm_name));
...
dbus_bool_t _dbus_get_autolaunch_address(const char *scope, DBusString *address, DBusError *error) {
  ...
  if (!SearchPathA(NULL, daemon_name, NULL, sizeof(dbus_exe_path), dbus_exe_path, &lpFile)) {
      ...
      DWORD rc;
      _dbus_verbose("did not found dbus daemon executable on default search path, trying path where dbus shared library is located");
  ...
  if(CreateProcessA(dbus_exe_path, dbus_args, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
      ...
      if (retval == FALSE) dbus_set_error_const(error, DBUS_ERROR_FAILED, "Failed to get autolaunch address from launched dbus-daemon");
  } else {
...
dbus_bool_t _dbus_get_install_root(DBusString *str) {
  ...
  do {
      ...
      SetLastError(0);
      pathLength = GetModuleFileNameA(_dbus_win_get_dll_hmodule(), _dbus_string_get_data(str), _dbus_string_get_length(str));
  ...
  lastSlash[1] = 0;
  if (lastSlash - prefix >= 4 && _mbsnicmp(lastSlash - 4, (const unsigned char*)"\\bin", 4) == 0) lastSlash[-3] = 0;
  else if (lastSlash - prefix >= 10 && _mbsnicmp(lastSlash - 10, (const unsigned char*)"\\bin\\debug", 10) == 0) lastSlash[-9] = 0;
  else if (lastSlash - prefix >= 12 && _mbsnicmp (lastSlash - 12, (const unsigned char *)"\\bin\\release", 12) == 0) lastSlash[-11] = 0;
...
dbus_bool_t _dbus_append_keyring_directory_for_credentials(DBusString *directory, DBusCredentials *credentials) {
  ...
  if (homedrive != NULL && *homedrive != '\0') _dbus_string_append(&homedir,homedrive);
  homepath = _dbus_getenv("HOMEPATH");
  if (homepath != NULL && *homepath != '\0') _dbus_string_append(&homedir,homepath);
...
dbus_bool_t _dbus_file_exists(const char *file) {
  ...
  if (attributes != INVALID_FILE_ATTRIBUTES && GetLastError() != ERROR_PATH_NOT_FOUND) return TRUE;
  else */return FALSE;
}
...
void _dbus_win_set_error_from_win_error(DBusError *error, int code) {
  /*char *msg;
  FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, ULL, code,
                 MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPSTR)&msg, 0, NULL);
  if (msg) {
      dbus_set_error(error, "win32.error", "%s", msg);
      LocalFree(msg);
  } else dbus_set_error(error, "win32.error", "Unknown error code %d or FormatMessage failed", code);*/
}
...
dbus_bool_t _dbus_path_is_absolute(const DBusString *filename) {
  if (_dbus_string_get_length(filename) > 0) {
    return _dbus_string_get_byte(filename, 1) == ':' || _dbus_string_get_byte(filename, 0) == '\\' || _dbus_string_get_byte(filename, 0) == '/';
  } else return FALSE;
}
