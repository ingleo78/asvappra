...
DBusServer *_dbus_server_new_for_launchd(const char *launchd_env_var, DBusError * error) {
    ...
    if (launchd_socket_path == NULL || *launchd_socket_path == '\0') {
        dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS, "launchd's environment variable %s is empty, but should contain a socket path.\n", launchd_env_var);
        return NULL;
    }
    ...
    if (launchd_fd < 0) {
        ...
        if (display == NULL || *display == '\0') {
            ...
            if (NULL == environment_dict) _dbus_warn("Unable to retrieve user environment from launchd.");
            else {
                environment_param = launch_data_dict_lookup(environment_dict, "DISPLAY");
                if (NULL == environment_param) _dbus_warn("Unable to retrieve DISPLAY from launchd.");
                else {
    ...
    if (server == NULL) goto l_failed_0;
    _dbus_string_free(&address);
    return server;
l_failed_0:
    if (launchd_fd >= 0) close(launchd_fd);
    _dbus_string_free(&address);
    return NULL;
#else
    dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS,"address type 'launchd' requested, but launchd support not compiled in");
    return NULL;
#endif
}