...
static void new_connection_callback(DBusServer *server, DBusConnection *new_connection, void *data) {
  ...
  if (!test_connection_try_setup(testdata->loop, new_connection)) dbus_connection_close(new_connection);
}
static DBusHandlerResult filter_session_message(DBusConnection *connection, DBusMessage *message, void *user_data) {
  ...
  if (dbus_message_is_method_call(message,"org.freedesktop.DBus.TestSuite.PrivServer","GetPrivateAddress")) {
       ...
       if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &(testdata->private_addr), DBUS_TYPE_INVALID)) die("OOM");
