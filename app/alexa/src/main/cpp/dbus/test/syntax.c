...
static void test_syntax(Fixture *f, gconstpointer arg) {
  ...
  for (i = 0; test->valid[i] != NULL; i++) {
      ...
      if (dbus_error_is_set(&f->e)) g_error("%s was considered invalid: %s: %s", test->valid[i], f->e.name, f->e.message);
  ...
  for (i = 0; test->invalid[i] != NULL; i++) {
      ...
      if (!dbus_validate_error_name(f->e.name, NULL)) g_error("%s produced an invalid error name: %s", test->invalid[i], f->e.name);
      if (!dbus_validate_utf8 (f->e.message, NULL)) g_error("%s produced an invalid error message: %s", test->invalid[i], f->e.message);
      dbus_error_free(&f->e);
  }
}
