...
static DBusHandlerResult server_message_cb(DBusConnection *conn, DBusMessage *message, void *data) {
  ...
  if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_CALL) {
      ...
      unsigned long uid;
      char *sid;
      if (dbus_connection_get_unix_user(conn, &uid)) g_message("message from uid %lu", uid);
      else if (dbus_connection_get_windows_user(conn, &sid)) {
          if (sid == NULL) oom();
          g_message("message from sid \"%s\"", sid);
          dbus_free(sid);
      } else if (dbus_connection_get_is_anonymous(conn)) g_message("message from Anonymous");
      else g_message("message from ... someone?");
      if (reply == NULL) oom();
...
static dbus_bool_t broken_unix_func(DBusConnection *conn, unsigned long uid, void *data) {
  g_error("libdbus called the Unix user function for an ANONYMOUS-only connection");
  return FALSE;
}
static dbus_bool_t broken_win_func(DBusConnection *conn, const char *sid, void *data) {
  g_error("libdbus called the Windows user function for an ANONYMOUS-only connection");
  return FALSE;
}
...
static dbus_bool_t same_uid_win_func(DBusConnection *conn, const char *sid, void *data) {
  ...
  g_message("Stub implementation consistent with dbus-sysdeps-util-win: assume they do");
  return TRUE;
}
static void new_conn_cb(DBusServer *server, DBusConnection *conn, void *data) {
  ...
  else if (server == f->anon_allowed_server) dbus_connection_set_allow_anonymous(conn, TRUE);
...
int main(int argc, char **argv) {
  ...
  if (argc >= 2) setup(&f, argv[1]);
  else setup(&f, "tcp:host=127.0.0.1");
