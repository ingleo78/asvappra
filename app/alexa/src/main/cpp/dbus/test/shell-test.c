...
static dbus_bool_t test_command_line_internal(dbus_bool_t should_work, const char *arg1, va_list var_args) {
  ...
  for (i = 0, node = _dbus_list_get_first_link(&list); i < original_argc && node; i++, node = _dbus_list_get_next_link(&list, node)) {
  ...
  if (!_dbus_shell_parse_argv(command_line, &shell_argc, &shell_argv, &error)) {
      printf("# Error%s parsing command line: %s\n", should_work ? "" : " (as expected)", error.message ? error.message : "");
      dbus_free(original_argv);
      _dbus_string_free(&str);
      return !should_work;
  } else {
...
static void test_command_line(const char *arg1, ...) {
  ...
  if (test_command_line_internal(TRUE, arg1, var_args)) printf("ok %d\n", ++test_num);
  else {
      printf("not ok %d\n", ++test_num);
      num_failed++;
  }
...
static void test_command_line_fails(const char *arg1, ...) {
  va_list var_args;
  va_start(var_args, arg1);
  if (test_command_line_internal(FALSE, arg1, var_args)) printf("ok %d\n", ++test_num);
  else {
      printf("not ok %d\n", ++test_num);
      num_failed++;
  }
