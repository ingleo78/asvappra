...
static dbus_bool_t _dbus_write_pid_file(const DBusString *filename, unsigned long pid, DBusError *error) {
  ...
  hnd = CreateFileA(cfilename, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, INVALID_HANDLE_VALUE);
  ...
  if (snprintf(pidstr, sizeof(pidstr), "%lu\n", pid) < 0) {
      dbus_set_error(error, _dbus_error_from_system_errno (),"Failed to format PID for \"%s\": %s", cfilename, _dbus_strerror_from_errno());
      CloseHandle(hnd);
      return FALSE;
  }
  total = 0;
...
dbus_bool_t _dbus_stat(const DBusString *filename, DBusStat *statbuf, DBusError *error) {
  ...
  if (wfad.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) statbuf->mode = _S_IFDIR;
  else statbuf->mode = _S_IFREG;
...
#ifdef ENABLE_UID_TO_SID
  {
      ...
      sd = NULL;
      rc = GetNamedSecurityInfo((char*)filename_c, SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION, &owner_sid, &group_sid, NULL, NULL,
                                 &sd);
      if (rc != ERROR_SUCCESS) {
  ...
  statbuf->atime = (((dbus_int64_t)wfad.ftLastAccessTime.dwHighDateTime << 32) +
                    wfad.ftLastAccessTime.dwLowDateTime) / 10000000 - DBUS_INT64_CONSTANT(116444736000000000);
  statbuf->mtime = (((dbus_int64_t)wfad.ftLastWriteTime.dwHighDateTime << 32) +
                    wfad.ftLastWriteTime.dwLowDateTime) / 10000000 - DBUS_INT64_CONSTANT(116444736000000000);
  statbuf->ctime = (((dbus_int64_t)wfad.ftCreationTime.dwHighDateTime << 32) +
...
DBusDirIter* _dbus_directory_open(const DBusString *filename, DBusError *error) {
  ...
  if (iter->handle == INVALID_HANDLE_VALUE) {
      if (GetLastError() == ERROR_NO_MORE_FILES) iter->finished = TRUE;
      else {
          char *emsg = _dbus_win_error_string(GetLastError());
          dbus_set_error(error, _dbus_win_error_from_last_error(),"Failed to read directory \"%s\": %s", _dbus_string_get_const_data(filename), emsg);
          _dbus_win_free_error_string(emsg);
          dbus_free(iter);
          _dbus_string_free(&filespec);
          return NULL;
...
again:
  ...
  if (iter->fileinfo.cFileName[0] == '.' && (iter->fileinfo.cFileName[1] == '\0' || (iter->fileinfo.cFileName[1] == '.' && iter->fileinfo.cFileName[2] == '\0')))
      goto again;
...
dbus_bool_t _dbus_string_get_dirname(const DBusString *filename, DBusString *dirname) {
  ...
  while(sep > 0 && (_dbus_string_get_byte(filename, sep - 1) == '/' || _dbus_string_get_byte(filename, sep - 1) == '\\')) --sep;
  _dbus_assert(sep >= 0);
  if (sep == 0 || (sep == 2 && _dbus_string_get_byte(filename, 1) == ':' && isalpha(_dbus_string_get_byte(filename, 0))))
...
dbus_bool_t _dbus_replace_install_prefix(DBusString *path) {
  ...
  if (_dbus_string_starts_with_c_str(path, DBUS_PREFIX "/")) {
      if (!_dbus_string_replace_len(&runtime_prefix,0, _dbus_string_get_length(&runtime_prefix), path,0,strlen(DBUS_PREFIX) + 1)) {
          _dbus_string_free(&runtime_prefix);
          return FALSE;
      }
  }
...
dbus_bool_t
_dbus_set_up_transient_session_servicedirs(DBusList **dirs, DBusError *error) {
  return TRUE;
}
