...
#define KEY_LENGTH_BYTES 24
  _dbus_string_set_length(&bytes, 0);
  if (!_dbus_generate_random_bytes(&bytes, KEY_LENGTH_BYTES, error)) goto out;
...
static dbus_bool_t _dbus_keyring_reload(DBusKeyring *keyring, dbus_bool_t add_new, DBusError *error) {
  ...
  while(_dbus_string_pop_line(&contents, &line)) {
      ...
      if (timestamp < 0 || (now + MAX_TIME_TRAVEL_SECONDS) < timestamp || (now - EXPIRE_KEYS_TIMEOUT_SECONDS) > timestamp) {
...
out:
  ...
  if (! ((retval == TRUE && (error == NULL || error->name == NULL)) || (retval == FALSE && (error == NULL || error->name != NULL)))) {
...
DBusKeyring* _dbus_keyring_new_for_credentials(DBusCredentials *credentials, const DBusString *context, DBusError *error) {
  ...
  if (credentials != NULL) our_credentials = _dbus_credentials_copy(credentials);
  else our_credentials = _dbus_credentials_new_from_current_process();
...
int _dbus_keyring_get_best_key(DBusKeyring *keyring, DBusError *error) {
  ...
  if (key) return key->id;
      ...
      dbus_set_error_const(error, DBUS_ERROR_FAILED,"No recent-enough key found in keyring, and unable to create a new key");
      return -1;
  }
}
...
dbus_bool_t _dbus_keyring_test(void) {
  ...
  _dbus_assert(!_dbus_keyring_validate_context(&context));
  if (!_dbus_string_init(&context)) _dbus_test_fatal ("no memory");
  if (!_dbus_string_append_byte(&context, '\0')) _dbus_test_fatal("no memory");
  _dbus_assert(!_dbus_keyring_validate_context(&context));
