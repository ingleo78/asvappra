...
DBUS_PRIVATE_EXPORT void _dbus_warn(const char *format, ...) _DBUS_GNUC_PRINTF(1, 2);
DBUS_PRIVATE_EXPORT void _dbus_warn_check_failed(const char *format, ...) _DBUS_GNUC_PRINTF(1, 2);
DBUS_PRIVATE_EXPORT void _dbus_warn_return_if_fail(const char *function, const char *assertion, const char *file, int line);
...
#define _DBUS_BOOLEAN_EXPR(expr)                \
 __extension__ ({                               \
   ...
   if (expr) _dbus_boolean_var_ = 1;            \
   else _dbus_boolean_var_ = 0;                 \
   _dbus_boolean_var_;                          \
})
...
DBUS_PRIVATE_EXPORT void _dbus_verbose_real(const char *file, const int line, const char *function, const char *format,...) _DBUS_GNUC_PRINTF(4, 5);
#define _dbus_verbose(fmt,...) _dbus_verbose_real( __FILE__,__LINE__,__FUNCTION__,fmt, ## __VA_ARGS__)
#else
DBUS_PRIVATE_EXPORT void _dbus_verbose_real(const char *format, ...) _DBUS_GNUC_PRINTF (1, 2);
#define _dbus_verbose _dbus_verbose_real
#endif
DBUS_PRIVATE_EXPORT void _dbus_verbose_reset_real (void);
DBUS_PRIVATE_EXPORT dbus_bool_t _dbus_is_verbose_real (void);
DBUS_PRIVATE_EXPORT dbus_bool_t _dbus_get_verbose (void);
DBUS_PRIVATE_EXPORT void _dbus_set_verbose (dbus_bool_t state);
...
#endif
DBUS_PRIVATE_EXPORT void _dbus_trace_ref(const char *obj_name, void *obj, int old_refcount, int new_refcount, const char *why, const char *env_var, int *enabled);
DBUS_PRIVATE_EXPORT const char* _dbus_strerror(int error_number);
...
#define _DBUS_ALIGNOF(type)  (_DBUS_STRUCT_OFFSET(struct { char _1; type _2; }, _2))
#ifdef DBUS_DISABLE_CHECKS
...
#define _DBUS_ALIGN_VALUE(this, boundary)  (( ((uintptr_t)(this)) + (((uintptr_t)(boundary)) -1)) & (~(((uintptr_t)(boundary))-1)))
...
DBUS_PRIVATE_EXPORT extern const char *_dbus_no_memory_message;
#define _DBUS_SET_OOM(error)  dbus_set_error_const((error), DBUS_ERROR_NO_MEMORY, _dbus_no_memory_message)
DBUS_PRIVATE_EXPORT void _dbus_set_error_valist(DBusError *error, const char *name, const char *format, va_list args) _DBUS_GNUC_PRINTF(3, 0);
typedef dbus_bool_t (*DBusTestMemoryFunction)(void *data, dbus_bool_t have_memory);
...
DBUS_PRIVATE_EXPORT dbus_bool_t _dbus_test_oom_handling(const char *description, DBusTestMemoryFunction func, void *data);
#else
...
DBUS_PRIVATE_EXPORT dbus_bool_t _dbus_threads_init_debug(void);
