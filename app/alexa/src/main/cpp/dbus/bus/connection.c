...
typedef struct {
  ...
  long connection_tv_sec;
  long connection_tv_usec;
  int stamp;
#ifdef DBUS_ENABLE_STATS
  int peak_match_rules;
  int peak_bus_names;
...
static dbus_bool_t adjust_connections_for_uid(BusConnections *connections, dbus_uid_t uid, int adjustment) {
  ...
  _dbus_verbose("Adjusting connection count for UID " DBUS_UID_FORMAT ": was %d adjustment %d making %d\n", uid, current_count, adjustment,
  ...
  if (current_count == 0) {
      ...
      dbus_bool_t retval;
      retval = _dbus_hash_table_insert_uintptr(connections->completed_by_user, uid, _DBUS_INT_TO_POINTER(current_count));
...
void bus_connection_disconnected(DBusConnection *connection) {
  ...
  bus_containers_remove_connection(bus_context_get_containers(d->connections->context), connection);
  if (d->link_in_connection_list != NULL) {
      if (d->name != NULL) {
          ...
          if (dbus_connection_get_unix_user(connection, &uid)) {
              if (!adjust_connections_for_uid(d->connections, uid, -1)) _dbus_assert_not_reached("adjusting downward should never fail");
          }
      } else {
...
BusConnections* bus_connections_new(BusContext *context) {
  ...
  connections->expire_timeout = _dbus_timeout_new(100, expire_incomplete_timeout, connections, NULL);
  if (connections->expire_timeout == NULL) goto failed_3;
  _dbus_timeout_disable(connections->expire_timeout);
  connections->pending_replies = bus_expire_list_new(bus_context_get_loop(context), bus_context_get_reply_timeout(context), bus_pending_reply_expired,
                                                     connections);
...
static dbus_bool_t cache_peer_loginfo_string(BusConnectionData *d, DBusConnection *connection) {
  ...
  prev_added = FALSE;
  if (dbus_connection_get_unix_user(connection, &uid)) {
      if (!_dbus_string_append_printf(&loginfo_buf, "uid=%ld", uid)) goto oom;
      else prev_added = TRUE;
  }
  ...
  if (bus_containers_connection_is_contained(connection, &container, &container_type, &container_name)) {
      dbus_bool_t did_append;
      if (prev_added) {
          if (!_dbus_string_append_byte(&loginfo_buf, ' ')) goto oom;
      }
      did_append = _dbus_string_append_printf(&loginfo_buf,"container=%s %s=\"%s\")", container, container_type, container_name);
      if (!did_append) goto oom;
      else prev_added = TRUE;
  }
...
static void check_pending_fds_cb(DBusConnection *connection) {
  ...
  if (n_pending_unix_fds_old > 0 && n_pending_unix_fds_new == 0) _dbus_timeout_disable(d->pending_unix_fds_timeout);
...
static dbus_bool_t pending_unix_fds_timeout_cb(void *data) {
  ...
  bus_context_log(d->connections->context, DBUS_SYSTEM_LOG_WARNING,"Connection \"%s\" (%s) has had Unix fds pending for too long, "
                  "closing it (pending_fd_timeout=%d ms)", d->name != NULL ? d->name : "(null)", bus_connection_get_loginfo(connection), limit);
  dbus_connection_close(connection);
  return TRUE;
}
  ...
  if (connections->incomplete != NULL) {
      ...
      while(link != NULL) {
          ...
          if (elapsed >= (double)auth_timeout) {
              bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING,"Connection has not authenticated soon enough, closing it "
                              "(auth_timeout=%dms, elapsed: %.0fms)", auth_timeout, elapsed);
...
dbus_bool_t bus_connection_get_unix_groups(DBusConnection *connection, unsigned long **groups, int *n_groups, DBusError *error) {
  unsigned long uid;
  *groups = NULL;
  *n_groups = 0;
  if (dbus_connection_get_unix_user(connection, &uid)) {
...
dbus_bool_t bus_connection_mark_stamp(DBusConnection *connection) {
  ...
  d = BUS_CONNECTION_DATA(connection);
  _dbus_assert(d != NULL);
  if (d->stamp == d->connections->stamp) return FALSE;
  else {
...
dbus_bool_t bus_connection_preallocate_oom_error(DBusConnection *connection) {
  ...
  if (!dbus_message_set_error_name(message, DBUS_ERROR_NO_MEMORY) || !dbus_message_set_destination(message, d->name) ||
...
void bus_connection_send_oom_error(DBusConnection *connection, DBusMessage *in_reply_to) {
  ...
  bus_context_log(d->connections->context, DBUS_SYSTEM_LOG_WARNING,"dbus-daemon transaction failed (OOM), sending error to "
...
dbus_bool_t bus_connections_reload_policy(BusConnections *connections, DBusError *error) {
  ...
  for (link = _dbus_list_get_first_link(&(connections->completed)); link; link = _dbus_list_get_next_link(&(connections->completed), link)) {
      connection = link->data;
      d = BUS_CONNECTION_DATA(connection);
      _dbus_assert(d != NULL);
      _dbus_assert(d->policy != NULL);
...
dbus_bool_t bus_connections_check_limits(BusConnections *connections, DBusConnection *requesting_completion, const char **limit_name_out, int *limit_out,
                                          DBusError *error) {
  unsigned long uid;
  int limit;
  ...
  if (dbus_connection_get_unix_user(requesting_completion, &uid)) {
      ...
      if (get_connections_for_uid(connections, uid) >= limit) {
          ...
          if (limit_out != NULL) *limit_out = limit;
          dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,"The maximum number of active connections for UID %lu has been reached", uid);
          return FALSE;
      }
  }
  return TRUE;
...
static void bus_pending_reply_free(BusPendingReply *pending) {
  _dbus_verbose("Freeing pending reply %p, replier %p receiver %p serial %u\n", pending, pending->will_send_reply, pending->will_get_reply, pending->reply_serial);
  dbus_free(pending);
}
static dbus_bool_t bus_pending_reply_send_no_reply(BusConnections *connections, BusTransaction *transaction, BusPendingReply *pending) {
  DBusMessage *message;
  ...
  if (pending->will_send_reply == NULL) errmsg = "Message recipient disconnected from message bus without replying";
  else errmsg = "Message did not receive a reply (timeout by message bus)";
...
static void cancel_pending_reply(void *data) {
  ...
  bus_pending_reply_free(d->pending);
}
...
dbus_bool_t bus_connections_expect_reply(BusConnections *connections, BusTransaction *transaction, DBusConnection *will_get_reply, DBusConnection *will_send_reply,
                                         DBusMessage *reply_to_this, DBusError *error) {
  BusPendingReply *pending;
  dbus_uint32_t reply_serial;
  DBusList *link;
  ...
  if (dbus_message_get_no_reply(reply_to_this)) return TRUE;
  reply_serial = dbus_message_get_serial(reply_to_this);
  link = bus_expire_list_get_first_link(connections->pending_replies);
  count = 0;
  while(link != NULL) {
      pending = link->data;
      if (pending->reply_serial == reply_serial && pending->will_get_reply == will_get_reply && pending->will_send_reply == will_send_reply) {
          dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,"Message has the same reply serial as a currently-outstanding existing method call");
          return FALSE;
      }
  ...
  if (count >= limit) {
      bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING,"The maximum number of pending replies for \"%s\" (%s) has been reached "
                      "(max_replies_per_connection=%d)", bus_connection_get_name(will_get_reply), bus_connection_get_loginfo(will_get_reply), limit);
      dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,"The maximum number of pending replies per connection has been reached");
      return FALSE;
  }
...
dbus_bool_t bus_connections_check_reply(BusConnections *connections, BusTransaction *transaction, DBusConnection *sending_reply, DBusConnection *receiving_reply,
  ...
  while(link != NULL) {
      BusPendingReply *pending = link->data;
      if (pending->reply_serial == reply_serial && pending->will_get_reply == receiving_reply && pending->will_send_reply == sending_reply) {
  ...
  if (!bus_transaction_add_cancel_hook (transaction, cancel_check_pending_reply, cprd, check_pending_reply_data_free)) {
      BUS_SET_OOM(error);
      dbus_free(cprd);
      return FALSE;
  }
...
dbus_bool_t bus_transaction_capture(BusTransaction *transaction, DBusConnection *sender, DBusConnection *addressed_recipient, DBusMessage *message) {
  BusConnections *connections;
  BusMatchmaker *mm;
  DBusList *link;
  ...
  for (link = _dbus_list_get_first_link(&recipients); link != NULL; link = _dbus_list_get_next_link(&recipients, link)) {
...
dbus_bool_t bus_transaction_capture_error_reply(BusTransaction *transaction, DBusConnection *addressed_recipient, const DBusError *error,
...
dbus_bool_t bus_transaction_send_from_driver(BusTransaction *transaction, DBusConnection *connection, DBusMessage *message) {
  ...
  _dbus_verbose("Sending %s %s %s from driver\n", dbus_message_get_interface(message) ? dbus_message_get_interface(message) : "(no interface)",
                dbus_message_get_member(message) ? dbus_message_get_member(message) : "(no member)", dbus_message_get_error_name(message) ?
  ...
  if (!bus_context_check_security_policy(bus_transaction_get_context(transaction), transaction,NULL, connection, connection, message,NULL,
      &error)) {
      if (!bus_transaction_capture_error_reply(transaction, connection, &error, message)) {
          bus_context_log(transaction->context, DBUS_SYSTEM_LOG_WARNING,"message from dbus-daemon rejected but not enough memory to capture it");
      }
      dbus_error_free(&error);
      return TRUE;
  }
...
dbus_bool_t bus_transaction_send(BusTransaction *transaction, DBusConnection *connection, DBusMessage *message) {
  ...
  _dbus_verbose("  trying to add %s interface=%s member=%s error=%s to transaction%s\n", dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR ? "error" :
                dbus_message_get_reply_serial(message) != 0 ? "reply" : "message", dbus_message_get_interface(message) ?
                dbus_message_get_interface(message) : "(unset)", dbus_message_get_member(message) ? dbus_message_get_member(message) : "(unset)",
                dbus_message_get_error_name(message) ? dbus_message_get_error_name(message) : "(unset)", dbus_connection_get_is_connected(connection) ?
                "" : " (disconnected)");
  _dbus_assert(dbus_message_get_sender (message) != NULL);
  if (!dbus_connection_get_is_connected (connection)) return TRUE;
  d = BUS_CONNECTION_DATA(connection);
  _dbus_assert(d != NULL);
  to_send = dbus_new(MessageToSend, 1);
  if (to_send == NULL) return FALSE;
...
void bus_transaction_cancel_and_free(BusTransaction *transaction) {
  ...
  while((connection = _dbus_list_pop_first(&transaction->connections))) connection_cancel_transaction(connection, transaction);
...
void bus_transaction_execute_and_free(BusTransaction *transaction) {
  ...
  while((connection = _dbus_list_pop_first(&transaction->connections))) connection_execute_transaction(connection, transaction);
  transaction_free(transaction);
}
static void bus_connection_remove_transactions(DBusConnection *connection) {
  MessageToSend *to_send;
...
dbus_bool_t bus_transaction_send_error_reply(BusTransaction *transaction, DBusConnection *connection, const DBusError *error, DBusMessage *in_reply_to) {
  DBusMessage *reply;
  _dbus_assert(error != NULL);
  _DBUS_ASSERT_ERROR_IS_SET(error);
...
dbus_bool_t bus_transaction_add_cancel_hook(BusTransaction *transaction, BusTransactionCancelFunction cancel_function, void *data,
                                            DBusFreeFunction free_data_function) {
  CancelHook *ch;
...
dbus_bool_t bus_connection_be_monitor(DBusConnection *connection, BusTransaction *transaction, DBusList **rules, DBusError *error) {
  BusConnectionData *d;
  DBusList *link;
  DBusList *tmp;
  DBusList *iter;
  ...
  _dbus_list_clear(&tmp);
  bus_context_log(transaction->context, DBUS_SYSTEM_LOG_INFO,"Connection %s (%s) became a monitor.", d->name, d->cached_loginfo_string);
  if (d->n_match_rules > 0) {
      BusMatchmaker *mm;
