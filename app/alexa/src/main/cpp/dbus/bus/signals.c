...
static char* match_rule_to_string(BusMatchRule *rule) {
  ...
  if (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) {
      ...
      if (!append_key_and_escaped_value(&str, "eavesdrop", (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) ? "true" : "false")) goto nomem;
  }
  if (rule->flags & BUS_MATCH_ARGS) {
      ...
      while(i < rule->args_len) {
          if (rule->args[i] != NULL) {
              ...
              if (!_dbus_string_append_printf(&str, "arg%d%s", i, is_path ? "path" : is_namespace ? "namespace" : "")) goto nomem;
...
void bus_match_rule_set_client_is_eavesdropping(BusMatchRule *rule, dbus_bool_t is_eavesdropping) {
  if (is_eavesdropping) rule->flags |= BUS_MATCH_CLIENT_IS_EAVESDROPPING;
  else rule->flags &= ~(BUS_MATCH_CLIENT_IS_EAVESDROPPING);
}
dbus_bool_t bus_match_rule_get_client_is_eavesdropping(BusMatchRule *rule) {
  if (rule->flags & BUS_MATCH_CLIENT_IS_EAVESDROPPING) return TRUE;
  else return FALSE;
}
dbus_bool_t bus_match_rule_set_path(BusMatchRule *rule, const char *path, dbus_bool_t is_namespace) {
  ...
  if (is_namespace) rule->flags |= BUS_MATCH_PATH_NAMESPACE;
  else rule->flags |= BUS_MATCH_PATH;
  dbus_free(rule->path);
  rule->path = new;
  return TRUE;
}
...
static dbus_bool_t find_value(const DBusString *str, int start, const char *key, DBusString *value, int *value_end, DBusError *error) {
  const char *p;
  const char *s;
  char quote_char;
  int orig_len;
  ...
  if (quote_char == '\\') {
      ...
      goto failed;
  } else _dbus_assert(quote_char == '\0');
  *value_end = p - s;
  return TRUE;
failed:
...
static dbus_bool_t bus_match_rule_parse_arg_match(BusMatchRule *rule, const char *key, const DBusString *value, DBusError *error) {
  ...
  if (_dbus_string_get_length (&key_str) < 4) {
      dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"Key '%s' in match rule starts with 'arg' but lacks an arg number. Should be 'arg0' or 'arg7' "
                     "for example.\n", key);
      goto failed;
  }
  if (!_dbus_string_parse_uint(&key_str, 3, &arg, &end)) {
      dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"Key '%s' in match rule starts with 'arg' but could not parse arg number. Should be 'arg0' or "
                     "'arg7' for example.\n", key);
      goto failed;
  }
  if (end != length) {
      int len1 = strlen("path");
      if ((end + len1) == length && _dbus_string_ends_with_c_str(&key_str, "path")) is_path = TRUE;
          ...
          if (!_dbus_validate_bus_namespace(value, 0, value_len)) {
              dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"arg0namespace='%s' is not a valid prefix of a bus name",
                             _dbus_string_get_const_data(value));
              goto failed;
          }
      } else {
          dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"Key '%s' in match rule contains junk after argument number (%lu). Only 'arg%lupath' (for "
                         "example) or 'arg0namespace' are valid", key, arg, arg);
          goto failed;
      }
  }
  if (arg > DBUS_MAXIMUM_MATCH_RULE_ARG_NUMBER) {
      dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"Key '%s' in match rule has arg number %lu but the maximum is %d.\n", key, (unsigned long)arg,
...
BusMatchRule* bus_match_rule_parse(DBusConnection *matches_go_to, const DBusString *rule_text, DBusError *error) {
  BusMatchRule *rule;
  RuleToken tokens[MAX_RULE_TOKENS+1];
  int i;
  _DBUS_ASSERT_ERROR_IS_CLEAR(error);
  if (_dbus_string_get_length(rule_text) > DBUS_MAXIMUM_MATCH_RULE_LENGTH) {
      dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,"Match rule text is %d bytes, maximum is %d", _dbus_string_get_length(rule_text),
  ...
  while(tokens[i].key != NULL) {
      ...
      if (strcmp(key, "type") == 0) {
          ...
          if (rule->flags & (BUS_MATCH_PATH | BUS_MATCH_PATH_NAMESPACE)) {
              dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID, "path or path_namespace specified twice in match rule\n");
              goto failed;
          }
          ...
          if ((strcmp(value, "true") == 0)) bus_match_rule_set_client_is_eavesdropping(rule, TRUE);
          else if (strcmp(value, "false") == 0) bus_match_rule_set_client_is_eavesdropping(rule, FALSE);
          else {
              dbus_set_error(error, DBUS_ERROR_MATCH_RULE_INVALID,"eavesdrop='%s' is invalid, it should be 'true' or 'false'\n", value);
              goto failed;
          }
...
nomem:
  for (i = DBUS_MESSAGE_TYPE_INVALID; i < DBUS_NUM_MESSAGE_TYPES; i++) {
      RulePool *p = matchmaker->rules_by_type + i;
      if (p->rules_by_iface == NULL) break;
      else _dbus_hash_table_unref(p->rules_by_iface);
  }
  dbus_free(matchmaker);
  return NULL;
}
static DBusList **bus_matchmaker_get_rules(BusMatchmaker *matchmaker, int message_type, const char *interface, dbus_bool_t create) {
  RulePool *p;
  ...
  if (interface == NULL) return &p->rules_without_iface;
  else {
      DBusList **list;
...
static void bus_matchmaker_gc_rules (BusMatchmaker *matchmaker, int message_type, const char *interface, DBusList **rules) {
  RulePool *p;
...
dbus_bool_t bus_matchmaker_add_rule(BusMatchmaker *matchmaker, BusMatchRule *rule) {
  ...
  _dbus_verbose("Adding rule with message_type %d, interface %s\n", rule->message_type, rule->interface != NULL ? rule->interface : "<null>");
...
static void rule_list_remove_by_connection(DBusList **rules, DBusConnection *connection) {
  ...
  while(link != NULL) {
      ...
      if (rule->matches_go_to == connection) bus_matchmaker_remove_rule_link(rules, link);
      else if (((rule->flags & BUS_MATCH_SENDER) && *rule->sender == ':') || ((rule->flags & BUS_MATCH_DESTINATION) && *rule->destination == ':')) {
          const char *name;
          name = bus_connection_get_name(connection);
          _dbus_assert(name != NULL);
          if (((rule->flags & BUS_MATCH_SENDER) && strcmp (rule->sender, name) == 0) || ((rule->flags & BUS_MATCH_DESTINATION) &&
...
static dbus_bool_t match_rule_matches(BusMatchRule *rule, DBusConnection *sender, DBusConnection *addressed_recipient, DBusMessage *message,
  ...
  if (flags & BUS_MATCH_ARGS) {
      ...
      while(i < rule->args_len) {
          ...
          if (expected_arg != NULL) {
              ...
              int actual_length;
              if (current_type != DBUS_TYPE_STRING && (!is_path || current_type != DBUS_TYPE_OBJECT_PATH)) return FALSE;
              actual_arg = NULL;
              ...
              if (is_path) {
                  ...
                  if (expected_length != actual_length || memcmp(expected_arg, actual_arg, expected_length) != 0) return FALSE;
              }
          }
...
static dbus_bool_t get_recipients_from_list(DBusList **rules, DBusConnection *sender, DBusConnection *addressed_recipient, DBusMessage *message,
  ...
  #endif
      if (match_rule_matches(rule,sender, addressed_recipient, message,BUS_MATCH_MESSAGE_TYPE | BUS_MATCH_INTERFACE)) {
          _dbus_verbose("Rule matched\n");
          if (bus_connection_mark_stamp(rule->matches_go_to)) {
              if (!_dbus_list_append(recipients_p, rule->matches_go_to))
                return FALSE;
          } else { _dbus_verbose("Connection already receiving this message, so not adding again\n"); }
      }
...
dbus_bool_t bus_matchmaker_get_recipients(BusMatchmaker *matchmaker, BusConnections *connections, DBusConnection *sender, DBusConnection *addressed_recipient,
  ...
  if (interface != NULL) just_iface = bus_matchmaker_get_rules(matchmaker,DBUS_MESSAGE_TYPE_INVALID, interface, FALSE);
  ...
  if (!(get_recipients_from_list(neither, sender, addressed_recipient, message, recipients_p) && get_recipients_from_list(just_iface, sender, addressed_recipient,
      message, recipients_p) && get_recipients_from_list(just_type, sender, addressed_recipient, message, recipients_p) && get_recipients_from_list(both, sender,
      addressed_recipient, message, recipients_p))) {
...
static dbus_bool_t test_parsing(void *data, dbus_bool_t have_memory) {
  BusMatchRule *rule;
  rule = check_parse(TRUE, "type='signal',sender='org.freedesktop.DBusSender',interface='org.freedesktop.DBusInterface',member='Foo',path='/bar/foo',destination='"
                     ":452345.34'");
  if (rule != NULL) {
      assert_large_rule(rule);
      bus_match_rule_unref(rule);
  }
  rule = check_parse(TRUE, "    type='signal',  \tsender='org.freedes''ktop.DBusSender',   interface='org.freedesktop.DBusInterface''''', \tmember='Foo',path="
  ...
  while (i < _DBUS_N_ELEMENTS(equality_tests)) {
      ...
      while(j < _DBUS_N_ELEMENTS(equality_tests)) {
          if (i != j) {
              ...
              _dbus_assert(second != NULL);
              if (match_rule_equal(first, second)) {
                  _dbus_warn("rule %s and %s should not have been equal", equality_tests[i].first, equality_tests[j].second);
                  exit (1);
              }
              bus_match_rule_unref(second);
          }
...
static void check_matches(dbus_bool_t expected_to_match, int number, DBusMessage *message, const char *rule_text) {
  ...
  if (matched != expected_to_match) {
      _dbus_warn("Expected rule %s to %s message %d, failed", rule_text, expected_to_match ? "match" : "not match", number);
      exit(1);
  }
  bus_match_rule_unref(rule);
}
...
static void test_matching(void) {
  ...
  if (!dbus_message_set_member(message1, "Frobated")) _dbus_test_fatal("oom");
  v_STRING = "foobar";
  v_INT32 = 3;
  if (!dbus_message_append_args(message1, DBUS_TYPE_STRING, &v_STRING, DBUS_TYPE_INT32, &v_INT32, NULL)) _dbus_test_fatal("oom");
  ...
  if (!dbus_message_set_member(message2, "NameOwnerChanged")) _dbus_test_fatal("oom");
  v_STRING = EXAMPLE_NAME;
  if (!dbus_message_append_args(message2, DBUS_TYPE_STRING, &v_STRING, NULL)) _dbus_test_fatal("oom");
...
static void test_path_match(int type, const char *path, const char *rule_text, BusMatchRule *rule, dbus_bool_t should_match) {
  DBusMessage *message = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
  dbus_bool_t matched;
  _dbus_assert(message != NULL);
  if (!dbus_message_set_member(message, "Foo")) _dbus_test_fatal("oom");
  if (!dbus_message_append_args(message, type, &path, NULL)) _dbus_test_fatal("oom");
  matched = match_rule_matches(rule, NULL, NULL, message, 0);
  if (matched != should_match) {
      _dbus_warn("Expected rule %s to %s message " "with first arg %s of type '%c', failed", rule_text, should_match ? "match" : "not match", path, (char)type);
      exit (1);
  }
...
static void test_path_matching(void) {
  ...
  _dbus_assert(rule != NULL);
  for (s = paths_that_should_be_matched; *s != NULL; s++) test_path_match(DBUS_TYPE_STRING, *s, PATH_MATCH_RULE, rule, TRUE);
...
static const char* path_namespace_should_not_match_message_2[] = {
  NULL
};
...
static void test_matching_path_namespace(void) {
  ...
  message1 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
  _dbus_assert(message1 != NULL);
  if (!dbus_message_set_path(message1, "/foo/TheObjectManager")) _dbus_test_fatal("oom");
  message2 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
  _dbus_assert(message2 != NULL);
  if (!dbus_message_set_path(message2, "/foo/TheObjectManager/child_object")) _dbus_test_fatal("oom");
  message3 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
  _dbus_assert(message3 != NULL);
  if (!dbus_message_set_path(message3, "/foo/TheObjectManagerOther")) _dbus_test_fatal ("oom");
  message4 = dbus_message_new(DBUS_MESSAGE_TYPE_SIGNAL);
  _dbus_assert(message4 != NULL);
  if (!dbus_message_set_path(message4, "/")) _dbus_test_fatal("oom");
...
dbus_bool_t bus_signals_test(const DBusString *test_data_dir) {
  ...
  bus_matchmaker_unref(matchmaker);
  if (!_dbus_test_oom_handling("parsing match rules", test_parsing, NULL)) _dbus_test_fatal("Parsing match rules test failed");
  test_equality();
  test_matching();
  test_path_matching();
