...
static void expat_StartElementHandler(void *userData, const XML_Char *name, const XML_Char **atts) {
  ...
  if (!bus_config_parser_start_element(context->parser, name, (const char**)names, (const char**)values, context->error)) {
      dbus_free(names);
      dbus_free(values);
      context->failed = TRUE;
      return;
  ...
  XML_SetCharacterDataHandler(expat, expat_CharacterDataHandler);
  {
      ...
      if (XML_Parse (expat, data_str, _dbus_string_get_length(&data), TRUE) == XML_STATUS_ERROR) {
          if (context.error != NULL && !dbus_error_is_set(context.error)) {
              ...
              if (e == XML_ERROR_NO_MEMORY) dbus_set_error (error, DBUS_ERROR_NO_MEMORY, NULL);
              else {
                  dbus_set_error(error, DBUS_ERROR_FAILED,"Error in file %s, line %lu, column %lu: %s\n", filename,
                                 (unsigned long)XML_GetCurrentLineNumber(expat), (unsigned long)XML_GetCurrentColumnNumber(expat), XML_ErrorString(e));
              }
          }
          _dbus_string_free(&data);
          goto failed;
