...
BusDriverFound bus_driver_get_conn_helper(DBusConnection *connection, DBusMessage *message, const char *what_we_want, const char **name_p,
                                          DBusConnection **peer_conn_p, DBusError *error) {
  DBusConnection *conn;
  const char *name;
  if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &name, DBUS_TYPE_INVALID)) return BUS_DRIVER_FOUND_ERROR;
  ...
  if (conn == NULL) {
      dbus_set_error(error, DBUS_ERROR_NAME_HAS_NO_OWNER,"Could not get %s of name '%s': no such name", what_we_want, name);
      return BUS_DRIVER_FOUND_ERROR;
  }
  if (peer_conn_p != NULL) *peer_conn_p = conn;
  return BUS_DRIVER_FOUND_PEER;
}
static dbus_bool_t bus_driver_check_caller_is_not_container(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  if (bus_containers_connection_is_contained(connection, NULL, NULL, NULL)) {
      const char *method = dbus_message_get_member(message);
      bus_context_log_and_set_error(bus_transaction_get_context(transaction),DBUS_SYSTEM_LOG_SECURITY, error, DBUS_ERROR_ACCESS_DENIED,
                               "rejected attempt to call %s by connection %s (%s) in container", method, nonnull(bus_connection_get_name(connection),
                              "(inactive)"), bus_connection_get_loginfo(connection));
      return FALSE;
  }
  return TRUE;
}
static dbus_bool_t bus_driver_check_caller_is_privileged(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
#ifdef DBUS_UNIX
  unsigned long uid;
...
#endif
  if (!bus_driver_check_caller_is_not_container(connection, transaction, message, error)) return FALSE;
#ifdef DBUS_UNIX
  if (!dbus_connection_get_unix_user(connection, &uid)) {
      const char *method = dbus_message_get_member(message);
      bus_context_log_and_set_error(bus_transaction_get_context(transaction),DBUS_SYSTEM_LOG_SECURITY, error, DBUS_ERROR_ACCESS_DENIED,
                               "rejected attempt to call %s by connection %s (%s) with unknown uid", method, nonnull(bus_connection_get_name(connection),
                             "(inactive)"), bus_connection_get_loginfo(connection));
      return FALSE;
  }
  if (_dbus_unix_user_is_process_owner(uid));
      ...
      const char *method = dbus_message_get_member(message);
      bus_context_log_and_set_error(bus_transaction_get_context(transaction),DBUS_SYSTEM_LOG_SECURITY, error, DBUS_ERROR_ACCESS_DENIED,
                               "rejected attempt to call %s by connection %s (%s) with uid %lu", method, nonnull(bus_connection_get_name(connection),
                             "(inactive)"),bus_connection_get_loginfo(connection), uid);
      return FALSE;
  }
  return TRUE;
#elif defined(DBUS_WIN)
  if (!dbus_connection_get_windows_user(connection, &windows_sid)) {
      const char *method = dbus_message_get_member(message);
      bus_context_log_and_set_error(bus_transaction_get_context(transaction), DBUS_SYSTEM_LOG_SECURITY, error, DBUS_ERROR_ACCESS_DENIED, "rejected attempt to "
                                    "call %s by unknown uid", method);
      goto out;
  }
  if (!_dbus_windows_user_is_process_owner(windows_sid)) {
      const char *method = dbus_message_get_member(message);
      bus_context_log_and_set_error(bus_transaction_get_context(transaction), DBUS_SYSTEM_LOG_SECURITY, error, DBUS_ERROR_ACCESS_DENIED, "rejected attempt to "
                                    "call %s by uid %s", method, windows_sid);
      goto out;
  }
  ret = TRUE;
out:
...
static dbus_bool_t bus_driver_send_welcome_message (DBusConnection *connection, DBusMessage *hello_message, BusTransaction *transaction, DBusError *error);
dbus_bool_t bus_driver_send_service_owner_changed(const char *service_name, const char *old_owner, const char *new_owner, BusTransaction *transaction,
  ...
  null_service = "";
  _dbus_verbose("sending name owner changed: %s [%s -> %s]\n", service_name, old_owner ? old_owner : null_service, new_owner ? new_owner : null_service);
  ...
  if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &service_name, DBUS_TYPE_STRING, old_owner ? &old_owner : &null_service, DBUS_TYPE_STRING,
...
dbus_bool_t bus_driver_send_service_lost(DBusConnection *connection, const char *service_name, BusTransaction *transaction, DBusError *error) {
  ...
  if (!dbus_message_set_destination(message, bus_connection_get_name(connection)) || !dbus_message_append_args(message, DBUS_TYPE_STRING, &service_name,
      DBUS_TYPE_INVALID)) {
      dbus_message_unref(message);
      BUS_SET_OOM(error);
      return FALSE;
...
static dbus_bool_t bus_driver_handle_hello(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
...
static dbus_bool_t bus_driver_handle_list_services(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply;
  int len;
  char **services;
  BusRegistry *registry;
...
static dbus_bool_t bus_driver_handle_list_activatable_services(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply;
  int len;
  char **services;
...
static dbus_bool_t bus_driver_handle_acquire_service(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply;
  ...
  _DBUS_ASSERT_ERROR_IS_CLEAR(error);
  registry = bus_connection_get_registry(connection);
  if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &name, DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID)) return FALSE;
  _dbus_verbose("Trying to own name %s with flags 0x%x\n", name, flags);
  retval = FALSE;
  reply = NULL;
  _dbus_string_init_const(&service_name, name);
  if (!bus_registry_acquire_service(registry, connection, &service_name, flags, &service_reply, transaction, error)) goto out;
  reply = dbus_message_new_method_return(message);
  if (reply == NULL) {
      BUS_SET_OOM(error);
      goto out;
...
static dbus_bool_t bus_driver_handle_release_service(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply;
  DBusString service_name;
  const char *name;
  dbus_uint32_t service_reply;
  ...
  reply = NULL;
  _dbus_string_init_const(&service_name, name);
  if (!bus_registry_release_service(registry, connection, &service_name, &service_reply, transaction, error)) goto out;
  reply = dbus_message_new_method_return(message);
  if (reply == NULL) {
      BUS_SET_OOM(error);
      goto out;
...
static dbus_bool_t bus_driver_handle_service_exists(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply;
  DBusString service_name;
  BusService *service;
  ...
  if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &name, DBUS_TYPE_INVALID)) return FALSE;
  retval = FALSE;
  if (strcmp(name, DBUS_SERVICE_DBUS) == 0) service_exists = TRUE;
  else {
      _dbus_string_init_const(&service_name, name);
...
static dbus_bool_t bus_driver_handle_activate_service(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  dbus_uint32_t flags;
  const char *name;
  dbus_bool_t retval;
  BusActivation *activation;
...
static dbus_bool_t bus_driver_handle_update_activation_environment(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  dbus_bool_t retval;
  BusActivation *activation;
  BusContext *context;
  DBusMessageIter iter;
  ...
  if (bus_context_get_servicehelper(context) != NULL) {
      dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,"Cannot change activation environment on a system bus.");
      return FALSE;
  }
  activation = bus_connection_get_activation(connection);
  ...
  if (bus_context_get_systemd_activation (bus_connection_get_context(connection))) {
      systemd_message = dbus_message_new_method_call("org.freedesktop.systemd1","/org/freedesktop/systemd1","org.freedesktop.systemd1.Manager",
                                                     "SetEnvironment");
...
static dbus_bool_t bus_driver_handle_add_match(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  BusMatchRule *rule;
  ...
  if (bus_connection_get_n_match_rules(connection) >= limit) {
      DBusError tmp_error;
      dbus_error_init(&tmp_error);
      dbus_set_error(&tmp_error, DBUS_ERROR_LIMITS_EXCEEDED,"Connection \"%s\" is not allowed to add more match rules (increase limits in configuration "
                     "file if required; max_match_rules_per_connection=%d)", bus_connection_is_active(connection) ? bus_connection_get_name(connection) :
                     "(inactive)", limit);
  ...
  if (bus_match_rule_get_client_is_eavesdropping(rule)) {
      if (!bus_driver_check_caller_is_privileged(connection, transaction, message, error) || !bus_apparmor_allows_eavesdropping(connection, bustype, error))
          goto failed;
  }
...
static dbus_bool_t bus_driver_handle_remove_match(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  BusMatchRule *rule;
  const char *text;
  DBusString str;
  BusMatchmaker *matchmaker;
...
static dbus_bool_t bus_driver_handle_get_service_owner(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  const char *text;
  ...
  if (service == NULL && _dbus_string_equal_c_str(&str, DBUS_SERVICE_DBUS)) base_name = DBUS_SERVICE_DBUS;
...
static dbus_bool_t bus_driver_handle_list_queued_owners(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
...
static dbus_bool_t bus_driver_handle_get_connection_unix_user(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusConnection *conn;
  DBusMessage *reply;
  dbus_uid_t uid;
  dbus_uint32_t uid32;
  ...
  switch(found) {
      case BUS_DRIVER_FOUND_SELF: uid = _dbus_getuid(); break;
...
static dbus_bool_t bus_driver_handle_get_connection_unix_process_id(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusConnection *conn;
  DBusMessage *reply;
  dbus_pid_t pid;
  dbus_uint32_t pid32;
  ...
  switch(found) {
      case BUS_DRIVER_FOUND_SELF: pid = _dbus_getpid(); break;
      case BUS_DRIVER_FOUND_PEER:
...
static dbus_bool_t bus_driver_handle_get_adt_audit_session_data(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusConnection *conn;
  DBusMessage *reply;
  void *data = NULL;
  ...
  reply = dbus_message_new_method_return(message);
  if (reply == NULL) goto oom;
  if (found != BUS_DRIVER_FOUND_PEER || !dbus_connection_get_adt_audit_session_data(conn, &data, &data_size) || data == NULL) {
      dbus_set_error(error, DBUS_ERROR_ADT_AUDIT_DATA_UNKNOWN,"Could not determine audit session data for '%s'", service);
      goto failed;
  }
  if (! dbus_message_append_args(reply, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE, &data, data_size, DBUS_TYPE_INVALID)) goto oom;
  if (! bus_transaction_send_from_driver(transaction, connection, reply)) goto oom;
  dbus_message_unref(reply);
  return TRUE;
oom:
...
static dbus_bool_t bus_driver_handle_get_connection_selinux_security_context(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message,
							                                                 DBusError *error) {
  DBusConnection *conn;
  DBusMessage *reply;
  ...
  reply = dbus_message_new_method_return(message);
  if (reply == NULL) goto oom;
  if (found == BUS_DRIVER_FOUND_SELF) context = bus_selinux_get_self();
  else if (found == BUS_DRIVER_FOUND_PEER) context = bus_connection_get_selinux_id(conn);
  else context = NULL;
  if (!context) {
      dbus_set_error(error, DBUS_ERROR_SELINUX_SECURITY_CONTEXT_UNKNOWN,"Could not determine security context for '%s'", service);
      goto failed;
  }
...
dbus_bool_t bus_driver_fill_connection_credentials(DBusConnection *conn, DBusMessageIter *asv_iter) {
  ...
  if (ulong_pid <= _DBUS_UINT32_MAX && ulong_pid != DBUS_PID_UNSET && !_dbus_asv_add_uint32(asv_iter, "ProcessID", ulong_pid)) return FALSE;
  if (ulong_uid <= _DBUS_UINT32_MAX && ulong_uid != DBUS_UID_UNSET && !_dbus_asv_add_uint32(asv_iter, "UnixUserID", ulong_uid)) return FALSE;
  if (conn != NULL && dbus_connection_get_windows_user(conn, &s)) {
      DBusString str;
      dbus_bool_t result;
  ...
  if (conn != NULL && _dbus_connection_get_linux_security_label(conn, &s)) {
      if (s == NULL) return FALSE;
      if (!_dbus_asv_add_byte_array(asv_iter, "LinuxSecurityLabel", s, strlen (s) + 1)) {
          dbus_free(s);
          return FALSE;
      }
      dbus_free(s);
  ...
  if (conn != NULL && bus_containers_connection_is_contained(conn, &path, NULL, NULL)) {
      if (!_dbus_asv_add_object_path(asv_iter, DBUS_INTERFACE_CONTAINERS1 ".Instance", path)) return FALSE;
  }
  return TRUE;
}
static dbus_bool_t bus_driver_handle_get_connection_credentials(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusConnection *conn;
  DBusMessage *reply;
  ...
  switch(found) {
      case BUS_DRIVER_FOUND_SELF: conn = NULL; break;
      case BUS_DRIVER_FOUND_PEER: _dbus_assert(conn != NULL); break;
      default: goto failed;
  }
  reply = _dbus_asv_new_method_return(message, &reply_iter, &array_iter);
  if (reply == NULL || !bus_driver_fill_connection_credentials(conn, &array_iter) || !_dbus_asv_close(&reply_iter, &array_iter)) goto oom;
  if (! bus_transaction_send_from_driver(transaction, connection, reply)) {
      dbus_message_unref(reply);
      reply = NULL;
      goto oom;
...
static dbus_bool_t bus_driver_handle_reload_config(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  BusContext *context;
  DBusMessage *reply;
  _DBUS_ASSERT_ERROR_IS_CLEAR(error);
  reply = NULL;
...
static dbus_bool_t bus_driver_handle_enable_verbose(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
...
static dbus_bool_t
bus_driver_handle_disable_verbose(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
    DBusMessage *reply = NULL;
    _DBUS_ASSERT_ERROR_IS_CLEAR(error);
    reply = dbus_message_new_method_return(message);
    if (reply == NULL) goto oom;
...
#endif
static dbus_bool_t
bus_driver_handle_get_id(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  BusContext *context;
  DBusMessage *reply;
  DBusString uuid;
  const char *v_STRING;
...
static dbus_bool_t bus_driver_handle_become_monitor(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  ...
  if (!dbus_message_get_args(message, error, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING, &match_rules, &n_match_rules, DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID))
      goto out;
  if (flags != 0) {
  ...
  ret = TRUE;
out:
  if (ret) { _DBUS_ASSERT_ERROR_IS_CLEAR(error); }
  else { _DBUS_ASSERT_ERROR_IS_SET(error); }
  for (iter = _dbus_list_get_first_link(&rules); iter != NULL; iter = _dbus_list_get_next_link(&rules, iter)) bus_match_rule_unref(iter->data);
  _dbus_list_clear(&rules);
  dbus_free_string_array(match_rules);
  return ret;
}
static dbus_bool_t bus_driver_handle_get_machine_id(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusMessage *reply = NULL;
  DBusString uuid;
  const char *str;
  _DBUS_ASSERT_ERROR_IS_CLEAR(error);
...
static dbus_bool_t bus_driver_handle_ping(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  return bus_driver_send_ack_reply (connection, transaction, message, error);
}
static dbus_bool_t bus_driver_handle_get(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error);
static dbus_bool_t bus_driver_handle_get_all(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error);
static dbus_bool_t bus_driver_handle_set (DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error);
...
typedef struct {
  ...
  const char *in_args;
  const char *out_args;
  dbus_bool_t (*handler)(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error);
  MethodFlags flags;
} MessageHandler;
typedef struct {
  const char *name;
...
static InterfaceHandler interface_handlers[] = {
  ...
  { DBUS_INTERFACE_INTROSPECTABLE, introspectable_message_handlers, NULL,INTERFACE_FLAG_ANY_PATH | INTERFACE_FLAG_UNINTERESTING },
...
#endif
  { DBUS_INTERFACE_PEER, peer_message_handlers, NULL,INTERFACE_FLAG_ANY_PATH | INTERFACE_FLAG_UNINTERESTING },
  { NULL, NULL, NULL }
};
...
dbus_bool_t bus_driver_generate_introspect_string(DBusString *xml, dbus_bool_t is_canonical_path, DBusMessage *message) {
  ...
  for (ih = interface_handlers; ih->name != NULL; ih++) {
      ...
      for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++) {
          if (!_dbus_string_append_printf(xml,"    <property name=\"%s\" type=\"%s\" access=\"read\">\n", ph->name, ph->type)) return FALSE;
          if (!_dbus_string_append(xml,"      <annotation name=\"org.freedesktop.DBus.Property.EmitsChangedSignal\" value=\"const\"/>\n    </property>\n"))
              return FALSE;
      }
...
static dbus_bool_t bus_driver_handle_introspect(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  DBusString xml;
  DBusMessage *reply;
  const char *v_STRING;
...
dbus_bool_t bus_driver_handle_message(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  ...
  if (dbus_message_is_signal(message, "org.freedesktop.systemd1.Activator", "ActivationFailure")) {
      ...
      if (systemd != connection) {
          ...
          bus_context_log(context, DBUS_SYSTEM_LOG_SECURITY,"Ignoring forged ActivationFailure message from connection %s (%s)",
                          attacker ? attacker : "(unauthenticated)", bus_connection_get_loginfo(connection));
          return TRUE;
      }
      if (!bus_context_get_systemd_activation(context)) {
          bus_context_log(context, DBUS_SYSTEM_LOG_WARNING,"Ignoring unexpected ActivationFailure message while not using systemd activation");
          return FALSE;
      }
  ...
  dbus_set_error(error, found_interface ? DBUS_ERROR_UNKNOWN_METHOD : DBUS_ERROR_UNKNOWN_INTERFACE,"%s does not understand message %s",
...
static dbus_bool_t features_getter(BusContext *context, DBusMessageIter *variant_iter) {
  ...
  const char *s;
  if (!dbus_message_iter_open_container(variant_iter, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING_AS_STRING, &arr_iter)) return FALSE;
...
static const PropertyHandler *interface_handler_find_property(const InterfaceHandler *ih, const char *name, DBusError *error) {
  const PropertyHandler *ph;
  for (ph = ih->property_handlers; ph != NULL && ph->name != NULL; ph++) {
...
static dbus_bool_t bus_driver_handle_get(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  ...
  if (!dbus_message_get_args(message, error, DBUS_TYPE_STRING, &iface, DBUS_TYPE_STRING, &prop, DBUS_TYPE_INVALID)) return FALSE;
...
static dbus_bool_t bus_driver_handle_set(DBusConnection *connection, BusTransaction *transaction, DBusMessage *message, DBusError *error) {
  ...
  if (!dbus_message_iter_init(message, &iter)) _dbus_assert_not_reached("Message type was already checked to be 'ssv'");
