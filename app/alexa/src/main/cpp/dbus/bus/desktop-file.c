...
static dbus_bool_t grow_lines_in_section(BusDesktopFileSection *section) {
  BusDesktopFileLine *lines;
  int new_n_lines;
  if (section->n_allocated_lines == 0) new_n_lines = 1;
  else new_n_lines = section->n_allocated_lines*2;
...
static dbus_bool_t grow_sections(BusDesktopFile *desktop_file) {
  int new_n_sections;
  BusDesktopFileSection *sections;
  if (desktop_file->n_allocated_sections == 0) new_n_sections = 1;
  else new_n_sections = desktop_file->n_allocated_sections*2;
...
static char *unescape_string(BusDesktopFileParser *parser, const DBusString *str, int pos, int end_pos, DBusError *error) {
  char *retval, *q;
  ...
  while(pos < end_pos) {
      if (_dbus_string_get_byte(str, pos) == 0) {
	      dbus_free(retval);
          report_error(parser, "Text to be unescaped contains embedded nul", BUS_DESKTOP_PARSE_ERROR_INVALID_ESCAPES, error);
          return NULL;
	  }
      if (_dbus_string_get_byte(str, pos) == '\\') {
          ...
          if (pos >= end_pos) {
              dbus_free(retval);
              report_error(parser, "Text to be unescaped ended in \\", BUS_DESKTOP_PARSE_ERROR_INVALID_ESCAPES, error);
              return NULL;
          }
          switch(_dbus_string_get_byte(str, pos)) {
               ...
               default:
                   dbus_free(retval);
                   report_error(parser, "Text to be unescaped had invalid escape sequence", BUS_DESKTOP_PARSE_ERROR_INVALID_ESCAPES, error);
                   return NULL;
          }
          pos++;
	  } else {
...
static void parse_comment_or_blank(BusDesktopFileParser *parser) {
  ...
  if (line_end == parser->len) parser->pos = parser->len;
  else parser->pos = line_end + eol_len;
  parser->line_num += 1;
}
...
static void report_error(BusDesktopFileParser *parser, const char *message, const char *error_name, DBusError *error) {
  ...
  _DBUS_ASSERT_ERROR_IS_CLEAR(error);
  if (parser->current_section != -1) section_name = parser->desktop_file->sections[parser->current_section].section_name;
  if (section_name) dbus_set_error(error, error_name,"Error in section %s at line %d: %s\n", section_name, parser->line_num, message);
  else dbus_set_error(error, error_name,"Error at line %d: %s\n", parser->line_num, message);
}
BusDesktopFile* bus_desktop_file_load(DBusString *filename, DBusError *error) {
  ...
  while(parser.pos < parser.len) {
      if (_dbus_string_get_byte(&parser.data, parser.pos) == '[') {
          if (!parse_section_start(&parser, error)) return NULL;
	  } else if (is_blank_line(&parser) || _dbus_string_get_byte(&parser.data, parser.pos) == '#') parse_comment_or_blank(&parser);
      else if (parser.current_section < 0) {
          dbus_set_error(error, DBUS_ERROR_FAILED, "invalid service file: key=value before [Section]");
          return NULL;
	  } else {
          if (!parse_key_value(&parser, error)) return NULL;
	  }
  }
  _dbus_string_free(&parser.data);
...
static BusDesktopFileLine *lookup_line(BusDesktopFile *desktop_file, BusDesktopFileSection *section, const char *keyname) {
  BusDesktopFileLine *line;
  int i;
