...
static void service_dirs_append_link_unique_or_free(DBusList **service_dirs, DBusList *dir_link) {
  if (!service_dirs_find_dir(service_dirs, dir_link->data)) _dbus_list_append_link(service_dirs, dir_link);
  else {
...
dbus_bool_t bus_config_parser_start_element(BusConfigParser *parser, const char *element_name, const char **attribute_names, const char **attribute_values,
  ...
  switch(parser->type) {
      case ELEMENT_SERVICEDIR: case ELEMENT_SERVICEHELPER: case ELEMENT_USER: case ELEMENT_CONFIGTYPE: break;
      case ELEMENT_STANDARD_SYSTEM_SERVICEDIRS: {
            ...
            while ((link = _dbus_list_pop_first_link(&dirs))) service_dirs_append_link_unique_or_free(&parser->service_dirs, link);
            break;
        }
      default: {
...
static dbus_bool_t make_full_path(const DBusString *test_data_dir, const char *subdir, const char *file, DBusString *full_path) {
  DBusString filename;
  dbus_bool_t retval;
  retval = FALSE;
...
static dbus_bool_t check_file_valid(DBusString *full_path, Validity validity) {
  dbus_bool_t retval;
  if (validity == VALID) _dbus_test_diag("Testing valid file:");
  else if (validity == INVALID) _dbus_test_diag("Testing invalid file:");
  else _dbus_test_diag("Testing unknown file:");
...
dbus_bool_t bus_config_parser_trivial_test(const DBusString *test_data_dir) {
  ...
  if (!check_file_valid(&full_path, INVALID)) goto finish; _dbus_string_free(&full_path);
#ifndef DBUS_WIN
