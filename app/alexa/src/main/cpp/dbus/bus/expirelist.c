...
BusExpireList* bus_expire_list_new(DBusLoop *loop, int expire_after, BusExpireFunc expire_func, void *data) {
  ...
  list->timeout = _dbus_timeout_new(100, expire_timeout_handler, list, NULL);
...
static int do_expiration_with_monotonic_time(BusExpireList *list, long tv_sec, long tv_usec) {
  ...
  while(link != NULL) {
      ...
      if (((item->added_tv_sec == 0) && (item->added_tv_usec == 0)) || ((list->expire_after > 0) && (elapsed >= (double)list->expire_after))) {
...
static dbus_bool_t
test_expire_func(BusExpireList *list, DBusList *link, void *data) {
  TestExpireItem *t;
...
#define EXPIRE_AFTER 100
  ...
  if (!bus_expire_list_add(list, &item->item)) _dbus_test_fatal("out of memory");
