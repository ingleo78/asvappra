...
typedef dbus_bool_t (*DBusMessageGeneratorFunc)(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity);
static void set_reply_serial(DBusMessage *message) {
  if (message == NULL) _dbus_test_fatal("oom");
  if (!dbus_message_set_reply_serial(message,100)) _dbus_test_fatal("oom");
}
static dbus_bool_t generate_trivial_inner(DBusMessageDataIter *iter, DBusMessage **message_p) {
  DBusMessage *message;
  switch(iter_get_sequence(iter)) {
      case 0:
          message = dbus_message_new_method_call("org.freedesktop.TextEditor","/foo/bar","org.freedesktop.DocumentFactory","Create");
          break;
      case 1:
      ...
      case 3:
          ...
          if (!dbus_message_set_error_name(message,"org.freedesktop.TestErrorName")) _dbus_test_fatal("oom");
          {
              DBusMessageIter iter2;
              const char *v_STRING = "This is an error";
              dbus_message_iter_init_append(message, &iter2);
              if (!dbus_message_iter_append_basic(&iter2, DBUS_TYPE_STRING, &v_STRING)) _dbus_test_fatal("oom");
          }
          set_reply_serial(message);
          break;
...
static dbus_bool_t generate_many_bodies_inner(DBusMessageDataIter *iter, DBusMessage **message_p) {
  ...
  if (dbus_internal_do_not_use_generate_bodies (iter_get_sequence(iter), byte_order, &signature, &body)) {
      ...
      if (!_dbus_header_set_field_basic(&message->header, DBUS_HEADER_FIELD_SIGNATURE, DBUS_TYPE_SIGNATURE, &v_SIGNATURE)) _dbus_test_fatal("oom");
      if (!_dbus_string_move(&body, 0, &message->body, 0)) _dbus_test_fatal("oom");
      _dbus_marshal_set_uint32(&message->header.data, BODY_LENGTH_OFFSET, _dbus_string_get_length(&message->body), byte_order);
      *message_p = message;
  } else {
...
static void generate_from_message(DBusString *data, DBusValidity *expected_validity, DBusMessage *message) {
  ...
  *expected_validity = DBUS_VALID;
  if (!_dbus_string_move(&message->header.data, 0, data, 0)) _dbus_test_fatal("oom");
  if (!_dbus_string_copy(&message->body, 0, data, _dbus_string_get_length (data))) _dbus_test_fatal("oom");
}
static dbus_bool_t generate_outer(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity, DBusInnerGeneratorFunc func) {
  DBusMessage *message;
  message = NULL;
...
static DBusMessage* simple_error(void) {
  ...
  message =  dbus_message_new(DBUS_MESSAGE_TYPE_ERROR);
  if (message == NULL) _dbus_test_fatal("oom");
  if (!dbus_message_set_error_name(message, "foo.bar")) _dbus_test_fatal("oom");
  set_reply_serial(message);
  return message;
}
static DBusMessage* message_with_nesting_levels(int levels) {
  ...
  while(i < levels) {
      if (!dbus_message_iter_open_container(&parents[i], DBUS_TYPE_VARIANT,i == (levels - 1) ? DBUS_TYPE_INT32_AS_STRING :
          DBUS_TYPE_VARIANT_AS_STRING, &children[i]))
          _dbus_test_fatal("oom");
      ++i;
      parents[i] = children[i-1];
  }
  --i;
  if (!dbus_message_iter_append_basic(&children[i], DBUS_TYPE_INT32, &v_INT32)) _dbus_test_fatal("oom");
  while(i >= 0) {
      if (!dbus_message_iter_close_container(&parents[i], &children[i])) _dbus_test_fatal("oom");
      --i;
  }
  dbus_free(parents);
  dbus_free(children);
...
static dbus_bool_t generate_special(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity) {
  ...
  if (item_seq == 0) {
      message = simple_method_call();
      if (!dbus_message_append_args(message, DBUS_TYPE_INT32, &v_INT32, DBUS_TYPE_INT32, &v_INT32, DBUS_TYPE_INT32, &v_INT32, DBUS_TYPE_INVALID))
          _dbus_test_fatal("oom");
      ...
      *expected_validity = DBUS_INVALID_UNKNOWN_TYPECODE;
      ...
      v_STRING = long_sig;
      if (!_dbus_header_set_field_basic(&message->header, DBUS_HEADER_FIELD_SIGNATURE, DBUS_TYPE_SIGNATURE, &v_STRING)) _dbus_test_fatal("oom");
      _dbus_header_get_field_raw(&message->header, DBUS_HEADER_FIELD_SIGNATURE,NULL, &pos);
      generate_from_message(data, expected_validity, message);
      ...
      *expected_validity = DBUS_INVALID_MESSAGE_TOO_LONG;
      ...
      message = simple_method_call();
      if (!_dbus_header_set_field_basic(&message->header, DBUS_HEADER_FIELD_SENDER, DBUS_TYPE_STRING, &v_STRING)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
      *expected_validity = DBUS_INVALID_BAD_SENDER;
  } else if (item_seq == 10) {
      message = simple_method_call();
      if (!dbus_message_set_interface(message, DBUS_INTERFACE_LOCAL)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
      *expected_validity = DBUS_INVALID_USES_LOCAL_INTERFACE;
  } else if (item_seq == 11) {
      message = simple_method_call();
      if (!dbus_message_set_path(message, DBUS_PATH_LOCAL)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
      *expected_validity = DBUS_INVALID_USES_LOCAL_PATH;
  } else if (item_seq == 12) {
      message = simple_method_call();
      if (!dbus_message_set_interface(message, NULL)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
      *expected_validity = DBUS_VALID;
  } else if (item_seq == 13) {
      ...
      *expected_validity = DBUS_INVALID_MISSING_INTERFACE;
      ...
      if (!_dbus_header_delete_field(&message->header, DBUS_HEADER_FIELD_REPLY_SERIAL)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
      *expected_validity = DBUS_INVALID_MISSING_REPLY_SERIAL;
  } else if (item_seq == 15) {
      message = simple_error();
      if (!dbus_message_set_error_name (message, NULL)) _dbus_test_fatal("oom");
      generate_from_message(data, expected_validity, message);
...
restart:
  ...
  if (adjust < 0) {
      if ((_dbus_string_get_length(data) + adjust) < DBUS_MINIMUM_HEADER_SIZE) _dbus_string_set_length(data, DBUS_MINIMUM_HEADER_SIZE);
      else _dbus_string_shorten(data, - adjust);
      *expected_validity = DBUS_INVALID_FOR_UNKNOWN_REASON;
  } else {
      if (!_dbus_string_lengthen(data, adjust)) _dbus_test_fatal("oom");
...
restart:
  ...
  if (byte_seq == _dbus_string_get_length(data)) {
      ...
      iter_unrecurse (iter);
      goto restart;
  } else iter_set_sequence (iter, iter_get_sequence (iter) - 1);
...
#if 0
static dbus_bool_t find_next_typecode(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity) {
  int body_seq;
  int byte_seq;
  int base_depth;
...
restart:
  ...
  while(TRUE) {
      ...
      _dbus_assert(byte_seq < _dbus_string_get_length(data));
      if (dbus_type_is_valid(_dbus_string_get_byte(data, byte_seq))) break;
      else iter_next(iter);
  }
...
static dbus_bool_t generate_typecode_changed(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity) {
  int byte_seq;
  int typecode_seq;
  int base_depth;
  base_depth = iter->depth;
restart:
  _dbus_assert(iter->depth == (base_depth + 0));
  _dbus_string_set_length(data, 0);
  if (!find_next_typecode(iter, data, expected_validity)) return FALSE;
  iter_recurse(iter);
  byte_seq = iter_get_sequence(iter);
  _dbus_assert(byte_seq < _dbus_string_get_length(data));
  iter_recurse(iter);
  ...
  _dbus_assert(iter->depth == (base_depth + 0));
#if 0
  _dbus_test_diag("Changing byte %d in message %d to %c", byte_seq, iter_get_sequence(iter), typecodes[typecode_seq]);
#endif
...
static dbus_bool_t generate_uint32_changed(DBusMessageDataIter *iter, DBusString *data, DBusValidity *expected_validity) {
  int body_seq;
  int byte_seq;
  int change_seq;
...
next_change:
  ...
  if (change->type == CHANGE_TYPE_ADJUST) v_UINT32 += (int)change->value;
  else v_UINT32 = change->value;
#if 0
  _dbus_test_diag("body %d change %d pos %d ", body_seq, change_seq, byte_seq);
  if (change->type == CHANGE_TYPE_ADJUST) _dbus_test_diag("adjust by %d", (int)change->value);
  else _dbus_test_diag("set to %u", change->value);
  _dbus_test_diag(" \t%u -> %u", _dbus_marshal_read_uint32(data, byte_seq, byte_order, NULL), v_UINT32);
#endif
...
restart:
  ...
  iter_recurse(iter);
  if (iter_first_in_series(iter)) _dbus_test_diag(" testing message loading: %s ", generators[generator].name);
  func = generators[generator].func;
  if (!_dbus_string_init(&data->data)) _dbus_test_fatal("oom");
