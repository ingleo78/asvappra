...
DBusValidity _dbus_validate_signature_with_reason(const DBusString *type_str, int type_pos, int len) {
  ...
  while(p != end) {
      switch(*p) {
          case DBUS_TYPE_BYTE: case DBUS_TYPE_BOOLEAN: case DBUS_TYPE_INT16: case DBUS_TYPE_UINT16: case DBUS_TYPE_INT32: case DBUS_TYPE_UINT32:
          case DBUS_TYPE_UNIX_FD: case DBUS_TYPE_INT64: case DBUS_TYPE_UINT64: case DBUS_TYPE_DOUBLE: case DBUS_TYPE_STRING: case DBUS_TYPE_OBJECT_PATH:
          case DBUS_TYPE_SIGNATURE: case DBUS_TYPE_VARIANT:
              break;
          ...
          case DBUS_DICT_ENTRY_END_CHAR:
              ...
              if (element_count != 2) {
                  if (element_count == 0) result = DBUS_INVALID_DICT_ENTRY_HAS_NO_FIELDS;
                  else if (element_count == 1) result = DBUS_INVALID_DICT_ENTRY_HAS_ONLY_ONE_FIELD;
                  else result = DBUS_INVALID_DICT_ENTRY_HAS_TOO_MANY_FIELDS;
                  goto out;
              }
              break;
          default: result = DBUS_INVALID_UNKNOWN_TYPECODE; goto out;
      }
...
static DBusValidity validate_body_helper(DBusTypeReader *reader, int byte_order, dbus_bool_t walk_reader_to_end, int total_depth, const unsigned char *p,
                                         const unsigned char *end, const unsigned char **new_p) {
  int current_type;
  if (total_depth > (DBUS_MAXIMUM_TYPE_RECURSION_DEPTH * 2)) return DBUS_INVALID_NESTED_TOO_DEEPLY;
  while((current_type = _dbus_type_reader_get_current_type(reader)) != DBUS_TYPE_INVALID) {
      const unsigned char *a;
      int alignment;
  #if 0
      _dbus_verbose("   validating value of type %s type reader %p type_pos %d p %p end %p %d remain\n", _dbus_type_to_string(current_type), reader,
                    reader->type_pos, p, end, (int)(end - p));
  #endif
      if (p == end) return DBUS_INVALID_NOT_ENOUGH_DATA;
      switch(current_type) {
          case DBUS_TYPE_BYTE: ++p; break;
          case DBUS_TYPE_BOOLEAN: case DBUS_TYPE_INT16: case DBUS_TYPE_UINT16: case DBUS_TYPE_INT32: case DBUS_TYPE_UINT32: case DBUS_TYPE_UNIX_FD:
          case DBUS_TYPE_INT64: case DBUS_TYPE_UINT64: case DBUS_TYPE_DOUBLE:
              ...
              if (current_type == DBUS_TYPE_BOOLEAN) {
                  dbus_uint32_t v = _dbus_unpack_uint32(byte_order, p);
          ...
          case DBUS_TYPE_ARRAY: case DBUS_TYPE_STRING: case DBUS_TYPE_OBJECT_PATH: {
                  ...
                  if (current_type == DBUS_TYPE_ARRAY) {
                      ...
                      if (!dbus_type_is_valid(array_elem_type)) return DBUS_INVALID_UNKNOWN_TYPECODE;
                  ...
                  if (current_type == DBUS_TYPE_OBJECT_PATH) {
                      ...
                      DBusString str;
                      _dbus_string_init_const_len(&str, (const char*)p, claimed_len);
                      if (!_dbus_string_validate_utf8(&str, 0, _dbus_string_get_length(&str))) return DBUS_INVALID_BAD_UTF8_IN_STRING;
                      p += claimed_len;
          ...
          case DBUS_TYPE_VARIANT: {
                  ...
                  claimed_len = *p;
                  ++p;
                  if (claimed_len + 1 > (unsigned long)(end - p)) return DBUS_INVALID_VARIANT_SIGNATURE_LENGTH_OUT_OF_BOUNDS;
                  ...
                  if (!(reason == DBUS_VALID)) {
                      if (reason == DBUS_VALIDITY_UNKNOWN_OOM_ERROR) return reason;
                      else return DBUS_INVALID_VARIANT_SIGNATURE_BAD;
                  }
                  p += claimed_len;
          ...
          case DBUS_TYPE_DICT_ENTRY: case DBUS_TYPE_STRUCT: {
              ...
              break;
          default: _dbus_assert_not_reached("invalid typecode in supposedly-validated signature"); break;
      }
  #if 0
...
DBusValidity _dbus_validate_body_with_reason(const DBusString *expected_signature, int expected_signature_start, int byte_order, int *bytes_remaining,
                                             const DBusString *value_str, int value_pos, int len) {
  ...
  if (bytes_remaining) {
      *bytes_remaining = end - p;
      return DBUS_VALID;
  } else if (p < end) return DBUS_INVALID_TOO_MUCH_DATA;
  else {
      _dbus_assert(p == end);
      return DBUS_VALID;
  }
...
#define VALID_NAME_CHARACTER(c)  (((c) >= '0' && (c) <= '9') || ((c) >= 'A' && (c) <= 'Z') || ((c) >= 'a' && (c) <= 'z') || ((c) == '_'))
dbus_bool_t _dbus_validate_path(const DBusString *str, int start, int len) {
  ...
  while(s != end) {
      if (*s == '/') {
          if ((s - last_slash) < 2) return FALSE;
          last_slash = s;
      } else {
  ...
  if ((end - last_slash) < 2 && len > 1) return FALSE;
  return TRUE;
}
...
dbus_bool_t _dbus_validate_interface(const DBusString *str, int start, int len) {
  ...
  s = iface;
  if (_DBUS_UNLIKELY(*s == '.')) return FALSE;
  else if (_DBUS_UNLIKELY(!VALID_INITIAL_NAME_CHARACTER(*s))) return FALSE;
  else ++s;
  while(s != end) {
      if (*s == '.') {
          if (_DBUS_UNLIKELY((s + 1) == end)) return FALSE;
          else if (_DBUS_UNLIKELY(!VALID_INITIAL_NAME_CHARACTER(*(s + 1)))) return FALSE;
          last_dot = s;
          ++s;
      } else if (_DBUS_UNLIKELY(!VALID_NAME_CHARACTER(*s))) return FALSE;
      ++s;
  }
...
dbus_bool_t _dbus_validate_member(const DBusString *str, int start, int len) {
  ...
  s = member;
  if (_DBUS_UNLIKELY(!VALID_INITIAL_NAME_CHARACTER(*s))) return FALSE;
  else ++s;
  while(s != end) {
      if (_DBUS_UNLIKELY(!VALID_NAME_CHARACTER(*s))) return FALSE;
      ++s;
...
#define VALID_INITIAL_BUS_NAME_CHARACTER(c)  (((c) >= 'A' && (c) <= 'Z') || ((c) >= 'a' && (c) <= 'z') || ((c) == '_') || ((c) == '-'))
#define VALID_BUS_NAME_CHARACTER(c)  (((c) >= '0' && (c) <= '9') || ((c) >= 'A' && (c) <= 'Z') || ((c) >= 'a' && (c) <= 'z') || ((c) == '_') || ((c) == '-'))
  ...
  if (*s == ':') {
      ...
      while(s != end) {
          if (*s == '.') {
              ...
              ++s;
          } else if (_DBUS_UNLIKELY(!VALID_BUS_NAME_CHARACTER(*s))) return FALSE;
          ++s;
      }
      return TRUE;
  } else if (_DBUS_UNLIKELY(*s == '.')) return FALSE;
  else if (_DBUS_UNLIKELY(!VALID_INITIAL_BUS_NAME_CHARACTER(*s))) return FALSE;
  else ++s;
  while(s != end) {
      if (*s == '.') {
          if (_DBUS_UNLIKELY((s + 1) == end)) return FALSE;
          else if (_DBUS_UNLIKELY(!VALID_INITIAL_BUS_NAME_CHARACTER(*(s + 1)))) return FALSE;
          last_dot = s;
          ++s;
      } else if (_DBUS_UNLIKELY(!VALID_BUS_NAME_CHARACTER(*s))) return FALSE;
      ++s;
