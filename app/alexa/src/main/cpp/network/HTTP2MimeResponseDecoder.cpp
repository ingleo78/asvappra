    ...
    namespace avsCommon {
        namespace utils {
            namespace http2 {
                ...
                HTTP2MimeResponseDecoder::HTTP2MimeResponseDecoder(std::shared_ptr<HTTP2MimeResponseSinkInterface> sink) : m_sink{sink}, m_responseCode{0},
                        m_lastStatus{HTTP2ReceiveDataStatus::SUCCESS}, m_index{0}, m_leadingCRLFCharsLeftToRemove{LEADING_CRLF_CHAR_SIZE},
                        m_boundaryFound{false}, m_lastSuccessIndex{0} {
                ...
                void HTTP2MimeResponseDecoder::partBeginCallback(const MultipartHeaders& headers, void* userData) {
                    ...
                    switch (decoder->m_lastStatus) {
                        case HTTP2ReceiveDataStatus::SUCCESS: if (!decoder->m_sink->onBeginMimePart(headers)) decoder->m_lastStatus = HTTP2ReceiveDataStatus::ABORT; break;
                ...
                void HTTP2MimeResponseDecoder::partDataCallback(const char* buffer, size_t size, void* userData) {
                    ...
                    if (decoder->m_lastStatus == HTTP2ReceiveDataStatus::SUCCESS) {
                        ...
                        if (decoder->m_lastStatus == HTTP2ReceiveDataStatus::SUCCESS) decoder->m_lastSuccessIndex = decoder->m_index;
                    }
                }
