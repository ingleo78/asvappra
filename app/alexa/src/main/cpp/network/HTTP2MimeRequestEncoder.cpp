        ...
        namespace utils {
            namespace http2 {
                ...
                std::ostream& operator<<(std::ostream& stream, HTTP2MimeRequestEncoder::State state) {
                    switch (state) {
                        case HTTP2MimeRequestEncoder::State::NEW: return stream << "NEW";
                        case HTTP2MimeRequestEncoder::State::GETTING_1ST_PART_HEADERS: return stream << "GETTING_1ST_PART_HEADERS";
                        case HTTP2MimeRequestEncoder::State::SENDING_1ST_BOUNDARY: return stream << "SENDING_1ST_BOUNDARY";
                        case HTTP2MimeRequestEncoder::State::SENDING_PART_HEADERS: return stream << "SENDING_PART_HEADERS";
                        case HTTP2MimeRequestEncoder::State::SENDING_PART_DATA: return stream << "SENDING_PART_DATA";
                        case HTTP2MimeRequestEncoder::State::SENDING_END_BOUNDARY: return stream << "SENDING_END_BOUNDARY";
                        case HTTP2MimeRequestEncoder::State::GETTING_NTH_PART_HEADERS: return stream << "GETTING_NTH_PART_HEADERS";
                        case HTTP2MimeRequestEncoder::State::SENDING_CRLF_AFTER_BOUNDARY: return stream << "SENDING_CRLF_AFTER_BOUNDARY";
                        case HTTP2MimeRequestEncoder::State::SENDING_TERMINATING_DASHES: return stream << "SENDING_TERMINATING_DASHES";
                        case HTTP2MimeRequestEncoder::State::DONE: return stream << "DONE";
                        case HTTP2MimeRequestEncoder::State::ABORT: return stream << "ABORT";
                    }
                    return stream << "UNKNOWN(" << static_cast<int>(state) << ")";
                }
                HTTP2MimeRequestEncoder::HTTP2MimeRequestEncoder(const std::string& boundary, std::shared_ptr<HTTP2MimeRequestSourceInterface> source) : m_state{State::NEW},
                        m_rawBoundary{boundary}, m_prefixedBoundary{CRLF + TWO_DASHES + boundary}, m_source{source}, m_getMimeHeaderLinesResult{HTTP2GetMimeHeadersResult::ABORT},
                        m_stringIndex{0} { ACSDK_DEBUG9(LX(__func__).d("boundary", boundary).d("source", source.get())); }
                HTTP2SendDataResult HTTP2MimeRequestEncoder::onSendData(char* bytes, size_t size) {
                    ...
                    if (!m_source) return HTTP2SendDataResult::COMPLETE;
                    m_bytesCopied = 0;
                    while (true) {
                        switch (m_state) {
                            case State::NEW: setState(State::GETTING_1ST_PART_HEADERS); break;
                            case State::GETTING_1ST_PART_HEADERS:
                                ...
                                switch (m_getMimeHeaderLinesResult.status) {
                                    case HTTP2SendStatus::CONTINUE: m_stringIndex = 0; setState(State::SENDING_1ST_BOUNDARY); break;
                                    case HTTP2SendStatus::PAUSE:
                                        if (m_bytesCopied != 0) return continueResult();
                                        return HTTP2SendDataResult::PAUSE;
                                    case HTTP2SendStatus::COMPLETE: setState(State::DONE); return continueResult();
                                    case HTTP2SendStatus::ABORT: setState(State::ABORT); return HTTP2SendDataResult::ABORT;
                                }
                                break;
                            case State::SENDING_1ST_BOUNDARY:
                                if (!sendStringAndCRLF(bytes, size, m_prefixedBoundary)) return continueResult();
                            ...
                            case State::SENDING_PART_HEADERS:
                                while (m_headerLine != m_getMimeHeaderLinesResult.headers.end()) {
                                    if (sendStringAndCRLF(bytes, size, *m_headerLine)) {
                                        m_headerLine++;
                                        m_stringIndex = 0;
                                    } else return continueResult();
                                }
                                if (sendString(bytes, size, CRLF)) setState(State::SENDING_PART_DATA);
                                else return continueResult();
                                break;
                            case State::SENDING_PART_DATA: {
                                    ...
                                    switch (sendPartResult.status) {
                                        case HTTP2SendStatus::CONTINUE:
                                            ...
                                            if (m_bytesCopied == size) return continueResult();
                                        ...
                                        case HTTP2SendStatus::PAUSE:
                                            if (m_bytesCopied != 0) return continueResult();
                                            return HTTP2SendDataResult::PAUSE;
                                        case HTTP2SendStatus::COMPLETE: m_stringIndex = 0; setState(State::SENDING_END_BOUNDARY); break;
                                        case HTTP2SendStatus::ABORT: setState(State::ABORT); return HTTP2SendDataResult::ABORT;
                                    }
                                }
                                break;
                            case State::SENDING_END_BOUNDARY:
                                if (!sendString(bytes, size, m_prefixedBoundary)) return continueResult();
                                setState(State::GETTING_NTH_PART_HEADERS);
                                break;
                            case State::GETTING_NTH_PART_HEADERS:
                                m_getMimeHeaderLinesResult = m_source->getMimePartHeaderLines();
                                switch (m_getMimeHeaderLinesResult.status) {
                                    case HTTP2SendStatus::CONTINUE: m_stringIndex = 0; setState(State::SENDING_CRLF_AFTER_BOUNDARY); break;
                                    case HTTP2SendStatus::PAUSE:
                                        if (m_bytesCopied != 0) return continueResult();
                                        return HTTP2SendDataResult::PAUSE;
                                    case HTTP2SendStatus::COMPLETE: m_stringIndex = 0; setState(State::SENDING_TERMINATING_DASHES); break;
                                    case HTTP2SendStatus::ABORT: setState(State::ABORT); return HTTP2SendDataResult::ABORT;
                                }
                                break;
                            case State::SENDING_CRLF_AFTER_BOUNDARY:
                                if (!sendString(bytes, size, CRLF)) return continueResult();
                                m_headerLine = m_getMimeHeaderLinesResult.headers.begin();
                                m_stringIndex = 0;
                                setState(State::SENDING_PART_HEADERS);
                                break;
                            case State::SENDING_TERMINATING_DASHES:
                                if (sendStringAndCRLF(bytes, size, TWO_DASHES)) setState(State::DONE);
                                return continueResult();
                                break;
                            case State::DONE: return HTTP2SendDataResult::COMPLETE; break;
                            case State::ABORT: return HTTP2SendDataResult::ABORT;
                        }
                    }
                }
                ...
                bool HTTP2MimeRequestEncoder::sendStringAndCRLF(char* bytes, size_t size, const std::string& text) {
                    if (m_stringIndex < text.size()) {
                        if (!sendString(bytes, size, text)) return false;
                    }
