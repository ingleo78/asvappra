...
namespace alexaClientSDK {
    namespace acsdkNotifications {
        ...
        static const string CREATE_NOTIFICATION_INDICATOR_TABLE_SQL_STRING = string("CREATE TABLE ") + NOTIFICATION_INDICATOR_TABLE_NAME + " (" +
                                                                             DATABASE_COLUMN_PERSIST_VISUAL_INDICATOR_NAME + " INT NOT NULL," +
                                                                             DATABASE_COLUMN_PLAY_AUDIO_INDICATOR_NAME + " INT NOT NULL," +
                                                                             DATABASE_COLUMN_ASSET_ID_NAME + " TEXT NOT NULL," + DATABASE_COLUMN_ASSET_URL_NAME +
                                                                            " TEXT NOT NULL);";
        static const string INDICATOR_STATE_NAME = "indicatorState";
        static const string CREATE_INDICATOR_STATE_TABLE_SQL_STRING = string("CREATE TABLE ") + INDICATOR_STATE_NAME + " (" + INDICATOR_STATE_NAME +
                                                                      " INT NOT NULL);";
        ...
        unique_ptr<SQLiteNotificationsStorage> SQLiteNotificationsStorage::create(const ConfigurationNode& configurationRoot) {
            ...
            string notificationDatabaseFilePath;
            ...
            return unique_ptr<SQLiteNotificationsStorage>(new SQLiteNotificationsStorage(notificationDatabaseFilePath));
        }
        ...
        bool SQLiteNotificationsStorage::enqueue(const NotificationIndicator& notificationIndicator) {
            const string sqlString = "INSERT INTO " + NOTIFICATION_INDICATOR_TABLE_NAME + " (" + DATABASE_COLUMN_PERSIST_VISUAL_INDICATOR_NAME + "," +
                                     DATABASE_COLUMN_PLAY_AUDIO_INDICATOR_NAME + "," + DATABASE_COLUMN_ASSET_ID_NAME + "," + DATABASE_COLUMN_ASSET_URL_NAME +
                                     ") VALUES (" + "?, ?, ?, ?);";
        ...
        bool SQLiteNotificationsStorage::setIndicatorState(IndicatorState state) {
            lock_guard<mutex> lock{m_databaseMutex};
            {
                ...
                string sqlString = "DELETE FROM " + INDICATOR_STATE_NAME + " WHERE ROWID IN (SELECT ROWID FROM " + INDICATOR_STATE_NAME + " limit 1);";
        ...
        bool SQLiteNotificationsStorage::getIndicatorState(IndicatorState* state) {
            ...
            IndicatorState indicatorState = intToIndicatorState(statement->getColumnInt(0));
            statement->finalize();
            if (IndicatorState::UNDEFINED == indicatorState) {
                ACSDK_ERROR(LX("getIndicatorStateFailed").d("reason", "Unknown indicator state retrieved from the table. Returning default indicator state."));
