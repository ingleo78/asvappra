...
namespace alexaClientSDK {
    namespace acsdkNotifications {
        namespace test {
            using namespace avs::initialization;
            ...
            private:
                static pair<unique_ptr<istream>, const MediaType> defaultNotification() {
                    return pair<unique_ptr<istream>, const MediaType>(unique_ptr<istream>(new stringstream(DEFAULT_NOTIFICATION_AUDIO)),MediaType::MPEG);
                }
            };
            ...
            public:
                ...
                static shared_ptr<NiceMock<MockNotificationRenderer>> create() {
                    ...
                    //ON_CALL(*renderer.get(), renderNotificationShim(_, _)).WillByDefault(Invoke(renderer.get(), &MockNotificationRenderer::mockRender));
                    ON_CALL(*renderer.get(), cancelNotificationRenderingShim()).WillByDefault(InvokeWithoutArgs(renderer.get(), &MockNotificationRenderer::mockCancel));
                    return renderer;
                }
            ...
            MockNotificationRenderer::MockNotificationRenderer() : m_observer{nullptr}, m_startedRendering{false}, m_finishedRendering{false}, m_cancelling{false},
                                                                   m_renderStartedPromise{}, m_renderStartedFuture{m_renderStartedPromise.get_future()},
                                                                   m_renderFinishedPromise{}, m_renderFinishedFuture{m_renderFinishedPromise.get_future()} {}
            void MockNotificationRenderer::addObserver(
                shared_ptr<NotificationRendererObserverInterface> observer) {
                if (observer) m_observer = observer;
            }
            void MockNotificationRenderer::removeObserver(
                shared_ptr<acsdkNotificationsInterfaces::NotificationRendererObserverInterface> observer) {
                m_observer = nullptr;
            }
            bool MockNotificationRenderer::renderNotification(function<pair<unique_ptr<istream>, const MediaType>()> audioFactory, const string& url) {
            ...
            public:
                ...
                const string generatePayload(bool persistVisualIndicator, bool playAudioIndicator, const string& assetId = ASSET_ID1, const string& assetUrl = ASSET_URL1);
            ...
            void NotificationsCapabilityAgentTest::initializeCapabilityAgent() {
                m_notificationsCapabilityAgent = NotificationsCapabilityAgent::create(m_notificationsStorage,m_renderer,m_mockContextManager,
                                                                        m_mockExceptionSender, m_testNotificationsAudioFactory, m_dataManager,
                                                                         m_metricRecorder);
            ...
            void NotificationsCapabilityAgentTest::SetUp() {
                ...
                m_notificationsStorage = make_shared<TestNotificationsStorage>();
            ...
            void NotificationsCapabilityAgentTest::sendSetIndicatorDirective(
                ...
                //shared_ptr<AVSDirective> setIndicatorDirective = AVSDirective::create("", avsMessageHeader, payload, mockAttachmentManager, CONTEXT_ID_TEST);
                shared_ptr<DirectiveHandlerInterface> agentAsDirectiveHandler = m_notificationsCapabilityAgent;
                //agentAsDirectiveHandler->preHandleDirective(setIndicatorDirective, move(m_mockDirectiveHandlerResult));
            ...
            void NotificationsCapabilityAgentTest::sendClearIndicatorDirective(const string& messageId) {
                ...
                //auto mockAttachmentManager = make_shared<MockAttachmentManager>();
                //shared_ptr<AVSDirective> clearIndicatorDirective = AVSDirective::create("", avsMessageHeader, "", mockAttachmentManager, CONTEXT_ID_TEST);
            ...
            const string NotificationsCapabilityAgentTest::generatePayload(bool persistVisualIndicator, bool playAudioIndicator, const string& assetId,
                ...
                const string payload = "{\"persistVisualIndicator\":" + stringPersistVisualIndicator + ",\"playAudioIndicator\":" + stringPlayAudioIndicator +
                                       ",\"asset\": {\"assetId\":\"" + assetId + "\",\"url\":\"" + assetUrl + "\"}}";
                return payload;
            }
            TEST_F(NotificationsCapabilityAgentTest, test_create) {
                ...
                testNotificationsCapabilityAgent = NotificationsCapabilityAgent::create(nullptr,m_renderer,m_mockContextManager,
                                                                          m_mockExceptionSender,m_testNotificationsAudioFactory,
                                                                            m_dataManager,m_metricRecorder);
                EXPECT_EQ(testNotificationsCapabilityAgent, nullptr);
                testNotificationsCapabilityAgent = NotificationsCapabilityAgent::create(m_notificationsStorage,nullptr,m_mockContextManager,
                                                                          m_mockExceptionSender, m_testNotificationsAudioFactory,
                                                                            m_dataManager, m_metricRecorder);
                EXPECT_EQ(testNotificationsCapabilityAgent, nullptr);
                testNotificationsCapabilityAgent = NotificationsCapabilityAgent::create(m_notificationsStorage,m_renderer,nullptr,
                                                                          m_mockExceptionSender, m_testNotificationsAudioFactory,
                                                                            m_dataManager, m_metricRecorder);
                EXPECT_EQ(testNotificationsCapabilityAgent, nullptr);
                testNotificationsCapabilityAgent = NotificationsCapabilityAgent::create(m_notificationsStorage,m_renderer,m_mockContextManager,
                                                                          nullptr, m_testNotificationsAudioFactory,
                                                                            m_dataManager, m_metricRecorder);
                EXPECT_EQ(testNotificationsCapabilityAgent, nullptr);
                testNotificationsCapabilityAgent = NotificationsCapabilityAgent::create(m_notificationsStorage,m_renderer,m_mockContextManager,
